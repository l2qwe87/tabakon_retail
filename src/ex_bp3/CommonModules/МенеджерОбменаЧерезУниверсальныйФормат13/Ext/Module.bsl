...
Процедура TBK_BP_ПКО_Документ_СБДСРасчетыСКонтрагентами_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	ДанныеДляЗаписиВИБ	=	ДанныеИБ;
	Попытка
		Если ДанныеДляЗаписиВИБ <> Неопределено тогда
			Если Найти(НРег(ДанныеИБ.НазначениеПлатежа),"заработная плата") тогда
				ДанныеДляЗаписиВИБ.РасшифровкаПлатежа[0].СчетУчетаРасчетовСКонтрагентом	=	планыСчетов.Хозрасчетный.НайтиПоКоду("70");
				ДанныеДляЗаписиВИБ.РасшифровкаПлатежа[0].СчетУчетаРасчетовПоАвансам		=	планыСчетов.Хозрасчетный.НайтиПоКоду("70");
			КонецЕсли;
		КонецЕсли;
	Исключение
	КонецПопытки;
