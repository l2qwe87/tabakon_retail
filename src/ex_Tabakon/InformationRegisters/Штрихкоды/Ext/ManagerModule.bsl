...
Функция УдалитьНеВерные_ШтрихкодыУпаковокТоваров() ЭКСПОРТ
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
	               |ГДЕ
	               |	ШтрихкодыУпаковокТоваров.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяССылка)
	               |	И ШтрихкодыУпаковокТоваров.Номенклатура.ВидНоменклатуры.ИспользоватьХарактеристики";
...
Функция ФиксПробелыВШтрихкодах()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Штрихкоды.*
	               |ИЗ
	               |	РегистрСведений.Штрихкоды КАК Штрихкоды
	               |ГДЕ
	               |	Штрихкоды.Штрихкод ПОДОБНО ""[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]% %""";
	...
	Пока Рез.Следующий() Цикл
		...
		Попытка
			//Сообщить("["+Рез.Штрихкод+"] "+Рез.Владелец);		
			нз = РегистрыСведений.Штрихкоды.СоздатьНаборЗаписей();
			нз.Отбор.Владелец.Установить(Рез.Владелец);
			нз.Отбор.Штрихкод.Установить(Рез.Штрихкод);
			нз.Записать(ИСТИНА);
			
			
			нзНовый = РегистрыСведений.Штрихкоды.СоздатьНаборЗаписей();
			нзНовый.Отбор.Владелец.Установить(Рез.Владелец);
			новаяЗапись = нзНовый.Добавить();
			ЗаполнитьЗначенияСвойств(новаяЗапись,рез);
			новаяЗапись.Штрихкод = СокрЛП(новаяЗапись.Штрихкод);		
			новаяЗапись.ТипШтрихкода = ?(СтрДлина(новаяЗапись.Штрихкод) = 8,ПланыВидовХарактеристик.ТипыШтрихкодов.EAN8,ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13);
			нзНовый.Записать(); 	
		исключение
			//Сообщить("Ошибка фикса штрихкодов "+"["+Рез.Штрихкод+"] "+Рез.Владелец);
		КонецПопытки;
	КонецЦикла;
...
Функция УдалениеНеверныхШтрихкодов() ЭКСПОРТ
	...
	НоваяСтрока.НеверныйШК = "6971578995705";
	НоваяСтрока.НевернаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию("Электронное устройство HQD SUPER Lychee");
