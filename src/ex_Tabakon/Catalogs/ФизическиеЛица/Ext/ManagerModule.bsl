Функция ГенерацияЮзеровДляФЛ() ЭКСПОРТ
	...
	ВремяТекущее	 = Формат(ТекущаяДата(), "ДЛФ=T");
	ВремяЧасНочи 	 = Формат('00010101010000', "ДЛФ=T");
	ВремяШестьУтра	 = Формат('00010101060000', "ДЛФ=T");
	
	Если ВремяТекущее > ВремяЧасНочи и ВремяТекущее < ВремяШестьУтра тогда
		Пауза 		= 1;
		КолИтераций	=	100;
	иначе
		Пауза 		= 10;
