...
&НаСервере
&После("ОбработатьДокументыККМСервер")
Процедура ТБКОбработатьДокументыККМСервер1(Отказ, СсылкаНаОтчет, НужноДополнительноеПредупреждение)
	
	//Марк 2024-11-27
	Если не Отказ И СсылкаНаОтчет <> Неопределено Тогда
		
		Если Ложь Тогда СсылкаНаОтчет = Документы.ОтчетОРозничныхПродажах.НайтиПоНомеру("",ТекущаяДата()); КонецЕсли;
		
		ОбъектОтчет = СсылкаНаОтчет.ПолучитьОбъект();
		Для каждого СтрокаАкцизныеМарки Из ОбъектОтчет.АкцизныеМарки Цикл
			
			ПараметрыОтбора = Новый Структура();
			ПараметрыОтбора.Вставить("Номенклатура", СтрокаАкцизныеМарки.АкцизнаяМарка.Номенклатура);
			ПараметрыОтбора.Вставить("Характеристика", СтрокаАкцизныеМарки.АкцизнаяМарка.Характеристика);
			МассивСтрок = ОбъектОтчет.Товары.НайтиСтроки(ПараметрыОтбора);
			
			Для каждого Строка Из МассивСтрок Цикл
				Если Ложь Тогда Строка = СсылкаНаОтчет.Товары.Добавить(); КонецЕсли;
				
				Если Строка.QR = "" Тогда
					Строка.QR = Строка(СтрокаАкцизныеМарки.АкцизнаяМарка);		
				иначе
					Строка.QR = Строка.QR + символ(169) + Строка(СтрокаАкцизныеМарки.АкцизнаяМарка);	
				КонецЕсли;
		...
		Попытка
			ОбъектОтчет.Записать(РежимЗаписиДокумента.Проведение);	
		Исключение
			Сообщить("не удалось проставить qr в ООРП. " + ОписаниеОшибки());  
		КонецПопытки;
