...
Процедура ВыполнитьОбмен() ЭКСПОРТ
	Проверка_ПоследнийУспешныйОбмен();
	...
	Пока планы.Следующий() Цикл
		Если планы.НомерПринятого > 0 Тогда
			...
			ПланыОбмена.УдалитьРегистрациюИзменений(планы.Ссылка ,Метаданные.Справочники.ТоварныеГруппы);
			//КонецМарк
	...
	//Марк 2025-08-21 
	//Косталь по оишбкам видов Номенклатур
	Попытка
		Справочники.ВидыНоменклатуры.ЗапуститьПроверкуВидовНоменклатуры_И_Измерениеневерных();
	Исключение
	КонецПопытки;
	//КонецМарк
	...
	Обработки.ТБК_Миграции.М_00070_УстановкаРолиКассир();
	//КонецМарк
	
	
	ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая("ПоследнийУспешныйОбмен", ТекущаяДата());				
...
Процедура ОтправкаСообщенийВ_ТГ()
	...
	Если НЕ ПустойРезультат Тогда
		...
		ОбщегоНазначенияВызовСервера.SendMessageВVKTeams(5001, "В фокусной мотивации имеется пустая номенклатура. Магазин: " + Магазин);
...
Процедура ВыполнитьЗадачиПоВебСервису() Экспорт
	...
	//Вик 2026-01-29
	Попытка
		ОтправитьДанные_Инкасация(Прокси);
	Исключение
	КонецПопытки;
	// 
	...
	//Марк 2026-03-02
	Попытка
		ОбновитьТокенЧЗ(Прокси);
	Исключение
	КонецПопытки;
	//КонецМарк 
	
	//Марк 2026-04-27 https://jira.tabkn.ru/browse/DV1C-246
	Попытка
		ОтправитьНеотправленнуюАктивностьПользователя(Прокси);	
	Исключение
	КонецПопытки; 
	//КонецМарк
	
	// Толстоухов 27.04.2026 ++
	Попытка
		ОтправитьХозТовары(Прокси);
	Исключение
	КонецПопытки;
	// Толстоухов 27.04.2026 --  
	...
	//Вик 2024-04-22
	Попытка
		ОтправитьДанные_ПоДокументамУценки(Прокси);
	Исключение
	КонецПопытки;
	//
	...
	//Марк 2026-08-04
	Попытка
		ОтправитьТекущуюИнформаицюККТ(Прокси);
	Исключение
	КонецПопытки;
	
	// Толстоухов 05.08.2026 PRIC-137 ++
	Попытка
		ОтправитьПогашенныеВаучеры(Прокси);
	Исключение
	КонецПопытки;
	// Толстоухов 05.08.2026 PRIC-137 ++
	
	Прокси = Неопределено;
	Прокси = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User_TBK_IstoriyaPokupok", "http://mx.tbkon.ru:1777/ut/ws/tbk?wsdl");
	Попытка
	...
	Прокси = Неопределено;
	Прокси = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User_TBK_zayavki", "http://mx.tbkon.ru:1777/ut/ws/tbk?wsdl");
	Попытка
...
//Сервисы
Процедура ОтправитьДанныеТБК_МаркиДляБК(Прокси) Экспорт
...
Процедура ОтправкаКодовСкидки(Прокси) Экспорт
...
Процедура УдалениеНеверныхШтрихкодов(Прокси) Экспорт
	Ответ = Прокси.DelSHK();
...
Процедура ЗагрузкаДанныхПоСкидкам(Прокси) Экспорт
	...
	Если ЗначениеЗаполнено(Ответ) тогда
		...
		Для каждого СтрокаМассива из ОтветМассив цикл //добавляем
			...
			ДанныеАкции	=	СтрокаМассива.ТипАкции 					+ "/" + СтрокаМассива.Скидка 			+ "/" +
							...
							СтрокаМассива.КоличествоШагов			+ "/" + СтрокаМассива.КоличествоЧеков	+ "/" +	
							СтрокаМассива.НоменклатураИсключение	+ "/" + СтрокаМассива.Наименование;
...
Процедура ОтправкаДанныхОПокупкеСотрудников(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
...
Процедура ВыровнятьОстатки() Экспорт 
...
Процедура ОтправкаДанныхОЗаявкахМагазина(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
	...
	Для каждого Строка из рез цикл 
		...
		эл.Вставить("НовыйМеханизм", 				"");
		
		эл.Вставить("ДатаСформирован",				Строка(Строка.Документ.ДатаСформирован));  // Толстоухов 06.02.2026 DV1C-205 ++
	...
	Если Рез.Количество() > 0 тогда 
		...
		JSON = ОбщегоНазначения.СтруктураВjson_Общая(МассивВозврата);
		ОбщегоНазначения.СообщитьПользователю(JSON);
		
		ОтветJSON	=	Прокси.writerequests(JSON);
		МассивОтвета = ОбщегоНазначения.JSONВЗначение(ОтветJSON,,Ложь);
		
		Если ТипЗнч(МассивОтвета) = Тип("Массив") тогда
			
			Сообщить("ответ получен");
			
			Для каждого Строка из МассивОтвета цикл
				
				НомерДокумента		=	Строка.НомерДокумента;
				ДатаДокумента		=	ОбщегоНазначения.РазобратьДату(Строка.ДатаДокумента); 
				ДокументСсылка = ПолучитьСсылкуТБК_Заявки(Строка.НомерДокумента, Строка.ДатаДокумента); 
				
				Если НЕ ДокументСсылка.Пустая() Тогда
					
					рег = РегистрыСведений.ТБК_ДанныеПоЗаявкамМагазинов.СоздатьНаборЗаписей();
					рег.Отбор.Документ.Установить(ДокументСсылка);
					
					НоваяСтрока	=	Рег.добавить();  
					НоваяСтрока.Отправлен 	=  истина;
					НоваяСтрока.Документ 	=  ДокументСсылка;
					НоваяСтрока.Экспедитор 	=  ДокументСсылка.Экспедитор;
					НоваяСтрока.Товары		=  ДокументСсылка.Товары; 
					рег.Записать();	
					
					Если не (ДокументСсылка.Статус	= "Собран" или ДокументСсылка.Статус	= "Собирать" 
						ИЛИ ДокументСсылка.Статус = "Нет в наличии" ИЛИ ДокументСсылка.Статус = "Собран частично") тогда
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю("ответ не получен");	
		КонецЕсли;  
	...
	JSON = ОбщегоНазначения.СтруктураВjson_Общая(МассивВозврата);
	ОтветJSON	=	Прокси.getrequests(JSON);
	...
	ОбщегоНазначения.СообщитьПользователю("Сервис выполнился"); 
	
КонецПроцедуры  

Функция ПолучитьСсылкуТБК_Заявки(НомерДокумента, ДатаДокумента)

	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ТБК_ЗаявкаМагазина.Ссылка КАК ТБК
	                      |ИЗ
	                      |	Документ.ТБК_ЗаявкаОтМагазина КАК ТБК_ЗаявкаМагазина
	                      |ГДЕ
	                      |	ТБК_ЗаявкаМагазина.Номер = &Номер
	                      |	И ТБК_ЗаявкаМагазина.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&Дата, ГОД) И КОНЕЦПЕРИОДА(&Дата, ГОД)"); 
	
	Запрос.УстановитьПараметр("Номер", НомерДокумента);
	Запрос.УстановитьПараметр("Дата", Дата(ДатаДокумента)); 
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ТБК;
	иначе
		Возврат Документы.ТБК_ЗаявкаМагазина.ПустаяСсылка();
	КонецЕсли;

КонецФункции // ()


Процедура ОтправкаДанныхЗаказовПокупателей(Прокси) Экспорт
...
Процедура ОтправкаДанныхИсторияПокупок(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
...
Процедура ОтправкаДанныхСуммаОплат(Прокси) Экспорт
...
Процедура ОтправкаДанныхОЗаявкахНаЦенники(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
	...
	Для каждого Строка из рез цикл
		...
		Для каждого СтрокаДанные из Строка.Документ.Товары цикл	
			...
			эл.Вставить("Цена"						+	Строка(СтрокаДанные.НомерСтроки),	Строка(СтрокаДанные.Цена));
			эл.Вставить("QR"						+	Строка(СтрокаДанные.НомерСтроки),	Строка(СтрокаДанные.QR));    // Толстоухов 16.06.2026 PRIC-131 
		КонецЦикла;
...
Процедура ЗагрузкаПлановПродаж(Прокси) Экспорт
	//Для общей мотивации
	...
	Если запрос.Выполнить().Пустой() или ФлПринудительнаяЗагрузка  тогда 	
		...
		ОбновитьРезультатФокуснойМотивации = Ложь;
		...
		Если ЗначениеЗаполнено(Ответ)  и Ответ<>"[]"  тогда
			...
			Для каждого СтрокаМассива из ОтветМассив цикл //добавляем
				Номенклатура	=	Справочники.Номенклатура.НайтиПоКоду(СтрокаМассива.НоменклатураКод); // Толстоухов 02.04.2026 ++
			...
			ОбновитьРезультатФокуснойМотивации = Истина;
		КонецЕсли;
		
		Если ОбновитьРезультатФокуснойМотивации = Истина Тогда
			 ТБК_Миграции.ОбновитьРезультатФокуснойМотивации();
		КонецЕсли;	
	...
	Если запрос.Выполнить().Пустой() или ФлПринудительнаяЗагрузка тогда 
		...
		Если ЗначениеЗаполнено(Ответ) тогда   			
			...
			Для каждого СтрокаМассива из ОтветМассив цикл
				...
				Попытка //Вик 2025-06-27 фин отдел не всегда загружает проценты поэтому тут падает
					...
					Для каждого СтрокаПроцент из СтрокаМассива.Проценты цикл
						...
						Попытка
							новСтрока.ПроцентПремииСувениры	=	Число(СтрокаПроцент.ПроцентПремииСувениры);
						Исключение
						КонецПопытки;
	...
	// Толстоухов 02.12.2025 FINANCE-38 ++
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	|	КОЛИЧЕСТВО(ТБК_ПланыМагазинаПоПериодам.Сумма) КАК Сумма
	|ИЗ
	...
	Если Запрос.Выполнить().Пустой() или ФлПринудительнаяЗагрузка Тогда
		...
		Ответ	=	Прокси.get_Plans(НужСклад, НачалоДня, "6");	
		
		Если ЗначениеЗаполнено(Ответ) и Ответ <> "[]"  Тогда
			ОтветМассив	=	ОбщегоНазначения.jsonВСтруктура_Общая(Ответ);//В массив
			
			Для каждого СтрокаМассива из ОтветМассив Цикл //добавляем
				нз = РегистрыСведений.ТБК_ПланыМагазинаПоПериодам.СоздатьНаборЗаписей();
													 
				ПериодС	=	Дата(СтрЗаменить(лев(СтрокаМассива.ПериодС,10), "-", ""));
				ПериодПо	=	КонецДня(Дата(СтрЗаменить(лев(СтрокаМассива.ПериодПо, 10),"-","")));
													  
				нз.Отбор.ПериодС.Установить(ПериодС);
				нз.Отбор.ПериодПо.Установить(ПериодПо); 
				
				НоваяЗапись	=	нз.Добавить();
				
				НоваяЗапись.ПериодС		=	ПериодС;
				НоваяЗапись.ПериодПо	=	ПериодПо;
				НоваяЗапись.Сумма		=	Число(СтрокаМассива.Сумма);		
...
Процедура ОтправкаСтатусовЗаказовПокупателей(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
	      	 	             |	ЗаказПокупателя.Ссылка КАК Ссылка,
...
Процедура ОтправкаДанныхПоЗарегистрированнымКлиентам() Экспорт
	Попытка
...
Процедура ОтправкаДанныхПоПодаркам(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
...
Процедура ОтправкаДанныхПоСертификатам(Прокси) Экспорт
	Запрос	=	новый запрос("ВЫБРАТЬ
...
Процедура ОтправитьДанныеФокуснаяМотивация(Прокси) Экспорт
...
Процедура ОтправитьДанныеФокуснаяМотивация_ЗаДень(Прокси) Экспорт
	...
	ОтправитьДанныеФокуснаяМотивация_ЗаДеньНеотправленные(Прокси);  // Толстоухов 12.01.2026 ++
	
КонецПроцедуры

// Толстоухов 12.01.2026 ++
Процедура ОтправитьДанныеФокуснаяМотивация_ЗаДеньНеотправленные(Прокси)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	НАЧАЛОПЕРИОДА(ОтчетОРозничныхПродажах.Дата, ДЕНЬ) КАК ДеньПродажи
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НАЧАЛОПЕРИОДА(ТБК_РезультатФокуснойМотивации.ПериодМотивации, ДЕНЬ) КАК ДеньРегистра
		|ПОМЕСТИТЬ вт_Регистр
		|ИЗ
		|	РегистрСведений.ТБК_РезультатФокуснойМотивации КАК ТБК_РезультатФокуснойМотивации
		|ГДЕ
		|	ТБК_РезультатФокуснойМотивации.ПериодМотивации МЕЖДУ &Дата1 И &Дата2
		|
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ТБК_РезультатФокуснойМотивации.ПериодМотивации, ДЕНЬ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	вт_Продажи.ДеньПродажи КАК ДеньПродажи
		|ИЗ
		|	вт_Продажи КАК вт_Продажи
		|ГДЕ
		|	НЕ вт_Продажи.ДеньПродажи В
		|				(ВЫБРАТЬ
		|					т.ДеньРегистра
		|				ИЗ
		|					вт_Регистр КАК т)
	...
	Запрос.УстановитьПараметр("Дата1", НачалоДня(ТекущаяДата() - 60 * 60 * 24 * 10));
	Запрос.УстановитьПараметр("Дата2", НачалоДня(ТекущаяДата()));
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаНеотправленные = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаНеотправленные.Следующий() Цикл
		
	Период = ВыборкаНеотправленные.ДеньПродажи;	
...
Процедура ОтправитьДанныеРучныеСкидкиЗапись(Прокси) Экспорт
	Запрос = Новый запрос("ВЫБРАТЬ
...
Процедура ОтправитьДанныеДолгиеЧеки(Прокси) Экспорт
	Запрос = Новый запрос("ВЫБРАТЬ
...
Процедура ОтправитьДанные_УценкаНовая(Прокси) Экспорт
	Запрос = Новый запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ ТБКУценка_Новая.флОтправленВУт И ТБКУценка_Новая.Чек.Ссылка ЕСТЬ NOT NULL");
...
Процедура ПолучитьПробитиеПоБК(Прокси) Экспорт
...
Процедура ЗапросДанныхUtTrade(Прокси) Экспорт

	//Получить Магазин по Складу
	Запрос = Новый запрос("ВЫБРАТЬ
	...
	ВыборкаМагазин = Запрос.Выполнить().Выбрать();
	Если ВыборкаМагазин.Следующий() Тогда
		Магазин  = ВыборкаМагазин.Магазин;
	Иначе
		Возврат;
	КонецЕсли;
	
	Запрос = Новый запрос("ВЫБРАТЬ
	                      |	ТБК_ЗапросДанныхУ_УТ.Данные КАК Данные,
	                      |	ТБК_ЗапросДанныхУ_УТ.ТипДанных КАК ТипДанных
	                      |ИЗ
	                      |	РегистрСведений.ТБК_ЗапросДанныхУ_УТ КАК ТБК_ЗапросДанныхУ_УТ
	                      |ГДЕ
	                      |	НЕ ТБК_ЗапросДанныхУ_УТ.Отправлено"); 
	Результат = ЗАпрос.Выполнить().Выгрузить();
	
	СтруктураОтрправки = Новый Структура();
	СтруктураОтрправки.Вставить("Магазин", Строка(Магазин));
	СтруктураОтрправки.Вставить("МассивДанных", Новый Массив);
	
	для каждого Выборка из Результат Цикл
		
		//Проврека данных если ссылка уже есть то пропускаем
		Если Выборка.ТипДанных = "Ссылка" Тогда
			
			НайденнаяСсылка = ОбщегоНазначения.ПоискПоГуиду(Новый УникальныйИдентификатор(Выборка.Данные));	
			Если НайденнаяСсылка <> Неопределено Тогда
				набор = РегистрыСведений.ТБК_ЗапросДанныхУ_УТ.СоздатьНаборЗаписей();		
				набор.Отбор.Данные.Установить(Выборка.Данные);
				набор.Записать();
                Продолжить;
			КонецЕсли;
		...
		СтруктураОтрправки.МассивДанных.Добавить(Новый Структура("Данные, ТипДанных", Выборка.Данные, Выборка.ТипДанных));	
	
	КонецЦикла;
	
	Если СтруктураОтрправки.МассивДанных.Количество() Тогда
		
		JS = ОбщегоНазначения.СтруктураВjson_Общая(СтруктураОтрправки);
		Ответ = Прокси.RregistrationData(JS);
		
		Если Ответ = "true" Тогда
			
			Для каждого Выборка Из СтруктураОтрправки.МассивДанных Цикл
				
				набор = РегистрыСведений.ТБК_ЗапросДанныхУ_УТ.СоздатьНаборЗаписей();		
				набор.Отбор.Данные.Установить(Выборка.Данные);
				набор.Отбор.ТипДанных.Установить(Выборка.ТипДанных);
				набор.Прочитать();
...
Процедура ОтправитьДанные_Инкасация(Прокси) Экспорт
	Запрос = Новый запрос("ВЫБРАТЬ
	                      |	ТБКИнкасация.Ссылка КАК Ссылка,
	                      |	ТБКИнкасация.Дата КАК Дата,
	                      |	ТБКИнкасация.Продавец КАК Продавец,
	                      |	ТБКИнкасация.Экспедитор КАК Экспедитор,
	                      |	ТБКИнкасация.СуммаКСдаче КАК СуммаКСдаче,
	                      |	ТБКИнкасация.СуммаСдаваемыхНаличных КАК СуммаСдаваемыхНаличных,
	                      |	ТБКИнкасация.Комментарий КАК Комментарий
	                      |ИЗ
	                      |	Документ.ТБКИнкасация КАК ТБКИнкасация
	                      |ГДЕ
	                      |	НЕ ТБКИнкасация.ОтправленВУТ
	                      |	И НЕ ТБКИнкасация.ПометкаУдаления");
	...
	Для каждого Строка Из Тз Цикл
		...
		Структура = Новый Структура();
		Структура.Вставить("Дата", 						Строка(Строка.Дата));
		Структура.Вставить("Магазин", 					Строка(Магазин));
		Структура.Вставить("Продавец",					Строка(Строка.Продавец));
		Структура.Вставить("Экспедитор",				Строка(Строка.Экспедитор));
		Структура.Вставить("СуммаКСдаче",				Строка(Строка.СуммаКСдаче));
		Структура.Вставить("СуммаСдаваемыхНаличных",	Строка(Строка.СуммаСдаваемыхНаличных));
		Структура.Вставить("Комментарий",				Строка(Строка.Комментарий));
		
		МассивОтправки.Добавить(Структура);
		
	КонецЦикла;
	
	JS = ОбщегоНазначения.СтруктураВjson_Общая(МассивОтправки);
	Результат = Прокси.WriteInkasaciya(JS);
...
Процедура Проверка_ПоследнийУспешныйОбмен()
	ПоследнийУспешныйОбмен	=	ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("ПоследнийУспешныйОбмен");  
	
	Если ПоследнийУспешныйОбмен<>Неопределено тогда		
		Если ТекущаяДата() - ПоследнийУспешныйОбмен > 2*24*60*60 тогда
			
			ДатаСообщенияОНеРаботающемОбмене	=	ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("ДатаСообщенияОНеРаботающемОбмене");  
			
			Если ДатаСообщенияОНеРаботающемОбмене <> Неопределено И НачалоДня(ДатаСообщенияОНеРаботающемОбмене) <> НачалоДня(ТекущаяДата())  тогда		
				
				Магазин 				=	ОбщегоНазначения.ПолучитьМагазин(); 
				
				Текст	=	"Внимание! Не работает обмен на магазине: " + Магазин + " с " + ПоследнийУспешныйОбмен + символы.ПС 
				+ "Возможно файл базы данных поврежден.";
				
				//Токен	=	"5966721453:AAHn8QgTocNeVFZ0yiehFELLHPo1ay5oApg";
				//ИДКРАША	=	"-1001403844951";
				//ОбщегоНазначенияВызовСервера.ОтправитьСообщениеВТелеграмм(Токен, ИДКРАША, Текст);
				ОбщегоНазначенияВызовСервера.SendMessageВVKTeams(5001, Текст);
				
				ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая("ДатаСообщенияОНеРаботающемОбмене", ТекущаяДата());				
...
Процедура ОтправитьНеотправленнуюАктивностьПользователя(Прокси)

	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТБКАктивностьПользователя.Дата КАК Дата,
	|	ТБКАктивностьПользователя.Пользователь КАК Пользователь,
	|	ТБКАктивностьПользователя.Событие КАК Событие
	|ИЗ
	|	РегистрСведений.ТБКАктивностьПользователя КАК ТБКАктивностьПользователя
	|ГДЕ
	|	НЕ ТБКАктивностьПользователя.Отправлено";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	СтруктураОтправки = Новый Структура;
	СтруктураОтправки.Вставить("Магазин", Строка(ОбщегоНазначения.ПолучитьМагазин()));
	...
	Пока Выборка.Следующий() Цикл
		...
		Структура = Новый Структура;
		Структура.Вставить("Дата", Строка(Выборка.Дата));
		Структура.Вставить("Пользователь", Строка(Выборка.Пользователь));
		Структура.Вставить("Событие", Строка(Выборка.Событие));
		
		МассивОтправки.Добавить(Структура);
		
	КонецЦикла;
	
	СтруктураОтправки.Вставить("Массив", МассивОтправки);
	
	Тело = ОбщегоНазначения.СтруктураВjson_Общая(СтруктураОтправки);
	
	Ответ = Прокси.fetchUnsentUserActivity(Тело);
	
	Если Ответ <> "false" Тогда
		
		Пока Выборка.Следующий() Цикл
			
			Набор = РегистрыСведений.ТБКАктивностьПользователя.СоздатьНаборЗаписей();
			Набор.Отбор.Дата.Установить(Дата(Выборка.Дата));	
...
// Толстоухов 27.04.2026 ++
Процедура ОтправитьХозТовары(Прокси) Экспорт 
	
	ТекущаяДата = ТекущаяДата();
	НомерДня = День(ТекущаяДата);
	
	Период_15_20 = НомерДня >= 15 И НомерДня < 21;
	Период_21_31 = НомерДня >= 21 И НомерДня <= 31;
		
	Если Период_15_20 Или Период_21_31 Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если Ответ = "true" И Период_21_31 Тогда
...
Процедура ОтправитьДанные_ПоДокументамУценки(Прокси) Экспорт
	Запрос = Новый запрос("ВЫБРАТЬ
	                      |	ТБКУценка.Ссылка КАК Ссылка
	                      |ИЗ
	                      |	Документ.ТБКУценка КАК ТБКУценка
	                      |ГДЕ
	                      |	ТБКУценка.Сформирован
	                      |	И НЕ ТБКУценка.флОтправленВУТ
	                      |	И НЕ ТБКУценка.ПометкаУдаления");
	...
	Для каждого Строка Из Тз Цикл
		...
		СтруктураДокумента.Вставить("Номер", 			Строка(Строка.Ссылка.Номер));
		СтруктураДокумента.Вставить("Дата", 			Строка(Строка.Ссылка.Дата));
		СтруктураДокумента.Вставить("Магазин", 			Строка(Строка.Ссылка.Магазин));    
		СтруктураДокумента.Вставить("Ответственный", 	Строка(Строка.Ссылка.Ответственный));    
		...
		Для каждого СтрокаТовар из Строка.Ссылка.Товары цикл
			...
			Если Ном = 0 тогда  //Для поиска в УТ  
				КодУценки	=	СтрокаТовар.QR_Уценки;
				Поз4				=	СтрНайти(КодУценки,"_",,,4);
				Поз5				=	СтрНайти(КодУценки,"_",,,5); 			
				НомерДокументаВУТ	=	Сред(КодУценки,Поз4+1, Поз5-Поз4-1); 
				
				СтруктураДокумента.Вставить("НомерДокументаВУТ", 	НомерДокументаВУТ);
				СтруктураДокумента.Вставить("КодУценки", 			КодУценки);

			КонецЕсли;
			
			ДляЗаписи	=	Новый Структура;
			
			ДляЗаписи.Вставить("Номенклатура",			Строка(СтрокаТовар.Номенклатура));
			ДляЗаписи.Вставить("НоменклатураКод",		Строка(СтрокаТовар.Номенклатура.Код));
			ДляЗаписи.Вставить("QR",					Строка(СтрокаТовар.QR));
			ДляЗаписи.Вставить("QR_Уценки",				Строка(СтрокаТовар.QR_Уценки));
			ДляЗаписи.Вставить("ПричинаУценки",			Строка(СтрокаТовар.QR_Уценки));
		...
		СтруктураДокумента.Вставить("ДляЗаписи", МассивДляЗаписи);
		
		МассивОтправки.Добавить(СтруктураДокумента);
		
	КонецЦикла;   
	
	JS = ОбщегоНазначения.СтруктураВjson_Общая(МассивОтправки);
	Результат = Прокси.WriteDokymentYcenka_New1(JS);
...
Процедура ОтправитьТекущуюИнформаицюККТ(Прокси)
	
	УстановитьПривилегированныйРежим(Истина);
	
	ДатаОтправки = ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("ДатаОтправкиДанныхИнформацииККТ");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ККТ_ТекущаяИнформация.Магазин КАК Магазин,
	|	ККТ_ТекущаяИнформация.ККМ КАК ККМ,
	|	ККТ_ТекущаяИнформация.ХранилищеЗначения КАК ХранилищеЗначения
	|ИЗ
	|	РегистрСведений.ККТ_ТекущаяИнформация КАК ККТ_ТекущаяИнформация";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Массив = Новый Массив;
	Пока Выборка.Следующий() Цикл
		Массив.Добавить(Выборка.ХранилищеЗначения.Получить());
	КонецЦикла;
	
	DataJson = ОбщегоНазначения.СтруктураВjson_Общая(Массив);
	Ответ = Прокси.ReportKktInfo(DataJson);
	
	Если Ответ Тогда
		
		МенеджерЗаписи = РегистрыСведений.ТБК_Константы.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.ЗначениеКонстанты = ТекущаяДата();
		МенеджерЗаписи.ИмяКонстанты = "ДатаОтправкиДанныхИнформацииККТ";
		МенеджерЗаписи.Записать();
...
// Толстоухов 05.08.2026 PRIC-137 ++
Процедура ОтправитьПогашенныеВаучеры(Прокси)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТБК_ПогашенныйВаучер.QR КАК QR,
	|	ТБК_ПогашенныйВаучер.СуммаСкидки КАК СуммаСкидки,
	|	ТБК_ПогашенныйВаучер.Чек КАК Чек,
	|	ТБК_ПогашенныйВаучер.Магазин КАК Магазин,
	|	ТБК_ПогашенныйВаучер.Отправлен КАК Отправлен
	|ИЗ
	|	РегистрСведений.ТБК_ПогашенныйВаучер КАК ТБК_ПогашенныйВаучер
	|ГДЕ
	|	ТБК_ПогашенныйВаучер.Отправлен = ЛОЖЬ";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураДанных = Новый Структура;
		СтруктураДанных.Вставить("Магазин", 	Строка(Выборка.Магазин));
		СтруктураДанных.Вставить("QR", 	        Выборка.QR);
		СтруктураДанных.Вставить("Чек", 	    Строка(Выборка.Чек)); 
		СтруктураДанных.Вставить("СуммаСкидки", Строка(Выборка.СуммаСкидки)); 
		
		JS = ОбщегоНазначения.СтруктураВjson_Общая(СтруктураДанных);
		
		Ответ =	Прокси.VoucherExit(JS);
		Если Ответ = "true" Тогда
			Сообщить("Данные отправлены в офис");
			
			МенеджерЗаписи = РегистрыСведений.ТБК_ПогашенныйВаучер.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.QR = Выборка.QR; 				
			МенеджерЗаписи.Прочитать();
