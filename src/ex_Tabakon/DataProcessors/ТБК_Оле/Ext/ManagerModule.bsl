...
Процедура ПолучитьШтрихкодаОбезличенныхМарок(Прокси)
	...
	Если НЕ Ответ = Ложь Тогда
		...
		Набор = РегистрыСведений.ОбезличенныеМарки.СоздатьНаборЗаписей();	
		Набор.Записать();
		...
		Для каждого Значение Из ОтветJSOON Цикл
			...
			Рег = РегистрыСведений.ОбезличенныеМарки.СоздатьМенеджерЗаписи();
			Рег.ШтрихкодОбезличеннойМарки = Значение;                                        
			Рег.Записать();
