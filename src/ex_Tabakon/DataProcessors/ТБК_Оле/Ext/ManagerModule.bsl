...
Процедура ОтправитьДанныеФокуснаяМотивация_ЗаДень(Прокси) Экспорт
	...
	Для каждого ПродавецМагазин из МассивОбщий цикл
		...
		Для каждого СтрокаДанных из ДляЗаписи Цикл
			...
			//рег.ФокусГруппа		=	СтрокаДанных.ФокусГруппа; 
			//рег.ПланПродаж		=	СтрокаДанных.ПланПродаж; 
			//рег.ФактПродаж		=	СтрокаДанных.ФактПродаж; 
			//
			////Если СтрокаДанных.КоличествоСверх <> "" Тогда	
			////	рег.СуммаСверх		=	СтрокаДанных.СуммаСверх; 
			////	рег.КоличествоСверх	=	СтрокаДанных.КоличествоСверх; 
			////КонецЕсли;
			//
			//рег.СуммаФакт		=	СтрокаДанных.СуммаФакт; 
			//рег.СуммаПремии		=	СтрокаДанных.СуммаПремии; 
			рег.Записать();
		КонецЦикла;  			
	КонецЦикла;
...
Процедура ОтправитьДанныеФокуснаяМотивация_ЗаДеньНеотправленные(Прокси)
	...
	Запрос.УстановитьПараметр("Дата1", ТекущаяДата() - 60 * 60 * 24 * 10);
...
Процедура ОтправитьХозТовары(Прокси) Экспорт 
	...
	ТекущийМесяц = Месяц(ТекущаяДата); // Добавляем определение переменной
	
	// Условие: (июнь 1–5) ИЛИ (ежемесячно 20–28)                         
	//Если (ТекущийМесяц = 6 И НомерДня >= 1 И НомерДня <= 5)
	//	ИЛИ (ТекущийМесяц <> 6 И НомерДня >= 20 И НомерДня <= 28) Тогда
		
	Если НомерДня >= 21 И НомерДня <= 31 Тогда
