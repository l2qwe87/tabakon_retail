...
Процедура УдалениеНеверныхШтрихкодов(Прокси)
	...
	Для каждого Строка из СтруктураОтвета цикл
		...
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             ...
		      	 	             |ГДЕ
		      	 	             |	ШтрихкодыУпаковокТоваров.Наименование ПОДОБНО &Наименование
...
Процедура ОтправкаДанныхОЗаявкахМагазина(Прокси)	
	...
	Для каждого Строка из рез цикл
		...
		Для каждого СтрокаДанные из Строка.Документ.Товары цикл	
			...
			Если Строка(Строка.Документ.Проект) = "Неиспользованные QR" тогда
...
Процедура ОтправитьДанные_УценкаНовая(Прокси)
	...
	Для каждого Строка Из Тз Цикл
		...
		СтруктураЧека.Вставить("НоменклатураКод"		+	Строка(Строка.НомерСтроки),	Строка(Строка.Номенклатура.Код));
		СтруктураЧека.Вставить("ПричинаУценки"			+	Строка(Строка.НомерСтроки),	Строка(Строка.ПричинаУценки));
		СтруктураЧека.Вставить("ПроцентСкидки"			+	Строка(Строка.НомерСтроки),	Строка(Строка.ПроцентСкидки));    
		СтруктураЧека.Вставить("Код"					+	Строка(Строка.НомерСтроки),	Строка(Строка.Код));
		СтруктураЧека.Вставить("Цена"					+	Строка(Строка.НомерСтроки),	Строка(Строка.Цена));
