...
Процедура ОтправкаДанныхОНевернойОрганизации(Прокси)	
	...
	Для каждого Строка из рез цикл
		эл	=	Новый Структура;
		эл.Вставить("Дата",Строка.Дата);		
		Данные		=	Строка.Запрос.Получить();
		
		эл.Вставить("Магазин",Данные.Магазин);
		эл.Вставить("Организация",Данные.Организация);
		эл.Вставить("ИНН",Данные.ИНН);
		эл.Вставить("Чек",Данные.Чек);
	...
	Если Ответ тогда
		Для каждого Строка из рез цикл
			...
			рег.Отбор.Запрос.Установить(Строка.Запрос);
			рег.Отбор.Дата.Установить(Строка.Дата);
			рег.Прочитать();
			рег[0].Идентификатор = рег[0].Идентификатор + "загружен";
			рег.Записать();			
		КонецЦикла;
	КонецЕсли;
