...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			Если Найти(Данные,"4660313541520") или Найти(Данные,"4627175411217") или Найти(Данные,"4627175411200") тогда
...
Процедура ДобавитьЗаписьВИсториюПокупок(НужныйЧек, ТБК_IDКлиента, флНадоНачислить, КолСписано, ТелефонКлиента)
	...
	Рег.Товары						=	Новый ХранилищеЗначения(НужныйЧек.Товары.Выгрузить());
...
Процедура ОбработатьСкидкиИзУТ(флНадо, ФлПринудительныйПересчет = Ложь, СтруктураПодарки = Неопределено)
	...
	Для каждого Строка из рез цикл
		...
		Позиция8 = СтрНайти(Строка.Наименование,"/",,,8);
		...
		КоличествоЧеков			=	Сред(Строка.Наименование,Позиция7+1,Позиция8-Позиция7-1);
		...
		Если ТипАкции = "Подарок за товар" тогда
			ОбработатьПодарокЗаТовар(ТоварВПодарок, Номенклатура, КоличествоНоменклатуры, СуммаЧека,  КоличествоЧеков, СтруктураПодарки, Строка.Идентификатор);
		КонецЕсли;
...
&НаСервере
Процедура ОбработатьПодарокЗаТовар(ТоварВПодарок, Номенклатура, КоличествоНоменклатуры, СуммаЧека,  КоличествоЧеков, СтруктураПодарки, Идентификатор)
...
Процедура ЗаписатьДанныеОНевернойОрганизации(ПроверкаМаркиИНН,НужныйЧек, НомерСтроки)
	...
	Поз				=	Найти(ПроверкаМаркиИНН,":");
	МаркаСтр		=	лев(ПроверкаМаркиИНН,Поз - 2);
	НеверныйИННСтр	=	Сред(ПроверкаМаркиИНН,Поз + 2 );
	
	Значение.Вставить("Марка",МаркаСтр);
	
	
	Если Не найти(ПроверкаМаркиИНН,"Статус") Тогда		
		Значение.Вставить("НеверныйИНН", НеверныйИННСтр);  		
	иначе
