		...
		Если Результат.КодСостояния = 200 Тогда
			...
			Если Структура.status <> "not found" Тогда
				...
				КонецЕсли;
					...
					СтруктураПречек = ЗаполнитьСтруктуруИзОтветаJSON(Чтение);	
					Чтение.Закрыть();
...
Процедура ТБКНачислитьБаллыПослеНаСервере(СуммаБаллов, ИмяАкции = "" , ТекстСообщения = "")
	Если ЭтаФорма.Samosale_НомерТелефона = 0 Тогда Возврат КонецЕсли;
	...
	Если Результат.КодСостояния = 200 Тогда
		...
		Если Структура.status <> "not found" Тогда
			...
			СтруктураДляJSON.Вставить("description", ИмяАкции);
...
&НаКлиенте
Процедура ТБКНачислитьБаллыПосле(Команда)
	ТБКНачислитьБаллыПослеНаСервере(10);
КонецПроцедуры
...
&НаСервере
Процедура ТБКВвестиНомерСоставногоБукетаПослеНаСервере(ПолученноеЗначение, ПереданныеПараметры) экспорт
	Если ПолученноеЗначение >0 тогда
		СосБук	=	Справочники.Номенклатура.НайтиПоНаименованию("Составной букет");
		...
		Запрос.УстановитьПараметр("Владелец", СосБук);
		Запрос.УстановитьПараметр("Наименование",СокрЛП(Строка(ПолученноеЗначение)));
		Рез = Запрос.Выполнить().Выгрузить();
		Если Рез.Количество() = 0 тогда
			Сообщить("Не найдена характеристика составного букета №: "+ПолученноеЗначение);
			Возврат;
		иначе
			НужХар	=	рез[0].Ссылка;
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("Характеристика",НужХар);
		Запрос.УстановитьПараметр("СтатусЧекаККМ",Перечисления.СтатусыЧековККМ.Пробитый);
		Если не Запрос.Выполнить().Пустой() тогда
			Сообщить("Букет уже продан. Чек: "+ запрос.Выполнить().Выгрузить()[0].Ссылка);
			Возврат;	
		КонецЕсли;		
		...
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             ...
		      	 	             |ГДЕ
		      	 	             |	ДействующиеЦеныНоменклатурыСрезПоследних.Характеристика = &Характеристика");
		Запрос.УстановитьПараметр("Характеристика",НужХар);
		Рез = Запрос.Выполнить().Выгрузить();
		
		Если рез.Количество() >0 тогда			
			НоваяСтрока	=	Объект.Товары.Добавить();
			НоваяСтрока.Номенклатура 		= СосБук;
			НоваяСтрока.Характеристика		= рез[0].Характеристика;
			НоваяСтрока.Продавец 			= Объект.Продавец;
			НоваяСтрока.Организация			= Объект.Организация;
			НоваяСтрока.Количество			= 1;
			НоваяСтрока.КоличествоУпаковок	= 1;
			...
			НоваяСтрока.СтавкаНДС			= Перечисления.СтавкиНДС.БезНДС;
		иначе
			Сообщить("Не найдена цена на букет с номером: "+ПолученноеЗначение);
		КонецЕсли;
...
&НаКлиенте
Процедура ТБКВвестиНомерСоставногоБукетаПосле(Команда)
	НомерБукета = 0;
	ОписаниеОповещения = Новый ОписаниеОповещения("ТБКВвестиНомерСоставногоБукетаПослеНаСервере", ЭтаФорма);
	ПоказатьВводЧисла(ОписаниеОповещения, НомерБукета, "Введите номер Составного букета: ");	
КонецПроцедуры
