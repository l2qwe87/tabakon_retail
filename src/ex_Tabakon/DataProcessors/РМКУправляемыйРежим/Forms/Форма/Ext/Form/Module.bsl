...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			Если ПроверитьЧтоЭтоВода(СтрокаДанные) и СтрДлина(данные) > 13 и СтрДлина(данные) < 37 и ТекущаяДата()>'20230627' тогда
			...
			//    флЭтоВода	=	Ложь;
			//	Если СтрДлина(Данные) = 37 и Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") тогда //КОСТЫЛЬ 2023-06-15. Это скорее всего вода
			...
			//Костыль на макбарен
			Если Лев(Данные,3) = "010" и СтрДлина(Данные) = 31   тогда
...
Процедура ТБКОплатитьТовары(ИмяФормыОплаты, ОплатаПлатежнойКартой = Ложь, ОплатаПлатежнойСистемой = Ложь, Печать)
	//Вик 2022-09-26
...
Функция ЭтуМаркуБитьПоБК(Данные)			
	...
	//Вик 2024-09-20 С 21.09.24 принято решение все жидкости бить по бк. Ранее эти жидкости пробивались как акция 2 по цене 1.
	Если ТекущаяДата()>'20240921' тогда
