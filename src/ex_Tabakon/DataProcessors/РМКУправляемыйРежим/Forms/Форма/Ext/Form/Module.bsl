...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			СтрокаДанные	=	СтрЗаменить(СтрокаДанные,Символ(21),"");
			СтрокаДанные	=	СтрЗаменить(СтрокаДанные,Символ(29),"");
			...
			Если СтрДлина(СтрокаДанные) >=29 тогда
				Если ЭтуМаркуБитьПоБК(Данные) тогда
					Сообщить("Для этой номенклатуры сканируйте штрихкод!");  	
					Возврат; 	
				КонецЕсли;
			КонецЕсли;
			...
			//Вик 2024-04-15
			МаркаСписана	=	ПроверитьСписаниеМарки(СтрокаДанные);
			...
			Если ПроверитьЧтоЭтоВода(СтрокаДанные) и СтрДлина(данные)>13 и СтрДлина(данные) < 31 и ТекущаяДата()>'20230627' тогда
			...
			Если флДобавитьВоВторыеЭлектронки тогда
				...
				Если ВходитВАкциюДВаПоЦенеОдного(ПК) тогда
					ОбработатьДобавлениеВоВторыеЭлектронки(СтрокаДанные, НужНоменклатура);
					Возврат;
				КонецЕсли; 
			КонецЕсли;
			//
...
&НаСервереБезКонтекста
Функция ЭтуМаркуБитьПоБК(Данные)			
	Запрос	=	Новый запрос("ВЫБРАТЬ
	...
	Если Рез.Следующий() тогда
		...
		Если Найти(ПК, "Ю2-00039463") или Найти(ПК, "Ю2-00021839") или Найти(ПК, "Ю2-00039464") или 
			...
			Найти(ПК, "Ю2-00040598") или Найти(ПК, "Ю2-00039007") или Найти(ПК, "Ю2-00024201") 			
			тогда
	...
	Если ТекущаяДата()>'20240921' тогда
		Номенклатура	=	 ОбщегоНазначенияВызовСервера.ПолучитьНоменклатуруПоКуару(Данные);
...
Процедура ОбработатьДобавлениеВоВторыеЭлектронки(Данные, НужНоменклатура)
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.ДействующиеЦеныНоменклатуры.СрезПоследних(&дата, ) КАК ДействующиеЦеныНоменклатурыСрезПоследних
	      	 	             |ГДЕ
