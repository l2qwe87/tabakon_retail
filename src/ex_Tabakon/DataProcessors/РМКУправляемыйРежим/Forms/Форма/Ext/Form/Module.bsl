...
Процедура ПроверитьСтатусыМарок_Результат() ЭКСПОРТ
	...
	Для Каждого стр из Объект.Товары Цикл
		...
		Для Каждого стрМарка из строкиМарки Цикл
			...
			Если ЗначениеЗаполнено(стрМарка.ПроверкаМаркиИНН) И СтрНайти(агрегатИНН, стрМарка.ПроверкаМаркиИНН) = 0 Тогда
				агрегатИНН = стрМарка.ПроверкаМаркиИНН + Символы.ПС + агрегатИНН; 
			КонецЕсли;
		КонецЦикла;
		
		
		стр.ПроверкаМаркиИНН = агрегатИНН;
