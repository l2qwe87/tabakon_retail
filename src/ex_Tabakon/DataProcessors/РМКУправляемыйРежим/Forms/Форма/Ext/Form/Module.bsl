...
Процедура ТБКПробитьЧекККМ(ОповещениеПриЗавершении, ЧекККМСсылка, КассаККМ, ПараметрыЗаполнения)
	...
	Иначе
		...
		//Вик  
		Если НеПечататьЧек и ЗначениеЗаполнено(ПочтаКлиента) тогда  
			
			Если не ОбщегоНазначенияВызовСервера.EmailValid(ПочтаКлиента) тогда
...
&НаКлиенте
Процедура ТБКПочтаКлиентаОкончаниеВводаТекстаПосле(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
КонецПроцедуры

&НаКлиенте
Процедура ТБКПочтаКлиентаПриИзмененииПосле(Элемент)
		Если не ОбщегоНазначенияВызовСервера.EmailValid(ПочтаКлиента) тогда
		Сообщить("Неверно заполнена почта клиента!");
		Возврат;
	КонецЕсли;	
