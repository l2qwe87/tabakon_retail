...
Процедура ТБКПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	//Вик 2021_07_01
	Для Каждого Стр из Объект.Товары Цикл
		Если ТекущаяДата() > '20240401' и ЗначениеЗаполнено(Стр.Характеристика) и Стр.Цена < 129 тогда
			Отказ = Истина;
...
Процедура ТБКЗаявкиИМПосле(Команда) //Марк 2024-02-14 Отбор (Интернет заказов)
	Оповещение = Новый ОписаниеОповещения("ВыполнитьПослеОткрытияЗаказов",ЭтотОбъект);
	ЗначениеОтбора = Новый Структура("Проект", ПолучтьПроектИнтернетМагазин());
	ПараметрыВыбора = Новый Структура("Отбор", ЗначениеОтбора);	
	Форма	= ОткрытьФорму("Документ.ТБК_ЗаявкаОтМагазина.ФормаСписка",ПараметрыВыбора,,,,,Оповещение);
...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			//Вик 2024-01-17 Костыль на группу MON, обезличенные марки
			...
			или Найти(Данные,"4670227010678") или Найти(Данные,"4670227010777")

			//или Найти(Данные,"6937105450284") или Найти(Данные,"6937105450246") или Найти(Данные,"6937105450123")
			//или Найти(Данные,"6937105450062") или Найти(Данные,"6937105450147") или Найти(Данные,"6937105449967")
			//или Найти(Данные,"6937105450048") или Найти(Данные,"6937105457511") или Найти(Данные,"6937105457467")
			//или Найти(Данные,"6937105450321") или Найти(Данные,"6937105457405") или Найти(Данные,"6937105457481")
			//или Найти(Данные,"6937105457436") или Найти(Данные,"6937105457443") или Найти(Данные,"6937105450345")
			//или Найти(Данные,"6937105450086") или Найти(Данные,"6937105450307") или Найти(Данные,"6937105457559")
			//или Найти(Данные,"6937105450000") или Найти(Данные,"6937105450024") или Найти(Данные,"6937105457535")	
			//КонецМарк
			тогда
			...
			Если МаркаСписана тогда //это списанная марка
				...
				ТекстСообщения 		=	"Эта марка списана! Пробейте по БК!";
				ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокИнформации, ТекстСообщения);	
				Возврат
			КонецЕсли;
			//
			
			Если ПроверитьЧтоЭтоВода(Данные) и СтрДлина(данные)>13 и СтрДлина(данные) < 31 и ТекущаяДата()>'20230627' тогда
				
				ЗаголовокИнформации =	НСтр("ru = 'Ошибка!'");
				ТекстСообщения 		=	"Для продажи напитков отсканируйте QR-код";
				ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокИнформации, ТекстСообщения);	
				Возврат
			КонецЕсли;
			...
			Если СтрДлина(Данные) > 29 тогда //этоБлок
			//Если ШтрихкодированиеМОТПКлиентСервер.ЭтоКодУпаковки(Данные) тогда
			    флЭтоВода	=	Ложь;
				Если (СтрДлина(Данные) = 37 или  СтрДлина(Данные) = 31)  и Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") тогда //КОСТЫЛЬ 2023-06-15. Это скорее всего вода
					ШК_ЕАН		=	Сред(Данные,4,13);
					флЭтоВода	=	ПроверитьЧтоЭтоВода(ШК_ЕАН);
				КонецЕсли;

				Если не флЭтоВода тогда //обрабатываем блок			
					Попытка
						флВсеХорошо	=	Истина;
			...
			Если ИспользоватьВторыеЭлектронки и СтрДлина(Данные) > 28 тогда
				...
				Если Объект.Товары.Итог("КоличествоУпаковок")=1 тогда 					
					Для Каждого Строка из Объект.Товары цикл
						...
						Если Найти(ПК, "00048205348") и не Найти(ПК, "Ю2-00019140") тогда
			...
			Если флДобавитьВоВторыеЭлектронки тогда
				ОбработатьДобавлениеВоВторыеЭлектронки(Данные);
				Возврат;
			КонецЕсли;
			//
...
&НаСервере
Процедура ОбработатьДобавлениеВоВторыеЭлектронки(Данные)
	СканКуар = Данные;
...
Процедура СоздатьНефискальныйЧек(ЭтоВторыеЭлектронки = Ложь, НужныйЧек = Неопределено)
	...
	КонецЕсли;
	ТБКНеФискальнаяПечать.НапечататьЧек(Объект.Товары, Сдача, СНО , Объект.Магазин, НужныйКассир, Объект.ТБКВторыеЭлектронки);
...
Функция СоздатьЧекККМ(ЭтоВторыеЭлектронки = Ложь, НужныйЧек = Неопределено)
	...
	иначе
		Для каждого Строка Из НужныйЧек.Оплата цикл
			НоваяСтрока = НовыйДокументЧек.Оплата.Добавить();
...
&НаКлиенте
Процедура Подключаемый_КомандаСкидкиАксессуарыIQOSСпидка30() //Марк 2024-06-06
...
Процедура ТБКОплатитьТовары(ИмяФормыОплаты, ОплатаПлатежнойКартой, Печать)
	//Вик 2022-09-26
	...
	Если Объект.СуммаДокумента > 15000 И ИмяФормыОплаты = "ФормаОплатыЯндексКасса" Тогда
		...
		Если Не НаличиеДоговора Тогда
			...
			ТекстИнформации = НСтр("ru = 'Превышена максимальная сумма чека оплаты в яндекс кассе.'");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокИнформации, ТекстИнформации);
	...
	Если РозничныеПродажиКлиент.ЭтоВозвратОбувиБезУказанияМарок(Объект, СписокНоменклатуры) Тогда 
		ПодтвердитьВозвратОбувиБезМарок(ИмяФормыОплаты, ОплатаПлатежнойКартой, Печать, СписокНоменклатуры);
	Иначе
		ПродолжитьОплатуТоваров(ИмяФормыОплаты, ОплатаПлатежнойКартой, Печать);
	КонецЕсли;
...
Процедура ТБКПробитьЧекККМ(ОповещениеПриЗавершении, ЧекККМСсылка, КассаККМ, ПараметрыЗаполнения)
...
Процедура ТБКНапечататьЧекиККМ(МассивСсылокЧеков)
...
Процедура ЗатолнитьТоварыПоЗаказу(ВведенныйНомер)
	...
	Для каждого Строка из ТоварыЗаказа цикл
		...
		Если ЭтоМаркированнаяНоменклатура(НоваяСтрока.Номенклатура) тогда
			...
			НоваяСтрока.ВидПродукцииИС					= Перечисления.ВидыПродукцииИС.Табачная;
...
Процедура ВыбратьНужнуюХарактеристикуПоНеобходимости(ИсходныеДанные, ПараметрыСканирования)
	...
	Если МассивШтрихкодов.Найти(ШК)<>Неопределено тогда
		...
		ХарактеристикиПоШК	=	ПолучитьХарактеристикиПоШК(ШК);
		Если ХарактеристикиПоШК.Количество() = 0 тогда
			//Перейти ~ЕслиНеЗавелиХарактеристик;
			ИсходныеДанные.Штрихкод = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBAse64(ИсходныеДанные.Штрихкод);
...
&НаСервереБезКонтекста
Функция ПолучитьХарактеристикиПоШК(ШК)
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	Штрихкоды.Владелец КАК Владелец
	      	 	             |ПОМЕСТИТЬ Номен
	      	 	             |ИЗ
			...
			Если ТипЗнч(сведенияМарок) = Тип("Массив") Тогда
				Если сведенияМарок.Количество() = 1 Тогда
					...
					Для каждого ДочерняяМарка из ДочерниеМарки_Массив цикл 
						...
						ИсходныеДанные.Штрихкод = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBAse64(ДочерняяМарка + МРЦБлокаКодированнная);
						РезультатОбработки = ОбработатьВводШтрихкода(ИсходныеДанные, Неопределено, ПараметрыСканирования);
...
Процедура ИзменитьМотивациюНаНовую()

	Если НачалоДня(ТекущаяДата()) = '20240901' Тогда
		
		МассивМагазиновНаПереходНовойМотивации = новый Массив;  //48
		//Полтавцев Артем Олегович
		МассивМагазиновНаПереходНовойМотивации.Добавить("Верхний1МАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ВыборгскоешоссеСММ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ГрандКаньонБЮА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Ларина15МВС");
		МассивМагазиновНаПереходНовойМотивации.Добавить("МегаПарнасПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Норд2СЕМ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("НордШИЛ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ПарнасСитиМДБ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ПарнасТКПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Приозерское26ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Просвещения60МАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Просвещения74ЦНГ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("РодеоЦНГ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Свирская1МВГ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ЭнколовоПДВ");
		//Савин Александр Анатольевич
		МассивМагазиновНаПереходНовойМотивации.Добавить("АтмосфераОСА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("БайконурскийЦНГ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Богатырский7ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ВыборгРебусПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ГулливерСДП");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Камышовая13ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("МеркурийСМА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("МиксерЛыжныйУИБ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("МонпансьеБЮА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ОкейБогатырскийПАО");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Парашютная63МАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ПитерлэндТАВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Плесецкая8ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ПятьОзёрВЛГ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Савушкина128ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("СитиМоллМАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Туристская26НЗВ");
		//Шейнова Наталия Николаевна
		МассивМагазиновНаПереходНовойМотивации.Добавить("Ветеранов171ПЕВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ЖемчужинаВАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Корзуна34ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Ленинский94ПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ЛенинскийТЦСДА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ЛомоносовЛентаБАА");
		МассивМагазиновНаПереходНовойМотивации.Добавить("МиксерПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("НовосельеНЗВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ОкейГерманаСДП");
		МассивМагазиновНаПереходНовойМотивации.Добавить("СолнечныйгородПДВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("СосновыйБорЗЕВ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("СтрельнаУИБ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ТетрисВТЕ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Ульянка");
		МассивМагазиновНаПереходНовойМотивации.Добавить("Фиолент2УИБ");
		МассивМагазиновНаПереходНовойМотивации.Добавить("ФиолентСДП");
		
		Магазин = ОбщегоНазначения.ПолучитьМагазин();
		
		Если МассивМагазиновНаПереходНовойМотивации.Найти(Строка(Магазин)) <> Неопределено Тогда
			
			ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая("ВариантМотивации", 6);
