...
Процедура ТБКОбновитьФормуНаКлиентеНаКлиенте()
...
Процедура ТБКПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	МассивДопПользователей.Добавить("Волкова Т. Е.");
...
Функция ЭтуМаркуБитьПоБК(Данные)			
	...
	Если Рез.Следующий() тогда
		...
		Если Найти(ПК, "ВЭ100000499") тогда // это жидкости
			Возврат истина;
		КонецЕсли;
...
Процедура ОбработатьПокупкуДляСебя(Данные)
	...
	Если НужНаим = "" тогда 
		...
		Возврат;	
	КонецЕсли;
	НужНаим = ОбщегоНазначенияКлиентСервер.УдалитьНедопустимыеСимволыXML(НужНаим);
...
Функция ПодключитьсяКУТ()
	Попытка
		Веб_Соединение = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User", "http://vp.tbkon.ru:1776/vape/ws/tbk?wsdl", 10);
		Массив	=	новый Массив;
...
Процедура Расш_SamosaleСоздатьКлиентаПосле(Команда)
	...
	Телефон = Samosale_НомерТелефона;
	...
	Samosale_НомерТелефона = Телефон;
	//ЭтаФорма.Элементы.СоздатьКлиента.Видимость = Ложь;
	//Расш_SamosaleНайтиКлиентаПослеНаСервере();
	//ЭтаФорма.Элементы.Samosale_Списать.Видимость = Истина;
	//ЭтаФорма.Элементы.СписатьБаллы.Видимость = Истина;	
...
Процедура ОтправитьЧек(СтруктураДляJSON,ПробитыйЧек)
	...
	Если АкцияУтилизация тогда
		Если не ПробитыйЧек.АналитикаХозяйственнойОперации.Наименование = "Возврат от покупателя" Тогда
			...
			ТБКНачислитьБаллыПослеНаСервере(СуммаБонусов, ИмяАкции, ТекстСообщения, ПробитыйЧек);
			АкцияУтилизация = Ложь;
			//элементы.АкцияУтилизация.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ЭтаФорма.Samosale_НомерТелефона <> "" Тогда
		...
		Если Результат.КодСостояния = 200 Тогда
			...
			Если Структура.status <> "not found" Тогда
				...
				//Марк 2023-11-20
				Если НЕ ЗначениеЗаполнено(ЭтаФорма.Samosale_ФИО) Тогда
			...
			Иначе
				...
				ЭтаФОрма.Элементы.СписатьБаллы.Видимость = Ложь;
				//ЭтаФорма.Элементы.АкцияУтилизация.Видимость = Ложь;
				Samosale_ФИО = "Клиент не найден";
				Samosale_КоличествоБаллов = "";
				//Сообщить(Структура.message);
				Возврат;
			КонецЕсли;
			Если Структура.promo.hasPromo = Истина Тогда
				ЭтаФорма.Элементы.ГруппаОК.Видимость = Истина;
...
Процедура Расш_SamosaleТоварыПриАктивизацииСтрокиВместо(Элемент)
	//ПодключитьОбработчикОжидания("ОбновитьФормуНаКлиентеНаКлиенте", 0.2, Истина);
КонецПроцедуры
...
Процедура ТБКНачислитьБаллыПослеПсле()
		...
		Если СтрДлина(Телефон) = 12 Тогда
			...
			Исключение
				...
				Рег.Записать();
				Сообщить("Проблемы с интернатом. " + Строка(Samosale_Бонус) + " - будут начислены потом");	
			КонецПопытки;
...
&НаСервере
Процедура ТБКНачислитьБаллыПослеНаСервере(СуммаБаллов, ИмяАкции = "" , ТекстСообщения = "", Чек = Неопределено)
	...
	Если Результат.КодСостояния = 200 Тогда
		...
		Если Структура.status <> "not found" Тогда
			...
			Если ОтветПречек.КодСостояния = 200 Тогда
				...
				//Марк 2024-03-25 DV1C-26
				Рег.Чек = Чек;
				Рег.Дата = ТекущаяДата();
				//КонецМарк
				Рег.Записать();
				Samosale_КоличествоБаллов = СуммаБаллов + Samosale_КоличествоБаллов;
			иначе	
				Рег = РегистрыСведений.ТБК_SamosaleДопБаллы.СоздатьМенеджерЗаписи();		
			    Рег.Телефон = "+7" + Формат(Samosale_НомерТелефона,"ЧГ=0");
				Рег.СуммаБаллов = СуммаБаллов;
				...
				//КонецМарк
				Рег.Записать();
				Сообщить("" + ИмяАкции + " - не начислено. Начисляется позже");	
			КонецЕсли;
...
&НаКлиенте
Процедура ТБКОстаткиПосле(Команда)
	ОткрытьФорму("Отчет.ТБК_Остатки.Форма");	
КонецПроцедуры
...
Функция ВходитВАкциюДВаПоЦенеОдного(ПК)
	...
	Если Найти(ПК, "ВЭ100000499")  тогда 
...
&НаКлиенте
Процедура ПробитьЧекККМСложныйСлучайЗавершение(РезультатОперации, ПараметрыОперации) Экспорт;//добавил вик 2024-12-2 
	...
	Если ПараметрыОперации.ТекущийЧек < ПараметрыОперации.КоличествоЧеков Тогда
		...
		ПробитьЧекККМ(ЧекККМСсылка, СтрокаТабличногоПоляЧеков.КассаККМ);
...
&НаСервере
Функция ЗаписатьЧекККМПоВсемДанным(Организация, ПараметрыОперации, ПараметрыИнформации)//вик 2024-12-26
