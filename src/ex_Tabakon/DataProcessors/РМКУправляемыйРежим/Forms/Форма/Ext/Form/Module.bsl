...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			//Вик 2024-09-11 Костыль, эти группы товаров бить по БК
			Если СтрДлина(СтрокаДанные) >=29 тогда
				НоменклатураИзКуара	=	 ОбщегоНазначенияВызовСервера.ПолучитьНоменклатуруПоКуару(Данные);
				...
				КонецЕсли;
			иначе
				НоменклатураИзКуара = Неопределено;
			КонецЕсли;
