...
Процедура ОбработатьНовуюУценку_ПоНовому(КодУценки)  
	КуарНоменклатуры_ПривязаннойККуаруУценки	=	ПолучитьКуарНоменклатурыОтУценки(КодУценки);
	Если КуарНоменклатуры_ПривязаннойККуаруУценки = Неопределено тогда возврат КонецЕсли;
	
	//Отбор	=	Новый Структура("КодМаркировки", КуарНоменклатуры_ПривязаннойККуаруУценки);
	//Строки	=	Объект.ТБК_КодыМаркировки.НайтиСтроки(Отбор);    
	
	//Если не Строки.количество() тогда
	//	Сообщить("Этот куар уценки привязан к другой номенклатуре (куару)");
	//	Возврат;
	//КонецЕсли;
	//НужКлючСвязи	=	Строки[0].КлючСвязи-1;
	
	флНашли	=	Ложь; 
	Для каждого Строка из Объект.ТБК_КодыМаркировки цикл   
		Если Лев(Строка.КодМаркировки, 21) = Лев(КуарНоменклатуры_ПривязаннойККуаруУценки, 21) тогда
			флНашли = Истина;  
	...
	Если не флНашли тогда
		Для каждого Строка из Объект.АкцизныеМарки цикл   
			Если Лев(Строка.ТБКОтсканированнаяМарка, 21) = Лев(КуарНоменклатуры_ПривязаннойККуаруУценки, 21) тогда
				флНашли = Истина;  
				НужКлючСвязи	=	Строка.КлючСвязи-1;
				Прервать;
			КонецЕсли;
