	...
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда
			...
			Если СтрЧислоВхождений(Данные,"*") >5 тогда //это куар клиента
				...
				СтруктураБаллов					=	ТБК_ПрограммаЛояльности.ПолучитьКоличествоБаллов(Объект.ТБК_IDКлиента, ПеремТовары);
...
Процедура ТБКСоздатьЧекиЗавершение1(Отказ, ПечатьПакетаДокументовВозврат, МассивСсылокЧеков, Печать)
	...
	Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) и НужныйЧек <> Неопределено тогда
		...
		Если ПокупкаСотрудника			 тогда флНадоНачислить	=	Ложь; КонецЕсли;
...
Процедура ТБКОбновитьКоличествоБалловКоторыеМожноСписатьПосле(Команда)
	Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) тогда
		...
		КоличествоБалловМожноСписать	=	ТБК_ПрограммаЛояльности.ОбновитьКоличествоБаллов(КоличествоНакопленныхБаллов, Перем_Товары);	
	КонецЕсли;		
КонецПроцедуры
...
Процедура СписатьБаллы(Результат, ДополнительныеПараметры) экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) тогда
			Перем_Товары	=	Объект.Товары;
			КоличествоБалловСписано			= ТБК_ПрограммаЛояльности.СписатьБаллы(Число(КоличествоНакопленныхБаллов), Перем_Товары);	
