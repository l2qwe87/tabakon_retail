...
Процедура ТБКОбновитьФорму()
	...
	текущегоПродавца = ОбщегоНазначенияВызовСервера.ПолучитьТекущегоПродавца();
			...
			Если НайденнаяФорма = Неопределено тогда
				...
				Для каждого ОкноПриложения Из ОкнаПриложения Цикл 
					Если Найти(ОкноПриложения.Заголовок, "Табакон") тогда 
						...
						AutoItX3.WinMove(ОкноПриложения.Заголовок, "", СдвигПоРазрешениюЭкрана,0 ); 
					КонецЕсли;
				КонецЦикла;	
...
Процедура ТБКПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	НаборАйкос3			=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS 3 DUOS (золотой) + 3 пачки стиков HEETS (T/SB/A)");
	НаборАйкос			=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS 3 DUOS (синий) + 3 пачки стиков HEETS (T/SB/A)");
	НаборАйкос4			=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS 3 DUOS (золотой) + 3 пачки стиков HEETS (голубой) (A/P/R/)");
	НаборАйкос5			=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS 3 DUOS (синий) + 3 пачки стиков HEETS (голубой) (A/P/R/)");
	НаборАйкосЛил		=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS LIL SOLID 2.0  Графитовый + 3 пачки стиков FIIT (RD/V/S)"); 
	НаборАйкосЛил1		=	Справочники.Номенклатура.НайтиПоНаименованию("Набор  iQOS LIL SOLID 2.0  Синий + 3 пачки стиков FIIT (RD/V/S)");
	...
	Для Каждого стр из Объект.Товары Цикл
		...
		Если Стр.Номенклатура =  НаборАйкос3 или Стр.Номенклатура =  НаборАйкос или Стр.Номенклатура =  НаборАйкосЛил или Стр.Номенклатура =  НаборАйкосЛил1 или
			Стр.Номенклатура =  НаборАйкос4 или Стр.Номенклатура =  НаборАйкос5 тогда
			
			КолМарокДолжноБыть = КолМарокДолжноБыть + 3;	
		КонецЕсли;
	КонецЦикла;  
...
Процедура ТБКПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	//ТекстФоновойПроцедуры = " 
	//|Соед = ОбщегоНазначенияВызовСервера.ПолучитьПрокси(""123456"", ""WS_User"", ""http://mx.tbkon.ru:1777/ut/ws/tbk?wsdl"", 10);
	//|Массив	=	новый Массив;
	...
	Попытка
		Соед = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User", "http://mx.tbkon.ru:1777/ut/ws/tbk?wsdl", 10, Истина);
		Массив	=	новый Массив;
		Массив.Добавить(Соед);
	...
	Если ТипЗнч(сведенияМарок) = Тип("Массив") Тогда
		Если сведенияМарок.Количество() = 1 Тогда
			...
			Для каждого ДочерняяМарка из ДочерниеМарки_Массив цикл 
				...
				//Вик 2023-02-13
				Если Найти(данные,"7623900217823") тогда
					Элементы.Товары.ТекущиеДанные.Цена			=	150;
					Элементы.Товары.ТекущиеДанные.Сумма			=	150 * Элементы.Товары.ТекущиеДанные.Количество;
					Элементы.Товары.ТекущиеДанные.СуммаВсего	=	150 * Элементы.Товары.ТекущиеДанные.Количество;
				КонецЕсли;				
				//КонецВик
			КонецЦикла;   
		КонецЕсли;
...
Процедура ТБКСоздатьЧекиЗавершение1(Отказ, ПечатьПакетаДокументовВозврат, МассивСсылокЧеков, Печать)
	...
	//Вик 2022_04_22 	
	Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) и НужныйЧек <> Неопределено тогда
		...
		Для каждого Строка из Объект.Товары цикл
			...
			Если Строка.СуммаРучнойСкидки > 0 и не  ЗначениеЗаполнено(Строка.КодУценки)	тогда флНадоНачислить	=	Ложь; КонецЕсли; 
	    ...
	    Если флНадоСделатьЗапись тогда
			ДобавитьЗаписьВИсториюПокупок(НужныйЧек, Объект.ТБК_IDКлиента, флНадоНачислить, КолСписано);
		КонецЕсли;
...
&НаСервере
Процедура ДобавитьЗаписьВИсториюПокупок(НужныйЧек, ТБК_IDКлиента, флНадоНачислить, КолСписано)
...
Процедура СписатьБаллы(Результат, ДополнительныеПараметры) экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) тогда
			...
			КоличествоБалловСписано			= ТБК_ПрограммаЛояльности.СписатьБаллы(Число(КоличествоНакопленныхБаллов), Перем_Товары, Адрес_ВебСоединение);	
...
Функция ПодключитьсяКУТ()
	Попытка
		Веб_Соединение = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User", "http://mx.tbkon.ru:1777/ut/ws/tbk?wsdl", 10);
		Массив	=	новый Массив;
...
Процедура ОбработкаИндикацииКнопкичЧат(флНужноВключитьИндикацию)
	Если флНужноВключитьИндикацию тогда
		Элементы.ЧатСофисом.ЦветТекста = WebЦвета.Красный;
	иначе
		Элементы.ЧатСофисом.ЦветТекста = WebЦвета.Черный;
	КонецЕсли;
КонецПроцедуры
