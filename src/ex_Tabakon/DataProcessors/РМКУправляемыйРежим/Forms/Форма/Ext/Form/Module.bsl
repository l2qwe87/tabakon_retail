...
Процедура ТБКСоздатьЧекиЗавершение1(Отказ, ПечатьПакетаДокументовВозврат, МассивСсылокЧеков, Печать)
	...
	Если ЗначениеЗаполнено(Объект.ТБК_IDКлиента) и НужныйЧек <> Неопределено тогда
		ДобавитьЗаписьВИсториюПокупок(НужныйЧек, Объект.ТБК_IDКлиента);
		Объект.ТБК_IDКлиента	=	"";
	КонецЕсли;
	//
КонецПроцедуры 
...
&НаСервере
Процедура ДобавитьЗаписьВИсториюПокупок(НужныйЧек, ТБК_IDКлиента)
...
Процедура ОбработатьСкидкиИзУТ(флНадо, ФлПринудительныйПересчет = Ложь)
	...
	Для каждого Строка из рез цикл
		...
		Если ТипАкции = "Подарок за товар" тогда
			ОбработатьСкидкуНаПодарокЗаТовар(ТоварВПодарок, Число(СуммаЧека), Номенклатура, Число(КоличествоЧеков), Строка.Идентификатор, флНадо, ФлПринудительныйПересчет);
		КонецЕсли;
...
&НаСервере
Процедура ОбработатьСкидкуНаПодарокЗаТовар(ТоварВПодарок,СуммаЧека, Номенклатура, КоличествоЧеков, Идентификатор,  флНадо, ФлПринудительныйПересчет = Ложь)
	
	//Проверяем выполняется ли условие по сумме
	Если СуммаЧека >0 тогда
		Если Объект.Товары.Итог("СуммаВсего") < СуммаЧека тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;	
	
	//Проверяем выполняется ли условие по номенклатуре. Хотя бы 1 товар в чеке должен попадать в список номенклатуры
	Если Номенклатура <> "" тогда
		...
		Если не флВсеНорм тогда Возврат КонецЕсли;
	
	КонецЕсли;
	
	//Проверяем выполняется ли условие по количеству чеков
	Если КоличествоЧеков > 0 тогда
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТБКПродажиПоАкции.Чек) КАК КолЧек
		      	 	             |ИЗ
		      	 	             |	РегистрСведений.ТБКПродажиПоАкции КАК ТБКПродажиПоАкции
		      	 	             |ГДЕ
		      	 	             |	ТБКПродажиПоАкции.ИдентификаторАкции = &ИдентификаторАкции");
		Запрос.УстановитьПараметр("ИдентификаторАкции",Идентификатор);
		Рез = Запрос.Выполнить().Выгрузить();
		
		Если Рез.Количество() >0 тогда
			Если Рез[0].КолЧек >= КоличествоЧеков тогда
				Возврат;				
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	
...
Процедура ТБКОповещениеОткрытьФормуВыбораПричиныВозврата(РезультатОткрытияФормы, ДополнительныеПараметры)
	Если РезультатОткрытияФормы <> Неопределено Тогда
		Объект.ТБК_IDКлиента	=	ОбработатьИсториюПокупокПриВозврате(РезультатОткрытияФормы, ДополнительныеПараметры);
	КонецЕсли; 
КонецПроцедуры
