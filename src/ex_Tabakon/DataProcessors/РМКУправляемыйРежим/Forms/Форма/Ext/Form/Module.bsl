...
Процедура ТБКПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	//Вик 2021_07_01
	Для Каждого Стр из Объект.Товары Цикл
		Если ТекущаяДата() > '20230601' и ЗначениеЗаполнено(Стр.Характеристика) и Стр.Цена < 119 тогда
			Отказ = Истина;
			Сообщить("Для продажи сигарет < 119р. воспользуйтесь БК");
			
		ИначеЕсли ТекущаяДата() > '20230401' и ЗначениеЗаполнено(Стр.Характеристика) и Стр.Цена < 117 тогда
			Отказ = Истина;
			Сообщить("Для продажи сигарет < 117р. воспользуйтесь БК");
		КонецЕсли;
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	Если ТекущаяДата() >'20230501' тогда
...
Процедура ТБКСоздатьЧекиЗавершение1(Отказ, ПечатьПакетаДокументовВозврат, МассивСсылокЧеков, Печать)
	...
	//Марк
	Если СтрДлина(СокрЛП(элементы.НомерЗаказаПокупателя.ТекстРедактирования)) = 5 ИЛИ СтрДлина(СокрЛП(элементы.НомерЗаказаПокупателя.ТекстРедактирования)) = 4 Тогда	
		ЗакрытьЗаказКлиента(элементы.НомерЗаказаПокупателя.ТекстРедактирования, МассивСсылокЧеков);
		Элементы.НомерЗаказаПокупателя.ОбновитьТекстРедактирования();
		Элементы.НомерЗаказаПокупателя.ОбновлениеТекстаРедактирования = ОбновлениеТекстаРедактирования.Авто;
...
&НаСервере
Процедура ЗакрытьЗаказКлиента(ВведенныйНомерЗаказаПокупателя, НужныйЧек)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаказПокупателя.Ссылка КАК Ссылка
	...
	|	Документ.ЗаказПокупателя КАК ЗаказПокупателя
	|ГДЕ
	|	ЗаказПокупателя.НомерЗаказаНаСайте ПОДОБНО &НомерЗаказаНаСайте
	...
	Запрос.УстановитьПараметр("НомерЗаказаНаСайте", "%"+СокрЛП(ВведенныйНомерЗаказаПокупателя)+"%");
	Запрос.УстановитьПараметр("Дата",НачалоГода(ТекущаяДата()));
	
	Если Запрос.Выполнить().Пустой() Тогда 	Возврат; КонецЕсли; 		
		
	Документ = Запрос.Выполнить().Выгрузить();
	
	Док = Документ[0].Ссылка.получитьОбъект();
	Док.Статус  =  ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Закрыт");
	//Док.НаличиеНезарезервированныхСтрок  = Истина; //  метка, чтобы понимать отправлено изменение по заказу в розницу или нет
	Док.Записать();	
...
Процедура ТБКНомерЗаказаПокупателяАвтоПодборПосле(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрДлина(СокрЛП(Текст)) = 5 ИЛИ СтрДлина(СокрЛП(Текст)) = 4 Тогда
		Элементы.НомерЗаказаПокупателя.ОбновлениеТекстаРедактирования = ОбновлениеТекстаРедактирования.НеИспользовать;
