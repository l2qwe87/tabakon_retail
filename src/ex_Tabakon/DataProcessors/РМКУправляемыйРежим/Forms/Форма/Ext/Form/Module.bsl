...
Функция ПроверитьСтатусыМарок_Сервер()
	...
	марки = новый Массив;
	Для Каждого стр из Объект.Товары Цикл
		Если ЗначениеЗаполнено(стр.Номенклатура) И ОбщегоНазначения.ЭтоМаркированнаяНоменклатура(стр.Номенклатура) тогда
			Если стр.ПроверкаМаркиСтатус = 0 Тогда
				марки.Добавить(стр.Штрихкод);
			КонецЕсли;
		Иначе
	...
	тт = ПолучитьИзВременногоХранилища_Сервер(ЭтаФорма.адресРезультата_ПроверитьСтатусыМарок);
...
Процедура ПроверитьСтатусыМарок_Результат() ЭКСПОРТ
	...
	Для Каждого стр из Объект.Товары Цикл
		Если стр.ПроверкаМаркиСтатус = 0 Тогда
			...
			мкрка = ?(СтрДлина(стр.Штрихкод = 25),Лев(стр.Штрихкод, 25 - 4),стр.Штрихкод);
...
&НаКлиенте
Процедура ОбработатьБлок(Данные)	
	марки = Новый Массив();
...
&НаСервере
Процедура ОбработатьСкидкиИзУТ(флНадо)
	...
	Для каждого Строка из рез цикл
		...
		Если ТипАкции = "Процентная скидка на товар" тогда
			ОбработатьСкидкуПроцентнаяСкидкаНаТовар(Скидка,Номенклатура, флНадо);
		КонецЕсли;
	КонецЦикла; 	
КонецПроцедуры
	...
	Для каждого СтрокаТовары Из Объект.Товары цикл
		...
		Для каждого СтрокаСписка из СзНоменклатура цикл
			Если Найти(ПК,СтрокаСписка.Значение) тогда
				ВСегоПодходящихНоменклатур = ВСегоПодходящихНоменклатур + СтрокаТовары.Количество;
...
&НаСервере
Процедура ОбработатьСкидкуПроцентнаяСкидкаНаТовар(Скидка,Номенклатура, флНадо)
	//Проверяем выполняется ли условие по номенклатуре	
	СзНоменклатура = Новый СписокЗначений;
	...
	Для каждого СтрокаТовары Из Объект.Товары цикл
		...
		Для каждого СтрокаСписка из СзНоменклатура цикл
			Если Найти(ПК,СтрокаСписка.Значение) тогда
				СуммаВозможнойСкидки	=	СуммаВозможнойСкидки + (СтрокаТовары.Цена/100 * Скидка);
	...
	Для каждого СтрокаТовары Из СЗ_СтрокиДляСкидки цикл
		СтрокаТовары.Значение.СуммаРучнойСкидки	=	Окр((СтрокаТовары.Значение.Цена/100) * Скидка,0);
