...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда 
			...
			//Марк 2025-06-18
			Номенклатура = ОбщегоНазначенияВызовСервера.ПолучитьНоменклатуруИзДанныхСканирования(Данные, АвтоматическиПодгружатьНоменклатуру);
			//КонецМарк    
			

			//Вик 2024-09-11 Костыль, эти группы товаров бить по БК, на них наклеены Универсальные марки
			Если СтрДлина(Данные) >= 25 И Номенклатура <> Неопределено тогда
									
				Если ЭтуМаркуБитьПоБК(Данные, Номенклатура) тогда
					Сообщить("Для этой номенклатуры сканируйте штрихкод!");  	
					Возврат; 	
				КонецЕсли; 
				
				//Вик 2024-09-26 Костыль на наборы
				Если СтрДлина(Данные) > 29 и Лев(Строка(Номенклатура),5) = "Набор" тогда 
					ОбщегоНазначенияВызовСервера.ПроставитьПризнакиМаркировкиЕслиНужно(Номенклатура);	
				КонецЕсли;
				//КонецВик
