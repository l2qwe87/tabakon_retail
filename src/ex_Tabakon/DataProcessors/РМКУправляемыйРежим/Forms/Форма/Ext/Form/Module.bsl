...
Процедура ТБКПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	Для Каждого Стр из Объект.Товары Цикл
		...
		Если Стр.Цена >300 и ( Найти(Стр.Номенклатура.ПолныйКод(),"Ю2-00015938") или Найти(Стр.Номенклатура.ПолныйКод(),"Ю2-00015939") ) тогда 
			Отказ = Истина;
			Сообщить("Нельзя пробивать электронки дороже 300р. Возможно вы пробиваете блок");
		КонецЕсли;
	КонецЦикла;;
	//КонецВик
...
//&НаКлиенте
//Процедура ТБКТоварыПриОкончанииРедактированияПосле(Элемент, НоваяСтрока, ОтменаРедактирования)
//	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
//	Если ЭтоМаркированнаяНоменклатура(ТекущиеДанные.Номенклатура) тогда  
//		ТекущиеДанные.ПроверкаМаркиСтатус	=	1;	
//	КонецЕсли;
//КонецПроцедуры
