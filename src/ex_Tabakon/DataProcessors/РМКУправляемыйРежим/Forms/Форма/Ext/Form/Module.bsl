...
// ККТ текущей кассы ККМ, последнее показанное состояние связи с ней и настройки
// подключения к ТС ПИоТ. См. область ТБК_СвязьСОборудованием. Марк 2026-09-14.
&НаКлиенте
Перем ТБК_УстройствоККТ, ТБК_УстройствоОпределено, ТБК_СостояниеСвязиККТ, ТБК_НастройкиПИоТ,
	ТБК_СвязьПроверена;
...
Процедура ТБКОбновитьФормуНаКлиенте()	
	Если Объект.Товары.Количество()=0 тогда
		...
		Ваучер = Неопределено;
		СуммаСкидкиВаучера = Неопределено;
...
Процедура ТБКПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	Для Каждого Стр из Объект.Товары Цикл
		...
		//Марк 2026-02-10 https://jira.tabkn.ru/browse/SIG-13
		Если ТекущаяДата() > '20260401' и ЗначениеЗаполнено(Стр.Характеристика) и Стр.Цена < 153 тогда
			Отказ = Истина;
			Сообщить("Для продажи сигарет < 153р. воспользуйтесь БК"); 
		//КонецМарк
		//Марк 2025-03-24	
		ИначеЕсли ТекущаяДата() > '20250401' и ЗначениеЗаполнено(Стр.Характеристика) и Стр.Цена < 135 тогда
			Отказ = Истина;
		...
		Если 
			//Марк 2025-12-30 https://jira.tabkn.ru/browse/KRO-90
			НЕ СтрНайти(Строка(Стр.Номенклатура), "Набор") И
			//КоенцМарк
			Стр.Цена >300 и ( Найти(ПК,"Ю2-00015938") или Найти(ПК,"Ю2-00015939") ) тогда 
			Отказ = Истина;
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	//Марк 2026-09-14. Индикация связи с ККТ — обработчик читает список подключенного
	//оборудования в памяти клиента, к драйверу и к серверу не обращается.
	ПодключитьОбработчикОжидания("ТБК_ОбновитьИндикациюККТ",10);
	//КонецМарк
	
	Парам	=	Новый Структура;
	Парам.Вставить("КассаККМ",ТекущаяКассаККМ);
	ОткрытьФорму("ОбщаяФорма.ТБКФормаВыбораТипаПродавцаИВремениРаботы",Парам,,,,,Новый ОписаниеОповещения("ТБК_ПослеЗакрытияФормыПродавца", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	...
	Элементы.ГруппаТБК_2.Видимость		                 	=	Ложь;
	...
	//ПодключитьОбработчикОжидания("ОбновитьСтатусИнтернета_Результат",5);
	...
	Дата18	=	ДобавитьМесяц(ТекущаяДата(), -18*12)-24*60*60;//вик 2026-04-13
КонецПроцедуры
...
процедура ОбновитьСтатусИнтернета()
	...
	//Марк 2025-11-18 https://jira.tabkn.ru/browse/DV1C-191
	Попытка
	//КонецМарк
		PingInternet();	
	...
	//ТекущийПинг = ПолучитьПингWMI("77.88.8.8");
	//Элементы.ТекстОшибкиПодключенияИнтернета.Заголовок = ТекущийПинг;
	//Если СтрНайти(ТекущийПинг, " мс") Тогда
	//	
...
&НаКлиенте
Функция ПолучитьПингWMI(Адрес)
	Попытка
		Локатор = Новый COMОбъект("WbemScripting.SWbemLocator");
		СервисWMI = Локатор.ConnectServer(".", "root\cimv2");
		
		// ИСПРАВЛЕНИЕ: Добавляем StatusCode в запрос или пишем SELECT *
		Запрос = "SELECT ResponseTime, StatusCode FROM Win32_PingStatus WHERE Address = '" + Адрес + "'";
		Результаты = СервисWMI.ExecQuery(Запрос);
		...
		Для каждого Результат Из Результаты Цикл
			// StatusCode = 0 означает успешный пинг
			Если Результат.StatusCode = 0 Тогда
				ТекущийПинг = Результат.ResponseTime;
				Возврат Формат(ТекущийПинг, "ЧГ=0") + " мс";
			ИначеЕсли Результат.StatusCode = 11010 Тогда
				Возврат "Тайм-аут (нет ответа)";
			ИначеЕсли Результат.StatusCode = 11003 Тогда
				Возврат "Узел недоступен";
			Иначе
				Возврат "Ошибка сети: " + Результат.StatusCode;
			КонецЕсли;
		КонецЦикла;
	Исключение 
...
Процедура ТБКТекущаяКассаККМПриИзмененииПосле(Элемент) 
	...
	//Марк 2026-09-14. Смена кассы ККМ меняет ККТ — перечитать устройство и индикацию.
	ТБК_ОбновитьУстройствоККТ();
	//КонецМарк
...
Процедура ТБК_ОткрытьЗакрытьМенюПрочихОпераций()
	...
	Элементы.ГруппаТБК_2.Видимость			= НЕ Элементы.ГруппаТБК_2.Видимость;
	...
	Для каждого Строка из  ОбъектЧекККМ.Товары цикл
		Если Строка.Сумма = 0 тогда
			...
			Для каждого Строка2 из  ОбъектЧекККМ.Товары цикл
				...
				//Марк 2026-08-13
				Если Найти(Строка2.Номенклатура.ВидНоменклатуры.Наименование,"Табак (х)") тогда
					Продолжить;
				КонецЕсли;
				//КонецМарк
...
Процедура ТБКВнешнееСобытиеВместо(Источник, Событие, Данные)  
	...
	Если ПокупкаСотрудника и Элементы.Товары.Доступность = Ложь тогда
		Сообщить("Это покупка сотрудника. Нельзя добавлять позиции. Пробейте или Аннулируйте чек.");
		Возврат;
	КонецЕсли;
	...
	Если ВводДоступен() Тогда
		...
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") Тогда 
			...
			//Марк 2026-08-24
			Если НЕ ТБКОбработатьСтрокиТЧВозврат(Данные,НоменклатураИзКуара) Тогда
				Возврат;
			КонецЕсли;
			//КонецМарк
			...
			Если СтрДлина(Данные) >= 25 И НоменклатураИзКуара <> Неопределено тогда
				...
				ТБКОтсканированнаяМарка = Данные;
				
				Если ЭтуМаркуБитьПоБК(Данные, НоменклатураИзКуара, Объект.ВидОперации) тогда  
					
					//ROZ-50 меняем сообщение на автомат Вик 2026-09-24
					
					Данные = ОбщегоНазначенияВызовСервера.ПолучитьШтрихкодИзКуара(Данные);  //подмена куара на шк
			...
			//Марк 2026-09-21
			//Марку, по которой в последний час оформлен возврат, продавать нельзя.
			Если СтрДлина(Данные) >= 25 Тогда
				ЗапретПродажиМарки = ТБКЗапретПродажиМаркиПослеВозврата(Данные);
				Если ЗапретПродажиМарки.Запрещено Тогда
					ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(
						НСтр("ru = 'Продажа марки временно запрещена'"),
						ЗапретПродажиМарки.ТекстСообщения);
					Возврат;
				КонецЕсли;
			КонецЕсли;
			//КонецМарк
			...
			// Толстоухов 01.02.2026 ++
			Если СтрЧислоВхождений(Данные, "\") > 10 Тогда
				ОбработатьВаучер(Данные);
				Возврат;
			КонецЕсли;                  
			// Толстоухов 01.02.2026 --
			...
			Если Источник = ТБКDebugИсточникСобытия() Тогда
				// Сканер не подключен, штрихкод введен вручную в поле Debug_Марка.
				// Структура собирается в том же виде, что и в
				// МенеджерОборудованияКлиент.ПреобразоватьДанныеСоСканераВСтруктуру.
				ДанныеСоСканераСтруктура = Новый Структура("Штрихкод, Количество", Данные, 1);
			Иначе
...
Процедура ОбработатьНовуюУценку(Данные)
	...
	Если Найти(КодУценки, "UNI_") тогда
		ОбработатьНовуюУценку_ПоНовому(КодУценки); //2026-07-27 
		Возврат;
	КонецЕсли;
	...
	Для каждого Строка из Объект.Товары цикл
		...
		если ПолучитьКод(НужНом) = КодНоменклатуры тогда     
			...
			Для каждого Строка из Строки цикл
				НужноДобавить	=	НужноДобавить + Строка.СуммаСкидки;	
			КонецЦикла;

			СуммаСкидки = СуммаСкидки + НужноДобавить;
			//
			
			//Марк 2026-04-14 https://jira.tabkn.ru/browse/ 
			//закомментировал
			//ТекСтрока.СуммаРучнойСкидки	=	ТекСтрока.СуммаРучнойСкидки + СуммаСкидки;
			//Обнуляем скидку прошлую
			ТекСтрока.Сумма = ТекСтрока.Сумма + ТекСтрока.СуммаРучнойСкидки;
			ТекСтрока.СуммаРучнойСкидки	= СуммаСкидки;
			//КонецМарк
			...
			НовСтрока.СуммаСкидки		= окр((Строка.Цена/100) * ПроцентСкидки,2);
...
&НаКлиенте
Процедура ОбработатьНовуюУценку_ПоНовому(КодУценки) 
	//2026-09-24 Неожиданно! Выяснилось, что не на все товары, которые требуют уценки, есть куар коды. поэтому костыли

	КуарНоменклатуры__Структура	=	ПолучитьКуарНоменклатурыОтУценки(КодУценки);
	Если КуарНоменклатуры__Структура = Неопределено тогда возврат КонецЕсли;
	
	КуарНоменклатуры = КуарНоменклатуры__Структура.QR;
	НужНоменклатура  	= КуарНоменклатуры__Структура.Номенклатура;
	НужНоменклатура_БК  = ОбщегоНазначенияВызовСервера.ПолучитьНоменклатураБК(НужНоменклатура) ;

	Если СтрДлина(КуарНоменклатуры)>21 тогда
		
		флНашли	=	Ложь; 
		Для каждого Строка из Объект.ТБК_КодыМаркировки цикл   
			Если Лев(Строка.КодМаркировки, 21) = Лев(КуарНоменклатуры, 21) тогда
				флНашли = Истина;  
				НужКлючСвязи	=	Строка.КлючСвязи-1;
				Прервать;
			КонецЕсли;
		КонецЦикла; 
		...
		Если не флНашли тогда
			Для каждого Строка из Объект.АкцизныеМарки цикл   
				Если Лев(Строка.ТБКОтсканированнаяМарка, 21) = Лев(КуарНоменклатуры, 21) тогда
					флНашли = Истина;  
					НужКлючСвязи	=	Строка.КлючСвязи-1;
					Прервать;
				КонецЕсли;
	...
	иначе  
		...
		Для каждого Строка из Объект.Товары цикл
			...
			Если Строка.Номенклатура = НужНоменклатура или Строка.Номенклатура = НужНоменклатура_БК тогда
				НужКлючСвязи = Строка.НомерСтроки-1;  
				флНашли = Истина;
	...
	Если не флНашли тогда
		Сообщить("Этот куар уценки привязан к другой номенклатуре (куару)");
		Возврат;
	КонецЕсли;
	
	ТекСтрока	=	Объект.Товары[НужКлючСвязи];  

	Поз2	=	СтрНайти(КодУценки,"_",,,2);
	Поз3	=	СтрНайти(КодУценки,"_",,,3);
	Поз4	=	СтрНайти(КодУценки,"_",,,4);
	Поз5	=	СтрНайти(КодУценки,"_",,,5);
	
	КодПричины			=	Сред(КодУценки,Поз2+1, Поз3-Поз2-1);
	ПроцентСкидки		=	Сред(КодУценки,Поз3+1, Поз4-Поз3-1);
	НомерДокументаВУТ	=	Сред(КодУценки,Поз4+1, Поз5-Поз4-1); 
	
	Попытка
		ПроцентСкидкиУТ	=	ПолучитьПроцентСкидкиИзУТ(НомерДокументаВУТ, КодУценки);
	Исключение 	
	КонецПопытки;

	Если ПроцентСкидкиУТ <> Неопределено тогда
		ПроцентСкидки = ПроцентСкидкиУТ;	
	КонецЕсли;

	СуммаСкидки	=	окр((ТекСтрока.Цена/100) * ПроцентСкидки,2);
	
	//Вик 2026-07-28	
	Отбор	=	Новый Структура("КлючСвязи",ТекСтрока.НомерСтроки);  
	Строки	=	КодыНовойУценки.НайтиСтроки(Отбор);
	...
	//Марк 2026-04-14 https://jira.tabkn.ru/browse/PRIC-123
	//закомментировал
	//ТекСтрока.СуммаРучнойСкидки	=	ТекСтрока.СуммаРучнойСкидки + СуммаСкидки;
	//Обнуляем скидку прошлую
	ТекСтрока.Сумма 			= ТекСтрока.Сумма + ТекСтрока.СуммаРучнойСкидки;
	...
	НовСтрока.КлючСвязи			= ТекСтрока.НомерСтроки;
	НовСтрока.ПричинаУценки		= КодПричины;
	НовСтрока.ПроцентСкидки		= Число(ПроцентСкидки);
	НовСтрока.СуммаСкидки		= окр((ТекСтрока.Цена/100) * ПроцентСкидки,2);
...
&НаСервере
Процедура ОбработатьКодыНовойУценки()
	Копия	=	КодыНовойУценки.Выгрузить();	
	Копия.Свернуть("КлючСвязи" , "СуммаСкидки" );

	Для каждого Строка из Копия цикл
		Отбор	=	Новый Структура("НомерСтроки",Строка.КлючСвязи); 
		Строки	=	Объект.Товары.НайтиСтроки(Отбор); 
		Строки[0].СуммаРучнойСкидки	=	Строка.СуммаСкидки;
		Строки[0].Сумма				=	Строки[0].КоличествоУпаковок*Строки[0].Цена - Строки[0].СуммаРучнойСкидки;
...
&НаСервереБезКонтекста
Функция ПолучитьКуарНоменклатурыОтУценки(КодУценки)
	Запрос	=	Новый запрос("ВЫБРАТЬ
	      	 	             |	ТБКУценкаТовары.Номенклатура КАК Номенклатура,
	      	 	             |	ТБКУценкаТовары.QR КАК QR,
	      	 	             |	ТБКУценкаТовары.QR_Уценки КАК QR_Уценки
	      	 	             |ИЗ
	      	 	             |	Документ.ТБКУценка.Товары КАК ТБКУценкаТовары
	      	 	             |ГДЕ
	      	 	             |	ТБКУценкаТовары.QR_Уценки ПОДОБНО &QR_Уценки");
	Запрос.УстановитьПараметр("QR_Уценки", "%" + КодУценки + "%");   
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() тогда        
		//Возврат	Рез.QR;
		Структура = новый Структура;   
		Структура.Вставить("QR", Рез.QR);
		Структура.Вставить("Номенклатура", Рез.Номенклатура);
		Возврат Структура;
	иначе
		Сообщить("Не найден код уценки в базе");
		Возврат Неопределено;
	КонецЕсли; 
...
&НаСервере
Функция ПолучитьПроцентСкидкиИзУТ(НомерДокументаВУТ, КодУценки)
	
	Если Адрес_ВебСоединение  = "" тогда
		Веб_Соединение	=	ПодключитьсяКУТ();
		Если Веб_Соединение = Неопределено тогда возврат Неопределено КонецЕсли;
	иначе
		Попытка
			Массив = ПолучитьИзВременногоХранилища(Адрес_ВебСоединение);
	...
	Ответ	=	Веб_Соединение.GetPercentYcenka(НомерДокументаВУТ, КодУценки);
...
Функция ЭтоАйкос(Номенклатура)
	...
	Возврат Найти(ПК,"Ю2-00014185") или Найти(ПК,"00048208784") или Найти(ПК,"Ю2-00046531") или Найти(ПК,"Ю2-00048161") или Найти(ПК,"Ю2-00057415");     // Толстоухов 15.12.2025 PRIC-99 ++ //Ю2-00057415
КонецФункции
...
Процедура ПогаситьКодыНовойУценки()	    
	Если  ТабличноеПолеЧеков.Количество()>0 тогда
		НужЧек	=	ТабличноеПолеЧеков[0].ЧекККМСсылка; 
	иначе
		Возврат;
	КонецЕсли;    
	
	Если Не НужЧек.Проведен тогда возврат КонецЕсли;
...
Процедура ТБКDebug_ВнешнееСобытиеПосле(Команда)
	...
	ТБКDebugЭмулироватьСканирование();

КонецПроцедуры

// Эмулирует сканирование штрихкода, введенного в поле Debug_Марка, когда сканер не подключен.
// Данные передаются в обработчик внешнего события формы, поэтому дальнейшая обработка идет
// тем же путем, что и при реальном сканировании.
//
&НаКлиенте
Процедура ТБКDebugЭмулироватьСканирование()

	Если Не ЗначениеЗаполнено(Debug_Марка) Тогда
		Возврат;
	КонецЕсли;

	ТБКВнешнееСобытиеВместо(ТБКDebugИсточникСобытия(), "Штрихкод", Debug_Марка);

КонецПроцедуры

// Условный источник внешнего события: признак того, что сканирование эмулировано
// кнопкой Debug, а не получено от подключенного оборудования.
//
// Возвращаемое значение:
//  Строка
//
&НаКлиенте
Функция ТБКDebugИсточникСобытия()
...
Процедура ТБКНазначитьАвтоматическиеСкидкиКлиент(ТолькоСообщенияПослеОформления, НеВыводитьСообщения, ПараметрыДляОплаты)
	//Марк 2026-09-25 В режиме "только сообщения" типовая вызывает этот метод уже после оплаты,
	//перед созданием чеков. Пересчитывать акции здесь нельзя: деньги приняты по прежней сумме,
	//и чек разошелся бы с оплатой. Акции перед оплатой считает ТБКПересчитатьСкидкиПередОплатой.
		...
		Если СтруктураПодарки.Свойство("СЗПодарки") тогда
			Если СтруктураПодарки.СЗПодарки.Количество()>0 тогда
				...
				Если ОбработанныеАкцииПоПодаркам.НайтиСтроки(ОТборУидАкции).Количество() = 0 тогда
					...
					//Марк 2026-08-13
					//товар, за который выдается подарок - на него уйдет копеечная скидка
					НоменклатураОснования = Неопределено;
					СтруктураПодарки.Свойство("НоменклатураОснования", НоменклатураОснования);

					ПараметрыОповещения	=	Новый Структура;
					ПараметрыОповещения.Вставить("СтрокаАкции",				НовСтрока);
					ПараметрыОповещения.Вставить("НоменклатураОснования",	НоменклатураОснования);

					Оповещение = Новый ОписаниеОповещения("ПослеОткрытияФормы_ТБКФормаВыбораПодарка",ЭтотОбъект,ПараметрыОповещения);
                    //КонецМарк
...
Процедура ОбработатьСкидкиИзУТ(флНадо, ФлПринудительныйПересчет = Ложь, СтруктураПодарки = Неопределено)
	...
	Если КодыНовойУценки.Количество() >0 тогда 
		ОбработатьКодыНовойУценки();//если пробивать несоклько позиций, то скидка по уценке сбрасывается. поэтому ее нужно проставлять повторно 
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьПодарокЗаТовар(ТоварВПодарок, Номенклатура, КоличествоНоменклатуры, СуммаЧека,  КоличествоЧеков, СтруктураПодарки, Идентификатор, НоменклатураИсключение)
	...
	СуммаПодходящая			=	0;
	//Марк 2026-08-13
	НоменклатураОснования = Неопределено; //товар, за который выдается подарок
	//КонецМарк
		...
		Для каждого СтрокаТовары Из Объект.Товары цикл
			...
			Для каждого СтрокаСписка из СзНоменклатура цикл
				Если Найти(ПК,СтрокаСписка.Значение) тогда
					...
					//Марк 2026-08-13
					//запоминаем товар-основание акции - именно на него уйдет копеечная скидка
					Если НоменклатураОснования = Неопределено
						и СтрокаТовары.Сумма > 0.01
						и НЕ Найти(СтрокаТовары.Номенклатура.ВидНоменклатуры.Наименование,"Табак (х)") тогда

						НоменклатураОснования = СтрокаТовары.Номенклатура;
					КонецЕсли;
					//КонецМарк
	...
	СтруктураПодарки.Вставить("НоменклатураОснования", НоменклатураОснования);

КонецПроцедуры
	
//Подбирает строку товаров, на которую можно перенести копеечную скидку за подарок.
//Приоритет - товар, за который выдается подарок. Вид номенклатуры "Табак (х)" исключается:
//это товар с МРЦ, цена ниже МРЦ - нарушение. На остальную табачную продукцию скидка допустима.
//МассивСтрок - массив структур ("Номенклатура, Сумма") в порядке строк Объект.Товары.
//Возвращает индекс строки или -1, если подходящей строки в чеке нет.
&НаСервереБезКонтекста
Функция ТБК_ИндексСтрокиДляКопеечнойСкидки(МассивСтрок, НоменклатураОснования)
	ИндексЗапасной	=	-1;

	Для Индекс = 0 По МассивСтрок.ВГраница() Цикл
		СтрокаТовары	=	МассивСтрок[Индекс];

		//сама строка подарка (0.01) и строки без суммы не подходят - уйдут в ноль или в минус
		Если СтрокаТовары.Сумма <= 0.01 тогда
			Продолжить;
		КонецЕсли;

		Если Найти(СтрокаТовары.Номенклатура.ВидНоменклатуры.Наименование,"Табак (х)") тогда
			Продолжить;
		КонецЕсли;

		Если ЗначениеЗаполнено(НоменклатураОснования) и СтрокаТовары.Номенклатура = НоменклатураОснования тогда
			Возврат Индекс;
		КонецЕсли;

		Если ИндексЗапасной = -1 тогда
			ИндексЗапасной	=	Индекс;
		КонецЕсли;
	КонецЦикла;

	Возврат ИндексЗапасной;
КонецФункции
...
Процедура ПослеОткрытияФормы_ТБКФормаВыбораПодарка(Результат, Параметры) экспорт
	...
	Если ТипЗнч(Параметры) = Тип("Структура") тогда
		СтрокаАкции           = Параметры.СтрокаАкции;
		НоменклатураОснования = Параметры.НоменклатураОснования;
	иначе
		СтрокаАкции           = Параметры;
		НоменклатураОснования = Неопределено;
	КонецЕсли;
	//КонецМарк
	...
	Если ЗначениеЗаполнено(Подарок)  тогда
		...
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			МассивСтрок.Добавить(Новый Структура("Номенклатура, Сумма",
				СтрокаТовары.Номенклатура, СтрокаТовары.Сумма));
		КонецЦикла;
		
		ИндексСтрокиДляСкидки	=	ТБК_ИндексСтрокиДляКопеечнойСкидки(МассивСтрок, НоменклатураОснования);

		Если ИндексСтрокиДляСкидки >= 0 тогда
			СтрокаДляСкидки = Объект.Товары[ИндексСтрокиДляСкидки];
			СтрокаДляСкидки.СуммаРучнойСкидки = СтрокаДляСкидки.СуммаРучнойСкидки + 0.01;
			СтрокаДляСкидки.Сумма             = СтрокаДляСкидки.Сумма - 0.01;
			СтрокаДляСкидки.СуммаВсего        = СтрокаДляСкидки.СуммаВсего - 0.01;
		КонецЕсли;
		//КонецМарк
		...
		СтрокаАкции.Подарок = Подарок;
		СтрокаАкции.Сумма   = НоваяСтрока.Сумма;
...
// Толстоухов 01.10.2025 PRIC-96 ++ 
&НаКлиенте
Процедура Подключаемый_КомандаСкидкиАксессуарыIQOSСкидка30()
	
	Для каждого СтрокаТЧ Из Объект.Товары Цикл
		
		Если (СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "00048205496") > 0 или СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "МП-00002778") > 0)
			И Не(СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "Ю2-00002918") > 0 Или СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "Ю2-00002917") > 0) Тогда 
			СуммаБезСкидки 				= СтрокаТЧ.КоличествоУпаковок * СтрокаТЧ.Цена;
			СтрокаТЧ.СуммаРучнойСкидки 	= СуммаБезСкидки * 0.3;
			СтрокаТЧ.Сумма				= СуммаБезСкидки - СтрокаТЧ.СуммаРучнойСкидки;
		КонецЕсли;
...
// Толстоухов 01.10.2025 PRIC-96 ++ 
&НаКлиенте
Процедура Подключаемый_КомандаСкидкиАксессуарыIQOSСкидка90()
	...
	Для каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если (СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "00048205496") > 0 или СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "МП-00002778") > 0)
			И Не(СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "Ю2-00002918") > 0 Или СтрНайти(ПолучитьПолныйКод(СтрокаТЧ.Номенклатура), "Ю2-00002917") > 0) Тогда 
			СуммаБезСкидки 				= СтрокаТЧ.КоличествоУпаковок * СтрокаТЧ.Цена;
			СтрокаТЧ.СуммаРучнойСкидки 	= СуммаБезСкидки * 0.9;
			СтрокаТЧ.Сумма				= СуммаБезСкидки - СтрокаТЧ.СуммаРучнойСкидки;
		КонецЕсли;
...
//Марк 2026-09-25 Пересчет скидок перед открытием формы оплаты.
//Сумма для терминала берется из строк товаров в момент открытия формы оплаты, а типовая
//перед оплатой скидки не пересчитывает: флаг СкидкиРассчитаны в РМК Табакона не сбрасывается.
//Поэтому акции Табакона и уценку пересчитываем здесь явно - тем же вызовом, что и при изменении строки.
//Возвращает Ложь, если пересчет открыл форму выбора подарка: оплату нужно прервать,
//кассир выберет подарок и нажмет оплату еще раз.
&НаКлиенте
Функция ТБКПересчитатьСкидкиПередОплатой()
	
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа")
		Или Объект.ПродажаПоЗаказу
		Или Объект.ОперацияСДенежнымиСредствами
		Или Объект.Товары.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;
	
	КоличествоАкцийСПодаркамиДоПересчета = ОбработанныеАкцииПоПодаркам.Количество();
	
	НазначитьАвтоматическиеСкидкиКлиент(Ложь, Истина);
	
	Возврат ОбработанныеАкцииПоПодаркам.Количество() = КоличествоАкцийСПодаркамиДоПересчета;
...
Процедура ТБКОплатитьТовары(ИмяФормыОплаты, ОплатаПлатежнойКартой = Ложь, ОплатаПлатежнойСистемой = Ложь, Печать = Ложь)
	...
	//Марк 2026-09-25 Скидки пересчитываем до оплаты, чтобы на терминал ушла сумма со скидкой
	Если НЕ ТБКПересчитатьСкидкиПередОплатой() Тогда
		ЭтотОбъект.Доступность = Истина;
		Возврат;
	КонецЕсли;
	//КонецМарк
...
Процедура ДействияПриДобавленииНовойПозиции(Элемент = Неопределено, ОТказ = Ложь); 
	...
	Если Стр <> Неопределено Тогда
		...
		//Вик 2026-07-27               
		НовСтрока	=	Объект.ТБК_КодыМаркировки.Добавить();
		НовСтрока.КлючСвязи		=	Элементы.Товары.ТекущиеДанные.НомерСтроки;
		НовСтрока.КодМаркировки	=	ТекущиеДанныеСоСканера; 
		//КонецВик
		...
		Если Элемент <> Неопределено тогда
			Если  Прав(СокрЛП(Строка(Стр.Номенклатура)),3)=" БК" тогда
				...
				Если Строки.Количество()=0  тогда
					...
					//Вик 2026-09-24
					Если СтрДлина(ТБКОтсканированнаяМарка)> 21 тогда 
						...
						НовСтрока.Марка					=	ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ТБКОтсканированнаяМарка);
						НовСтрока.КоличествоВУпаковке	=	ОбщегоНазначенияВызовСервера.ПолучитьКоличествоВУпаковкеПоКуару(ТБКОтсканированнаяМарка); 	
						НовСтрока.КлючСвязи				=	Элементы.Товары.ТекущиеДанные.НомерСтроки; 					//
...
Процедура ТБКДобавитьНоменклатуруВТабЧастьКлиент(СтруктураПараметров, Знач ОткрыватьРедактированиеСтроки)
	...
	Если СтруктураПараметров.Свойство("Цена") тогда
		МаркаЗамены = "";
		СтруктураПараметров.Свойство("ТБКМаркаЗамены", МаркаЗамены);
		ПричинаЗамены = "";
		СтруктураПараметров.Свойство("ТБКПричинаЗамены", ПричинаЗамены);
...
&НаКлиенте
Процедура ТБКДобавитьНоменклатуруВТабЧасть(Знач Номенклатура, Знач Характеристика, Знач Упаковка, Знач Количество, Знач цена, Знач ПерейтиНаКоличество, Знач СерийныйНомер, ОткрыватьРедактированиеСтроки, Знач МаркаЗамены = "", Знач ПричинаЗамены = "")
	...
	ТБКЗаписатьМаркуЗаменыВБуфер(ТекущаяСтрока, МаркаЗамены, ПричинаЗамены);
...
Процедура ТБКТоварыПередУдалениемПосле(Элемент, Отказ)
	...
	//Марк 2026-03-05 https://jira.tabkn.ru/browse/ROZ-24
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") И НЕ РольДоступнаПолныеПрава() Тогда
		ВызватьИсключение "Запрещено удалять позиции" + Символы.ПС + "Воспользуйтесь аннулированием чека";	
	КонецЕсли;
	//КонецМарк              
...
Процедура ВместоПроцедурыТБКСоздатьЧекиЗавершение(МассивСсылокЧеков)     
	...
	// Толстоухов 05.08.2026 PRIC-137 ++
	Если НужныйЧек <> Неопределено Тогда
		Если ЗначениеЗаполнено(Ваучер) И СуммаСкидкиВаучера > 0 Тогда
			ПогаситьВаучер(НужныйЧек);
		КонецЕсли;
	КонецЕсли;
	// Толстоухов 05.08.2026 --
...
Процедура ВыбратьНужнуюХарактеристикуПоНеобходимости(ИсходныеДанные, ПараметрыСканирования)
	...
	Если ЗначениеЗаполнено(НоменклатураИзКуара) тогда
		Если (найти(Строка(НоменклатураИзКуара), "Bohem") 
			...
			или найти(Строка(НоменклатураИзКуара), "Dakota")) и не Найти(Строка(НоменклатураИзКуара), "Сигареты") тогда
	...
	Если флНуженВыборХарактеристики тогда
		...
		ХарактеристикиПоШК = ПолучитьХарактеристики(ШК, НоменклатураИзКуара, ИсходныйШтрихкод);
...
&НаСервереБезКонтекста
Функция ПолучитьХарактеристики(ШК, Номенклатура, ИсходныйШтрихкод)
	Если ЗначениеЗаполнено(Номенклатура) тогда
	...
	//Вик 2026-08-20 ROZ-46 контроль минимальной цены 
	Если Рез.Количество()>1 тогда
		марки = Новый Массив();
		марки.Добавить(ИсходныйШтрихкод);   
		...
		Попытка
			...
			Если ТипЗнч(сведенияМарок) = Тип("Массив") Тогда
				Если сведенияМарок.Количество() = 1 Тогда
					...
					Если сведения.cisInfo.Свойство("mcGtinCop") Тогда  
						
						МинЦена = сведения.cisInfo.mcGtinCop[0] / 100;
						
						УдалитьСтроки	=	Новый Массив;
						
						Для каждого Строка Из рез Цикл       
							Попытка
								Если Число(Строка.Характеристика.Наименование) <  МинЦена тогда
...
Процедура ПослеОткрытияФормы_ТБКФормаВыбораХарактеристики(Результат, ДопПараметры) экспорт
	Если Результат <> Неопределено тогда
		...
		//Марк 2026-05-15
		Если НЕ ПараметрыСканирования.Свойство("Характеристика") Тогда
			ПараметрыСканирования.Вставить("Характеристика", Результат);
		Иначе
		//	
...
&НаСервереБезКонтекста
Функция ЭтуМаркуБитьПоБК(Данные, Номенклатура, ВидОперации)
	...
	//Вик 2026-08-14
	//Проверка что эту марку уже продали
	Если Строка(ВидОперации) = "Продажа" тогда
		Запрос	=	новый Запрос("ВЫБРАТЬ
		      	 	             |	ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |				КОГДА ЧекККМАкцизныеМарки.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЧекККМ.Продажа)
		      	 	             |					ТОГДА 1
		      	 	             |				ИНАЧЕ -1
		      	 	             |			КОНЕЦ), 0) КАК МаркаПробита
		      	 	             |ИЗ
		      	 	             |	Документ.ЧекККМ.АкцизныеМарки КАК ЧекККМАкцизныеМарки
		      	 	             |ГДЕ
		      	 	             |	ЧекККМАкцизныеМарки.Ссылка.Проведен
		      	 	             |	И (ЧекККМАкцизныеМарки.Ссылка.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Архивный)
		      	 	             |			ИЛИ ЧекККМАкцизныеМарки.Ссылка.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробитый))
		      	 	             |	И (ЧекККМАкцизныеМарки.КодАкцизнойМарки ПОДОБНО &КодАкцизнойМарки
		      	 	             |			ИЛИ ЧекККМАкцизныеМарки.ТБКОтсканированнаяМарка ПОДОБНО &ТБКОтсканированнаяМарка)");  
		
		Запрос.УстановитьПараметр("КодАкцизнойМарки", "%" + Лев(Данные,СтрДлина(Данные)-4) + "%");
		Запрос.УстановитьПараметр("ТБКОтсканированнаяМарка", "%" + Лев(Данные,25) + "%");

		ВыборкаБитьПоБК = запрос.Выполнить().Выбрать(); 

		Если ВыборкаБитьПоБК.Следующий() Тогда
			
			Если ВыборкаБитьПоБК.МаркаПробита >0 Тогда  
				//Сообщить("Эта марка уже была продана! Сканируйте штрихкод.");
				Возврат Истина;
			КонецЕсли;
...
//Марк 2026-08-24 Запрет возврата (отмены чека текущего дня) без сканирования кодов маркировки.
&НаКлиенте
&Вместо("ПодтвердитьВозвратПродукцииИСМПБезМарок")
Процедура ТБКПодтвердитьВозвратПродукцииИСМПБезМарок(ДополнительныеПараметры, СписокНоменклатуры)

	Если НЕ ТБКЗапретВозвратаБезМарокВключен() Тогда
		ПродолжитьВызов(ДополнительныеПараметры, СписокНоменклатуры);
		Возврат;
	КонецЕсли;

	СписокБезПовторов = Новый СписокЗначений;
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл
		Если СписокБезПовторов.НайтиПоЗначению(ЭлементСписка.Значение) = Неопределено Тогда
			СписокБезПовторов.Добавить(ЭлементСписка.Значение);
		КонецЕсли;
	КонецЦикла;

	ТекстИнформации = НСтр("ru = 'Не отсканированы коды маркировки:'") + Символы.ПС;
	Для Каждого ЭлементСписка Из СписокБезПовторов Цикл
		ТекстИнформации = ТекстИнформации + Символы.ПС + Строка(ЭлементСписка.Значение);
	КонецЦикла;
	ТекстИнформации = ТекстИнформации + Символы.ПС + Символы.ПС
		+ НСтр("ru = 'Отсканируйте QR-код (DataMatrix) возвращаемого товара.'") + Символы.ПС
		+ НСтр("ru = 'Возврат без сканирования запрещен.'");

	ЭтотОбъект.Доступность = Истина;

	ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(
		НСтр("ru = 'Возврат маркируемой продукции'"),
		ТекстИнформации);

КонецПроцедуры

// Запрет включен по умолчанию. Выключается записью значения Ложь в
// РегистрСведений.ТБК_Константы по имени "ЗапретВозвратаБезСканированияМарок".
&НаСервереБезКонтекста
Функция ТБКЗапретВозвратаБезМарокВключен()

	Возврат НЕ (ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("ЗапретВозвратаБезСканированияМарок") = Ложь);

КонецФункции
//КонецМарк

&НаКлиенте
Процедура ТБКИнкасацияПосле(Команда)
	УжеСозданнаяИнкассация = ОбщегоНазначенияВызовСервера.ПолучитьИнкасациюЗаСегодня();
	
	Если ЗначениеЗаполнено(УжеСозданнаяИнкассация) Тогда
		ПараметрыОткрытия = Новый Структура("Ключ", УжеСозданнаяИнкассация);
		ОткрытьФорму("Документ.ТБКИнкасация.Форма.ФормаДокумента",ПараметрыОткрытия);
	иначе	
		ОткрытьФорму("Документ.ТБКИнкасация.Форма.ФормаДокумента");  
	КонецЕсли;

КонецПроцедуры

// Толстоухов 10.03.2026 ++
&НаКлиенте
Процедура ТБКДвиженияТоваровПосле(Команда)
	ОткрытьФорму("Отчет.ТБК_ДвиженияТоваров.Форма");
КонецПроцедуры

&НаКлиенте
Процедура ТБКОткрытьАкцииПосле(Команда)
	ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБК_Акции");
КонецПроцедуры

&НаКлиенте
Процедура ТБКОткрытьСпискиПосле(Команда)
	ОткрытьФорму("Документ.ТБК_ЗаявкаОтМагазина.Форма.Списки");
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработатьВаучер(Данные)
	Если ПокупкаСотрудника тогда Возврат КонецЕсли;
	Если Не Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") Тогда  Возврат КонецЕсли;
	
	ПолучитьТабличнуюЧасть(Данные);		
	ОбработкаТабличнойЧастиТоварыКлиентСервер.ЗаполнитьСуммуВсегоВТаблице(Объект.Товары, Объект.ЦенаВключаетНДС);		
	ОбновитьФормуНаКлиенте(); 
КонецПроцедуры

Процедура ПолучитьТабличнуюЧасть(Данные) Экспорт
	
	Ваучер = ТБК_Шифрование.ПолучитьРасшифрованныйТекст(Данные, "777", "\");
	
	Если ПогашенВаучер(Ваучер) Тогда
		Сообщить("Ваучер уже погашен");
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Структ.Вставить("Магазин", Строка(НужСклад));
		Структ.Вставить("Ваучер", Строка(Ваучер));
		
		ОбщаяСуммаДоРаспределения = 0;
		ОбщаяСуммаПослеРаспределения = 0;
		ВаучерПрименен = Ложь;
		...
		Для каждого Строка из Объект.Товары Цикл
			...
			ДляЗаписи.Вставить("Номенклатура",			Строка(Строка.Номенклатура));
			ДляЗаписи.Вставить("НоменклатураКод",		Строка(Строка.Номенклатура.Код));
			ДляЗаписи.Вставить("Количество",			Строка(Строка.Количество));
			ДляЗаписи.Вставить("Цена",				    Строка(Строка.Цена));
			ДляЗаписи.Вставить("НомерСтроки",			Строка(Строка.НомерСтроки));
			
			МассивТоваров.Добавить(ДляЗаписи);
			
			ОбщаяСуммаДоРаспределения = ОбщаяСуммаДоРаспределения + Строка.Количество * Строка.Цена;
			
		КонецЦикла;
		
		Структ.Вставить("Товары", МассивТоваров);
		
		JS = ОбщегоНазначения.СтруктураВjson_Общая(Структ); 
		
		Ответ =	Веб_Соединение.Voucher(JS);
		...
		Если ЗначениеЗаполнено(Ответ) Тогда
			Если не Найти(Ответ,"Внимание!") Тогда 
				...
				Для каждого СтрокаМассива из ОтветМассив Цикл 
					Строка = Объект.Товары[СтрокаМассива.НомерСтроки - 1];
					Строка.Цена = СтрокаМассива.Цена;
					Строка.Сумма = Строка.Цена * Строка.Количество;
					
					ВаучерПрименен = Истина;
				КонецЦикла; 
			Иначе
				Сообщить(Ответ); 		
			КонецЕсли;
		...
		Если ВаучерПрименен Тогда
			Для каждого Строка из Объект.Товары Цикл
				ОбщаяСуммаПослеРаспределения = ОбщаяСуммаПослеРаспределения + Строка.Количество * Строка.Цена;
			КонецЦикла;
			
			СуммаСкидкиВаучера = ОбщаяСуммаДоРаспределения - ОбщаяСуммаПослеРаспределения;
			
			Сообщить("Оплата ваучером на сумму " + СуммаСкидкиВаучера);
...
&НаСервере
Процедура ПогаситьВаучер(Чек)	    
	
	Рег = РегистрыСведений.ТБК_ПогашенныйВаучер.СоздатьМенеджерЗаписи();
	
	Рег.QR = Ваучер;
	Рег.СуммаСкидки = СуммаСкидкиВаучера;		
	Рег.Чек	= Чек;	
	Рег.Магазин	= Чек.Магазин.СкладПродажи;	
...
&НаСервере
Функция ПогашенВаучер(Ваучер)	    
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТБК_ПогашенныйВаучер.QR КАК QR
		|ИЗ
		|	РегистрСведений.ТБК_ПогашенныйВаучер КАК ТБК_ПогашенныйВаучер
		|ГДЕ
		|	ТБК_ПогашенныйВаучер.QR = &QR";
	
	Запрос.УстановитьПараметр("QR", Ваучер);
...
// Кладет уже отсканированную марку в буфер ТБКМаркиДляБК, чтобы механизм списания
// марок для БК-позиций отработал штатно и форма ввода марки кассиру не открывалась:
// ДействияПриДобавленииНовойПозиции открывает ее только если марок по КлючСвязи нет.
//
// Параметры:
//  ТекущаяСтрока  - ДанныеФормыЭлементКоллекции - строка Объект.Товары
//  МаркаЗамены    - Строка - полный код маркировки с МРЦ и криптохвостом
//  ПричинаЗамены  - Строка - почему марка не прошла проверку
//
&НаКлиенте
Процедура ТБКЗаписатьМаркуЗаменыВБуфер(ТекущаяСтрока, МаркаЗамены, ПричинаЗамены)

	Если Не ЗначениеЗаполнено(МаркаЗамены) Тогда
		Возврат;
	КонецЕсли;

	// Повторное добавление той же марки к той же строке - при объединении строк в чеке.
	Отбор = Новый Структура("КлючСвязи, Марка", ТекущаяСтрока.НомерСтроки, МаркаЗамены);
	Если Объект.ТБКМаркиДляБК.НайтиСтроки(Отбор).Количество() > 0 Тогда
		Возврат;
	КонецЕсли;

	ДанныеСтроки = Новый Структура;
	ДанныеСтроки.Вставить("Номенклатура",        ТекущаяСтрока.Номенклатура);
	ДанныеСтроки.Вставить("Харкатеристика",      ТекущаяСтрока.Характеристика);
	ДанныеСтроки.Вставить("Марка",               МаркаЗамены);
	ДанныеСтроки.Вставить("КлючСвязи",           ТекущаяСтрока.НомерСтроки);
	ДанныеСтроки.Вставить("КоличествоВУпаковке", ТБККоличествоВУпаковкеМарки(МаркаЗамены));
	ДанныеСтроки.Вставить("Причина",             ПричинаЗамены);

	ЗаполнитьЗначенияСвойств(Объект.ТБКМаркиДляБК.Добавить(), ДанныеСтроки);

КонецПроцедуры

// Количество в упаковке по коду маркировки.
// Повторяет логику формы ТБКФормаВводаМаркиДляБК: для кодов длиннее 29 символов
// количество берется по куару, иначе единица.
//
// Параметры:
//  Марка - Строка - полный код маркировки
//
// Возвращаемое значение:
//  Число
//
&НаКлиенте
Функция ТБККоличествоВУпаковкеМарки(Марка)

	Если СтрДлина(Марка) <= 29 Тогда
		Возврат 1;
	КонецЕсли;

	Попытка
		Возврат ОбщегоНазначенияВызовСервера.ПолучитьКоличествоВУпаковкеПоКуару(Марка);
	Исключение
		Возврат 1;
	КонецПопытки;
...
//Марк 2026-08-24
//При возврате по чеку строка с товаром, которого в чеке нет, добавляться не должна.
&НаСервере
Функция ТБКОбработатьСтрокиТЧВозврат(ДанныеСканирования, Номенклатура)
	
	Если Объект.ВидОперации <> Перечисления.ВидыОперацийЧекККМ.Возврат
		ИЛИ НЕ ЗначениеЗаполнено(Объект.ЧекККМПродажа) Тогда
		Возврат Истина;
	КонецЕсли;
	
	НоменклатураЧека = ТБКНоменклатураЧекаПродажи(Объект.ЧекККМПродажа);
	
	Если Номенклатура <> Неопределено  Тогда
		Если НоменклатураЧека[Номенклатура] <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
	Иначе
		
		СоответствиеАкцизныеМарки = Новый Соответствие;
		Для каждого Строка Из Объект.ЧекККМПродажа.АкцизныеМарки Цикл
			СоответствиеАкцизныеМарки.Вставить(Строка.ТБКОтсканированнаяМарка, Истина); 
		КонецЦикла;
		
		Если СоответствиеАкцизныеМарки[ДанныеСканирования] <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
	...
	Если Номенклатура <> Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Этого товара нет в возвращаемом чеке: %1'"), Строка(Номенклатура));
	Иначе
		ТекстСообщения = НСтр("ru = 'Этого товара нет в возвращаемом чеке'");
	КонецЕсли;
	
	Сообщить(ТекстСообщения);
	
	ЗаписьЖурналаРегистрации("ТБК.ВозвратЛишнийТовар",
		УровеньЖурналаРегистрации.Информация,,,
		ТекстСообщения + Символы.ПС + "Чек продажи: " + Строка(Объект.ЧекККМПродажа));
		
	Возврат Ложь;
	
КонецФункции

//Номенклатура, которая есть в чеке продажи.
&НаСервереБезКонтекста
Функция ТБКНоменклатураЧекаПродажи(Знач ЧекПродажи)
...
//Марк 2026-09-21
//Запрет продажи маркированной продукции, по которой недавно оформлен возврат.
//ГИС МТ возвращает код маркировки в оборот не мгновенно, поэтому сразу после
//возврата продавать эту марку нельзя. Период запрета берется из ТБК_Констант
//(МинутЗапретаПродажиПослеВозврата), по умолчанию 60 минут.
//
// Параметры:
//  ДанныеСканирования - Строка - код маркировки, как его считал сканер
//
// Возвращаемое значение:
//  Структура:
//   * Запрещено      - Булево - Истина, если продавать марку пока нельзя
//   * ТекстСообщения - Строка - что показать кассиру
//
&НаСервере
Функция ТБКЗапретПродажиМаркиПослеВозврата(ДанныеСканирования)
	
	Результат = Новый Структура;
	Результат.Вставить("Запрещено",      Ложь);
	...
	//Первые 25 символов кода - GTIN и серийный номер, они идентифицируют марку.
	ИдентификаторМарки = Лев(ДанныеСканирования, 25);
	Если СтрДлина(ИдентификаторМарки) < 25 Тогда
		Возврат Результат;
	КонецЕсли;
	
	МинутЗапрета = ТБКМинутЗапретаПродажиПослеВозврата();
	Если МинутЗапрета <= 0 Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЧекВозврата.Ссылка КАК ЧекВозврата,
	|	ЧекВозврата.Дата КАК ДатаВозврата,
	|	МаркиВозврата.КодАкцизнойМарки КАК КодАкцизнойМарки,
	|	МаркиВозврата.ТБКОтсканированнаяМарка КАК ОтсканированнаяМарка
	|ИЗ
	|	Документ.ЧекККМ КАК ЧекВозврата
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ.АкцизныеМарки КАК МаркиВозврата
	|		ПО ЧекВозврата.Ссылка = МаркиВозврата.Ссылка
	|ГДЕ
	|	ЧекВозврата.Дата >= &НачалоПериода
	|	И ЧекВозврата.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЧекККМ.Возврат)
	|	И ЧекВозврата.Проведен
	|	И (ЧекВозврата.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробитый)
	|			ИЛИ ЧекВозврата.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Архивный))
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаВозврата УБЫВ";
	
	Запрос.УстановитьПараметр("НачалоПериода", ТекущаяДатаСеанса() - МинутЗапрета * 60);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		//Сравниваем вхождением: в чеке возврата код может лежать и с криптохвостом, и без него.
		Если Найти(Строка(Выборка.КодАкцизнойМарки), ИдентификаторМарки) = 0
			И Найти(Строка(Выборка.ОтсканированнаяМарка), ИдентификаторМарки) = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ДатаРазблокировки = Выборка.ДатаВозврата + МинутЗапрета * 60;
		
		Результат.Запрещено = Истина;
		Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По этой марке в %1 оформлен возврат.
					|Продать ее можно будет после %2.'"),
			Формат(Выборка.ДатаВозврата, "ДЛФ=T"),
			Формат(ДатаРазблокировки, "ДЛФ=T"));
		
		ЗаписьЖурналаРегистрации("ТБК.ЗапретПродажиПослеВозврата",
			УровеньЖурналаРегистрации.Информация,,,
			Результат.ТекстСообщения + Символы.ПС
			+ "Марка: " + ИдентификаторМарки + Символы.ПС
			+ "Чек возврата: " + Строка(Выборка.ЧекВозврата));
...
// Сколько минут после возврата марку продавать нельзя.
// По умолчанию 60. Меняется записью числа в РегистрСведений.ТБК_Константы
// по имени "МинутЗапретаПродажиПослеВозврата", 0 полностью выключает запрет.
//
// Возвращаемое значение:
//  Число - период запрета в минутах
//
&НаСервереБезКонтекста
Функция ТБКМинутЗапретаПродажиПослеВозврата()
	
	ЗначениеКонстанты = ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("МинутЗапретаПродажиПослеВозврата");
	
	Если ТипЗнч(ЗначениеКонстанты) = Тип("Число") И ЗначениеКонстанты > 0 Тогда
...
#Область ТБК_СвязьСОборудованием

// Принудительное подключение ККТ и индикация связи ККТ / ТС ПИоТ.
//
// Типовая РМК при открытии формы подключает только сканер штрихкода, считыватель
// магнитных карт и дисплей покупателя (см. ПриОткрытии), а ККТ занимает лениво —
// при первой фискальной операции. После перезагрузки ПК порт ККТ свободен, ТС ПИоТ
// не инициализирован, и первая же марка уходит в ошибку разрешительного режима (514, 403).
//
// Кнопка "ККТ" занимает порт сразу, кнопка "ПИоТ" опрашивает экземпляр ТС ПИоТ по HTTP.
// Обе подсвечиваются по состоянию связи.
//
// Проверка запускается сама при открытии РМК и при смене кассы ККМ, молча - кнопки
// нужны только чтобы переспросить. Так порт ККТ занимается не дожидаясь первой продажи,
// и кассир не видит красного при живом оборудовании.
//
// Марк 2026-09-14.

&НаКлиенте
Процедура ТБКСвязьСККТПосле(Команда)

	ТБК_УстановитьСвязьСККТ(Истина);

КонецПроцедуры

&НаКлиенте
Процедура ТБКСвязьСПИоТПосле(Команда)

	ТБК_ПроверитьСвязьСПИоТ(Истина);

КонецПроцедуры

// Сбрасывает состояние связи. Вызывается при открытии формы и при смене кассы ККМ.
//
// Ничего тяжелого здесь быть не должно: процедура работает внутри ПриОткрытии,
// и каждый серверный вызов тут удлиняет открытие РМК. Определение ККТ, чтение
// настроек ТС ПИоТ и сама проверка связи выполняются позже - см.
// ТБК_ПослеЗакрытияФормыПродавца.
//
&НаКлиенте
Процедура ТБК_ОбновитьУстройствоККТ()

	ТБК_УстройствоККТ        = Неопределено;
	ТБК_УстройствоОпределено = Ложь;
	ТБК_СостояниеСвязиККТ    = Неопределено;
	ТБК_НастройкиПИоТ        = Неопределено;
	ТБК_СвязьПроверена       = Ложь;

	// До первой проверки красным не пугаем: связь еще не проверяли, а не оборвалась.
	ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьККТ,  НСтр("ru = 'ККТ: проверяется'"), 0);
	ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьПИоТ, НСтр("ru = 'ПИоТ: проверяется'"), 0);

КонецПроцедуры

// Проверка связи после того, как кассир закрыл форму выбора продавца.
//
// Раньше проверка висела на обработчике ожидания и срабатывала через секунду после
// открытия РМК - то есть прямо поверх формы выбора продавца, открытой в режиме
// БлокироватьВесьИнтерфейс. Подключение ККТ занимает секунды и выполняется синхронно,
// поэтому кассир видел зависший заблокированный интерфейс.
//
// Теперь проверка стартует в момент, когда кассир закрыл форму продавца и РМК
// становится рабочей. Форма продавца открывается дважды за одно открытие РМК
// (в ПриОткрытии и в обработчике смены кассы), поэтому проверка помечается
// выполненной - второй раз она не повторяется.
//
&НаКлиенте
Процедура ТБК_ПослеЗакрытияФормыПродавца(Результат, ДополнительныеПараметры) Экспорт

	Если ТБК_СвязьПроверена Тогда
		Возврат;
	КонецЕсли;

	ТБК_СвязьПроверена = Истина;

	ТБК_УстановитьСвязьСККТ(Ложь);

КонецПроцедуры

// Определяет ККТ текущей кассы ККМ. Серверный вызов, поэтому результат кешируется
// до следующей смены кассы.
//
&НаКлиенте
Процедура ТБК_ОпределитьУстройствоККТ()

	Если ТБК_УстройствоОпределено Тогда
		Возврат;
	КонецЕсли;

	ТБК_УстройствоОпределено = Истина;
	ТБК_УстройствоККТ        = Неопределено;

	Если Не ИспользоватьПодключаемоеОборудование
		Или ОтправкаЧековЧерезСервис
		Или Не ЗначениеЗаполнено(ТекущаяКассаККМ) Тогда
		Возврат;
	КонецЕсли;

	ПараметрыКассыККМ = ЗначениеНастроекВызовСервера.ПараметрыКассыККМ(ТекущаяКассаККМ);
	...
	Если Не ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования
		...
		ТБК_УстройствоККТ = ПараметрыКассыККМ.ИдентификаторУстройства;
	КонецЕсли;

КонецПроцедуры

// Индикация состояния связи на кнопке.
//
// Параметры:
//  Кнопка    - КнопкаФормы - подсвечиваемая кнопка
//  НадписьКнопки - Строка  - новый заголовок кнопки
//  Состояние - Число       - 1 связь есть, -1 связи нет, 0 состояние не известно
//
&НаКлиенте
Процедура ТБК_УстановитьИндикацию(Кнопка, НадписьКнопки, Состояние)

	Кнопка.Заголовок = НадписьКнопки;

	Если Состояние = 1 Тогда
		Кнопка.ЦветФона   = Новый Цвет(198, 239, 206);
		Кнопка.ЦветТекста = Новый Цвет(0, 97, 0);
	ИначеЕсли Состояние = -1 Тогда
		Кнопка.ЦветФона   = Новый Цвет(255, 199, 206);
		Кнопка.ЦветТекста = Новый Цвет(156, 0, 6);
	Иначе
		Кнопка.ЦветФона   = Новый Цвет(255, 235, 156);
		Кнопка.ЦветТекста = Новый Цвет(156, 101, 0);
	КонецЕсли;

КонецПроцедуры

// Связь с ККТ есть, если устройство уже подключено в этом сеансе 1С.
// Проверка идет по списку подключенного оборудования в памяти клиента,
// обращения к драйверу и к серверу нет — можно дергать по таймеру.
//
&НаКлиенте
Функция ТБК_СвязьСККТУстановлена()

	Если ТБК_УстройствоККТ = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	Попытка
		Возврат МенеджерОборудованияРМККлиент.ПолучитьПодключенноеУстройство(ТБК_УстройствоККТ) <> Неопределено;
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
// Обработчик ожидания: держит индикацию ККТ в актуальном состоянии.
// Если порт займет типовой код при первой продаже, кнопка позеленеет сама.
//
&НаКлиенте
Процедура ТБК_ОбновитьИндикациюККТ()

	// Пока ККТ не определена, на кнопке честное "проверяется" - не затираем его.
	Если Не ТБК_УстройствоОпределено Тогда
		Возврат;
	КонецЕсли;

	Если ТБК_УстройствоККТ = Неопределено Тогда
		НовоеСостояние = 0;
		НовыйЗаголовок = НСтр("ru = 'ККТ: не используется'");
	ИначеЕсли ТБК_СвязьСККТУстановлена() Тогда
		НовоеСостояние = 1;
		НовыйЗаголовок = НСтр("ru = 'ККТ: связь есть'");
	Иначе
		НовоеСостояние = -1;
		НовыйЗаголовок = НСтр("ru = 'ККТ: нет связи'");
	КонецЕсли;

	// Перерисовывать кнопку каждые несколько секунд незачем — только при смене состояния.
	Если ТБК_СостояниеСвязиККТ = НовоеСостояние Тогда
		Возврат;
	КонецЕсли;

	ТБК_СостояниеСвязиККТ = НовоеСостояние;
	ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьККТ, НовыйЗаголовок, НовоеСостояние);

КонецПроцедуры

// Занимает порт ККТ через подключаемое оборудование.
//
// Параметры:
//  ПоказыватьСообщения - Булево - показывать кассиру результат.
//                                 Ложь при автоматическом вызове из цепочки подключения.
//
&НаКлиенте
Процедура ТБК_УстановитьСвязьСККТ(ПоказыватьСообщения)

	ЗаголовокСообщения = НСтр("ru = 'Связь с ККТ'");

	ТБК_ОпределитьУстройствоККТ();

	Если ТБК_УстройствоККТ = Неопределено Тогда

		ТБК_ОбновитьИндикациюККТ();
		ТБК_ПроверитьСвязьСПИоТ(Ложь);

		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'Для текущей кассы ККМ не указано подключаемое оборудование
				|либо касса работает без подключения оборудования.'"));
		КонецЕсли;
	...
	// Устройство уже подключено в этом сеансе. Повторно подключать нельзя:
	// типовая на каждое подключение увеличивает счетчик клиентов соединения,
	// и при закрытии формы порт останется занятым.
	Если ТБК_СвязьСККТУстановлена() Тогда

		ТБК_ОбновитьИндикациюККТ();

		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'Порт уже используется 1С, попробуйте повторно отсканировать марку.'"));
		КонецЕсли;
	...
	Контекст   = Новый Структура("ПоказыватьСообщения", ПоказыватьСообщения);
	Оповещение = Новый ОписаниеОповещения("ТБК_УстановитьСвязьСККТЗавершение", ЭтотОбъект, Контекст);

	Попытка
		МенеджерОборудованияРМККлиент.ПодключитьОборудованиеПоИдентификатору(Оповещение,
			УникальныйИдентификатор, ТБК_УстройствоККТ);
	Исключение
		ТБК_СостояниеСвязиККТ = -1;
		ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьККТ, НСтр("ru = 'ККТ: нет связи'"), -1);
		ТБК_ПроверитьСвязьСПИоТ(Ложь);
		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'Нет связи.'") + Символы.ПС + ОписаниеОшибки());
		КонецЕсли;
	КонецПопытки;
...
&НаКлиенте
Процедура ТБК_УстановитьСвязьСККТЗавершение(РезультатВыполнения, Контекст) Экспорт

	ЗаголовокСообщения = НСтр("ru = 'Связь с ККТ'");

	ЭтоСтруктура = ТипЗнч(РезультатВыполнения) = Тип("Структура");
	Успешно      = ЭтоСтруктура И РезультатВыполнения.Свойство("Результат") И РезультатВыполнения.Результат;

	ТБК_ОбновитьИндикациюККТ();

	Если Успешно Тогда

		Если Контекст.ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'Связь установлена.'"));
		КонецЕсли;
	...
	ТекстИнформации = НСтр("ru = 'Нет связи.'");

	Если ЭтоСтруктура И РезультатВыполнения.Свойство("ОписаниеОшибки")
		И ЗначениеЗаполнено(РезультатВыполнения.ОписаниеОшибки) Тогда
		ТекстИнформации = ТекстИнформации + Символы.ПС + СокрЛП(Строка(РезультатВыполнения.ОписаниеОшибки));
	КонецЕсли;

	Если Контекст.ПоказыватьСообщения Тогда
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения, ТекстИнформации);
	КонецЕсли;

КонецПроцедуры

// Читает настройки подключения к экземпляру ТС ПИоТ.
//
// Внутренний "1С:ПИОТ" в этой Рознице недоступен: ОбщегоНазначенияБПОКлиентСервер.Доступен1СПИоТ()
// возвращает Ложь намертво, и команда драйвера PiotStatus всегда отвечает
// "Данная команда доступна только при использовании 1С:ПИОТ". Проверка марок идет по HTTP
// в экземпляр ТС ПИоТ (ИнтеграцияТСПИоТКлиентСервер.ДанныеПоКодуМаркировкиПриРозничнойПродаже,
// вызывается прямо с клиента), поэтому и индикация опрашивает его тем же способом.
//
// Настройки лежат в двух местах в зависимости от версии БПО, поэтому смотрим оба:
// справочник ТСПИоТ по фискальному устройству и регистр сведений по рабочему месту.
//
// Параметры:
//  Устройство   - СправочникСсылка.ПодключаемоеОборудование - ККТ текущей кассы ККМ
//  РабочееМесто - СправочникСсылка.РабочиеМеста             - рабочее место РМК
//
&НаСервереБезКонтекста
Функция ТБК_НастройкиТСПИоТ(Устройство, РабочееМесто)

	Настройки = Новый Структура;
	Настройки.Вставить("Настроено",                             Ложь);
	Настройки.Вставить("Внутренний",                            Ложь);
	Настройки.Вставить("Сервер",                                "");
	Настройки.Вставить("Порт",                                  0);
	Настройки.Вставить("ЗащищенноеСоединение",                  Ложь);
	Настройки.Вставить("ИсключитьПроверкуКорневыхСертификатов",  Истина);
	Настройки.Вставить("АдресПодключения",                      "");

	УстановитьПривилегированныйРежим(Истина);

	// Схема с привязкой экземпляра ТС ПИоТ к фискальному устройству.
	Если ЗначениеЗаполнено(Устройство)
		И ОбщегоНазначенияИСМП.ВерсияБПОПоддерживаетПроверкуКМЧерезТСПИоТ() Тогда

		ТСПИоТ = МенеджерОборудованияТСПИоТ.ПолучитьТСПИоТ(Устройство);

		Если ЗначениеЗаполнено(ТСПИоТ) Тогда

			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТСПИоТ,
				"Тип, Сервер, Порт, ЗащищенноеСоединение, ИсключитьПроверкуКорневыхСертификатов");

			ЗаполнитьЗначенияСвойств(Настройки, Реквизиты);

			Настройки.Внутренний       = (Реквизиты.Тип = Перечисления.ТипыТСПИоТ.Внутренний1С);
			Настройки.Настроено        = Истина;
			Настройки.АдресПодключения = ТБК_АдресТСПИоТ(Настройки);
	...
	// Схема с настройками по рабочему месту.
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	НастройкиПодключенияТСПИоТ.РабочееМесто = &РабочееМесто";
...
&НаСервереБезКонтекста
Функция ТБК_АдресТСПИоТ(Настройки)

	Если Не ЗначениеЗаполнено(Настройки.Сервер) Тогда
		Возврат "";
	КонецЕсли;

	Если ЗначениеЗаполнено(Настройки.Порт) Тогда
		Порт = Формат(Настройки.Порт, "ЧГ=0");
	Иначе
		Порт = ?(Настройки.ЗащищенноеСоединение, "443", "80");
	КонецЕсли;

	Возврат СтрШаблон("%1://%2:%3",
		?(Настройки.ЗащищенноеСоединение, "https", "http"),
		Настройки.Сервер,
		Порт);

КонецФункции

// Опрашивает экземпляр ТС ПИоТ методом api/v1/info - тем же, которым типовая читает
// настройки экземпляра (см. ИнтеграцияТСПИоТКлиентСервер.ПолучитьНастройкиЭкземпляраТСПИоТ).
//
// Запрос синхронный, поэтому таймаут короткий: типовая ставит 60 секунд, а РМК не должна
// вставать на минуту у очереди, если ТС ПИоТ не отвечает.
//
// Параметры:
//  Настройки - см. ТБК_НастройкиТСПИоТ
//
// Возвращаемое значение:
//  Структура:
//   * Связь       - Булево - Истина, если сервис ответил кодом 200
//   * ТекстОшибки - Строка
//   * Сведения    - Строка - номер ФН и заводской номер ККТ из ответа
//
&НаКлиенте
Функция ТБК_ОпроситьТСПИоТ(Настройки)

	Ответ = Новый Структура;
	Ответ.Вставить("Связь",       Ложь);
	Ответ.Вставить("ТекстОшибки", "");
	Ответ.Вставить("Сведения",    "");
	...
	ПараметрыОтправки.Вставить("ПредставлениеСервиса",                  НСтр("ru = 'ТС ПИоТ'"));
	ПараметрыОтправки.Вставить("Сервер",                                Настройки.Сервер);
	ПараметрыОтправки.Вставить("Порт",                                  Настройки.Порт);
	ПараметрыОтправки.Вставить("Таймаут",                               1);
	ПараметрыОтправки.Вставить("ИспользоватьЗащищенноеСоединение",      Настройки.ЗащищенноеСоединение);
	ПараметрыОтправки.Вставить("ИсключитьПроверкуКорневыхСертификатов", Настройки.ИсключитьПроверкуКорневыхСертификатов);

	Попытка

		РезультатЗапроса = ОбменДаннымиИСМПКлиентСервер.ОтправитьДанныеВСервис(
			"api/v1/info", , , "POST", ПараметрыОтправки, ЗаголовокHTTP);

		РезультатОтправки = ОбменДаннымиИСМПКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON(
	...
	Если Не РезультатОтправки.ОтветПолучен Тогда

		Если ЗначениеЗаполнено(РезультатОтправки.ТекстОшибки) Тогда
			Ответ.ТекстОшибки = СокрЛП(Строка(РезультатОтправки.ТекстОшибки));
		Иначе
			Ответ.ТекстОшибки = НСтр("ru = 'ТС ПИоТ не ответил.'");
		КонецЕсли;
	...
	Если Не РезультатОтправки.КодСостояния = 200 Тогда

		Ответ.ТекстОшибки = СтрШаблон(НСтр("ru = 'ТС ПИоТ ответил кодом %1.'"),
			Строка(РезультатОтправки.КодСостояния));

		Если ЗначениеЗаполнено(РезультатОтправки.ТекстОшибки) Тогда
			Ответ.ТекстОшибки = Ответ.ТекстОшибки + Символы.ПС + СокрЛП(Строка(РезультатОтправки.ТекстОшибки));
		КонецЕсли;
	...
	Попытка
		Данные = ОбменДаннымиИСМПКлиентСервер.ТекстJSONВОбъект(
			РезультатОтправки.ТекстВходящегоСообщенияJSON, Истина);
		Ответ.Сведения = СтрШаблон(НСтр("ru = 'ФН %1, ККТ %2'"),
			Строка(Данные.Получить("fnSerial")),
			Строка(Данные.Получить("kktSerial")));
	Исключение
		// Связь есть, разбор ответа нужен только для подписи - молчим.
	КонецПопытки;
...
// Проверяет связь с ТС ПИоТ и обновляет индикацию.
//
// Параметры:
//  ПоказыватьСообщения - Булево - показывать кассиру результат.
//
&НаКлиенте
Процедура ТБК_ПроверитьСвязьСПИоТ(ПоказыватьСообщения)

	ЗаголовокСообщения = НСтр("ru = 'Связь с ТС ПИоТ'");

	// Ленивое чтение: серверные вызовы не должны попадать в открытие РМК.
	ТБК_ОпределитьУстройствоККТ();

	Если ТипЗнч(ТБК_НастройкиПИоТ) <> Тип("Структура") Тогда
		ТБК_НастройкиПИоТ = ТБК_НастройкиТСПИоТ(ТБК_УстройствоККТ, Объект.РабочееМесто);
	КонецЕсли;

	Если Не ТБК_НастройкиПИоТ.Настроено Тогда

		ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьПИоТ, НСтр("ru = 'ПИоТ: не настроен'"), 0);

		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'Для этой кассы ККМ и рабочего места подключение к ТС ПИоТ не настроено.'"));
		КонецЕсли;
	...
	Если ТБК_НастройкиПИоТ.Внутренний Тогда

		ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьПИоТ, НСтр("ru = 'ПИоТ: внутренний'"), 0);

		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокСообщения,
				НСтр("ru = 'ТС ПИоТ настроен как внутренний (1С:ПИОТ), опросить его по сети нельзя.'"));
		КонецЕсли;
	...
	Ответ = ТБК_ОпроситьТСПИоТ(ТБК_НастройкиПИоТ);

	Если Ответ.Связь Тогда

		ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьПИоТ, НСтр("ru = 'ПИоТ: связь есть'"), 1);

		ТекстИнформации = НСтр("ru = 'Связь установлена.'") + Символы.ПС + ТБК_НастройкиПИоТ.АдресПодключения;

		Если ЗначениеЗаполнено(Ответ.Сведения) Тогда
			ТекстИнформации = ТекстИнформации + Символы.ПС + Ответ.Сведения;
		КонецЕсли;

	Иначе

		ТБК_УстановитьИндикацию(Элементы.ТБК_КнопкаСвязьПИоТ, НСтр("ru = 'ПИоТ: нет связи'"), -1);

		ТекстИнформации = НСтр("ru = 'Нет связи.'") + Символы.ПС + ТБК_НастройкиПИоТ.АдресПодключения;

		Если ЗначениеЗаполнено(Ответ.ТекстОшибки) Тогда
			ТекстИнформации = ТекстИнформации + Символы.ПС + Ответ.ТекстОшибки;
		КонецЕсли;
