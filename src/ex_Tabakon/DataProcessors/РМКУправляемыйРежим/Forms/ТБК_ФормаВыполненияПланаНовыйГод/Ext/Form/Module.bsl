...
Процедура ЗаполнитьПланы()
	...
	|	ТБК_ПланыМагазинаПоПериодам.ПериодПо КАК ПериодПо,
	|	ТБК_ПланыМагазинаПоПериодам.Сумма КАК Сумма,
	|	РАЗНОСТЬДАТ(ТБК_ПланыМагазинаПоПериодам.ПериодС, ДОБАВИТЬКДАТЕ(ТБК_ПланыМагазинаПоПериодам.ПериодПо, ДЕНЬ, 1), ДЕНЬ) КАК КоличествоДней
	|ИЗ
	|	РегистрСведений.ТБК_ПланыМагазинаПоПериодам КАК ТБК_ПланыМагазинаПоПериодам
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		ПланСегодня		=	Окр(Выборка.Сумма /  Выборка.КоличествоДней,0)
	КонецЕсли;
