...
Процедура ЗарегистрироватьНаСервере()
	...
	эл.Вставить("birthday", ДатаРождения);
	...
	Для каждого Строка из МассивПринятый цикл 
		...
		иначе
			...
			Куар	=	Строка.ID_S;
			Результат	=	Расш_SamosaleСоздатьКлиентаПослеНаСервере(ТелефонКлиента, Куар);
...
Функция  Расш_SamosaleСоздатьКлиентаПослеНаСервере(ТелефонКлиента, Куар)
