...
Процедура ПриАктивизацииСтрокиТаблицы();
	Если Элементы["ТаблицаНаФорме0"].ТекущиеДанные <> Неопределено тогда
		...
		МассивН = ПолучитьСписокНоменклатуры(ФокусГруппа);
		Элементы.ТаблицаНаФорме0.ПодчиненныеЭлементы.КолонкаФокусГруппа0.СписокВыбора.ЗагрузитьЗначения(МассивН);
...
// Толстоухов 11.11.2025 ROZ-12 ++
&НаСервере
Функция ПолучитьСписокНоменклатуры(ФокусГруппа)
