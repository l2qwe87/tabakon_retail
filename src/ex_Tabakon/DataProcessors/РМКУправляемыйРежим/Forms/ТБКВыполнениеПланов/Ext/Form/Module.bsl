				...
				Если не Найти(Ответ,"Внимание!") тогда 
					...
					Для каждого СтрокаМассива из ОтветМассив цикл //добавляем
						...
						Если НомСч = 0 тогда //В первом элементе массива передаю оклады замен и мобильных
							...
							Если Число(СтрокаМассива.ОкладЗамена) = 0 И Булево(СтрокаМассива.флТекущийПользователь_ЭтоЗамена) = Истина Или
								Число(СтрокаМассива.ОкладЗамена) > 0 И Булево(СтрокаМассива.флТекущийПользователь_ЭтоЗамена) = Ложь Тогда
...
Функция ПолучитьДанныеПоОбщемуПлану_2(Период)
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |	И ТБКПланыМагазинаПоМесяцам.ТипПлана = ""План ТО""
	      	 	             |
	      	 	             |УПОРЯДОЧИТЬ ПО
	      	 	             |	ТипПлана");
