	...
	Если рез.Следующий() тогда	
		Если ПереключательМесяцДень = 0 тогда// строим за МЕСЯЦ		
			...
			Для Сч = 1 по 2 цикл // 1- общий план, 2 - личный план
				...
				Если Сч =1 тогда
					Заголовок		= "Общие показатели магазина";
				...
				Если ПереключательМесяцДень = 0 и  Сч = 1 тогда 
					...
					ТочкаПланСреднее = НДиаг1.УстановитьТочку("План среднее");				
					ТочкаФактСреднее = НДиаг1.УстановитьТочку("Факт среднее");
				...
				Если СЧ	= 2 и ПереключательМесяцДень = 0 тогда
...
Процедура ЗаполнитьФокусныеПремииОбщаяМотивация()
	...
	ТаблицаРасшифровка.Колонки.Добавить("КоличествоСверх",,	"Количество сверх");
	ТаблицаРасшифровка.Колонки.Добавить("СуммаФакт",,		"Сумма факт");
	ТаблицаРасшифровка.Колонки.Добавить("СуммаСверх",,		"Сумма сверх");
	...
	Для каждого СтрокаГруппа из ГруппыНоменклатуры цикл
		...
		Если СтрокаГруппа.СверхПлана тогда   //только за то что сверх плана
			ТЗПродано	=	ПолучитьКоличествоПродажПоГруппе(Продавец, СтрокаГруппа.Номенклатура);			
			...
			Для каждого Строка из ТЗПродано цикл
				...
				Если ИтогоПродано > ПланКоличество тогда
					...
					иначе
						...
						СуммаПродажСверхПлана	=	ИтогоПроданоСумма;
					КонецЕсли;
		...
		иначе //начисляем за все
			ТЗПродано	=	ПолучитьКоличествоПродажПоГруппе(Продавец, СтрокаГруппа.Номенклатура, Истина);			
	...
	иначе
		...
		Для каждого СтрокаТЗ из ТаблицаРасшифровка цикл   
			...
			НДиаг         		= Новый РеквизитФормы(НазваниеДиаграммы, ДопустимыеТипы);
			НДиаг.Заголовок		= Строка(СтрокаТЗ.ФокусГруппа) + ". Премия: " + СтрокаТЗ.СуммаПремии;
			...
			ТочкаКолФакт = НДиаг.УстановитьТочку("Факт");						
...
Функция ПолучитьДанныеПоУценке(ПереключательМесяцДень, Период)	
	...
	Для каждого Строка из рез цикл
		...
		иначе
			...
			ПремияСувениры	=	ПремияСувениры  + 20;	
			Попытка
