...
Функция ПолучитьСуммуФактПоТипуПлана(ТипПлана, ЗаДень = Ложь, ЗаСегодня = Ложь)     	
	...
	Если ТипПлана = "План личный" тогда
		...
		Запрос	=	Новый запрос("ВЫБРАТЬ
		      	 	             ...
		      	 	             |	И ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
		      	 	             |	И ЧекККМТовары.Продавец = &Продавец
		      	 	             |	И (ЧекККМТовары.Ссылка.Комментарий ПОДОБНО &Комментарий
		...
		Если Рез.Следующий() тогда
			СуммаФАкт = СуммаФАкт - Рез.СуммаЧеки;	
		КонецЕсли;
...
Процедура ЗаполнитьВариантНовойМотивации_2()
	...
	Если НЕ ЗначениеЗаполнено(Продавец) тогда
		...
		СтруктураПремииЗаПроцент.Вставить("СуммаПродажВторой",				0);
		СтруктураПремииЗаПроцент.Вставить("КолСменОсновной",				0);
		СтруктураПремииЗаПроцент.Вставить("КолСменВторой",					0);
		СтруктураПремииЗаПроцент.Вставить("ПремияОсновной",					0);
		СтруктураПремииЗаПроцент.Вставить("ПремияВторой",					0);
		СтруктураПремииЗаПроцент.Вставить("ПроцентПремии",					0);
		СтруктураПремииЗаПроцент.Вставить("ПроцентПремииВторойПродавец",	0);  
	...
	//Был и вторым и первым
	Если  СтруктураПремииЗаПроцент.КолСменОсновной > 0 и СтруктураПремииЗаПроцент.КолСменВторой >0 тогда
		МассЧастей = Новый Массив;
		МассЧастей.Добавить(Новый ФорматированнаяСтрока("Второй продавец. Кол.смен: "+ СтруктураПремииЗаПроцент.КолСменВторой + Символы.ПС	,, ЦветСиний));	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока("Процент за продажи: " 												,, ЦветСиний));	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Строка(СтруктураПремииЗаПроцент.ПроцентПремииВторойПродавец)+"%."	,, ЦветКрасный));
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(" Сумма ваших личных продаж: "										,, ЦветСиний));   	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Строка(Окр(СтруктураПремииЗаПроцент.СуммаПродажВторой,0))+ "р."		,, ЦветКрасный));
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(" Сумма премии: "													,, ЦветСиний));  	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Строка(Окр(СтруктураПремииЗаПроцент.ПремияВторой,0))+ "р."+ Символы.ПС			,, ЦветКрасный));
		

		МассЧастей.Добавить(Новый ФорматированнаяСтрока("Основной продавец. Кол.смен: "+ СтруктураПремииЗаПроцент.КолСменОсновной + Символы.ПС	,, ЦветСиний));	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока("Процент за продажи: " 												,, ЦветСиний));	
		...
		ПремиязаПроцент	=	СтруктураПремииЗаПроцент.ПремияВторой + СтруктураПремииЗаПроцент.ПремияОсновной;
		
	//Был только вторым
	ИначеЕсли СтруктураПремииЗаПроцент.КолСменВторой >0 тогда
		...
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Строка(Окр(СтруктураПремииЗаПроцент.СуммаПродажВторой,0))+ "р."		,, ЦветКрасный));
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Символы.ПС + "Сумма премии: "										,, ЦветСиний));  	
		МассЧастей.Добавить(Новый ФорматированнаяСтрока(Строка(Окр(СтруктураПремииЗаПроцент.ПремияВторой,0))+ "р."			,, ЦветКрасный));
		
		Элементы.ДекорацияНадписьПремияЗаПроцент2.Заголовок = Новый ФорматированнаяСтрока(МассЧастей);	
			
		ПремиязаПроцент	=	СтруктураПремииЗаПроцент.ПремияВторой;
...
Функция ПолучитьДанныеПоПремиямЗаПроцент_2(Продавец_, ПереключательМесяцДень, Период)
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |	ТБКСменыСотрудников.ТипПродавца,
	      	 	             |	ТБКСменыСотрудников.Продавец
	      	 	             |;
	      	 	             |
	      	 	             |////////////////////////////////////////////////////////////////////////////////
	      	 	             ...
	      	 	             |	ТБКСменыСотрудников.Продавец КАК Продавец,
	      	 	             |	ТБКСменыСотрудников.ТипПродавца КАК ТипПродавца,
	      	 	             |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)) КАК КолСменПоТипуПродавца
	      	 	             |ПОМЕСТИТЬ КолСмен
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
	      	 	             |ГДЕ
	      	 	             ...
	      	 	             |		ИНАЧЕ 0
	      	 	             |	КОНЕЦ КАК Коэф,
	      	 	             |	ДниЭтогоПродавца.ТипПродавца КАК ТипПродавца
	      	 	             ...
	      	 	             |	ДниСКоэффициентом.ТипПродавца КАК ТипПродавца
	      	 	             |ПОМЕСТИТЬ Итог
	      	 	             |ИЗ
	      	 	             ...
	      	 	             |
	      	 	             |СГРУППИРОВАТЬ ПО
	      	 	             |	ДниСКоэффициентом.ТипПродавца
	      	 	             |;
	      	 	             |
	      	 	             |////////////////////////////////////////////////////////////////////////////////
	      	 	             |ВЫБРАТЬ
	      	 	             |	Итог.СуммаПродажПродавца КАК СуммаПродажПродавца,
	      	 	             |	Итог.ТипПродавца КАК ТипПродавца,
	      	 	             |	КолСмен.КолСменПоТипуПродавца КАК КолСменПоТипуПродавца
	      	 	             |ИЗ
	      	 	             |	Итог КАК Итог
	      	 	             |		ЛЕВОЕ СОЕДИНЕНИЕ КолСмен КАК КолСмен
	      	 	             |		ПО Итог.ТипПродавца = КолСмен.ТипПродавца");
	...
	Пока Рез.Следующий() цикл
		Для каждого Строка Из Рез.Ссылка.Проценты Цикл
			Если Строка.ПроцентС = 0 и 	Строка.ПроцентДо = 0 тогда
				ПроцентПремииВторойПродавец	=	Строка.ПроцентПремии;
	...
	Для каждого Строка из Итог цикл
		...
		Если Строка.ТипПродавца			=	Перечисления.ТБКТипыПродавцов.ОсновнойПродавец или Строка.ТипПродавца	=	Перечисления.ТБКТипыПродавцов.НочнойПродавец или Строка.ТипПродавца	=	Перечисления.ТБКТипыПродавцов.МладшийПродавец тогда
			ПремияОсновной				=	Окр((Строка.СуммаПродажПродавца/100)*ПроцентПремии,0);
			КолСменОсновной				=	Строка.КолСменПоТипуПродавца;
			СуммаПродажОсновной			=	Строка.СуммаПродажПродавца;
		КонецЕсли;
		
		
		Если Строка.ТипПродавца			=	Перечисления.ТБКТипыПродавцов.ВторойПродавец тогда
			ПремияВторой				=	Окр((Строка.СуммаПродажПродавца/100)*ПроцентПремииВторойПродавец,0);
			КолСменВторой				=	Строка.КолСменПоТипуПродавца; 
			СуммаПродажВторой			=	Строка.СуммаПродажПродавца;
	...
	Струк.Вставить("СуммаПродажВторой",				СуммаПродажВторой);
	Струк.Вставить("КолСменОсновной",				КолСменОсновной);
	Струк.Вставить("КолСменВторой",					КолСменВторой);
	Струк.Вставить("ПремияОсновной",				ПремияОсновной);
	Струк.Вставить("ПремияВторой",					ПремияВторой);
	Струк.Вставить("ПроцентПремии",					ПроцентПремии);
	Струк.Вставить("ПроцентПремииВторойПродавец",	ПроцентПремииВторойПродавец);
