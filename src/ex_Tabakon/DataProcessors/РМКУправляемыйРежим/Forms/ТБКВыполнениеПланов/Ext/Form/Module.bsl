...
Процедура ВыводДиаграммыПоВысокомаржинальномуТовару();    
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             ...
	      	 	             |	И ТБКПланыМагазинаПоМесяцам.Период МЕЖДУ &ПериодН И &ПериодК
	      	 	             |
	      	 	             |УПОРЯДОЧИТЬ ПО
	      	 	             |	Период УБЫВ"); 
...
Процедура ВывестиТаблицу(Родитель, ТаблицаРасшифровка, ИтогоБонус, Продавец, СЧ);
	...
	Для Каждого Ст ИЗ ТаблицаРасшифровка.Колонки Цикл
		...
		Если  Ст.Имя	= "ФокусГруппа" Тогда 
			...
			// Толстоухов 11.11.2025 ROZ-12 ++
			Рек.Вид = ВидПоляФормы.ПолеВвода;
			
			//Рек.КнопкаВыпадающегоСписка = истина;
			//Рек.РежимВыбораИзСписка = Истина;
			Рек.ВыбиратьТип = Ложь;
			Рек.ИсторияВыбораПриВводе = ИсторияВыбораПриВводе.НеИспользовать;
			Рек.РедактированиеТекста = Ложь;
			Рек.ОбновлениеТекстаРедактирования = ОбновлениеТекстаРедактирования.НеИспользовать;
			
			Рек.УстановитьДействие("ОбработкаВыбора", "ВыборСтрокиТаблицы");
			// Толстоухов 11.11.2025 ROZ-12 --
		Иначе
...
Процедура УстановитьДействиеПриАктивизацииСтрокиНаФорме()
	...
	Элементы["ТаблицаНаФорме0"].УстановитьДействие("Выбор", "Выбор");   
КонецПроцедуры
...
Процедура Выбор(ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
	Если Элементы["ТаблицаНаФорме0"].ТекущиеДанные <> Неопределено тогда
		...
		ФокусГруппа = Элементы["ТаблицаНаФорме0"].ТекущиеДанные.ФокусГруппа;
		СтруктураПараметров = Новый Структура("ФокусГруппа", ФокусГруппа);
		СтруктураПараметров.Вставить("Период", Период); 
		ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБКФормаОстатокПоФокуснымГруппам", СтруктураПараметров, ЭтаФорма, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);		
...
// Толстоухов 11.11.2025 ROZ-12 ++
&НаСервере
Функция ПолучитьСписокНоменклатуры(ФокусГруппа) // пока не используется
		...
		ЗапросН.УстановитьПараметр("Тип", ФокусГруппа);
...
// Толстоухов 11.11.2025 ROZ-12 ++ // пока не используется
&НаКлиенте
Процедура ВыборСтрокиТаблицы(Элемент, ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура Группа8ПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Заголовок = "Маркетинговые акции" И ДеревоДанных.ПолучитьЭлементы().Количество() = 0  Тогда
		ЗаполнитьДеревоМеркетинговыхАкций();
	КонецЕсли;
...
Процедура ЗаполнитьФокусныеПремииОбщаяМотивация()
	ИтогоПремииФокус = 0;                
	Если ОкладЗамена <> Неопределено И ОкладЗамена > 0 Или ОкладЗамена <> Неопределено И ОкладМобильный > 0 тогда
	...
	иначе
		...
		Для каждого СтрокаТЗ из ТаблицаРасшифровка цикл   
			...
			Если СтрокаТЗ.ПланПродаж >0 тогда
				...
				// Толстоухов 11.11.2025 ROZ-12 ++
				ТочкаКолВсего = НДиаг.УстановитьТочку("ПродажиВсего");						
				ТочкаКолВсего.ПриоритетЦвета	= Истина;   		
				ТочкаКолВсего.Цвет	=	WebЦвета.БледноКрасноФиолетовый;  
				
				ТочкаКолВсего.Текст			=	"Продажи всего";
				
				НДиаг.УстановитьЗначение(ТочкаКолВсего
				, НДиаг.Серии[0]
				, СтрокаТЗ.ИтогоКоличество
				, Неопределено
				, Сред(СтрокаТЗ.Заголовок,Найти(СтрокаТЗ.Заголовок, Символы.ПС)+1));
				// Толстоухов 11.11.2025 ROZ-12 --
...
&НаКлиенте
Процедура НовыйГод(Команда)
	ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБК_ФормаВыполненияПланаНовыйГод",, ЭтаФорма, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ЗаполнитьДеревоАкцийИзБазы()
    ...
    Если СписокКодов.Количество() > 0 Тогда
        ...
        Для Каждого СтрТ Из ТЗТоваров Цикл
            МапТоваров.Вставить(СокрЛП(СтрТ.Код), Новый Структура("Наименование, Остаток, Ссылка", СтрТ.Наименование, СтрТ.Остаток, СтрТ.Ссылка));
		КонецЦикла;
    ...
    Для Каждого Стр Из Выборка Цикл
		...
		Для Каждого Код Из МассивКодов Цикл
			...
			Если ДанныеТовара.Остаток = 0 И ДанныеТовара.ссылка.ЭтоГруппа Тогда
				...
				|ИЗ
				|	РегистрНакопления.ТоварыНаСкладах.Остатки(, Номенклатура В ИЕРАРХИИ (&Группа)) КАК Ост";
				
				Запрос3.УстановитьПараметр("Группа", ДанныеТовара.ссылка); 
	...
	Если Прокси = Неопределено Тогда
		...
		СтрГр = Акции.ПолучитьЭлементы().Добавить();
		СтрГр.ЭтоГруппа   = Истина;
        СтрГр.Наименование = "не удалось получить акции по желтым ценникам. проверьте интеренет";
	    Возврат;
			...
			Если ДанныеТовара <> Неопределено Тогда
				...
				Если ДанныеТовара.Остаток <> 0 Тогда
					...
					Если ДанныеТовара.ссылка.ЭтоГруппа Тогда
						...
						Запрос4.УстановитьПараметр("Группа", ДанныеТовара.ссылка); 
...
&НаКлиенте
Процедура ОбновитьСтраницуАкций(Команда)
...
Процедура Табель(Команда)
	...
	ПараметрыОткрытия.Вставить("Продавец", Продавец);
	
	ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБК_Табель", ПараметрыОткрытия, ЭтаФорма, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ЭффективностьПродавцов(Команда)
    СформироватьОтчетСКДНаСервере();
КонецПроцедуры

&НаСервере
Процедура СформироватьОтчетСКДНаСервере()
	
	ТаблицаЭффективностьПродавцов.Очистить();
	ОтчетОбъект = Отчеты.ТБК_ЭффективностьПродавцов.Создать();
	...
	// Создаём объект для сбора данных расшифровки
	ДанныеРасшифровки = Новый ДанныеРасшифровкиКомпоновкиДанных;
	
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	// Передаём настройки с установленными параметрами в выполнение макета
	...
	ПроцессорВывода.УстановитьДокумент(ТаблицаЭффективностьПродавцов);
...
&НаСервере
Процедура ЗаполнитьДеревоМеркетинговыхАкций()
    
    // Получаем существующее дерево из объекта формы
    ДеревоРек = РеквизитФормыВЗначение("ДеревоДанных");
	
    // Очищаем дерево, если нужно
	...
	Для каждого Строка из рез цикл
		...
		Если ТипАкции = "Скидка на самую дешёвую позицию" Тогда
			...
			Описание = "";
			Описание = Описание + ?(Скидка <> "0", "Скидка: " +  Скидка + "% ", "");
			Описание = Описание + ?(КоличествоНоменклатуры <> "0", "Количество позиций номенклатуры: " +  КоличествоНоменклатуры + " ", "");
			СписокНоменклатуры = ПолучитьНоменклатуру(Номенклатура, НоменклатураИсключение);
			
			ДобавитьВДерево(ДеревоРек, ТипАкции, Описание, СписокНоменклатуры);
		...
		Если ТипАкции = "Подарок за товар" Тогда
			Описание = "";
			Описание = Описание + ?(КоличествоНоменклатуры <> "0", "Количество позиций номенклатуры: " +  КоличествоНоменклатуры + " ", "");
			Описание = Описание + ?(КоличествоЧеков <> "0", "Количество чеков: " +  КоличествоЧеков + " ", "");
			Описание = Описание + ?(СуммаЧека <> "0", "Сумма чека: " +  СуммаЧека + " ", "");
			
			СписокНоменклатуры = ПолучитьНоменклатуру(Номенклатура, НоменклатураИсключение);
			СписокТоварВПодарок = ПолучитьНоменклатуру(ТоварВПодарок);
			НоменклатураВПодарок = "";
			
			Для каждого Строка Из СписокТоварВПодарок Цикл
				НоменклатураВПодарок = НоменклатураВПодарок + Строка.Номенклатура.Наименование + " ;";
			КонецЦикла;
			
			Описание = Описание + ". Номенклатура в подарок: " + НоменклатураВПодарок;
	...
	ЗначениеВРеквизитФормы(ДеревоРек,"ДеревоДанных");
	
КонецПроцедуры

&НаСервере
Функция ПолучитьНоменклатуру(Номенклатура, НоменклатураИсключение = Неопределено)
	//Проверяем выполняется ли условие по номенклатуре	
	СзНоменклатура = Новый Массив;
	...
	//Исключения
	СзНоменклатураИсключения = Новый Массив;
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ вт_ДанныеНоменклатура
		|ИЗ
		...
		|ВЫБРАТЬ
		|	Номенклатура.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ вт_ДанныеИсключения
		|ИЗ
		|	Справочник.Номенклатура КАК Номенклатура
		|ГДЕ
		|	Номенклатура.Код В(&КодыИсключения)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Номенклатура.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ вт_НужнаяНом
		|ИЗ
		|	Справочник.Номенклатура КАК Номенклатура
		|ГДЕ
		...
		|				т.Ссылка
		|			ИЗ
		|				вт_ДанныеНоменклатура КАК т)
		|	И НЕ Номенклатура.Ссылка В ИЕРАРХИИ
		|				(ВЫБРАТЬ
		|					т1.Ссылка
		|				ИЗ
		|					вт_ДанныеИсключения КАК т1)
		...
		|			Номенклатура В
		|				(ВЫБРАТЬ
		|					вт_НужнаяНом.Ссылка КАК Ссылка
		|				ИЗ
		|					вт_НужнаяНом КАК вт_НужнаяНом)) КАК ТоварыНаСкладахОстатки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ вт_НужнаяНом КАК вт_НужнаяНом
		|		ПО ТоварыНаСкладахОстатки.Номенклатура = вт_НужнаяНом.Ссылка
		|ГДЕ
		|	ТоварыНаСкладахОстатки.Номенклатура В (вт_НужнаяНом.Ссылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Коды", СзНоменклатура);
	Запрос.УстановитьПараметр("КодыИсключения", СзНоменклатураИсключения);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции

Процедура ДобавитьВДерево(Дерево, ТипАкции, Описание, СписокНоменклатуры)
	
	// Уровень 1 — группы
	Группа1 = Дерево.Строки.Добавить();
	Группа1.Акция = ТипАкции;
	Группа1.Описание = Описание;
	
	// Уровень 2 — товары в группах 
	Для Каждого Строка Из СписокНоменклатуры Цикл 
		Товар1 = Группа1.Строки.Добавить();
		Товар1.Акция = Строка.Номенклатура;
		Товар1.Описание = Строка.КоличествоОстаток;
	КонецЦикла;
...
Процедура ДеревоДанныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ДеревоДанныхАкция" И ТипЗнч(Элемент.ТекущиеДанные.Акция) = Тип("СправочникСсылка.Номенклатура") Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыФормы = Новый Структура("Ключ", Элемент.ТекущиеДанные.Акция);
