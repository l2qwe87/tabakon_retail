...
Функция ПолучитьДанныеПоПремиямЗаПроцент_2(Продавец_, ПереключательМесяцДень, Период)
	//Исключения
	НоменклатураИсключения	= Новый Массив;
	Запрос	=	Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             |	ТБКОкладыПремииИПроцентыНоваяМотивация.Ссылка КАК Ссылка
	      	 	             |ИЗ
	      	 	             |	Справочник.ТБКОкладыПремииИПроцентыНоваяМотивация КАК ТБКОкладыПремииИПроцентыНоваяМотивация
	...
	Если МассивНоменклатурыИсключения.Количество() тогда
		МассивНоменклатурыНовый	=	МассивНоменклатурыИсключения.ВыгрузитьЗначения();	
	иначе
		МассивНоменклатурыНовый 		= Обработки.РМКУправляемыйРежим.ПолучитьСписокНоменклатуры(НоменклатураИсключения);
		МассивНоменклатурыИсключения.ЗагрузитьЗначения(МассивНоменклатурыНовый);	
	КонецЕсли;
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |	И НЕ ЧекККМТовары.Номенклатура В (&НоменклатураИсключения)
	      	 	             ...
	      	 	             |	И НЕ ЧекККМТовары.Номенклатура В (&НоменклатураСувениры)
	      	 	             |
	      	 	             |СГРУППИРОВАТЬ ПО
	...
	Если ПроцентПремииСувениры > 0 Тогда
		НоваяНоменклатураСувениры = Обработки.РМКУправляемыйРежим.ПолучитьСписокНоменклатуры(Справочники.Номенклатура.НайтиПоКоду("00048218449"));
		
		Запрос.УстановитьПараметр("НоменклатураСувениры", НоваяНоменклатураСувениры); //папка сувенирная продукция  
		
		ЗапросСувениры	=	Новый Запрос();
		ЗапросСувениры.Текст	=	Запрос.Текст;
		ЗапросСувениры.Текст	=	СтрЗаменить(ЗапросСувениры.Текст, "И НЕ ЧекККМТовары.Номенклатура В (&НоменклатураСувениры)", "И ЧекККМТовары.Номенклатура В (&НоменклатураСувениры)");	
		
	Иначе
		Запрос.Текст	=	СтрЗаменить(Запрос.Текст, "И НЕ ЧекККМТовары.Номенклатура В (&НоменклатураСувениры)", "");	
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("НоменклатураИсключения", МассивНоменклатурыНовый);
	Запрос.УстановитьПараметр("Продавец", Продавец_);
...
Процедура ЗаполнитьФокусныеПремииОбщаяМотивация()
	...
	Если ОкладЗамена <> Неопределено И ОкладЗамена > 0 Или ОкладЗамена <> Неопределено И ОкладМобильный > 0 тогда
		ТаблицаРасшифровка	=		Обработки.РМКУправляемыйРежим.ПолучитьТаблицуПоФокуснойМотивации(ИтогоПремииФокус, ТекущаяДата(), 1, Продавец, ТаблицаСоответствийНоменклатурыДляФокуснойМотивации);
	иначе
		ТаблицаРасшифровка	=		Обработки.РМКУправляемыйРежим.ПолучитьТаблицуПоФокуснойМотивации(ИтогоПремииФокус, Период, ПереключательМесяцДень, Продавец, ТаблицаСоответствийНоменклатурыДляФокуснойМотивации);
	КонецЕсли;
