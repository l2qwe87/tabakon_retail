...
Процедура ЗаполнитьВариантНовойМотивации()
	...
	Элементы.ДекорацияНадписьОклад.Заголовок	=	"За этот период вы отработали: "+СтруктураОкладВремя.КоличествоЧасов+" часов."+ Символы.ПС +"Сумма оклада: " + СтруктураОкладВремя.СуммаОклада+"р.";
	...
	Если СтруктураОбщийПлан.ПроцентВыполнения >100 тогда
		ПремияЗаОбщий	=	СтруктураОбщийПлан.ПремияЗаПлан;
	иначе
		ПремияЗаОбщий	=	0;
	КонецЕсли;  
	...
	Элементы.ДекорацияНадписьОбщийПлан.Заголовок	=	"План магазина за выбранный месяц составляет: "+СтруктураОбщийПлан.СуммаПлана+"р." + Символы.ПС + 
													"Вы выполнили: "+СтруктураОбщийПлан.ПроцентВыполнения+"%."+ Символы.ПС +"Сумма премии: "+ПремияЗаОбщий+"р.";
	...
	Элементы.ДекорацияНадписьПремияЗаПроцент.Заголовок	=	"Процент премии за выполнение плана составляет: "+СтруктураПремииЗаПроцент.Процент+"%."+ Символы.ПС +
													"Сумма ваших личных продаж: "+Окр(СтруктураПремииЗаПроцент.СуммаПродаж,0)+"р." + Символы.ПС + "Сумма премии :"+Окр(СтруктураПремииЗаПроцент.ПремиязаПроцент,0)+"р.";
...
Функция ПолучитьДанныеПоОбщемуПлану(ПереключательМесяцДень, Период)
	...
	Струк	=	новый Структура;
	Струк.Вставить("СуммаПлана",		СуммаПлана);
	Струк.Вставить("ПроцентВыполнения",	ПроцентВыполнения);
	Струк.Вставить("ПремияЗаПлан",		ПремияЗаОбщий);
