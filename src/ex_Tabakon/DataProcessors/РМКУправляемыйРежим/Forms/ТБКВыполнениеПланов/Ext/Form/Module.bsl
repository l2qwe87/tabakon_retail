...
Процедура Группа8ПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Заголовок = "Маркетинговые акции" Тогда
		ОбработатьСкидкиИзУТ();
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьСкидкиИзУТ()
...
Процедура ДеревоДанныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ДеревоДанныхНаименование" И ТипЗнч(Элемент.ТекущиеДанные.Акция) = Тип("СправочникСсылка.Номенклатура") Тогда
		СтандартнаяОбработка = Ложь;
