...
Процедура ТБКДекорацияОткрытиеСменыНажатиеПеред(Элемент)
	...
	ОборудованияОтбор.ТипыПО.ККТ = Истина;
	...
	Для каждого Оборудование Из СписокОборудования Цикл
		...
		ПараметрыФормы.Вставить("Ключ", Оборудование.Ссылка);
		ПараметрыФормы.Вставить("Идентификатор", Оборудование.Ссылка);
		ПараметрыФормы.Вставить("КодЯзыка", "ru"); 
		ФормаНастройки = ПолучитьФорму("Справочник.ПодключаемоеОборудование.Форма.ФормаНастройки", ПараметрыФормы); 
		
		Если ФормаНастройки <> Неопределено Тогда
			
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("ПервыйЗапуск", Истина);
			ПараметрыКоманды.Вставить("ОбновлениеИнформацииДрайверОборудования_Завершение", Новый ОписаниеОповещения("ОбновлениеИнформацииДрайверОборудования_Завершение", ЭтаФорма)); 
			
			Оповещение = Новый ОписаниеОповещения("ОбновитьИнформациюДрайверОборудования_Завершение", ФормаНастройки, ПараметрыКоманды);
			МенеджерОборудованияКлиент.НачатьПолучениеОписанияОборудования(Оповещение, , ФормаНастройки.ДанныеУстройства);
...
&НаКлиенте
Процедура ОбновлениеИнформацииДрайверОборудования_Завершение(РезультатВыполнения, Параметры) Экспорт 
	
	ВремНастройки = РезультатВыполнения.ТБК_ОпределитьВремНастройки();
	ПараметрыЗапершения = Новый Структура;
	ПараметрыЗапершения.Вставить("ОповещениеПриЗавершении");
	МенеджерОборудованияКлиент.ВыполнитьНастройкуОборудования_Завершение(ВремНастройки, ПараметрыЗапершения);
