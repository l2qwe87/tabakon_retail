...
Процедура ТБКОповещениеОткрытьФормуВопросаЗакрытьСмену(РезультатОткрытияФормы, ДополнительныеПараметры)
	...
	Если НЕ РезультатОткрытияФормы = Неопределено Тогда
		...
		//Марк 2025-02-21
		//Когда прочивается чек и в этот момент разрывается с фискалкой кучу данных не записывается для прбития этого чека
		//Такие чеки удаляем так как продавцы в любом случае заного делать чек чтоб продать товар
