...
Процедура ТБКОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеСостоянияКассовойСмены" Тогда
		...
		//Марк 2026-04-27
		ОбщегоНазначенияВызовСервера.ЗафиксироватьАктивнойПользователя("" + Параметр + "Статус кассовой смены - " + СтруктураСостояниеКассовойСмены.СтатусКассовойСмены);
		//КонецМарк
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	ПодключитьОбработчикОжидания("ПроверкаНовыхНовостей",900); 
	
	//Вкл Новую мотивацию 
	ВклНовуюМотивациюнаСервере(Объект.Магазин);
	
	ПроверкаЗаполненияЗаявкиНаХозТовары();  // Толстоухов 24.04.2026 ROZ-26 ++
...
Процедура ТБКОткрытьСмену(КассаККМ)
	МенеджерОборудованияКлиент.СинхронизироватьСДатойКомпьютера(КассаККМ);  
	
	//Марк 2025-05-05
	//КонстантаКлишеИзмененоУспешно	=	ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("КлишеИзмененоУспешно");
	РабочееМесто = МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента();
	НазвениеКонтанетыКлише = "КлишеИзмененоУспешно" + "_" + КассаККМ;
	КонстантаКлишеИзменено	= ПолучитьЗначениеТБККонстанты(НазвениеКонтанетыКлише);
	
	Если КонстантаКлишеИзменено = Неопределено Тогда
		
		ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая(НазвениеКонтанетыКлише, Ложь);	
		КонстантаКлишеИзмененоУспешно = Ложь; 
		
	иначе
		КонстантаКлишеИзмененоУспешно = КонстантаКлишеИзменено;	
	КонецЕсли;   
	//КонецМарк
	
	Если КонстантаКлишеИзмененоУспешно <> Неопределено И НЕ КонстантаКлишеИзмененоУспешно тогда
		Ответ	=	МенеджерОборудованияКлиент.УстановитьКлише(НазвениеКонтанетыКлише);//делается 1 раз, от константы
		Если Ответ тогда КонстантаКлишеИзмененоУспешно = истина КонецЕсли; 
	КонецЕсли;
КонецПроцедуры

Функция ПолучитьЗначениеТБККонстанты(НазвениеКонтанетыКлише)

	Возврат ОбщегоНазначения.ПолучитьЗначениеТБККонстанты(НазвениеКонтанетыКлише);	
...
Процедура ТБКПоказатьЗаявкиПосле(Команда)
	...
	ПараметрыОтбора.Вставить("ПоказыватьВсе",Истина);
	//КонецМарк
	ОткрытьФорму("Документ.ТБК_ЗаявкаОтМагазина.ФормаСписка",ПараметрыОтбора);
...
Процедура ТБКПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка) 
	...
	Если КонстантаВариантМотивации <> Неопределено тогда
		Если КонстантаВариантМотивации = 0 тогда
			Элементы.ОткрытьПланы.Видимость = Ложь;
		КонецЕсли; 
	иначе
		Элементы.ОткрытьПланы.Видимость = Ложь;
	КонецЕсли;
    ...
    ДобавитьПользователяВСертификаты(); 
...
&НаКлиенте
Процедура ТБКОткрытьПланыПосле(Команда)
	ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБКВыполнениеПланов",Новый Структура("Развернуть",Истина));
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВклНовуюМотивациюнаСервере(Магазин)
	...
	Если ВариантМотивации<>6 тогда		
		...
		//МассивСкладовДляПерехода = Новый Массив();
		//МассивСкладовДляПерехода.Добавить("АВТОВО");
		//МассивСкладовДляПерехода.Добавить("Кировский1БОК");
		//МассивСкладовДляПерехода.Добавить("Обводный183ПЕВ");
		//МассивСкладовДляПерехода.Добавить("ЛиговВАА");
		//МассивСкладовДляПерехода.Добавить("Лигов2ПДВ");
		//МассивСкладовДляПерехода.Добавить("Лиговский53ХТА");
		//МассивСкладовДляПерехода.Добавить("Лиговский107ПДВ");
		//МассивСкладовДляПерехода.Добавить("ПлатформаПДВ");
		//МассивСкладовДляПерехода.Добавить("ГалереяПДВ");
		//МассивСкладовДляПерехода.Добавить("ЭлектраМВГ");
		//МассивСкладовДляПерехода.Добавить("ПаркПобедыМВС");
		//МассивСкладовДляПерехода.Добавить("МосковскаяПЕВ");
		//МассивСкладовДляПерехода.Добавить("ОкейВТЕ");
		//МассивСкладовДляПерехода.Добавить("Французский2ШИЛ");
		//МассивСкладовДляПерехода.Добавить("НарваПДВ");
		//МассивСкладовДляПерехода.Добавить("Ленинский2ПДВ");
		//МассивСкладовДляПерехода.Добавить("КонтинентСтачек99ЦНГ");
		//МассивСкладовДляПерехода.Добавить("Советская");
		//
		//Если МассивСкладовДляПерехода.Найти(Строка(НужныйМагазин)) <>Неопределено тогда
			ОбщегоНазначения.УстановитьЗначениеТБККонстанты("ВариантМотивации",6);				
		//КонецЕсли;
...
&НаСервереБезКонтекста
Функция ДобавитьПользователяВСертификаты()
	//Вик 2024-08-14 
	...
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() тогда
		Если Рез.Ссылка.Пользователи.найти(Пользователи.ТекущийПользователь()) = Неопределено тогда
			
			Об = Рез.Ссылка.ПолучитьОбъект();
			НовСтрока				=	Об.Пользователи.Добавить();
...
&НаКлиенте
&Вместо("ОповещениеОткрытьФормуВопросаЗакрытьСмену")
Процедура ТБКОповещениеОткрытьФормуВопросаЗакрытьСмену(РезультатОткрытияФормы, ДополнительныеПараметры)
	...
	Если НЕ РезультатОткрытияФормы = Неопределено Тогда
		...
		//Марк 2025-02-21
		ОбщегоНазначенияРТВызовСервера.ПолучитьТаблицуНеПробитыхЧеков_И_УдалитьГдеКонтрольнаяМаркаНеПроверена(КассаККМ);
		//КонецМарк
		
		//Марк 2026-02-26 https://jira.tabkn.ru/browse/ROZ-19
		ОбщегоНазначенияРТВызовСервера.УдалитьНеПробитыеВозвраты();
		//КонецМарк
...
Процедура ТБКПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	ПраставитьПраваПользователю();
	//КонецМарк

КонецПроцедуры   

Процедура ПраставитьПраваПользователю()
...
// Толстоухов 24.04.2026 ROZ-26 ++
&НаКлиенте
Процедура ТБКОткрытьХозНуждыПосле(Команда)
	//Если ТекущийДеньВПромежутке() Тогда
		ОткрытьФорму("Обработка.РМКУправляемыйРежим.Форма.ТБК_ХозНужды", Новый Структура("Развернуть", Истина));
	//Иначе
	//	Сообщить("Заполнение хоз. товаров доступно с 15 по 20 число текущего месяца!");
	//КонецЕсли;
КонецПроцедуры

// Толстоухов 24.04.2026 ROZ-26 ++
Процедура ПроверкаЗаполненияЗаявкиНаХозТовары()
	Если ТекущийДеньВПромежутке() Тогда
		Элементы.ОткрытьХозНужды.ЦветТекста = WebЦвета.Красный;
	Иначе
		Элементы.ОткрытьХозНужды.ЦветТекста = Элементы.ОТкрытьЗаявкиНаЦенники.ЦветТекста;
	КонецЕсли;   
КонецПроцедуры

// Толстоухов 24.04.2026 ROZ-26 ++
Функция ТекущийДеньВПромежутке() Экспорт
    ТекущаяДата = ТекущаяДата();
    НомерДня = День(ТекущаяДата);
    Возврат НомерДня >= 15 И НомерДня <= 21;
КонецФункции

&НаКлиенте
Процедура ТБКДекорация2НажатиеПосле(Элемент)

	ЗапуститьПриложение("https://docs.google.com/forms/d/1sQKQ1XT_LMX3ec2_7t22nQq5XBKA5oY6I_92lfpdSPk/")
...
&НаКлиенте
Процедура ТБКДекорацияОткрытиеСменыНажатиеПеред(Элемент)

	// Марк 2026-07-24 Перед открытием смены перезаписываем параметры ККТ АТОЛ
	// (связь с ОФД/ИСМ, таймауты проверки КМ). ККТ подключается заново при
	// выполнении операции открытия смены - драйвер получит новые параметры
	// и запишет настройки в кассу.
	Попытка
		Если ОбщегоНазначенияВызовСервера.ТБКОбновитьПараметрыККТРабочегоМеста() > 0 Тогда
			ОбновитьПовторноИспользуемыеЗначения();
		КонецЕсли;
	Исключение
		// ошибка обновления параметров не должна блокировать открытие смены
	КонецПопытки;
	//КонецМарк
...
Процедура ТБКОткрытьСменуЗавершение(РезультатВыполнения, Параметры)
	...
	// Марк 2026-08-04 Сбор версий ПО ККТ для централизованного мониторинга
	Если Не РезультатВыполнения.Результат Тогда
		Возврат;
	КонецЕсли;

	ПараметрыКассы = ЗначениеНастроекВызовСервера.ПараметрыКассыККМ(Параметры.КассаККМ);
	Если Не ЗначениеЗаполнено(ПараметрыКассы.ИдентификаторУстройства) Тогда
		Возврат;
	КонецЕсли;

	Контекст = Новый Структура("ИдентификаторУстройства", ПараметрыКассы.ИдентификаторУстройства);
	Оповещение = Новый ОписаниеОповещения("ТБКСборВерсииККТЗавершение", ЭтотОбъект, Контекст);
	ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьПолучениеПараметровФискальногоУстройства(
		Оповещение, УникальныйИдентификатор, ПараметрыКассы.ИдентификаторУстройства);
	//КонецМарк
	
КонецПроцедуры

&НаКлиенте
Процедура ТБКСборВерсииККТЗавершение(РезультатВыполнения, Контекст) Экспорт

	ПараметрыККТXML = "";
	Если Не РезультатВыполнения.Результат
		Или Не РезультатВыполнения.Свойство("ПараметрыККТXML", ПараметрыККТXML)
		Или ПустаяСтрока(ПараметрыККТXML) Тогда
		Возврат;
	КонецЕсли;

	Данные = ОбщегоНазначенияВызовСервера.ТБКДанныеМониторингаККТ(
		Контекст.ИдентификаторУстройства, ПараметрыККТXML, ИмяКомпьютера());

	Если Данные = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ТБКЗаписатьИнформациюККТ(Данные)

КонецПроцедуры

&НаСервере
Процедура ТБКЗаписатьИнформациюККТ(Данные)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Json = ОбщегоНазначения.СтруктураВjson_Общая(Данные);
	ХранилищеЗначения = Новый ХранилищеЗначения(Json);
	МенеджерЗаписи = РегистрыСведений.ККТ_ТекущаяИнформация.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.ККМ               = Данные.KKM;
	МенеджерЗаписи.Магазин           = Данные.shop;
...
&НаКлиенте
Процедура ТБКУценкаПосле(Команда)
		ОткрытьФорму("Документ.ТБКУценка.Форма.ФормаСписка",); 
