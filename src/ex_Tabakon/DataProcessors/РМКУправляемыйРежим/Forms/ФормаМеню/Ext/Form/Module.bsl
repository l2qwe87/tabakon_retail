...
Процедура ОповещениеОткрытьФормуЗакрытияКассовойСменыПосле(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
...
Процедура ТБКПриОткрытииПосле(Отказ)
	//РабочееМесто	 =	ПолучитьРабочееМесто1();
...
&НаСервере
Функция ПолучитьРабочееМесто1()
	КассаККМ	=	Справочники.КассыККМ.НайтиПоНаименованию("Советская ФР1");
	Возврат КассаККМ.РабочееМесто;
