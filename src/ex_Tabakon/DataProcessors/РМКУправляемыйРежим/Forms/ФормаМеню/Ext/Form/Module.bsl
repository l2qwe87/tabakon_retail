...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	ПровестиПереоценки01092024();//удалить после проведения
...
Процедура ТБКОткрытьСмену(КассаККМ)
	...
	Если КонстантаКлишеИзмененоУспешно<>Неопределено и не КонстантаКлишеИзмененоУспешно тогда
		Ответ	=	МенеджерОборудованияКлиент.УстановитьКлише();//делается 1 раз, от константы
...
Процедура ТБКПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка) 
	...
	//		Элементы.ОткрытьПланы.Видимость	= Истина;
	//	иначе
	//		Элементы.ОткрытьПланы.Видимость	= Ложь;		
	//	КонецЕсли;
	//Иначе
...
Процедура ИзменитьМотивациюНаНовую()
	ЗнаениеКонстанты  = ОбщегоНазначения.ПолучитьЗначениеТБККонстанты("ВариантМотивации");
	
	Если НачалоДня(ТекущаяДата()) = '20240901' И ЗнаениеКонстанты <> 6 Тогда
...
&НаСервереБезКонтекста  
Процедура ПровестиПереоценки01092024()
	
	Если НачалоДня(ТекущаяДата()) = '20240901' Тогда

		Коммент = "Новая переоценка. Одноразовые устройства.НЕ принимать, будет проведена только после согласования автоматически.";
		...
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             ...
		      	 	             |ГДЕ
		      	 	             |	ПрименениеЦенНоменклатуры.Комментарий ПОДОБНО &Комментарий
		      	 	             |	И НЕ ПрименениеЦенНоменклатуры.Проведен
		      	 	             |	И НЕ ПрименениеЦенНоменклатуры.ПометкаУдаления");
		...
		Пока Рез.Следующий() цикл
			...
			Док.ОбъектЦенообразования	=	ОбщегоНазначения.ПолучитьМагазин();
			Попытка
