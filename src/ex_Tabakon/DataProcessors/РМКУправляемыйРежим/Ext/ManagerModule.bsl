Функция ПолучитьТаблицуПоФокуснойМотивации(ИтогоПремииФокус=0, Период, ПереключательМесяцДень =0, Продавец, ТаблицаСоответствийНоменклатурыДляФокуснойМотивации = Неопределено) экспорт
//ФОКУСНАЯ	
	ИтогоПремииФокус	=	0;
	
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.ПланКоличество КАК ПланКоличество,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Процент КАК Процент,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Сумма КАК Сумма,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.СверхПлана КАК СверхПлана,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Тип КАК Тип
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.ТБКФокуснаяМотивация КАК ТБКФокуснаяМотивацияСрезПоследних
	      	 	             |ГДЕ
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Период МЕЖДУ &ДатаНач И &ДатаКон
	      	 	             |
	      	 	             |СГРУППИРОВАТЬ ПО
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.ПланКоличество,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Процент,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Сумма,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.СверхПлана,
	      	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Тип
	      	 	             |
	      	 	             |УПОРЯДОЧИТЬ ПО
	      	 	             |	ПланКоличество УБЫВ");
	Запрос.УстановитьПараметр("ДатаНач",НачалоМесяца(Период));
	Запрос.УстановитьПараметр("ДатаКон",КонецМесяца(Период));

	ГруппыНоменклатуры = Запрос.Выполнить().Выгрузить();
	
	ТаблицаРасшифровка = Новый ТаблицаЗначений;
	ТаблицаРасшифровка.Колонки.Добавить("ФокусГруппа",,		"Группа");
	ТаблицаРасшифровка.Колонки.Добавить("ПланПродаж",,		"План");
	ТаблицаРасшифровка.Колонки.Добавить("ФактПродаж",,		"Факт");
	ТаблицаРасшифровка.Колонки.Добавить("ИтогоКоличество",,	"Итого факт");
	ТаблицаРасшифровка.Колонки.Добавить("КоличествоСверх",,	"Количество сверх плана");
	ТаблицаРасшифровка.Колонки.Добавить("СуммаФакт",,		"Сумма факт");
	ТаблицаРасшифровка.Колонки.Добавить("ИтогоСумма",,		"Итого сумма факт");
	ТаблицаРасшифровка.Колонки.Добавить("СуммаСверх",,		"Сумма сверх плана");
	ТаблицаРасшифровка.Колонки.Добавить("СуммаПремии",,		"Сумма премии");
	ТаблицаРасшифровка.Колонки.Добавить("Заголовок",,		"Заголовок");
	
	
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	|	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
	|	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
	|ПОМЕСТИТЬ Смены
	|ИЗ
	|	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
	|ГДЕ
	|	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТБКСменыСотрудников.Продавец КАК Продавец,
	|	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
	|	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
	|ПОМЕСТИТЬ СменыПродавец
	|ИЗ
	|	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
	|ГДЕ
	|	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
	|	И ТБКСменыСотрудников.Продавец = &Продавец
	|
	|СГРУППИРОВАТЬ ПО
	|	ТБКСменыСотрудников.Продавец,
	|	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	0 КАК ИтогоКоличество,
	|	0 КАК ИтогоСумма,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
	|					ТОГДА ЕСТЬNULL(ВЫБОР
	|								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
	|									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
	|								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
	|							КОНЕЦ, 0)
	|				ИНАЧЕ ЕСТЬNULL(ВЫБОР
	|							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
	|								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
	|							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
	|						КОНЕЦ, 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
	|			КОНЕЦ, 0)) КАК Количество,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
	|					ТОГДА ЕСТЬNULL(ВЫБОР
	|								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
	|									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
	|								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
	|							КОНЕЦ, 0)
	|				ИНАЧЕ ЕСТЬNULL(ВЫБОР
	|							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
	|								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
	|							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
	|						КОНЕЦ, 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
	|			КОНЕЦ, 0)) КАК Сумма,
	|	ЧекККМТоварыПродавец.Ссылка.Дата КАК Дата,
	|	ЧекККМТоварыПродавец.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ вт_Продажи
	|ИЗ
	|	СменыПродавец КАК СменыПродавец
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТоварыПродавец
	|		ПО (НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ) = СменыПродавец.день)
	|			И (ЧекККМТоварыПродавец.Продавец = &Продавец
	|				ИЛИ ЧекККМТоварыПродавец.Продавец1 = &Продавец
	|				ИЛИ ЧекККМТоварыПродавец.Продавец2 = &Продавец)
	|ГДЕ
	|	ЧекККМТоварыПродавец.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И (ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
	|			ИЛИ ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
	|	И ЧекККМТоварыПродавец.Номенклатура.Наименование ПОДОБНО &Наименование
	|	И ЧекККМТоварыПродавец.Ссылка.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов,
	|	ЧекККМТоварыПродавец.Ссылка.Дата,
	|	ЧекККМТоварыПродавец.Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
	|					ТОГДА ЕСТЬNULL(ЧекККМТовары.Количество, 0)
	|				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Количество, 0)
	|			КОНЕЦ, 0)),
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
	|					ТОГДА ЕСТЬNULL(ЧекККМТовары.Сумма, 0)
	|				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Сумма, 0)
	|			КОНЕЦ, 0)),
	|	0,
	|	0,
	|	ЧекККМТовары.Ссылка.Дата,
	|	ЧекККМТовары.Номенклатура
	|ИЗ
	|	Документ.ЧекККМ.Товары КАК ЧекККМТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Смены КАК Смены
	|		ПО (НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ) = Смены.день)
	|ГДЕ
	|	ЧекККМТовары.Ссылка.Проведен
	|	И ЧекККМТовары.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И (ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
	|			ИЛИ ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
	|	И ЧекККМТовары.Номенклатура.Наименование ПОДОБНО &Наименование
	|
	|СГРУППИРОВАТЬ ПО
	|	ЧекККМТовары.Ссылка.КоличествоОсновныхПродавцов,
	|	ЧекККМТовары.Ссылка.Дата,
	|	ЧекККМТовары.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(вт_Продажи.ИтогоКоличество) КАК ИтогоКоличество,
	|	СУММА(вт_Продажи.ИтогоСумма) КАК ИтогоСумма,
	|	СУММА(вт_Продажи.Количество) КАК Количество,
	|	СУММА(вт_Продажи.Сумма) КАК Сумма,
	|	вт_Продажи.Номенклатура КАК Номенклатура,
	|	вт_Продажи.Дата КАК Дата
	|ИЗ
	|	вт_Продажи КАК вт_Продажи
	|
	|СГРУППИРОВАТЬ ПО
	|	вт_Продажи.Номенклатура,
	|	вт_Продажи.Дата
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата");                      // Толстоухов 13.11.2025 // поменял запрос 
	
	Запрос.УстановитьПараметр("Наименование","% БК%"); 	
	Если ПереключательМесяцДень = 0 тогда	
		Запрос.УстановитьПараметр("ДатаНач",НачалоМесяца(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецМесяца(Период));
	Иначе
		Запрос.УстановитьПараметр("ДатаНач",НачалоДня(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецДня(Период));  	
	КонецЕсли;

	Запрос.УстановитьПараметр("Продавец",Продавец);
	
	//Если не ЗначениеЗаполнено(Продавец)  Тогда	
	//	Запрос.Текст	=	СтрЗаменить(Запрос.Текст,"ИЛИ ЧекККМТовары.Продавец1 = &Продавец","");
	//	Запрос.Текст	=	СтрЗаменить(Запрос.Текст,"ИЛИ ЧекККМТовары.Продавец2 = &Продавец","");
	//КонецЕсли; 
	
	Запрос.УстановитьПараметр("СтатусЧекаККМ",	Перечисления.СтатусыЧековККМ.Пробитый);
	Запрос.УстановитьПараметр("СтатусЧекаККМ1",	Перечисления.СтатусыЧековККМ.Архивный);
	Запрос.УстановитьПараметр("ВидПродажа",		Перечисления.ВидыОперацийЧекККМ.Продажа);
	Запрос.УстановитьПараметр("ТипПродавца", 	Перечисления.ТБКТипыПродавцов.ОсновнойПродавец);
	
	//УЧЕТ БК
	ТаблицаБК	=	Запрос.Выполнить().Выгрузить();
	Для каждого Строка из ТаблицаБК цикл
		Строка.Номенклатура	=	Справочники.Номенклатура.НайтиПоНаименованию(СтрЗаменить(Строка.Номенклатура.Наименование, " БК",""));		
	КонецЦикла;
	//
	
	флБеремИзТаблицы	=	Ложь; 
	Если ТаблицаСоответствийНоменклатурыДляФокуснойМотивации <> Неопределено тогда 
		Если ТаблицаСоответствийНоменклатурыДляФокуснойМотивации.Количество() тогда  

			Отбор	=	Новый Структура;
			Отбор.Вставить("Месяц",  НачалоМесяца(Период));
            Строки = ТаблицаСоответствийНоменклатурыДляФокуснойМотивации.НайтиСтроки(Отбор);
			
			Если Строки.Количество() тогда
				флБеремИзТаблицы = истина;
			КонецЕсли;
			
		КонецЕсли;  
	КонецЕсли;
	
	Для каждого СтрокаГруппа из ГруппыНоменклатуры цикл
		
		Если флБеремИзТаблицы тогда
			Отбор	=	Новый Структура;
			Отбор.Вставить("Месяц",  НачалоМесяца(Период));
			Отбор.Вставить("Тип",  	 СтрокаГруппа.Тип);
			
			НужСтрока	=	 ТаблицаСоответствийНоменклатурыДляФокуснойМотивации.НайтиСтроки(Отбор);
			МассивНоменклатурыНовый	=	НужСтрока[0].Массив.ВыгрузитьЗначения();	
		иначе
			ЗапросН	=	Новый Запрос("ВЫБРАТЬ
			       	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Номенклатура КАК Номенклатура
			       	 	             |ИЗ
			       	 	             |	РегистрСведений.ТБКФокуснаяМотивация КАК ТБКФокуснаяМотивацияСрезПоследних
			       	 	             |ГДЕ
			       	 	             |	ТБКФокуснаяМотивацияСрезПоследних.Период МЕЖДУ &ДатаНач И &ДатаКон
			       	 	             |	И ТБКФокуснаяМотивацияСрезПоследних.Тип = &Тип");
			ЗапросН.УстановитьПараметр("Тип", СтрокаГруппа.Тип);
			ЗапросН.УстановитьПараметр("ДатаНач", НачалоМесяца(Период));
			ЗапросН.УстановитьПараметр("ДатаКон", КонецМесяца(Период));
			
			РезН	=	ЗапросН.Выполнить().Выгрузить();
			МассивНоменклатуры	=	РезН.ВыгрузитьКолонку("Номенклатура");
			
			МассивНоменклатурыНовый = ПолучитьСписокНоменклатуры(МассивНоменклатуры); 
			
			Если ТаблицаСоответствийНоменклатурыДляФокуснойМотивации <> Неопределено Тогда
				НовСтрока	=	ТаблицаСоответствийНоменклатурыДляФокуснойМотивации.Добавить();
				НовСтрока.Тип		=	СтрокаГруппа.Тип;
				НовСтрока.Месяц		=	НачалоМесяца(Период);
				НовСтрока.Массив.ЗагрузитьЗначения(МассивНоменклатурыНовый);
			КонецЕсли;	
			//СоответствиеНоменклатурыДляФокуснойМотивации.Вставить(СтрокаГруппа.Тип, МассивНоменклатуры);
		КонецЕсли;
		
		
		БонусЗаГруппу			=	0;
		СуммаПродажСверхПлана	=	0;
		
		
		ПланКоличество		=  СтрокаГруппа.ПланКоличество;   
		ПроцентПремии		=  СтрокаГруппа.Процент;
		СуммаПремии			=  СтрокаГруппа.Сумма;
		
		ИтогоПродано 		= 0;
		ИтогоПроданоСумма	= 0;
		
		Если СтрокаГруппа.СверхПлана тогда   //только за то что сверх плана
			ТЗПродано	=	ПолучитьКоличествоПродажПоГруппе(Продавец, МассивНоменклатурыНовый,,ТаблицаБК,ПереключательМесяцДень,Период);			

			Для каждого Строка из ТЗПродано цикл
				ИтогоПродано		=	ИтогоПродано 		+ Строка.Количество; 
				ИтогоПроданоСумма	=	ИтогоПроданоСумма 	+ Строка.Сумма;
				
				
				Если ИтогоПродано > ПланКоличество тогда
					Разница				= ИтогоПродано - ПланКоличество;
						
					КНачислению			= Мин(Разница, Строка.Количество); 
					СуммаПродажСверхПлана	=	0;
					
					Если ПроцентПремии>0  тогда    
						КНачислениюСумма		=   (Строка.Сумма /Строка.Количество)*КНачислению ;			
						ИтогоПремииФокус		=	ИтогоПремииФокус   		+ КНачислениюСумма/100 * ПроцентПремии;
						БонусЗаГруппу			=	БонусЗаГруппу 			+ КНачислениюСумма/100 * ПроцентПремии;
						СуммаПродажСверхПлана	=	СуммаПродажСверхПлана 	+ КНачислениюСумма;
					иначе
						ИтогоПремииФокус		=	ИтогоПремииФокус   		+ КНачислению * СуммаПремии;
						БонусЗаГруппу			=	БонусЗаГруппу 			+ КНачислению * СуммаПремии;
						СуммаПродажСверхПлана	=	СуммаПродажСверхПлана 	+ (Строка.Сумма /Строка.Количество) * КНачислению;
					КонецЕсли;
					
				КонецЕсли;				
			КонецЦикла; 
			
			Если ПроцентПремии>0  тогда    
				НужЗаголовок	=	"Процент премии на сумму сверх плана. "+ Символы.ПС +СуммаПродажСверхПлана +" * "+ ПроцентПремии+"% = "+БонусЗаГруппу;
			иначе
				НужЗаголовок	=	Строка(СуммаПремии) + " за каждую проданную штуку сверх плана. "+ Символы.ПС  + Макс(ИтогоПродано - ПланКоличество,0) + " * " +СуммаПремии + " = "+БонусЗаГруппу;
			КонецЕсли;
			
			НовСтр	=	ТаблицаРасшифровка.Добавить();
			НовСтр.Заголовок					=	НужЗаголовок;
			НовСтр.ФокусГруппа					=	СтрокаГруппа.Тип;
			НовСтр.ПланПродаж					=	ПланКоличество;
			НовСтр.ФактПродаж					=	ИтогоПродано;
			НовСтр.КоличествоСверх				=	Макс(НовСтр.ФактПродаж - ПланКоличество,0);
			НовСтр.СуммаФакт					=	ИтогоПроданоСумма;
			НовСтр.СуммаСверх					=	СуммаПродажСверхПлана;
			НовСтр.СуммаПремии					=	БонусЗаГруппу;
			
		иначе //начисляем за все
			ТЗПродано	=	ПолучитьКоличествоПродажПоГруппе(Продавец, МассивНоменклатурыНовый, Истина, ТаблицаБК, ПереключательМесяцДень, Период);			
			
			Если ТЗПродано.количество() > 0 тогда
				ИтогоПродано		=	ТЗПродано[0].Количество; 
				ИтогоПроданоСумма	=	ТЗПродано[0].Сумма;
				
				ИтогоКоличество		=	ТЗПродано[0].ИтогоКоличество; 
				ИтогоСумма	        =	ТЗПродано[0].ИтогоСумма;				
			КонецЕсли; 
			
			Если ИтогоПродано > ПланКоличество тогда
				
				Если ПроцентПремии > 0  тогда    
					КНачислениюСумма		=   (ИтогоПроданоСумма/100)	 * ПроцентПремии;			
					ИтогоПремииФокус		=	ИтогоПремииФокус   		 + КНачислениюСумма;
					БонусЗаГруппу			=	БонусЗаГруппу			 + КНачислениюСумма; 					
				иначе 					
					ИтогоПремииФокус		=	ИтогоПремииФокус    + ИтогоПродано * СуммаПремии;
					БонусЗаГруппу			=	БонусЗаГруппу 		+ ИтогоПродано * СуммаПремии;
				КонецЕсли;
				
			//Марк 2025-02-03 
			Иначе
				
				ИтогоПремииФокус		=	ИтогоПремииФокус    + ИтогоПродано * СуммаПремии;
				БонусЗаГруппу			=	БонусЗаГруппу 		+ ИтогоПродано * СуммаПремии;
			//КонецМарк	
			КонецЕсли;  			
			
			Если ПроцентПремии > 0  тогда    
				НужЗаголовок	=	"Процент премии на сумму продаж. "+ Символы.ПС +ИтогоПроданоСумма + " * "+ ПроцентПремии+"% = "+ БонусЗаГруппу;
			иначе
				НужЗаголовок	=	Строка(СуммаПремии) + " за каждую проданную штуку. "+ Символы.ПС + ИтогоПродано + " * " +СуммаПремии + " = "+БонусЗаГруппу;
			КонецЕсли;  

			
			НовСтр	=	ТаблицаРасшифровка.Добавить();
			НовСтр.ФокусГруппа					=	СтрокаГруппа.Тип;
			НовСтр.Заголовок					=	НужЗаголовок;
			НовСтр.ПланПродаж					=	ПланКоличество;
			НовСтр.ФактПродаж					=	ИтогоПродано;
			//НовСтр.КоличествоСверх				=	ИтогоПродано;
			НовСтр.СуммаФакт					=	ИтогоПроданоСумма;
			//НовСтр.СуммаСверх					=	ИтогоПроданоСумма;
			НовСтр.СуммаПремии					=	БонусЗаГруппу;
			
			НовСтр.ИтогоКоличество				=	ИтогоКоличество;
			НовСтр.ИтогоСумма					=	ИтогоСумма;
	
		КонецЕсли;
		
	КонецЦикла;
	
	//ОТДЕЛЬНО УЦЕНКА
	СтруктураДанныхУценка	=	ПолучитьДанныеПоУценке(ПереключательМесяцДень, Период, Продавец);  
	
	НовСтр	=	ТаблицаРасшифровка.Добавить();
	
	НужЗаголовок	=	"Процент премии на сумму продаж. " + Символы.ПС + СтруктураДанныхУценка.СуммаТабак + " * 10% = "+ СтруктураДанныхУценка.ПремияТабак;
	НовСтр.Заголовок					=	НужЗаголовок;
	НовСтр.ФокусГруппа					=	"Табак (Уценка)";
	НовСтр.ПланПродаж					=	0;
	НовСтр.ФактПродаж					=	СтруктураДанныхУценка.ПроданоТабак;
	//НовСтр.КоличествоСверх				=	НовСтр.ФактПродаж;
	НовСтр.СуммаФакт					=	СтруктураДанныхУценка.СуммаТабак;
	//НовСтр.СуммаСверх					=	СтруктураДанныхУценка.СуммаТабак;
	НовСтр.СуммаПремии					=	СтруктураДанныхУценка.ПремияТабак;
	
	НовСтр.ИтогоКоличество				=	СтруктураДанныхУценка.ПроданоТабак;
	НовСтр.ИтогоСумма					=	СтруктураДанныхУценка.СуммаТабак;
	
	НовСтр	=	ТаблицаРасшифровка.Добавить();
	НужЗаголовок	=	"Процент премии на сумму продаж. " + Символы.ПС +СтруктураДанныхУценка.СуммаСувениры + " * 20% = "+ СтруктураДанныхУценка.ПремияСувениры;
	НовСтр.Заголовок					=	НужЗаголовок;
	НовСтр.ФокусГруппа					=	"Сувениры (Уценка)";
	НовСтр.ПланПродаж					=	0;
	НовСтр.ФактПродаж					=	СтруктураДанныхУценка.ПроданоСувениры;
	//НовСтр.КоличествоСверх				=	НовСтр.ФактПродаж;
	НовСтр.СуммаФакт					=	СтруктураДанныхУценка.СуммаСувениры;
	//НовСтр.СуммаСверх					=	СтруктураДанныхУценка.СуммаСувениры;
	НовСтр.СуммаПремии					=	СтруктураДанныхУценка.ПремияСувениры; 
	
	ИтогоПремииФокус					=	ИтогоПремииФокус + СтруктураДанныхУценка.ПремияСувениры + СтруктураДанныхУценка.ПремияТабак;
	
	НовСтр.ИтогоКоличество				=	СтруктураДанныхУценка.ПроданоСувениры;
	НовСтр.ИтогоСумма					=	СтруктураДанныхУценка.СуммаСувениры;
	//      
	
	Возврат ТаблицаРасшифровка;
КонецФункции

Функция ПолучитьКоличествоПродажПоГруппе(Продавец_, Номенклатура, НеГруппироватьПоДате = Ложь, ТаблицаБК = Неопределено, ПереключательМесяцДень, Период) 
	
	Если НеГруппироватьПоДате тогда
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             |	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
		      	 	             |ПОМЕСТИТЬ Смены
		      	 	             |ИЗ
		      	 	             |	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
		      	 	             |ГДЕ
		      	 	             |	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	ТБКСменыСотрудников.Продавец КАК Продавец,
		      	 	             |	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
		      	 	             |ПОМЕСТИТЬ СменыПродавец
		      	 	             |ИЗ
		      	 	             |	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
		      	 	             |ГДЕ
		      	 	             |	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
		      	 	             |	И ТБКСменыСотрудников.Продавец = &Продавец
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	ТБКСменыСотрудников.Продавец,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	0 КАК ИтогоКоличество,
		      	 	             |	0 КАК ИтогоСумма,
		      	 	             |	ЕСТЬNULL(ВЫБОР
		      	 	             |			КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
		      	 	             |				ТОГДА ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |							КОНЕЦ), 0)
		      	 	             |			ИНАЧЕ ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |						КОНЕЦ), 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |		КОНЕЦ, 0) КАК Количество,
		      	 	             |	ЕСТЬNULL(ВЫБОР
		      	 	             |			КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
		      	 	             |				ТОГДА ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |							КОНЕЦ), 0)
		      	 	             |			ИНАЧЕ ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |						КОНЕЦ), 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |		КОНЕЦ, 0) КАК Сумма,
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ) КАК Дата
		      	 	             |ПОМЕСТИТЬ вт_Продажи
		      	 	             |ИЗ
		      	 	             |	СменыПродавец КАК СменыПродавец
		      	 	             |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТоварыПродавец
		      	 	             |		ПО (НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ) = СменыПродавец.день)
		      	 	             |			И (ЧекККМТоварыПродавец.Продавец = &Продавец
		      	 	             |				ИЛИ ЧекККМТоварыПродавец.Продавец1 = &Продавец
		      	 	             |				ИЛИ ЧекККМТоварыПродавец.Продавец2 = &Продавец)
		      	 	             |ГДЕ
		      	 	             |	ЧекККМТоварыПродавец.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И (ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
		      	 	             |			ИЛИ ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
		      	 	             |	И ЧекККМТоварыПродавец.Номенклатура В(&Номенклатура)
		      	 	             |	И ЧекККМТоварыПродавец.Ссылка.Проведен
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ),
		      	 	             |	ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |
		      	 	             |ОБЪЕДИНИТЬ ВСЕ
		      	 	             |
		      	 	             |ВЫБРАТЬ
		      	 	             |	ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |					ТОГДА ЕСТЬNULL(ЧекККМТовары.Количество, 0)
		      	 	             |				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Количество, 0)
		      	 	             |			КОНЕЦ), 0),
		      	 	             |	ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |					ТОГДА ЕСТЬNULL(ЧекККМТовары.Сумма, 0)
		      	 	             |				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Сумма, 0)
		      	 	             |			КОНЕЦ), 0),
		      	 	             |	0,
		      	 	             |	0,
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ)
		      	 	             |ИЗ
		      	 	             |	Документ.ЧекККМ.Товары КАК ЧекККМТовары
		      	 	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Смены КАК Смены
		      	 	             |		ПО (НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ) = Смены.день)
		      	 	             |ГДЕ
		      	 	             |	ЧекККМТовары.Ссылка.Проведен
		      	 	             |	И ЧекККМТовары.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И (ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
		      	 	             |			ИЛИ ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
		      	 	             |	И ЧекККМТовары.Номенклатура В(&Номенклатура)
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ),
		      	 	             |	ЧекККМТовары.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	СУММА(вт_Продажи.ИтогоКоличество) КАК ИтогоКоличество,
		      	 	             |	СУММА(вт_Продажи.ИтогоСумма) КАК ИтогоСумма,
		      	 	             |	СУММА(вт_Продажи.Количество) КАК Количество,
		      	 	             |	СУММА(вт_Продажи.Сумма) КАК Сумма
		      	 	             |ИЗ
		      	 	             |	вт_Продажи КАК вт_Продажи");
		
		Если ТаблицаБК <> Неопределено тогда
			ТаблицаБК.Свернуть("Номенклатура","Количество, Сумма, ИтогоСумма, ИтогоКоличество");
		КонецЕсли;
		
	иначе
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             |	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
		      	 	             |ПОМЕСТИТЬ Смены
		      	 	             |ИЗ
		      	 	             |	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
		      	 	             |ГДЕ
		      	 	             |	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	ТБКСменыСотрудников.Продавец КАК Продавец,
		      	 	             |	МАКСИМУМ(ТБКСменыСотрудников.ТипПродавца) КАК ТипПродавца,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ) КАК день
		      	 	             |ПОМЕСТИТЬ СменыПродавец
		      	 	             |ИЗ
		      	 	             |	РегистрСведений.ТБКСменыСотрудников КАК ТБКСменыСотрудников
		      	 	             |ГДЕ
		      	 	             |	ТБКСменыСотрудников.КассоваяСмена.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И ТБКСменыСотрудников.ТипПродавца = ЗНАЧЕНИЕ(Перечисление.ТБКТипыПродавцов.основнойПродавец)
		      	 	             |	И ТБКСменыСотрудников.Продавец = &Продавец
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	ТБКСменыСотрудников.Продавец,
		      	 	             |	НАЧАЛОПЕРИОДА(ТБКСменыСотрудников.КассоваяСмена.Дата, ДЕНЬ)
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	0 КАК ИтогоКоличество,
		      	 	             |	0 КАК ИтогоСумма,
		      	 	             |	ЕСТЬNULL(ВЫБОР
		      	 	             |			КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
		      	 	             |				ТОГДА ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |							КОНЕЦ), 0)
		      	 	             |			ИНАЧЕ ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Количество, 0)
		      	 	             |						КОНЕЦ), 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |		КОНЕЦ, 0) КАК Количество,
		      	 	             |	ЕСТЬNULL(ВЫБОР
		      	 	             |			КОГДА ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов = 0
		      	 	             |				ТОГДА ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |								КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |									ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |								ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |							КОНЕЦ), 0)
		      	 	             |			ИНАЧЕ ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |							КОГДА ЧекККМТоварыПродавец.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |								ТОГДА ЕСТЬNULL(ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |							ИНАЧЕ ЕСТЬNULL(-ЧекККМТоварыПродавец.Сумма, 0)
		      	 	             |						КОНЕЦ), 0) / ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |		КОНЕЦ, 0) КАК Сумма,
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ) КАК Дата
		      	 	             |ПОМЕСТИТЬ вт_Продажи
		      	 	             |ИЗ
		      	 	             |	СменыПродавец КАК СменыПродавец
		      	 	             |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТоварыПродавец
		      	 	             |		ПО (НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ) = СменыПродавец.день)
		      	 	             |			И (ЧекККМТоварыПродавец.Продавец = &Продавец
		      	 	             |				ИЛИ ЧекККМТоварыПродавец.Продавец1 = &Продавец
		      	 	             |				ИЛИ ЧекККМТоварыПродавец.Продавец2 = &Продавец)
		      	 	             |ГДЕ
		      	 	             |	ЧекККМТоварыПродавец.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И (ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
		      	 	             |			ИЛИ ЧекККМТоварыПродавец.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
		      	 	             |	И ЧекККМТоварыПродавец.Номенклатура В(&Номенклатура)
		      	 	             |	И ЧекККМТоварыПродавец.Ссылка.Проведен
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТоварыПродавец.Ссылка.Дата, ДЕНЬ),
		      	 	             |	ЧекККМТоварыПродавец.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |
		      	 	             |ОБЪЕДИНИТЬ ВСЕ
		      	 	             |
		      	 	             |ВЫБРАТЬ
		      	 	             |	ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |					ТОГДА ЕСТЬNULL(ЧекККМТовары.Количество, 0)
		      	 	             |				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Количество, 0)
		      	 	             |			КОНЕЦ), 0),
		      	 	             |	ЕСТЬNULL(СУММА(ВЫБОР
		      	 	             |				КОГДА ЧекККМТовары.Ссылка.ВидОперации = &ВидПродажа
		      	 	             |					ТОГДА ЕСТЬNULL(ЧекККМТовары.Сумма, 0)
		      	 	             |				ИНАЧЕ ЕСТЬNULL(-ЧекККМТовары.Сумма, 0)
		      	 	             |			КОНЕЦ), 0),
		      	 	             |	0,
		      	 	             |	0,
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ)
		      	 	             |ИЗ
		      	 	             |	Документ.ЧекККМ.Товары КАК ЧекККМТовары
		      	 	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Смены КАК Смены
		      	 	             |		ПО (НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ) = Смены.день)
		      	 	             |ГДЕ
		      	 	             |	ЧекККМТовары.Ссылка.Проведен
		      	 	             |	И ЧекККМТовары.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
		      	 	             |	И (ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ
		      	 	             |			ИЛИ ЧекККМТовары.Ссылка.СтатусЧекаККМ = &СтатусЧекаККМ1)
		      	 	             |	И ЧекККМТовары.Номенклатура В(&Номенклатура)
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.Дата, ДЕНЬ),
		      	 	             |	ЧекККМТовары.Ссылка.КоличествоОсновныхПродавцов
		      	 	             |;
		      	 	             |
		      	 	             |////////////////////////////////////////////////////////////////////////////////
		      	 	             |ВЫБРАТЬ
		      	 	             |	СУММА(вт_Продажи.ИтогоКоличество) КАК ИтогоКоличество,
		      	 	             |	СУММА(вт_Продажи.ИтогоСумма) КАК ИтогоСумма,
		      	 	             |	СУММА(вт_Продажи.Количество) КАК Количество,
		      	 	             |	СУММА(вт_Продажи.Сумма) КАК Сумма,
		      	 	             |	вт_Продажи.Дата КАК Дата
		      	 	             |ИЗ
		      	 	             |	вт_Продажи КАК вт_Продажи
		      	 	             |
		      	 	             |СГРУППИРОВАТЬ ПО
		      	 	             |	вт_Продажи.Дата");
	КонецЕсли;
		
	Если ПереключательМесяцДень = 0 тогда	
		Запрос.УстановитьПараметр("ДатаНач",НачалоМесяца(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецМесяца(Период));
	иначе
		Запрос.УстановитьПараметр("ДатаНач",НачалоДня(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецДня(Период));  	
	КонецЕсли;

	Запрос.УстановитьПараметр("Номенклатура",Номенклатура);
	Запрос.УстановитьПараметр("Продавец",Продавец_);
	Запрос.УстановитьПараметр("СтатусЧекаККМ",Перечисления.СтатусыЧековККМ.Пробитый);
	Запрос.УстановитьПараметр("СтатусЧекаККМ1",Перечисления.СтатусыЧековККМ.Архивный);
	Запрос.УстановитьПараметр("ВидПродажа",Перечисления.ВидыОперацийЧекККМ.Продажа);
	
	Если не ЗначениеЗаполнено(Продавец_)  Тогда	
		Запрос.Текст	=	СтрЗаменить(Запрос.Текст,"ИЛИ ЧекККМТовары.Продавец1 = &Продавец","");
		Запрос.Текст	=	СтрЗаменить(Запрос.Текст,"ИЛИ ЧекККМТовары.Продавец2 = &Продавец","");
	КонецЕсли; 
	
	Рез = Запрос.Выполнить().Выгрузить();
	
	Если НеГруппироватьПоДате тогда
		Рез.Свернуть("", "Количество, Сумма, ИтогоСумма, ИтогоКоличество");  	
	иначе
		Рез.Свернуть("Дата", "Количество, Сумма, ИтогоСумма, ИтогоКоличество");  	
	КонецЕсли;
	
	Если ТаблицаБК <> Неопределено тогда
		Если ТаблицаБК.Количество()>0 тогда
			Запрос	=	Новый запрос("ВЫБРАТЬ
			      	 	             |	ТаблицаБК.Количество КАК Количество,
			      	 	             |	ТаблицаБК.Сумма КАК Сумма,
			      	 	             |	ТаблицаБК.ИтогоСумма КАК ИтогоСумма,
			      	 	             |	ТаблицаБК.ИтогоКоличество КАК ИтогоКоличество
			      	 	             |ПОМЕСТИТЬ Итог
			      	 	             |ИЗ
			      	 	             |	&ТаблицаБК КАК ТаблицаБК
			      	 	             |ГДЕ
			      	 	             |	ТаблицаБК.Номенклатура В (&Номенклатура)
			      	 	             |;
			      	 	             |
			      	 	             |////////////////////////////////////////////////////////////////////////////////
			      	 	             |ВЫБРАТЬ
			      	 	             |	Итог.Количество КАК Количество,
			      	 	             |	Итог.Сумма КАК Сумма,
			      	 	             |	Итог.ИтогоСумма КАК ИтогоСумма,
			      	 	             |	Итог.ИтогоКоличество КАК ИтогоКоличество
			      	 	             |ИЗ
			      	 	             |	Итог КАК Итог");
			Запрос.УстановитьПараметр("Номенклатура", Номенклатура);    
			Запрос.УстановитьПараметр("ТаблицаБК", ТаблицаБК);

			РезБК	=	запрос.Выполнить().Выгрузить();
			Для каждого Строка из РезБК цикл
				НоваяСтрока	=	Рез.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
				Рез.Свернуть("","Количество, Сумма, ИтогоСумма, ИтогоКоличество");
			КонецЦикла;
			
			Если не НеГруппироватьПоДате тогда
				Рез.Свернуть("","Количество, Сумма, Дата, ИтогоСумма, ИтогоКоличество");
				Рез.Сортировать("Дата");
			КонецЕсли;
			
		КонецЕсли;
	КонецЕсли;
	                                                           
	
	Возврат рез;
	
КонецФункции

Функция ПолучитьСписокНоменклатуры(Номенклатура) Экспорт
	
	МассивНоменклатурыНовый = Новый Массив;   
	
	Для каждого Строка Из Номенклатура Цикл
		
		МассивГруппДляПовторногоОбхода = Новый Массив;
		МассивГруппДляПовторногоОбхода.Добавить(Строка);
		
		~ПоНовой:
		
		Ном = 0;
		Для каждого ГруппаПовтор Из МассивГруппДляПовторногоОбхода Цикл
			
			Если ГруппаПовтор.ЭтоГруппа Тогда	
				Спр = Справочники.Номенклатура.Выбрать(ГруппаПовтор);
				
				Пока Спр.Следующий() Цикл  
					Если Спр.ПометкаУдаления Тогда Продолжить; КонецЕсли;
					
					//рекурсия
					Если Спр.ЭтоГруппа Тогда
						МассивГруппДляПовторногоОбхода.Добавить(Спр.Ссылка); 
					Иначе
						МассивНоменклатурыНовый.Добавить(Спр.Ссылка); 
					КонецЕсли;
					
				КонецЦикла; 		
			Иначе
				МассивНоменклатурыНовый.Добавить(ГруппаПовтор); 
			КонецЕсли;
			
			МассивГруппДляПовторногоОбхода.Удалить(Ном); 
			
			Ном = Ном+1;  
		КонецЦикла;       
		
		Если МассивГруппДляПовторногоОбхода.Количество() Тогда      
			Перейти ~ПоНовой;
		КонецЕсли;  	
		
	КонецЦикла;
	
	Возврат МассивНоменклатурыНовый;
	
КонецФункции	

Функция ПолучитьДанныеПоУценке(ПереключательМесяцДень, Период, Продавец)	
	Запрос	=	новый запрос("ВЫБРАТЬ
	      	 	             |	КомментарииСтатистики.ХешНаименования КАК ХешНаименования,
	      	 	             |	КомментарииСтатистики.Наименование КАК Наименование
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.КомментарииСтатистики КАК КомментарииСтатистики
	      	 	             |ГДЕ
	      	 	             |	КомментарииСтатистики.Наименование ПОДОБНО &НаименованиеТБК
	      	 	             |	И ПОДСТРОКА(КомментарииСтатистики.ХешНаименования, 4, 2) = &Месяц
	      	 	             |	И ПОДСТРОКА(КомментарииСтатистики.ХешНаименования, 7, 4) = &Год
	      	 	             |	И ПОДСТРОКА(КомментарииСтатистики.ХешНаименования, 1, 2) = &День");
	Запрос.УстановитьПараметр("НаименованиеТБК","%tbkskidka%");

	ДеньСтрока	=	Строка(День(Период));
	МесяцСтрока	=	Строка(Месяц(Период));
	ГодСтрока	=	ОбщегоНазначения.УдалениеНезначимыхСимволов(Строка(Год(Период)));

	Если СтрДлина(МесяцСтрока) = 1 тогда МесяцСтрока = "0"+ МесяцСтрока; КонецЕсли;
	
	Запрос.УстановитьПараметр("Месяц",МесяцСтрока);
	Запрос.УстановитьПараметр("Год",ГодСтрока);
	
	Если ПереключательМесяцДень = 1 тогда
		Если СтрДлина(ДеньСтрока) = 1 тогда ДеньСтрока = "0"+ ДеньСтрока; КонецЕсли;
		Запрос.УстановитьПараметр("День",ДеньСтрока);   
	иначе
		Запрос.Текст	=	СтрЗаменить(Запрос.Текст,"И ПОДСТРОКА(КомментарииСтатистики.ХешНаименования, 1, 2) = &День","");		
	КонецЕсли;
	
	МассивВозврата	=	Новый Массив;
	
	
	ПроданоТабак	=	0;
	ПроданоСувениры	=	0;
	ПремияТабак		=	0;
	ПремияСувениры	=	0;
	СуммаТабак		=	0;
	СуммаСувениры	=	0;
	
	Рез = Запрос.Выполнить().Выгрузить();
	
	Для каждого Строка из рез цикл
		КолОсновныхПродавцов	=	0;

		//изменения 20240205
		Если ЗначениеЗаполнено(Продавец) и ТекущаяДата() > '20240301' тогда
			НомерЧека	=	Прав(Строка.ХешНаименования,11);
			СтрокаДата	=	Лев(Строка.ХешНаименования,19); 
			Если Прав(СтрокаДата,1) = ";" тогда
				СтрокаДата	=	Лев(СтрокаДата,18);	
			КонецЕсли;
			ДатаЧека	=	Дата(СтрокаДата);

			НужЧек		=	Документы.ЧекККМ.НайтиПоНомеру(НомерЧека, ДатаЧека);
			
			Если Не НужЧек.Пустая() тогда
				Если НужЧек.Товары.Количество() >0 тогда
					Если НужЧек.Товары[0].Продавец <> Продавец  тогда
						
						Если ЗначениеЗаполнено(Продавец) тогда
							Если НужЧек.Товары[0].Продавец1 <> Продавец и НужЧек.Товары[0].Продавец2 <> Продавец Тогда
								Продолжить;
							КонецЕсли;
						иначе
							Продолжить;												
						КонецЕсли; 
						
						
						
					КонецЕсли;
					КолОсновныхПродавцов	=	НужЧек.КоличествоОсновныхПродавцов;	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Данные		=	Строка.Наименование;		
		Поз3		=	СтрНайти(Данные,"/",,,3);
		Поз9		=	СтрНайти(Данные,"/",,,9);
		Поз10		=	СтрНайти(Данные,"/",,,10);
		ПозЗагружен	=	СтрНайти(Данные,"загружен");
		
		КодГруппы	=	Сред(Данные,Поз3+1,11);
		
		КодГруппы	=	СтрЗаменить(КодГруппы,"u","Ю");
		
		Попытка
			Если Поз10 = 0 тогда 
				Если ПозЗагружен >0 тогда
					Сумма		=	Сред(Данные,Поз9+1, ПозЗагружен-Поз9-1);
				иначе
					Сумма		=	Сред(Данные,Поз9+1);
				КонецЕсли;
				Сумма	=	Число(Сумма);
				
			иначе
				Если ПозЗагружен >0 тогда
					Сумма		=	Сред(Данные,Поз10+1, ПозЗагружен-Поз10-1);
				иначе
					Сумма		=	Сред(Данные,Поз10+1);
				КонецЕсли;
				Сумма	=	Число(Сумма);
			КонецЕсли;
			
		Исключение
			Продолжить;
		КонецПопытки;
		
		Если КолОсновныхПродавцов>0  Тогда
			Сумма	=	Сумма /КолОсновныхПродавцов;		
		КонецЕсли; 


		ПК			=	Справочники.Номенклатура.НайтиПоКоду(КодГруппы).ПолныйКод();
		Если найти(ПК, "00048205349") тогда //Это Табак
			ПроданоТабак	=	ПроданоТабак +1;
			ПремияТабак		=	ПремияТабак + (Сумма/100)*10;	
			Попытка
				СуммаТабак		=	СуммаТабак + Сумма;
			Исключение
			КонецПопытки;
		иначе
			ПроданоСувениры	=	ПроданоСувениры + 1;
			ПремияСувениры	=	ПремияСувениры  +  (Сумма/100)*20;	
			Попытка
				СуммаСувениры	=	СуммаСувениры + Сумма;
			Исключение
			КонецПопытки;
		КонецЕсли;
		
	КонецЦикла;
	
	//Новая Уценка
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	ТБКУценка_Новая.Чек КАК Чек,
	      	 	             |	ТБКУценка_Новая.ПроцентСкидки КАК ПроцентСкидки,
	      	 	             |	ТБКУценка_Новая.СуммаСкидки КАК СуммаСкидки,
	      	 	             |	ТБКУценка_Новая.Номенклатура КАК Номенклатура,
	      	 	             |	ТБКУценка_Новая.Цена КАК Цена
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.ТБКУценка_Новая КАК ТБКУценка_Новая
	      	 	             |ГДЕ
	      	 	             |	ТБКУценка_Новая.Чек.Дата МЕЖДУ &ДатаНач И &ДатаКон");
	Если ПереключательМесяцДень = 0 тогда	
		Запрос.УстановитьПараметр("ДатаНач",НачалоМесяца(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецМесяца(Период));
	иначе
		Запрос.УстановитьПараметр("ДатаНач",НачалоДня(Период));
		Запрос.УстановитьПараметр("ДатаКон",КонецДня(Период));  	
	КонецЕсли;
	
	Рез = Запрос.Выполнить().Выгрузить();
	
	Для каждого Строка из рез цикл
		КолОсновныхПродавцов	=	0;

		//изменения 20240205
		Если ЗначениеЗаполнено(Продавец) и ТекущаяДата() > '20240301' тогда
			НужЧек		=	Строка.Чек;
			
			Если Не НужЧек.Пустая() тогда
				Если НужЧек.Товары.Количество() >0 тогда
					Если НужЧек.Товары[0].Продавец <> Продавец  тогда
						
						Если ЗначениеЗаполнено(Продавец) тогда
							Если НужЧек.Товары[0].Продавец1 <> Продавец и НужЧек.Товары[0].Продавец2 <> Продавец Тогда
								Продолжить;
							КонецЕсли;
						иначе
							Продолжить;												
						КонецЕсли; 
	
					КонецЕсли;
					КолОсновныхПродавцов	=	НужЧек.КоличествоОсновныхПродавцов;	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		//СЧИТАЕТСЯ СУММА С УЧЕТОМ СКИДКИ!!
		//СуммаБезСкидки	=	  Строка.СуммаСкидки / (Строка.ПроцентСкидки / 100);
		//СуммаСоСкидкой	=	  СуммаБезСкидки	-	Строка.СуммаСкидки;	
		Сумма			=	Строка.Цена	-	Строка.СуммаСкидки;
		
		Если КолОсновныхПродавцов>0  Тогда
			Сумма	=	Сумма /КолОсновныхПродавцов;		
		КонецЕсли; 


		ПК			=	Строка.Номенклатура.ПолныйКод();
		Если найти(ПК, "00048205349") тогда //Это Табак
			ПроданоТабак	=	ПроданоТабак +1;
			ПремияТабак		=	ПремияТабак + (Сумма/100)*10;	
			Попытка
				СуммаТабак		=	СуммаТабак + Сумма;
			Исключение
			КонецПопытки;
		иначе
			ПроданоСувениры	=	ПроданоСувениры + 1;
			ПремияСувениры	=	ПремияСувениры  +  (Сумма/100)*20;	
			Попытка
				СуммаСувениры	=	СуммаСувениры + Сумма;
			Исключение
			КонецПопытки;
		КонецЕсли;
		
	КонецЦикла;	
	//
	
	СтруктураПремийУценка	=	Новый Структура;
	СтруктураПремийУценка.Вставить("ПроданоТабак", 		ПроданоТабак);
	СтруктураПремийУценка.Вставить("ПроданоСувениры", 	ПроданоСувениры);
	СтруктураПремийУценка.Вставить("СуммаТабак", 		СуммаТабак);
	СтруктураПремийУценка.Вставить("СуммаСувениры", 	СуммаСувениры);
	СтруктураПремийУценка.Вставить("ПремияТабак", 		Окр(ПремияТабак, 0));
	СтруктураПремийУценка.Вставить("ПремияСувениры", 	Окр(ПремияСувениры, 0));

	Возврат СтруктураПремийУценка;
	
	
КонецФункции
