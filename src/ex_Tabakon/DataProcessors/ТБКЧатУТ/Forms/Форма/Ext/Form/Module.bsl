	...
	для каждого Строка Из Объект.Чат Цикл
		...
		для каждого Фотка Из КаталогChat Цикл
			если НЕ СтрНайти(Фотка.ИмяБезРасширения, Строка.ИдентификаторСообщения) Тогда
				Продолжить;
			КонецЕсли;
			//СтрокаПутиКатинок = СтрокаПутиКатинок + Фотка.ИмяБезРасширения + ";";
			Строка.ПутьКартинки.Добавить(Фотка.ПолноеИмя);
			ЕстьФото = Истина;
		КонецЦикла;
			//f5ef42c6-56e8-48c3-8dec-885bbfa0ca62
	//787137fa-68ca-4733-941a-bb4abafc4b44

		Если ЕстьФото Тогда
			Строка.Фото = Истина;
			ЕстьФото = Ложь;
			//Строка.ПутьКартинки. = СтрокаПутиКатинок;
			//СтрокаПутиКатинок = "";
		КонецЕсли;
	КонецЦикла;
	//КонецМарк
