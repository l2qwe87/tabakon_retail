...
Функция М_00057_СделатьПользователяАдминистратором()
	...
	Попытка
		...
		запрос = Новый запрос("ВЫБРАТЬ
		                      |	Пользователи.Ссылка КАК сНужныйПользователь
		                      |ИЗ
		                      |	Справочник.Пользователи КАК Пользователи
		                      |ГДЕ
...
Функция М_00058_ЗаполнитьВыемкуЗа_19082024()
	НачатьТранзакцию();
	Запрос	=	Новый запрос("ВЫБРАТЬ
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Ссылка КАК Ссылка,
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.КассаККМ КАК КассаККМ,
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Дата КАК Дата
	      	 	             |ИЗ
	      	 	             |	Документ.ТБК_ВедомостьОПродажахЗаДень КАК ТБК_ВедомостьОПродажахЗаДень
	      	 	             |ГДЕ
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Дата МЕЖДУ &ДатаНач И &ДатаКон
	      	 	             |	И ТБК_ВедомостьОПродажахЗаДень.ВыручкаЗаСменуПоКассе = 0");
	Запрос.УстановитьПараметр("ДатаНач", '20240819');
	Запрос.УстановитьПараметр("ДатаКон", '20240819235959');
	Рез = Запрос.Выполнить().Выбрать();
	...
	Пока  рез.Следующий() цикл
		...
		Запрос.УстановитьПараметр("Дата",  '20240820');
		Запрос.УстановитьПараметр("КассаККМ",  Рез.КассаККМ);
		
		//ищем пко к выемке
		Рез_Выемки	=	Запрос.Выполнить().Выбрать();
		Пока Рез_Выемки.Следующий() цикл
			Запрос	=	Новый запрос("ВЫБРАТЬ
			      	 	             |	ПриходныйКассовыйОрдер.Ссылка КАК Ссылка
			      	 	             |ИЗ
			      	 	             |	Документ.ПриходныйКассовыйОрдер КАК ПриходныйКассовыйОрдер
			...
			Запрос.УстановитьПараметр("ДокументОснование", Рез_Выемки.Ссылка);
			
			рез_ПКО	=	Запрос.Выполнить().Выбрать();
			Если Не рез_ПКО.Следующий() тогда 
				//меняем дату выемки
				Об_выемка				=	Рез_Выемки.Ссылка.ПолучитьОбъект();	
				Об_выемка.Дата			=	Рез.Ссылка.дата;
				Об_выемка.Записать();
				...
				Запрос.УстановитьПараметр("КассаККМ",Рез.кассаККМ);
				рез_Касса = Запрос.Выполнить().Выбрать();
				Если рез_Касса.Следующий() Тогда
					док.Касса = рез_Касса.Ссылка;
				КонецЕсли;				
				
				док.Организация 	= док.Касса.Владелец;
				док.СуммаДокумента 	= Рез_Выемки.СуммаДокумента;
				док.Дата 			= Рез.Дата;
				
				//док.Комментарий = комментарий;
				//док.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочиеДоходы;
				док.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ;
				док.КассаККМ 			= Рез.КассаККМ;
				док.ДокументОснование 	= Рез_Выемки.Ссылка;
				
				док.Приложение			= "1.1 Торговая розничная выручка - НАЛ";
					
				стр = док.РасшифровкаПлатежа.Добавить();
				стр.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Прочие доходы");
				стр.Сумма = Рез_Выемки.СуммаДокумента;
				
				док.Записать(РежимЗаписиДокумента.Запись);  
				
				//Добавляем в ведомость
				
				НовСтрока	=	Об.ВыемкаДенежныхСредств.Добавить();
				НовСтрока.Выемка	=	Рез_Выемки.Ссылка;
				НовСтрока.ПКО		=	Док.Ссылка;
				НовСтрока.Сумма		=	Рез_Выемки.СуммаДокумента;
			КонецЕсли;
		КонецЦикла;
		
		//2. Ищем ООРП и заполняем
		Запрос	=	Новый Запрос("ВЫБРАТЬ
		      	 	             |	ОтчетОРозничныхПродажах.Ссылка КАК Ссылка
		      	 	             |ИЗ
		      	 	             |	Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
		...
		Рез_ООРП	=	Запрос.Выполнить().Выбрать();
		Если Рез_ООРП.Следующий() тогда
			Если Не Рез_ООРП.Ссылка.Проведен тогда
				Об_ООРП	=	Рез_ООРП.Ссылка.ПолучитьОбъект();
				Об_ООРП.Записать(РежимЗаписиДокумента.Проведение);
			КонецЕсли;
			...
			Запрос.УстановитьПараметр("Ссылка", Рез_ООРП.Ссылка);
			
			ОБ.ОтчетыОРозничныхПродажах.Очистить();
			
			отчетыПоПродажам = Запрос.Выполнить().Выгрузить();
			ОБ.ОтчетыОРозничныхПродажах.Загрузить(отчетыПоПродажам);
			...
			Запрос.Текст="ВЫБРАТЬ
				             ...
				             |	И ВозвратТоваровОтПокупателя.Дата <= &ДатаК
				             |	И ВозвратТоваровОтПокупателя.КассаККМ = &КассаККМ";
			Запрос.УстановитьПараметр("ДатаН",'20240819');
			Запрос.УстановитьПараметр("ДатаК",'20240819235959');
			Запрос.УстановитьПараметр("КассаККМ",Рез.КассаККМ);

			
			Рез_Возвраты = Запрос.Выполнить().Выгрузить();
			возвратОтПокупателяНАЛ 		= 0;
			возвратОтПокупателяКарта	= 0;
			Для каждого Строка из Рез_Возвраты цикл
				...
				Запрос	=	Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             ...
	      	 	             |	РасходныйКассовыйОрдер.Проведен
	      	 	             |	И РасходныйКассовыйОрдер.ДокументОснование = &ДокументОснование");
				Запрос.УстановитьПараметр("ДокументОснование",Строка.Ссылка);
				Рез_Расходники = Запрос.Выполнить().Выгрузить();
				
				Если Рез_Расходники.Количество() >0 тогда					
					возвратОтПокупателяНАЛ   = возвратОтПокупателяНАЛ   + Рез_Расходники[0].СуммаДокумента;
				КонецЕсли; 	
				...
				Запрос	=	Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             ...
	      	 	             |	И ОплатаОтПокупателяПлатежнойКартой.ДокументОснование = &ДокументОснование");
				Запрос.УстановитьПараметр("ДокументОснование",Строка.Ссылка);
				Рез_ООП = Запрос.Выполнить().Выгрузить();
				
				Если Рез_ООП.Количество() >0 тогда
					возвратОтПокупателяКарта = возвратОтПокупателяКарта + Рез_ООП[0].СуммаДокумента;
				КонецЕсли; 	
			...
			Если отчетыПоПродажам.Количество() = 1 Тогда
				ЗаполнитьЗначенияСвойств(Об, отчетыПоПродажам[0],"ВозвратОтПокупателяНал,ОплатаНаличными,ОплатаПоБанковскимКартам,ВозвратОтПокупателяБезНал");
				
				Об.ВыручкаЗаСменуПоКассе = 
					Об.ОплатаНаличными 
				   +Об.ОплатаПоБанковскимКартам - Об.ВозвратОтПокупателяБезНал
				; 
				Об.ОплатаПоБанковскимКартам		= Об.ОплатаПоБанковскимКартам - Об.ВозвратОтПокупателяБезНал;
				
				Об.ВозвратОтПокупателяНал 		= возвратОтПокупателяНАЛ;
				Об.ВозвратОтПокупателяБезНал 	= возвратОтПокупателяКарта;
			КонецЕсли;			
		КонецЕсли;
		
		Об.НаличныеКСдаче = 0
		+Об.ОплатаНаличными        //1
		+Об.ВедомостьПродажНеУчтено      //2
		-Об.ВозвратОтПокупателяНал       //4
		-Об.РасходыОтдела			     //5
		-Об.ЗП_Продавца;			     //6

		Об.Продавцы.Очистить();
		Для каждого Строка из Об.ОтчетыОРозничныхПродажах цикл
			Запрос	=	Новый Запрос("ВЫБРАТЬ
			      	 	             |	ТБКСменыСотрудников.Продавец КАК Продавец,
			...
			Запрос.УстановитьПараметр("КассоваяСмена", Строка.ОтчетОРозничныхПродажах.КассоваяСмена);
			
			Рез_КС = Запрос.Выполнить().Выгрузить();
			
			Для каждого СтрокаРез из Рез_КС цикл
				НоваяСтрока	=	Об.Продавцы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаРез);
			КонецЦикла; 		
		КонецЦикла;  
...
Функция М_00058_ПроставитьЗаПериодТБК_Заявки_флСозданВУТ()
...
Функция М_00059_ИсправлениеТоваровВНефискальныхЧеках()
	НачатьТранзакцию();
	
	Попытка
		//Удаляем оор уже созданные
		Запрос	=	Новый запрос("ВЫБРАТЬ
		      	 	             |	ОтчетОРозничныхПродажах.Ссылка КАК Ссылка
		      	 	             |ИЗ
		      	 	             |	Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
		...
		Запрос.УстановитьПараметр("Дата", '20240901');
		Запрос.УстановитьПараметр("Комментарий", "ООРП не ф.");
		Рез = Запрос.Выполнить().Выбрать();
		...
		Запрос	=	Новый запрос("ВЫБРАТЬ
		      	 	             ...
		      	 	             |			ИЛИ ЧекККМ.Комментарий ПОДОБНО &Комментарий1)");
		Запрос.УстановитьПараметр("Дата", '20240901');
		Запрос.УстановитьПараметр("СтатусЧекаККМ", Перечисления.СтатусыЧековККМ.Аннулированный);
		Запрос.УстановитьПараметр("Комментарий", "Добавлен в ООРП");  		
		Запрос.УстановитьПараметр("Комментарий1", "Чек н.ф.");  		
			
		Рез = Запрос.Выполнить().Выбрать();
		
		Пока Рез.Следующий() цикл
			Если рез.Ссылка.товары.Количество() = 1 тогда
			   	НоменклатураЧек	=	 рез.Ссылка.товары[0].Номенклатура;
				
				НоменклатураСпарвочник	=	Неопределено;
				Для каждого Строка из рез.Ссылка.АкцизныеМарки цикл
					НоменклатураСпарвочник	=	Строка.ШтрихкодУпаковки.Номенклатура;
				КонецЦикла;	
				
				
				Если НоменклатураСпарвочник<>Неопределено и НоменклатураСпарвочник<>НоменклатураЧек тогда
					//1. Исправляем ЧЕК
					Об = Рез.Ссылка.ПолучитьОбъект();
					ОБ.Товары[0].Номенклатура	=	НоменклатураСпарвочник;
					Об.ОбменДанными.Загрузка	=	Истина;				Об.Записать();
					Об.Записать();	
		...
		Для Ном = 1 по 3 цикл
			...
			Если Ном = 1 тогда
				...
				Запрос.УстановитьПараметр("ДатаКон", '20240902');
				
			ИначеЕсли Ном = 2 тогда
				Запрос.УстановитьПараметр("ДатаНач", '20240902');
				Запрос.УстановитьПараметр("ДатаКон", '20240903');
				
			ИначеЕсли Ном = 3 тогда
				Запрос.УстановитьПараметр("ДатаНач", '20240903');
				Запрос.УстановитьПараметр("ДатаКон", '20240904');
			...
			СуммаОплатыКарта	=	0;
			Пока Рез.Следующий() цикл  			 
				 Для каждого СтрокаТовары из Рез.Ссылка.товары цикл
				 	НоваяСтрока	=	КопияТовары.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаТовары);
				КонецЦикла;	
				
				 Для каждого СтрокаОплата из Рез.Ссылка.Оплата цикл
					Если Строка(СтрокаОплата.ВидОплаты) = "Наличные" тогда
						СуммаОплатыНаличных	=	СуммаОплатыНаличных + СтрокаОплата.Сумма;
					иначе
						СуммаОплатыКарта	=	СуммаОплатыКарта + СтрокаОплата.Сумма;
					КонецЕсли;
			...
			Если Ном = 1 тогда
				Отчет.Дата          = '20240901235959';
			ИначеЕсли Ном = 2 тогда
				Отчет.Дата          = '20240902235959';				
			ИначеЕсли Ном = 3 тогда
				Отчет.Дата          = '20240903094909';				
			КонецЕсли;

			
			Отчет.КассаККМ      = Рез.КассаККМ;
...
Функция М_00060_ПерепроведениеНефискальныхОпераций()
	//Запрос	=Новый Запрос("ВЫБРАТЬ
	//      	              |	ЧекККМ.Ссылка КАК Ссылка
	//      	              |ИЗ
	//      	              |	Документ.ЧекККМ КАК ЧекККМ
	...
	//Пока Рез.Следующий() цикл
	//	Об = рез.ссылка.ПолучитьОбъект();
	//	Об.ДополнительныеСвойства.Вставить("ЗагрузкаДанныхИзРабочегоМеста", Ложь);	
	//	Об.Записать(РежимЗаписиДокумента.Проведение);
	//КонецЦикла;
	...
	Запрос	=Новый Запрос("ВЫБРАТЬ
	      	              ...
	      	              |	И ОтчетОРозничныхПродажах.Проведен
	      	              |	И ОтчетОРозничныхПродажах.Комментарий ПОДОБНО &Комментарий");
	Запрос.УстановитьПараметр("дата", '20240903');
	Запрос.УстановитьПараметр("Комментарий", "ООРП не ф.");
	Рез = Запрос.Выполнить().Выбрать();
...
Функция М_00062_УбратьЧекиИзМотивации()
	Запрос	=	Новый запрос("ВЫБРАТЬ
	      	 	             |	ЧекККМ.Ссылка КАК Ссылка
	      	 	             |ИЗ
	      	 	             |	Документ.ЧекККМ КАК ЧекККМ
	...
	Пока рез.Следующий() цикл
		Рег = РегистрыНакопления.ТБКПремииПоЧекам.СоздатьНаборЗаписей();
		Рег.Отбор.Регистратор.Установить(рез.Ссылка);
		Рег.Записать();
	КонецЦикла;
