...
Функция М_00051_ЗаполнитьSamosaleApiКлюч() //Марк 2025-03-12

	Магазин = ОбщегоНазначения.ПолучитьМагазинИзООРП();
	
	Если Ложь Тогда Магазин = Справочники.Магазины.НайтиПоКоду("");	КонецЕсли;
	
	Если не Магазин.Пустая() Тогда
	
		ОбъектМагазин = Магазин.ПолучитьОбъект();
		ОбъектМагазин.Расш_SamosaleApiКлюч = "mTn7MrOkqdPLAHEVpwFXTxqAqN7JMvmTKp3FkST1EUla6h8knjW06C2kGS76UDVR";	
	    ОбъектМагазин.Записать();
		
	КонецЕсли;

КонецФункции // М_00051_ЗаполнитьSamosaleApiКлюч()

Функция М_00052_СделатьПользователяАдминистратором()
	...
	Попытка
		...
		МассивПользователей.Добавить("Админ");
		МассивПользователей.Добавить("Докторова Наталья Сергеевна 20");
		МассивПользователей.Добавить("Челидзе Богдан Эльдарович 22");
...
Функция М_00053_УстановитьКонстанты()

	Константы.ПечатьСлипЧека.Установить(Истина);
	Сообщить("Контсанты проставлены");
