...
Функция М_00047_ГенерацияЮзеровСПолнымиПравами()
	...
	//МассивП.Добавить("ИТ");
	МассивП.Добавить("Цыганкова Алёна Евгеньевна 19");
...
Функция М_00055_ПроведениеЧековМай2024()
	...
	Запрос.УстановитьПараметр("Комментарий", "Перепроведен автоматически");
	рез = Запрос.Выполнить().Выбрать();
	
	Пока рез.Следующий() цикл
		Об	=	рез.Ссылка.ПолучитьОбъект();
		Попытка
			Об.Комментарий	=	"Перепроведен автоматически";
			Об.Записать(РежимЗаписиДокумента.Проведение);
		Исключение			
			сообщить(ОписаниеОшибки());
		КонецПопытки; 		
