...
Функция М_00057_СделатьПользователяАдминистратором() Экспорт
	...
	Попытка
		...
		Для каждого НужныйПользователь Из МассивПользователей Цикл
			...
			Если АдминистраторыОбъект.Пользователи.Найти(НужныйПользователь) = Неопределено Тогда
				
				Нов = АдминистраторыОбъект.Пользователи.Добавить();
				нов.Пользователь = НужныйПользователь;
				АдминистраторыОбъект.Записать();	
				
			КонецЕсли; 
			
			Если ОткрытиеВнешнихОбработок.Пользователи.Найти(НужныйПользователь) = Неопределено Тогда
