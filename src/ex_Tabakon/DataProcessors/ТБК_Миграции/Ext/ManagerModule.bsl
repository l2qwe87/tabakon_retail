...
Функция М_00014_УдалитьДублиИПустыхПродавцовВВедомостях()
	Запрос	=	новый Запрос("ВЫБРАТЬ
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Ссылка КАК Ссылка
	      	 	             |ИЗ
	      	 	             |	Документ.ТБК_ВедомостьОПродажахЗаДень КАК ТБК_ВедомостьОПродажахЗаДень
	      	 	             |ГДЕ
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Проведен");
	Рез = Запрос.Выполнить().Выгрузить();
	флВсеХорошо = Истина;    
	
	Для каждого Строка из Рез цикл
		Об = Строка.Ссылка.ПолучитьОбъект();
		флНужноЗаписывать = Ложь;
		ПоискДублейСтрокИПустыхЗначенийВТЧПродавцы(Об,флНужноЗаписывать);
		
		Если флНужноЗаписывать тогда		
			Попытка
				Об.Записать();
			Исключение
				#Если Клиент тогда
	...
	Если не флВсеХорошо тогда 
		ВызватьИсключение НСтр("ru = 'Ошибка при записи ТБК_ВедомостьОПродажахЗаДень)'");
	КонецЕсли;
	
КонецФункции

Процедура ПоискДублейСтрокИПустыхЗначенийВТЧПродавцы(Об,флНужноЗаписывать) 
	ТЧ = ОБ.Продавцы;
	
	Отбор =  Новый Структура();
	Отбор.Вставить("Продавец", Справочники.ФизическиеЛица.ПустаяСсылка());
	
	Строки = ТЧ.НайтиСтроки(Отбор);	
	Для  каждого НайденнаяСтрока  Из Строки Цикл
		ТЧ.Удалить(НайденнаяСтрока);
		флНужноЗаписывать = Истина;
	КонецЦикла;	
...
Функция М_00030_УдалениеНеверныхШтрихкодов()
	...
	НоваяСтрока.НеверныйШК = "0000046209368";
	НоваяСтрока.НевернаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию("Glamour №1 Amber");
	
	НоваяСтрока = ТЗНеверныеШК.Добавить();
	НоваяСтрока.НеверныйШК = "0000046230638";
	НоваяСтрока.НевернаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию("Senator GRAND VIRGINIA");
	...
	НоваяСтрока = ТЗНеверныеШК.Добавить();
	НоваяСтрока.НеверныйШК = "46230638";
	НоваяСтрока.НевернаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию("Senator GRAND VIRGINIA");
