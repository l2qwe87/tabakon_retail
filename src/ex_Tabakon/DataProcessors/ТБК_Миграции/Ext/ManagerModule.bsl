...
Функция М_00032_ДатаЗапретИзменениеДанныхОбщая()
	...
	актуальнаДатаЗапрета = Дата(2023,07,31);
...
МассивП = новый Массив;
	...
	Для каждого Строка из МассивП цикл
		...
		Если Рез.Следующий() тогда //даем полные права  
		...
		ИначеЕсли Строка = "obmen_new" или Строка = "sys_ws" тогда //создаем
...
Функция М_00044_ЗаполнениеСправочникаПроекты()
	...
	Спр.Наименование	=	"Возврат брак одноразки/картриджа";
	Спр.Записать();
	
	Спр	=	Справочники.ТБКПроекты.СоздатьЭлемент();
	Спр.Наименование	=	"Возврат гарантия";
	Спр.Записать();
	
	Спр	=	Справочники.ТБКПроекты.СоздатьЭлемент();
	Спр.Наименование	=	"Возврат срок годности";
	Спр.Записать();
	
	Спр	=	Справочники.ТБКПроекты.СоздатьЭлемент();
	Спр.Наименование	=	"Между магазинам";
	Спр.Записать();
	...
	Спр	=	Справочники.ТБКПроекты.СоздатьЭлемент();
	Спр.Наименование	=	"Возврат некондиция";
	Спр.Записать();
	
КонецФункции

Функция М_00045_SamosaleЗаполнениеДополнительныйБалов()
	
	Если Справочники.ТБКSamosale_ДопБалы.НайтиПоНаименованию("Отзыв на картах (50)").Пустая() Тогда
		Спр = Справочники.ТБКSamosale_ДопБалы.СоздатьЭлемент();
	    Спр.Наименование = "Отзыв на картах (50)";
		Спр.Баллы = 50;
		Спр.Записать();
	КонецЕсли; 

	Если Справочники.ТБКSamosale_ДопБалы.НайтиПоНаименованию("Подписка в VK и/или Telegram (50)").Пустая() Тогда
		Спр = Справочники.ТБКSamosale_ДопБалы.СоздатьЭлемент();
	    Спр.Наименование = "Подписка в VK и/или Telegram (50)";
		Спр.Баллы = 50;                                  
		Спр.Записать();
	КонецЕсли;
КонецФункции

Функция М_00046_ИзменениеОбращенияК_FTP() //Марк 2024-02-27

	Шаблон_путь_mx_tbkon_ru = "mx.tbkon.ru";
	Шаблон_путь_mx_tabakon_ru = "mx.tabakon.ru";
	шаблон_путь_замена = "vp.tbkon.ru";
	набор = РегистрыСведений.НастройкиТранспортаОбменаДанными.СоздатьНаборЗаписей();
	набор.Прочитать();
	
	для каждого строка из набор Цикл
		Если СтрНайти(строка.FTPСоединениеПуть, Шаблон_путь_mx_tbkon_ru) тогда	
			строка.FTPСоединениеПуть = СтрЗаменить(строка.FTPСоединениеПуть, Шаблон_путь_mx_tbkon_ru, шаблон_путь_замена);
		ИначеЕсли  СтрНайти(строка.FTPСоединениеПуть, Шаблон_путь_mx_tabakon_ru) тогда
...
Функция М_00047_ЗаписьВ_РегпрошлыеАнулированныеЧеки() //Марк 2024-09-17
...
Функция М_00047_УдалениеПлоихШК()
	
	Массив = Новый массив;
	Массив.Добавить("4627184523017");
	
	Для каждого Значение Из Массив Цикл
		
		Набор = РегистрыСведений.Штрихкоды.СоздатьНаборЗаписей();
		Набор.Отбор.Штрихкод.Установить(Значение);
		Набор.Записать();
		
		Набор = РегистрыСведений.ТБК_Штрихкоды.СоздатьНаборЗаписей();
		Набор.Отбор.Штрихкод.Установить(Значение);
		Набор.Записать();
...
КонецФункции // М_00047_УдалениеПлоихШК()

Функция М_00049_ЗабытьСуществующиеШтрихкодыВРегистре()
	...
	Запрос = Новый запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	ШтрихкодыУпаковокТоваров.ДатаУпаковки >= &ДатаУпаковки");
	Запрос.УстановитьПараметр("ДатаУпаковки", '20240915');
	Выборка = ЗАпрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
	
		ОБ = Выборка.Ссылка.ПолучитьОбъект();
		ОБ.ЗначениеШтрихкода = "Я" + ОБ.ЗначениеШтрихкода;
		ОБ.Записать();
	
	КонецЦикла;

КонецФункции // М_00049_ЗабытьСуществующиеШтрихкодыВРегистре()

Функция М_00050_УдалитьПереоценкиБК()
	Попытка
		НачатьТранзакцию();
		Узел = ПланыОбмена.ОбменРозницаУправлениеТорговлей103.НайтиПоНаименованию("ВЭЙП");
		
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      |	ПрименениеЦенНоменклатуры.Ссылка КАК Ссылка,
		                      |	ПрименениеЦенНоменклатуры.Комментарий КАК Комментарий
		                      |ИЗ
		                      |	Документ.ПрименениеЦенНоменклатуры КАК ПрименениеЦенНоменклатуры
		                      |ГДЕ
		                      |	ПрименениеЦенНоменклатуры.Комментарий ПОДОБНО ""Создано автоматически для комплектации БК""
		                      |	И ПрименениеЦенНоменклатуры.Проведен
		                      |
		                      |УПОРЯДОЧИТЬ ПО
		                      |	ПрименениеЦенНоменклатуры.Дата");
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл
			
			Если СтрНайти(Строка(Выборка.Ссылка.ОбъектЦенообразования), "<Объект не найден>") или Выборка.Ссылка.ПометкаУдаления Тогда
			...
			об = Выборка.Ссылка.ПолучитьОбъект();
			Если ложь Тогда об = документы.ПрименениеЦенНоменклатуры.ПустаяСсылка().ПолучитьОбъект(); КонецЕсли;  
			об.ПометкаУдаления = истина;
			об.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			
			ПланыОбмена.УдалитьРегистрациюИзменений(Узел, об.Ссылка);
...
КонецФункции // М_00050_УдалитьПереоценкиБК()
