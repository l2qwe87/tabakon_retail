...
Функция М_00041_ИзменитьПутьFTP()
	Рег = РегистрыСведений.УдалитьНастройкиТранспортаОбменаДанными.СоздатьНаборЗаписей();
	Рег.Прочитать();
...
Запрос.УстановитьПараметр("Период2", Дата(2026,5,7));
...
КонецЦикла;
	...
	//Период = ПериодОбработки.ДатаНачала; 
	//ПериодКонец = НачалоДня(ПериодОбработки.ДатаОкончания)  + 60 * 60 * 24;
