...
Функция М_00032_ДатаЗапретИзменениеДанныхОбщая()
	...
	актуальнаДатаЗапрета = Дата(2021,01,31);
...
Функция М_00035_ПровестиОприходованиеПоОстаткам()
	...
	иначе
		...
		Попытка
			Об.Записать(РежимЗаписиДокумента.Проведение);
		Исключение
			ВызватьИсключение НСтр("ru = 'Не удалось провести оприходование!'");
		КонецПопытки;
		
		Пл = ПланыОбмена.ОбменРозницаУправлениеТорговлей103.НайтиПоКоду("УТ");

		ПланыОбмена.УдалитьРегистрациюИзменений(пл, ОБ);
		Сообщить("Проведено оприходование: !" + ОБ);
	КонецЕсли;  
