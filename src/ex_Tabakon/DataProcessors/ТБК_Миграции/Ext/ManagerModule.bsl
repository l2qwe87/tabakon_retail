...
Функция М_00057_СделатьПользователяАдминистратором() Экспорт
	...
	Попытка
		...
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Соколов Алексей Сергеевич 07"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Петрова Екатерина Ивановна 20"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Разумовский Данила Дмитриевич 16"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Цыганкова Алёна Евгеньевна 19"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Черкасова Софья Сергеевна 18"));
		//
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Базилевский Евгений Сергеевич 02"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Белоглазов Василий Николаевич 02"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Казаков Алексей Николаевич 03"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Киселева Евгения Александровна 10"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Курдюкова Ольга Владимировна 24"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Озеров Сергей Александрович 22"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Полтавцев Артем Олегович 31"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Савин Александр Анатольевич 01"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Сироткин Даниил Александрович 29"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Цысь Александр Юрьевич 18"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Шейнова Наталия Николаевна 23"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Горшкова Светлана Владимировна 22"));
		//МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("ИТ")); 
...
Функция М_00063_ФиксСсылкиПлатежнойКарты()
	...
	//	Об.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ПлатежнаяКарта;
	//	//Об.ПлатежнаяСистема = Перечисления.ТипыПлатежнойСистемыККТ.СертификатНСПК;
	//	Об.Записать();
	//	
	//КонецЦикла;
	...
	//Попытка
	//	
	//	НужныйВидОплаты = Справочники.ВидыОплатЧекаККМ.НайтиПоКоду("000000002");
	//	Если НЕ НужныйВидОплаты.Пустая() Тогда
	//		
	//		ГУИД = НужныйВидОплаты.УникальныйИдентификатор();
	//		Если Найти(Строка(ГУИД), "064bbca0-5fe9-11e7-80be-0cc47aabb907") Тогда
	//			ОтменитьТранзакцию();
	//			Возврат "М_00063_ФиксСсылкиПлатежнойКарты все хорошо"			
	//		КонецЕсли;
	//		
	//		НужныйОбъект = НужныйВидОплаты.ПолучитьОбъект();
	//		НужныйОбъект.Код = "000000003";
	//		НужныйОбъект.ПометкаУдаления = Истина;
	//		НужныйОбъект.Записать();
	//		
	//		ГУИДВидыОплатЧекаККМ = Новый УникальныйИдентификатор("064bbca0-5fe9-11e7-80be-0cc47aabb907");
	//		ВидыОплатЧекаККМСсылка = Справочники.ВидыОплатЧекаККМ.ПолучитьСсылку(ГУИДВидыОплатЧекаККМ);
	//		
	//		НовВидОплаты  = Справочники.ВидыОплатЧекаККМ.СоздатьЭлемент();
	//		НовВидОплаты.Код = "000000002";
	//		НовВидОплаты.Наименование = НужныйОбъект.Наименование;
	//		НовВидОплаты.ТипОплаты = НужныйОбъект.ТипОплаты;
	//		//НовВидОплаты.ПлатежнаяСистема = НужныйОбъект.ПлатежнаяСистема;
	//		НовВидОплаты.УстановитьСсылкуНового(ВидыОплатЧекаККМСсылка);
	//		НовВидОплаты.Записать();
	//		
	//		Выборка = Справочники.ЭквайринговыеТерминалы.Выбрать();
	//		
	//		Пока Выборка.Следующий() Цикл
	//			
	//			НужныйТерминал = Выборка.ПолучитьОбъект();
	//			НужныйТерминал.ТарифыЗаРасчетноеОбслуживание.Очистить();
	//			НовСтрока = НужныйТерминал.ТарифыЗаРасчетноеОбслуживание.Добавить();
	//			НовСтрока.ВидОплаты = НовВидОплаты.Ссылка;
	//			НужныйТерминал.Записать();
	//		
	//		КонецЦикла; 
	//			
	//	иначе
	//		Сообщить("не найден нужный вид оплаты. у нужного долен быть код 000000002");	
	//	КонецЕсли; 
	//	
	//	//ОтменитьТранзакцию();
	//	ЗафиксироватьТранзакцию();
	...
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Наличные").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.Наличные;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Банковский платеж").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.БанковскийКредит;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Встречное предоставление").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ВстречноеПредоставление;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Зачет аванса").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ЗачетАванса;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Оплата бонусами (распределяется как скидка)").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.Бонусы;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("Оплата в рассрочку").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ВРассрочку;
	Вид.Записать();
	
	Вид = Справочники.ВидыОплатЧекаККМ.НайтиПоНаименованию("ПодарочныйСертификат").ПолучитьОбъект();
	Вид.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ВРассрочку;
	Вид.Записать();
...
Функция М_00065_ФиксСсылкиКонтрагентаСбербанк()
     //только один раз 
 	//"064bbc9d-5fe9-11e7-80be-0cc47aabb907"
	//Попытка
	//	
	//	КонтрагентСбербанк = Справочники.Контрагенты.НайтиПоНаименованию("Сбербанк", Истина);
	//	Если НЕ КонтрагентСбербанк.Пустая() Тогда
	//		
	//		КонтрагентСбербанкОБ = КонтрагентСбербанк.ПолучитьОбъект();
	//		КонтрагентСбербанкОБ.ПометкаУдаления = Истина;
	//		КонтрагентСбербанкОБ.Код = "";
	//		КонтрагентСбербанкОБ.Наименование = КонтрагентСбербанкОБ.Наименование + " плохой";
	//		КонтрагентСбербанкОБ.Записать();
	//		
	//		НовыйСбербанк = Справочники.Контрагенты.СоздатьЭлемент();
	//		ЗаполнитьЗначенияСвойств(НовыйСбербанк, КонтрагентСбербанк,, "Владелец"); 
	//		
	//		НужныйГУИД_Сбербанк = Новый УникальныйИдентификатор("064bbca0-5fe9-11e7-80be-0cc47aabb907");
	//		НужныйГУИД_Сбербанк = Справочники.Контрагенты.ПолучитьСсылку(НужныйГУИД_Сбербанк);
	//		НовыйСбербанк.УстановитьСсылкуНового(НужныйГУИД_Сбербанк);
	//		НовыйСбербанк.Код = "АВ0000137";
	//		НовыйСбербанк.ПометкаУдаления = Ложь;
	//		НовыйСбербанк.Записать();
	//		
	//		Выборка = Справочники.ЭквайринговыеТерминалы.Выбрать();
	//		
	//		пока Выборка.Следующий() Цикл
	//			
	//			ОбъектЭквайринговыеТерминалы = Выборка.ПолучитьОбъект(); 	
	//			ОбъектЭквайринговыеТерминалы.Эквайрер = НовыйСбербанк.Ссылка;
	//			ОбъектЭквайринговыеТерминалы.Записать();
	//		КонецЦикла;
	//		
	//		набор = РегистрыСведений.КонтрагентыНаМониторингеСПАРКРиски.СоздатьНаборЗаписей();
	//		набор.Отбор.Контрагент.Установить(КонтрагентСбербанкОБ.Ссылка);
	//		набор.Прочитать();
	//		
	//		Для каждого Запись Из набор Цикл
	//		
	//			Запись.Контрагент = НовыйСбербанк.Ссылка;	
	//		
	//		КонецЦикла;
	//		набор.Записать();
	//		
	...
	//	
	//Исключение
	//	Сообщить("М_00065_ФиксСсылкиКонтрагентаСбербанк " + ОписаниеОшибки());	
	//КонецПопытки;
...
Функция М_00069_ПростивитьКофицент1вУпаковках()
	...
	запрос = Новый запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	УпаковкиНоменклатуры.Коэффициент = 0");
	
	Выборка = запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
	
		СсылкаУпаковкиНоменклатуры = Выборка.Ссылка;
		Если Ложь Тогда СсылкаУпаковкиНоменклатуры = Справочники.УпаковкиНоменклатуры.НайтиПоКоду(""); КонецЕсли;
	    Об = СсылкаУпаковкиНоменклатуры.ПолучитьОбъект();
		Об.Коэффициент = 1;
		Об.Наименование = Строка(Об.ЕдиницаИзмерения);
		Об.Записать();
...
Функция М_00070_УдалитьПомеченныеСертифигаты()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	СертификатыКлючейЭлектроннойПодписиИШифрования = Выборка.Ссылка.ПолучитьОбъект();
	СертификатыКлючейЭлектроннойПодписиИШифрования.Удалить();
