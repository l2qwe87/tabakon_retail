...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод")  Тогда
		...
		// Толстоухов 10.06.2026 PRIC-131 ++
		Если СтрЧислоВхождений(ДанныеСоСканераСтруктура.Штрихкод,"/") > 10 тогда //это куар новой уценки
			ОбработатьQR_Уценки(ДанныеСоСканераСтруктура.Штрихкод);
			Возврат;
		КонецЕсли;
		// Толстоухов 10.06.2026 --
...
// Толстоухов 10.06.2026 PRIC-131 ++
&НаКлиенте
Процедура ОбработатьQR_Уценки(Данные)
	...
	ДобавляемаяНоменклатура = ПолучитьНоменклатуру(КодНоменклатуры);
	
	Если ДобавляемаяНоменклатура.Пустая() Тогда
	...
	// Создаём структуру условий поиска
	УсловияПоиска = Новый Структура;
	УсловияПоиска.Вставить("Номенклатура", ДобавляемаяНоменклатура);
	
	// Выполняем поиск
	НайденныеСтроки = ТабличнаяЧасть.НайтиСтроки(УсловияПоиска);
	
	// Обрабатываем результат
	Если НайденныеСтроки.Количество() > 0 Тогда
		Для Каждого Строка Из НайденныеСтроки Цикл
			Если ПустаяСтрока(Строка.QR) Тогда
				Строка.QR = Данные;
			Иначе
				Сообщить("QR уже добавлен!");
			КонецЕсли;
		КонецЦикла;
	Иначе
		НоваяСтрока = ТабличнаяЧасть.Добавить();
		НоваяСтрока.Номенклатура = ДобавляемаяНоменклатура;
		НоваяСтрока.QR = Данные;
	КонецЕсли;	
