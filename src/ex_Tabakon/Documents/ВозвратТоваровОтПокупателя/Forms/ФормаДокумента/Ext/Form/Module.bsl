...
Функция  СоздатьЧекККМВозврата(Ссылка,ЧекККМ, СуммаНал = 0, СуммаКарта = 0)
	...
	НовыйДокументЧек.СистемаНалогообложения	=	ПолучитьВидНалогаСервер(НовыйДокументЧек.Организация);
	
	НовыйДокументЧек.Товары.Очистить();
	НовыйДокументЧек.Оплата.Очистить();
	НовыйДокументЧек.АкцизныеМарки.Очистить();
	НовыйДокументЧек.КодыМаркировки.Очистить();
	...
	Для каждого Строка из Ссылка.ШтрихкодыУпаковок цикл
		НоваяСтрокаМарка	=	НовыйДокументЧек.АкцизныеМарки.Добавить();
		НоваяСтрокаМарка.АкцизнаяМарка		=	Строка.ШтрихкодУпаковки;
		НоваяСтрокаМарка.КлючСвязи			=	Строка.НомерСтроки;
		НоваяСтрокаМарка.КодАкцизнойМарки	=	Строка.ШтрихкодУпаковки.ЗначениеШтрихкода;
		НоваяСтрокаМарка.ШтрихкодУпаковки	=	Строка.ШтрихкодУпаковки;					
	КонецЦикла;;
	...
	Попытка
		ЕстьОшибки = Ложь;
		//(ЕстьОшибки,НовыйДокументЧек);
		    //ПроверитьДанныеГосИСЗдесь
