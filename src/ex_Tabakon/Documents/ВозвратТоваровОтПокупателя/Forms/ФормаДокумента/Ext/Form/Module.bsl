...
Процедура ТБКОплатаКартаПосле(Команда)
	Если Объект.Товары.Количество() >0 тогда
		...
		Если НовыйДокументОплатаКартой <> Неопределено тогда
			...
			ТекстВопроса = НСтр("ru = 'По эквайрингу чек пробит?'");
			ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеВопросПробитЧекПоЭквайрингу",ЭтотОбъект);
...
&НаКлиенте
Процедура ОповещениеВопросПробитЧекПоЭквайрингу(РезультатВопроса, ДополнительныеПараметры) Экспорт
