		...
		Если ЗначениеЗаполнено(ЧекККМ) тогда			
			...
			Если НовыйДокументЧекККМ <> Неопределено тогда
				...
				Попытка						
					...
					УстановитьСтатусАрхивный(НовыйДокументЧекККМ);
				Исключение
					//Сообщить(ОписаниеОшибки());
				КонецПопытки;					
			КонецЕсли;
...
&НаСервереБезКонтекста
Функция УстановитьСтатусАрхивный(ссылкаЧекККМВозврат)
	док = ссылкаЧекККМВозврат.ПолучитьОбъект();
	док = Документы.ЧекККМ.СоздатьДокумент();
	док.СтатусЧекаККМ			=	Перечисления.СтатусыЧековККМ.Архивный;
