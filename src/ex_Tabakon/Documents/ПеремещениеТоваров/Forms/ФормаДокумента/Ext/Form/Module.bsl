...
Процедура ТБКПринятьТоварыПослеНаСервере(Ссылка,НоваяПереоценка)
	...
	Попытка
		...
		Попытка
			...
			Об.КоличествоПакетовФакт = Объект.КоличествоПакетовФакт;
		...
		Если  ДокументОбъектПрименениеЦен.Товары.Количество()>0 тогда
...
Процедура ТБКПринятьТоварыПосле(Команда)
	...
	Для Каждого Строка Из Объект.Товары Цикл
		Если СтрНайти(Строка.Характеристика, "<Объект не найден>") Тогда
			ПоказатьПредупреждение(, "В перемещении отсутствует характеристика на товар. Строка: " + Строка.номерСтроки + ". Обратитесь в офис или к администратору");
			Возврат;
		КонецЕсли;
	КонецЦикла;
	//КонецМарк
	...
	Если не флЭтоНеучтенка тогда
		//Марк 2024-11-07                     
		Если Найти(НРег(объект.Комментарий), "между магазинами") И объект.КоличествоПакетовФакт = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите фактическое количество пакетов",,"Объект.КоличествоПакетовФакт");
			Возврат;
		КонецЕсли;
		//КонецМарк
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	//Марк 2024-11-07
	Если Найти(НРег(объект.Комментарий), "между магазинами") Тогда
		//ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите фактическое количество пакетов",,"Объект.КоличествоПакетовФакт");
		Элементы.КоличествоПакетовФакт.Видимость = Истина;
		Элементы.КоличествоПакетов.Видимость = Истина;
	КонецЕсли;
	//КонецМарк
...
&НаКлиенте
Процедура ТБККоличествоПакетовФактПриИзмененииПосле(Элемент)

	Если Объект.КоличествоПакетов < Объект.КоличествоПакетовФакт Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Нельзя указать фактическое количество пакетов больше");
		Объект.КоличествоПакетовФакт = 0;
		Возврат;
	КонецЕсли;
	
	//Объект.Комментарий = Объект.Комментарий + " "; //Костыль для регистрации к обмену
	
КонецПроцедуры

&НаКлиенте
Процедура ТБККоличествоПакетовФактИзменениеТекстаРедактированияПосле(Элемент, Текст, СтандартнаяОбработка)
