...
Перем ТоварыТолькоПросмотр Экспорт;
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	ВосстановитьТовары(ТоварыТолькоПросмотр);
	Элементы.Товары.ТолькоПросмотр = ТоварыТолькоПросмотр;
	//КонецМарк
...
Процедура  ДобавитьКоличествоФакт(ДанныеСоСканераСтруктура) 
	...
	Если СтрДлина(СканированныйШК) >28 тогда //это маркировка 
		...
		Обрезать = Прав(СканированныйШК,8);
		Макра1 = СтрЗаменить(СканированныйШК, Обрезать, ""); 
		Запрос.УстановитьПараметр("Марка1", Макра1);                             
		Запрос.УстановитьПараметр("Марка", СканированныйШК); //04627184523017i4=CYDDAAAA
		Рез = Запрос.Выполнить().Выгрузить();
...
&НаСервере
Процедура ВосстановитьТовары(ТоварыТолькоПросмотр = Ложь)
 	Рег = РегистрыСведений.СохраненныеДанныеПомощникаПриемкиОтгрузки.СоздатьНаборЗаписей();
	Рег.Отбор.ДокументРаспоряжение.Установить(Объект.Ссылка);  
	рег.Прочитать();
	//Марк 2024-04-16
	...
	НетПриходник = Запрос.Выполнить().Пустой();
	Если НЕ НетПриходник Тогда
