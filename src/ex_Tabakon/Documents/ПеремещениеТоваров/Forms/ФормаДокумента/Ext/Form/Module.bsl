...
Процедура ТБКПриОткрытииПосле(Отказ)
	Если не ОбщегоНазначенияВызовСервера.ПроверитьВозможностьРедактирования_Общая()  тогда
		...
		Для каждого ЭлементФормы из  ЭтаФорма.Элементы Цикл
			Если Найти(ЭлементФормы.Имя,"Факт") или ЭлементФормы.Имя = "Товары" или Найти(ЭлементФормы.Имя,"Групп") или Найти(ЭлементФормы.Имя,"Стран") тогда
				Продолжить;
			КонецЕсли;
...
Процедура  ДобавитьКоличествоФакт(ДанныеСоСканераСтруктура) 
	...
	Если СтрДлина(СканированныйШК) >28 тогда //это маркировка
		ШтрихкодEAN = ШтрихкодированиеИС.ТБКEANИзКодаМаркировки(СканированныйШК);   
		Если не ЗначениеЗаполнено(ШтрихкодEAN) тогда
			ШтрихкодEAN	=	ПолучитьЕАН(СканированныйШК);
		КонецЕсли;
...
Процедура ТБКПередЗаписьюНаСервереПеред(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если не отказ тогда
		...
		НоваяСтрока.ДокументРаспоряжение	=	Объект.Ссылка;
		НоваяСтрока.ТоварыПеремещение		=	Новый ХранилищеЗначения(Объект.Товары.Выгрузить());
