...
&ИзменениеИКонтроль("ДополнитьТоварамиПараметрыПриПробитииЧека")
Процедура ТБКДополнитьТоварамиПараметрыПриПробитииЧека(РеквизитыДокумента, ОбщиеПараметры, ДанныеДляПробитияЧекаККМ, ТаблицаСоответствияТоваровСекциям, ПризнакСпособаРасчета)
	...
	ИспользоватьУчетВетПрепаратов = Ложь;
	ВидыПродукцииВетСертификат = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииПоЭлементуКонструктора(
	Справочники.ЭлементыКонструктораВидовПродукцииИС.ПолеВетеринарныйСертификат);
	Для Каждого ВидПродукцииВетСертификат Из ВидыПродукцииВетСертификат Цикл
		Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукцииВетСертификат) Тогда
			ИспользоватьУчетВетПрепаратов = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.Аванс Тогда
		...
		Если (ОбщиеПараметры.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.ОСН
			ИЛИ ОбщиеПараметры.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоход
			ИЛИ ОбщиеПараметры.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход)
			И НЕ ОсвобожденОтНДС Тогда

			Если РеквизитыДокумента.Дата > УчетНДС.ДатаПереходаНаНДС22() Тогда
				Если РеквизитыДокумента.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда 
					...
					Если ЗначениеЗаполнено(ЧекПродажи) И ЧекПродажи.Дата < УчетНДС.ДатаПереходаНаНДС22() Тогда
						// Возврат аванса за 2025 год.
			...
			Если СНООрганизации.СтавкаНДСУСН = Перечисления.СтавкиНДС.НДС5 Тогда
				СтрокаПозицииЧека.СтавкаНДС = 105;
			ИначеЕсли СНООрганизации.СтавкаНДСУСН = Перечисления.СтавкиНДС.НДС7 Тогда
				СтрокаПозицииЧека.СтавкаНДС = 107;
			КонецЕсли;
	...
	Иначе
		...
		Если Не ТаблицаТоваровГосИС.Количество() = 0 Тогда
			ОбщиеПараметры.ЧасоваяЗона = РеквизитыДокумента.ЧасоваяЗона;
		КонецЕсли;
		...
		Для Каждого СтрокаТЧ Из ТаблицаТоваровЧека Цикл
			...
			#Вставка
			//Вик 2021_07_01
			Если ТекущаяДата() > '20230401' и Найти(Нрег(НаименованиеТовара),"сигареты") и СтрокаТЧ.Цена < 117 тогда
			...
			Если ИспользоватьУчетТоваровФСС И Не ПустаяСтрока(СтрокаТЧ.КодТовараТРУ) Тогда
				...
				КонецЕсли;
			КонецЕсли;
			СтрокаПозицииЧека.ИспользованСертификатФСС = СтрокаТЧ.ИспользованСертификатФСС;
			...
			Иначе
				...
				Если СтрокаТЧ.ПродаетсяВРозлив Тогда
					ДанныеДляПробитияЧекаККМ.ЕстьРазливнаяПродукцияЕГАИС = Истина;
				КонецЕсли;
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ, "РезультатРаспределенияШтрихкодов")
					...
					Если ЗначениеЗаполнено(ПолныйКодМаркировки) Тогда
						...
						Иначе
							Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СтрокаТЧ.КлючВидовПродукции) Тогда
								СтрокаПозицииЧека.Штрихкод = ПолныйКодМаркировки;
							ИначеЕсли Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(СтрокаТЧ.КлючВидовПродукции) Тогда
								СтрокаПозицииЧека.КонтрольнаяМарка = ПолныйКодМаркировки;
							Иначе
								СтрокаПозицииЧека.Штрихкод = СтрокаТЧ.Штрихкод;
							КонецЕсли;
					...
					Если СтрокаТЧ.РезультатРаспределенияШтрихкодов.ЧастичноеВыбытие Тогда
						...
						Если Не КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(СтрокаТЧ.КлючВидовПродукции) Тогда
							Если СтрокаПозицииЧека.ПризнакПредметаРасчета  = Перечисления.ПризнакиПредметаРасчета.ТоварМаркируемыйСИИмеющийКМ Тогда
								СтрокаПозицииЧека.ПризнакПредметаРасчета  = Перечисления.ПризнакиПредметаРасчета.ТоварМаркируемыйСИНеИмеющийКМ;
							КонецЕсли;
						...
						Иначе
							...
							Если СтрокаТЧ.РезультатРаспределенияШтрихкодов.Количество = СтрокаТЧ.РезультатРаспределенияШтрихкодов.ЕмкостьПотребительскойУпаковки Тогда
					...
					Если ЗначениеЗаполнено(
						СтрокаТЧ.РезультатРаспределенияШтрихкодов.РазрешительныйРежимИдентификаторЭкземпляраПО) Тогда
						СтрокаПозицииЧека.ЗапросПроверкиКода.ИдентификаторЭкземпляра
						= СтрокаТЧ.РезультатРаспределенияШтрихкодов.РазрешительныйРежимИдентификаторЭкземпляраПО;
						СтрокаПозицииЧека.ЗапросПроверкиКода.ВерсияБазы
						= СтрокаТЧ.РезультатРаспределенияШтрихкодов.РазрешительныйРежимВерсияБазы;
					КонецЕсли;
				...
				Если СтрокаТЧ.Номенклатура.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга Тогда
					Если ЗначениеЗаполнено(СтрокаТЧ.Договор) Тогда
						Если СтрокаТЧ.Договор.ПризнакАгента = Перечисления.ПризнакиАгента.Комиссионер Тогда
							...
							УчетНДСУКомитента,
							ЭтоАванс,
							Новый Структура("Договор", СтрокаТЧ.Договор));
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если ИспользоватьУчетВетПрепаратов Тогда
				Если СтрокаТЧ.ПрепаратОтпускаетсяПоДокументу Тогда
					СтрокаПозицииЧека.ВетеринарныйПрепарат = Истина;
					СтрокаПозицииЧека.ВетеринарноеТребование = СтрокаТЧ.ЭтоВетТребование;
					Если ЗначениеЗаполнено(СтрокаТЧ.НомерВетРецепта) Тогда
						СтрокаПозицииЧека.РецептНомер = СтрокаТЧ.НомерВетРецепта;
					КонецЕсли;
					Если ЗначениеЗаполнено(СтрокаТЧ.ДатаВетРецепта) Тогда
						СтрокаПозицииЧека.РецептДата = СтрокаТЧ.ДатаВетРецепта;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
