...
Процедура ТБКДополнитьТоварамиПараметрыПриПробитииЧека(РеквизитыДокумента, ОбщиеПараметры, ДанныеДляПробитияЧекаККМ, ТаблицаСоответствияТоваровСекциям, ПризнакСпособаРасчета)
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из ТаблицаТоваровЧека Цикл
			...
			//Марк 2026-01-14 НДС22 Закоментил
			////Вик 2021_03_16
			//Попытка
...
Функция ТБКПодготовитьДанныеДляПробитияЧека(ДанныеДляЧека)
	...
	//Марк 2025-02-12
	Если РеквизитыДокумента.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход5 Тогда
		ОбщиеПараметры.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход;
	Иначе
