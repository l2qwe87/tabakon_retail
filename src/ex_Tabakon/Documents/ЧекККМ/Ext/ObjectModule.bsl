...
Процедура ТБКОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если не Отказ тогда    
		...
		Если не (Комментарий = "Чек н.ф." или Комментарий = "Добавлен в ООРП") тогда
		...
		//Вик 2024-09-03 Чтобы нефискальные чеки списывали товар
		Если Комментарий = "Чек н.ф." тогда
			Для каждого Строка из Товары цикл
				НовСтрока					=	движения.ТоварыНаСкладах.добавить();
				НовСтрока.ВидДвижения		=	ВидДвиженияНакопления.Расход;
				НовСтрока.Количество		=	Строка.КоличествоУпаковок;
				НовСтрока.Номенклатура		=	Строка.Номенклатура;
				НовСтрока.Характеристика	=	Строка.Характеристика;
				НовСтрока.Склад				=	Магазин.СкладПродажи;
