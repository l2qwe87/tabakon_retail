...
Процедура ТБКВозвратОтПокупателяПосле(Команда) 	
	...
	ЧекККМ = Объект.Ссылка;
		...
		//Марк 2024-07-15 открытие не пробитого возврата если есть
	ДокументыВозвратаБезЧека = ПолуфитьДокументаВозвратаЕслиЕсть(ЧекККМ); //Возвращает Массив форм док вовзарата
	Если ДокументыВозвратаБезЧека <> Неопределено Тогда
		Для каждого ДокСсылка Из ДокументыВозвратаБезЧека Цикл
			ПолучитьФорму("Документ.ВозвратТоваровОтПокупателя.Форма.ФормаДокумента", Новый Структура("Ключ",ДокСсылка)).открыть();			
		КонецЦикла; 
		Возврат;	
	КонецЕсли;
	//КонецМарк
...
//Марк 2024-07-15
Функция ПолуфитьДокументаВозвратаЕслиЕсть(ЧекККМ)
	МассивФормВозвратТоваровОтПокупателя = Новый Массив;
	Запрос = Новый запрос("ВЫБРАТЬ
	                      ...
	                      |	И ВозвратТоваровОтПокупателяТовары.ЧекККМ = &ЧекККМ
	                      |
	                      |СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		ДокВозврата = Выборка.Ссылка;
		ЕстьЧекВозврата = НайтиЧекВозврата(ДокВозврата);	
		Если НЕ ЕстьЧекВозврата Тогда
			МассивФормВозвратТоваровОтПокупателя.Добавить(ДокВозврата);
		КонецЕсли; 
	КонецЦикла; 
	
	Если МассивФормВозвратТоваровОтПокупателя.Количество() > 0 Тогда
		Возврат	МассивФормВозвратТоваровОтПокупателя; 	
	иначе
...
Функция НайтиЧекВозврата(ДокВозврата)
	...
	Запрос.УстановитьПараметр("ДокументОснование", ДокВозврата);
	Выборка = Запрос.Выполнить().Выбрать();
...
//Марк 2026-08-24 Запрет возврата без сканирования кодов маркировки.
&НаКлиенте
&Вместо("ПодтвердитьВозвратПродукцииИСМПБезМарок")
Процедура ТБКПодтвердитьВозвратПродукцииИСМПБезМарок(СписокНоменклатуры)

	Если НЕ ТБКЗапретВозвратаБезМарокВключен() Тогда
		ПродолжитьВызов(СписокНоменклатуры);
		Возврат;
	КонецЕсли;
