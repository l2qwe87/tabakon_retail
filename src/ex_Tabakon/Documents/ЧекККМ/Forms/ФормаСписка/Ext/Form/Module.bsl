...
Процедура ТБКВозвратОтПокупателяПосле(Команда) 	
	...
	Если ЗначениеЗаполнено(ЧекККМ) тогда
		...
		СуществующийВозврат	=	НайтиВозврат(ЧекККМ);
		
		Если СуществующийВозврат <>Неопределено тогда
			ПараметрыФормы = Новый Структура("Ключ", СуществующийВозврат);
			ОткрытьФорму("Документ.ВозвратТоваровОтПокупателя.Форма.ФормаДокумента", ПараметрыФормы);
			//ОткрытьЗначение(СуществующийВозврат);	
		иначе
...
Процедура ТБКИнструкцияПосле(Команда)
	ОбщегоНазначенияВызовСервера.ОткрытьНужнуюИнструкцию("Возврат+от+покупателя+на+следующий+день");		
КонецПроцедуры
...
Функция НайтиВозврат(ЧекККМ)
	Запрос	=	новый запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             ...
	      	 	             |	И НАЧАЛОПЕРИОДА(ВозвратТоваровОтПокупателяТовары.Ссылка.Дата, ДЕНЬ) = &Дата");
...
// Толстоухов 20.04.2026 KRO-119 ++
&НаКлиенте
Процедура ТБКОтчетАннулированныеЧекиПосле(Команда)
	ОткрытьФорму("Отчет.ТБК_ИнформацияПоАннулированнымЧекам.Форма");
КонецПроцедуры
