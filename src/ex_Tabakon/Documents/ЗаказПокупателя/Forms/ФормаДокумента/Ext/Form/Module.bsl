...
Процедура ТБКСогласоватьПосле(Команда)
	...
	//Марк 2024-04-18
	НомерЗаказа = Объект.НомерЗаказаНаСайте;
	Если НомерЗаказа <> "" Тогда
		ПроверкаПриепкиПоЗаказзу(НомерЗаказа, Объект.Дата, Отказ);
		Если Отказ Тогда
			Возврат;	
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПроверкаПриепкиПоЗаказзу(НомерЗаказа, ДатаЗаказа,  Отказ)
	...
	Запрос.УстановитьПараметр("Комментарий", "Заказ: " + НомерЗаказа);
	Запрос.УстановитьПараметр("Дата", НачалоГода(ДатаЗаказа));
	
	ВыборкаПеремещение = Запрос.Выполнить().Выбрать();
	Пока ВыборкаПеремещение.Следующий() Цикл
		...
		Запрос.УстановитьПараметр("ДокументОснование", ВыборкаПеремещение.ссылка);
		Запрос.УстановитьПараметр("Дата", НачалоГода(ДатаЗаказа));

		Если Запрос.Выполнить().Пустой() Тогда
			Отказ  = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Нельзя согласовать, так-как не приняты товары по перемещению " + ВыборкаПеремещение.Ссылка;
			Сообщение.Сообщить(); 
		КонецЕсли;
...
Процедура ТБКВыкупленПосле(Команда)
	...
	Если Объект.Статус	<>	ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Согласован") Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Статус должен быть согласован";
		Сообщение.Поле = "Статус";
		Сообщение.УстановитьДанные(Объект.Статус);
		Сообщение.Сообщить();
		Возврат;
...
Процедура ТБКПриОткрытииПосле(Отказ)
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Закрыт") или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.ОтмененВОфисе") Тогда
		...
		Для каждого ОкноПриложения Из ОкнаПриложения Цикл
			...
			Если ОкноПриложения.Заголовок = "" или ОкноПриложения.Заголовок = "*" Тогда
				...
				Для каждого Строка из Объект.Товары цикл
					...
					//Марк 2026-05-19 https://jira.tabkn.ru/browse/PRIC-129-----------
					Если ОбщегоНазначенияВызовСервера.ЭтуМаркуБитьПоБК(НоваяСтрока.Номенклатура) Тогда
						
						НоменклатураБК	=	ОбщегоНазначенияВызовСервера.ПолучитьНоменклатураБК(НоваяСтрока.Номенклатура);
						Если НоменклатураБК.Пустая() Тогда
							НоваяСтрока.Номенклатура = НоменклатураБК;
						КонецЕсли;
					...
					//Марк 2026-03-26 https://jira.tabkn.ru/browse/PRIC-117
					Цена = ЦенаПродажи(
										НайденнаяФорма.Объект.Магазин,
										НоваяСтрока.Номенклатура, 
										НоваяСтрока.Характеристика, НоваяСтрока.Упаковка);
					
					Если Цена <> 0 И Цена <> НоваяСтрока.Цена И Цена > НоваяСтрока.Цена Тогда
						
						НовСумма = Цена * НоваяСтрока.КоличествоУпаковок;
						НоваяСтрока.Цена = Цена;
						НоваяСтрока.СуммаРучнойСкидки = НовСумма - НоваяСтрока.Сумма;
						НайденнаяФорма.СуммаСкидки = НайденнаяФорма.СуммаСкидки + НоваяСтрока.СуммаРучнойСкидки;
					    НайденнаяФорма.СуммаДокументаБезСкидок = НайденнаяФорма.СуммаДокументаБезСкидок + НовСумма;
						//НоваяСтрока.Сумма = НовСумма;
					КонецЕсли; 
					//КонецМарк
				...
				//НайденнаяФорма.ОбновитьФормуНаКлиенте();
				
				НайденнаяФорма.Объект.ЗаказПокупателя = Объект.Ссылка;
...
//
&НаСервере
Функция ЦенаПродажи(ОбъектЦенообразования, Номенклатура, Характеристика, Упаковка)
	
	СтруктураПараметровДействия = Новый Структура;
	СтруктураПараметровДействия.Вставить("ОбъектЦенообразования", ОбъектЦенообразования); 
	СтруктураПараметровДействия.Вставить("Дата", ТекущаяДата());
	СтруктураПараметровДействия.Вставить("ПриводитьКМинимальнойЦене", Истина);
		
	Возврат ЗапасыСервер.ЦенаПродажи(
