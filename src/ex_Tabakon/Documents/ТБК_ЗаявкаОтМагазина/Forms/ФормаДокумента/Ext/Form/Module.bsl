...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Магазин) Тогда
		...
		//Марк 2025-12-05 https://jira.tabkn.ru/browse/KRO-88
		//Поле ИНН_Организации Всегда было пустое, не нашел где заполняется
		ИНН_Организации = ИННОрганизацииТБКЗаявки();
		//КонецМарк
	...
	// Толстоухов 16.02.2026 KRO-68 ++
	ПроектСписано = ПроектСписано();
	Если НЕ ПроектСписано.Пустая() И Объект.Проект = ПроектСписано Тогда
		ЭтаФорма.ТолькоПросмотр = Истина;
	КонецЕсли;
	// Толстоухов 16.02.2026 --
	...
	//Марк 2025-07-21
	Элементы.ТоварыСортироватьСписокПоВозрастанию.Доступность 	= Истина;
	Элементы.ТоварыСортироватьСписокПоУбыванию.Доступность 		= Истина;
	//КонецМарк
	//КонецМарк
...
Процедура ПоказатьОстатки()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	Товары.QR КАК QR,
	                      |	Товары.ПричинаБрака КАК ПричинаБрака
	                      |ПОМЕСТИТЬ Вт_Товары
	                      |ИЗ
	                      |	&Товары КАК Товары
	                      |
	                      ...
	                      |	Вт_Товары.QR КАК QR,
	                      |	Вт_Товары.ПричинаБрака КАК ПричинаБрака,
	...
	РезулттатТЗ = Запрос.Выполнить().Выгрузить();
	Объект.Товары.Загрузить(РезулттатТЗ);
...
Процедура ПроверкаЗаполненияПричиныБрака(Отказ)
	...
	// Толстоухов 14.05.2026 KRO-129 ++
	Для каждого Строка Из Объект.Товары Цикл
		
		Если Найти(Строка(Строка.ПричинаБрака),"Срок годности") > 0 И Не ЗначениеЗаполнено(Строка.СрокГодности) Тогда
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не заполнен срок годности в строке " + Строка.НомерСтроки;
			Сообщение.Сообщить(); 
			Отказ = Истина;
			Прервать;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод")  Тогда
		...
		//Марк 2025-11-19 появились марки где вначале указывается пробел
...
Процедура ДобавитьКоличество(ДанныеСоСканераСтруктура)
	...
	Если Строка(Объект.Проект) = "Между магазинами" или Строка(Объект.Проект) = "Заказ QR" Тогда
		...
		Если ТекущаяДата() <= '20250930235959' Тогда
			ПроверятьСканированияQR = Ложь;		
		КонецЕсли;
	...
	Иначе
		...
		Если ПроверятьСканированияQR Тогда
			...
			Если ЭтоМаркированнаяНоменклатура(НужнаяНоменклатура) Тогда // Толстоухов 22.10.2025 DV1C-184 ++
	...
	Если Объект.флСозданВУТ Тогда
		...
		иначе
			...
			Если ЭтоМаркированнаяНоменклатура(НужнаяНоменклатура) И ПроверятьСканированияQR Тогда // Толстоухов 22.10.2025 DV1C-184 ++
...
Процедура СформированПриИзменении(Элемент)
	...
	Если Отказ Тогда Объект.Сформирован = Ложь; Возврат; КонецЕсли;
...
Процедура ПроставитьСформирован(Результат, ДополнительныеПараметры) экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		//Объект.Дата = ТекущаяДата();                     // Толстоухов 06.02.2026 DV1C-205 ++
		...
		Объект.ДатаСформирован = ТекущаяДата();            // Толстоухов 06.02.2026 DV1C-205 ++
		Записать();
	иначе
		Объект.Сформирован		=	Ложь;
	КонецЕсли;
...
Процедура ТоварыПриИзменении(Элемент)
	...
	//Марк 2023-10-20     
	Если  Строка(Объект.Проект) <> "ЧИСТКА" и Строка(Объект.Проект) <> "Смена ИП" тогда
...
Функция ЭтоМаркированнаяНоменклатура(Номенклатура)
	Возврат ОбщегоНазначения.ЭтоМаркированнаяНоменклатура(Номенклатура) И НоменклатураНеВРегистреПробиватьПоБК(Номенклатура); // Толстоухов 13.10.2025 DV1C-184 ++
КонецФункции
...
Процедура ПроектНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если НЕ Объект.флСозданВУТ Тогда
		...
		ЭлементОтбора.ПравоеЗначение = "Между магазинами";		
		ЭлементОтбора.Использование = Истина;
	КонецЕсли;
	//КонецМарк
	
	// Толстоухов 31.03.2026 KRO-112 ++
	Если Не ЭтоАдминистратор() Тогда
		ЭлементОтбора = Форма.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Наименование");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;             
		
		ЭлементОтбора.ПравоеЗначение = "Списано";		
		ЭлементОтбора.Использование = Истина;
	КонецЕсли;
	// Толстоухов 31.03.2026 -- 
...
// Толстоухов 13.10.2025 DV1C-184 ++
Функция НоменклатураНеВРегистреПробиватьПоБК(Номенклатура)
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПробиватьПоБК.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа
		|ИЗ
		|	РегистрСведений.ПробиватьПоБК КАК ПробиватьПоБК,
		|	вт_ КАК вт_
		|ГДЕ
		|	ВЫБОР
		|			КОГДА вт_.Ссылка.Родитель В (ПробиватьПоБК.НоменклатурнаяГруппа)
		|				ТОГДА ИСТИНА
		|			КОГДА вт_.Ссылка.Родитель.Родитель В (ПробиватьПоБК.НоменклатурнаяГруппа)
		|				ТОГДА ИСТИНА
		|			КОГДА вт_.Ссылка.Родитель.Родитель.Родитель В (ПробиватьПоБК.НоменклатурнаяГруппа)
		|				ТОГДА ИСТИНА
		|			КОГДА вт_.Ссылка.Родитель.Родитель.Родитель.Родитель В (ПробиватьПоБК.НоменклатурнаяГруппа)
		|				ТОГДА ИСТИНА
		|		КОНЕЦ";
...
&НаСервере
Функция ИННОрганизацииТБКЗаявки() //Марк 2025-12-05

	Возврат Объект.Магазин.Организация.ИНН;

КонецФункции // ()

// Толстоухов 16.02.2026 KRO-68 ++
&НаСервереБезКонтекста
Функция ПроектСписано()
	Возврат Справочники.ТБКПроекты.НайтиПоНаименованию("Списано");	
КонецФункции // ПроектСписано()
