...
Процедура ПриОткрытии(Отказ)	
	...
	Объект.Продавец	=	ПолучитьТекущегоПродавца(); 
...
Функция  ТБКПолучитьМагазинНаСервере()
	НужныйМагазин	=	ОбщегоНазначения.ПолучитьСкладИзООРП();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если не Объект.Статус	=	"Подтверждено в офисе" и ЗначениеЗаполнено(Объект.Экспедитор) тогда  
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод")  Тогда
		...
		Если Найти(ДанныеСоСканераСтруктура.Штрихкод,"FL_") тогда
			Если не Объект.Статус	=	"Подтверждено в офисе" тогда
				Сообщить("Проставлять экспедитора можно только по подтвержденным заявкам!");
				Возврат;
			КонецЕсли;
			
			Объект.Экспедитор	=	ПроставитьЭкспедитора(Сред(ДанныеСоСканераСтруктура.Штрихкод,4));
			Если Объект.Экспедитор.Пустая() тогда
				Сообщить("Не найден экспедитор! "+Сред(ДанныеСоСканераСтруктура.Штрихкод,4));
			КонецЕсли;
			Записать();
			Возврат;
		КонецЕсли;
...
Функция ПроставитьЭкспедитора(НаименованиеСпр) 
	Возврат Справочники.ФизическиеЛица.НайтиПоНаименованию(НаименованиеСпр);	
КонецФункции
