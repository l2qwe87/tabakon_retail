...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() тогда
		Объект.ДатаПоследнейИнкасации	=	ПолучитьДатуПоследнейИнкасации(Объект.Ссылка, Объект.Дата);  
	...
	иначе
		...
		Объект.СуммаКСдаче	=	ПолучитьСуммуКСдаче(Объект.Дата, Объект.ДатаПоследнейИнкасации);
	КонецЕсли;   
...
&НаСервереБезКонтекста
Функция ПолучитьДатуПоследнейИнкасации(Ссылка, Дата)
	Запрос	=	Новый запрос("ВЫБРАТЬ
	      	 	             |	ТБКИнкасация.Дата КАК Дата
	      	 	             |ИЗ
	      	 	             |	Документ.ТБКИнкасация КАК ТБКИнкасация
	      	 	             |ГДЕ
	      	 	             |	ТБКИнкасация.Ссылка <> &Ссылка
	      	 	             |	И ТБКИнкасация.Дата <= &Дата
...
&НаСервереБезКонтекста
Функция ПолучитьСуммуКСдаче(Дата, ДатаПоследнейИнкасации)  
