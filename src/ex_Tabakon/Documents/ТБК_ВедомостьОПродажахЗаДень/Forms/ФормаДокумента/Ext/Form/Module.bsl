...
Функция ЗаполнитьОтчетОПродажах()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ГДЕ
	               |			НЕ ОплатаПоБанку.Ссылка.Комментарий ПОДОБНО &Комментарий
	               |		
	               |		СГРУППИРОВАТЬ ПО
	               ...
	               |	И НЕ ОтчетОРозничныхПродажах.Комментарий ПОДОБНО &Комментарий";
...
Функция ПолучитьЗаголовокСервер()
	...
	Для каждого стр из Объект.Продавцы Цикл
		...
		ИначеЕсли стр.ТипПродавца = Перечисления.ТБКТипыПродавцов.МладшийПродавец тогда
			СтрМладший	=	"(Не полный день) " + стр.Продавец;
		иначе
			СтрНочной	=	"(Ночной) " + стр.Продавец;
		КонецЕсли;;		 
	...
	Если ЗначениеЗаполнено(СтрМладший) тогда
		 СтрВозврата	=	?(ЗначениеЗаполнено(СтрВозврата), СтрВозврата + Символы.ПС + СтрМладший, СтрМладший);
	КонецЕсли;

	Если ЗначениеЗаполнено(СтрНочной) тогда
		 СтрВозврата	=	?(ЗначениеЗаполнено(СтрВозврата), СтрВозврата + Символы.ПС + СтрНочной, СтрНочной);
	КонецЕсли;
