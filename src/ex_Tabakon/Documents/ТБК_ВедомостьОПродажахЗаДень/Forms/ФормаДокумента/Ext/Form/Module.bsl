	...
	Если Объект.Продавцы.Количество() = 0 тогда
		Для каждого СтрокаООРП из Объект.ОтчетыОРозничныхПродажах цикл
			...
			Для каждого СтрокаРез из Рез цикл
				...
				//Вик 2022-10-12 Заполнить тип и время
				Запрос	=	Новый Запрос("ВЫБРАТЬ
				      	 	             |	ТБКСменыСотрудниковСрезПоследних.ТипПродавца КАК ТипПродавца,
				      	 	             |	ТБКСменыСотрудниковСрезПоследних.ВремяСмены КАК ВремяСмены
				      	 	             |ИЗ
				      	 	             |	РегистрСведений.ТБКСменыСотрудников.СрезПоследних(&датаКон, Продавец = &Продавец) КАК ТБКСменыСотрудниковСрезПоследних
				      	 	             |ГДЕ
				      	 	             |	ТБКСменыСотрудниковСрезПоследних.КассаККМ = &КассаККМ");
				
				Запрос.УстановитьПараметр("датаКон",КонецДня(Объект.Дата));
				Запрос.УстановитьПараметр("Продавец",СтрокаРез.Продавец);
				Запрос.УстановитьПараметр("КассаККМ",Объект.КассаККМ);
				Рез_Смены	=	запрос.Выполнить().Выгрузить();
				Если Рез_Смены.Количество() >0 тогда
					НоваяСтрока.ТипПродавца	=	Рез_Смены[0].ТипПродавца;
					НоваяСтрока.ВремяСмены	=	Рез_Смены[0].ВремяСмены;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;		
	КонецЕсли;
