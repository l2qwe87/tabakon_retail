...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущаяДата()>'20241101' тогда
		Элементы.ПродавцыПродавецНаЗамену.Видимость = Ложь;
		Элементы.ПродавцыТипПродавца.Видимость		= Ложь;
	КонецЕсли;
	//
...
Процедура ПриОткрытии(Отказ)
	...
	Если НачалоДня(Объект.Дата) <> НачалоДня(ТекущаяДата()) тогда
		Элементы.Группа_Купюры.ТолькоПросмотр	=Истина;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура Купюры(Команда)
	Элементы.Группа_Купюры.Видимость = НЕ Элементы.Группа_Купюры.Видимость; 
КонецПроцедуры
...
Функция ЗаполнитьВыемку()
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             ...
	      	 	             |ГДЕ
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Ссылка <> &Ссылка
	      	 	             |	И НЕ ТБК_ВедомостьОПродажахЗаДень.ПометкаУдаления
	      	 	             |	И ТБК_ВедомостьОПродажахЗаДень.Дата <= &Дата
	      	 	             |	И ТБК_ВедомостьОПродажахЗаДень.КассаККМ = &КассаККМ
	      	 	             |
	      	 	             |УПОРЯДОЧИТЬ ПО
	      	 	             |	ТБК_ВедомостьОПродажахЗаДень.Дата УБЫВ");
	Запрос.УстановитьПараметр("Ссылка",Объект.Ссылка);
	Запрос.УстановитьПараметр("Дата",НужДатаКон);
	Запрос.УстановитьПараметр("КассаККМ",объект.КассаККМ);  
	
	рез = Запрос.Выполнить().Выгрузить();
	Если рез.Количество()=1 тогда
		нужДатаНач = Рез[0].Дата;
	иначе
		нужДатаНач = НачалоДня(Объект.Дата);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ДатаН",нужДатаНач);
	Запрос.УстановитьПараметр("ДатаК",НужДатаКон);
	...
	//Вик 2026-08-24
	Если Запрос.Выполнить().Пустой() и Объект.ОтчетыОРозничныхПродажах.Количество() и Объект.ОплатаНаличными > 0 тогда 
		
		Для каждого Строка из Объект.ОтчетыОРозничныхПродажах цикл      
			Попытка
				НовДок = СоздатьВыемкуНаСервере(Строка.ОтчетОРозничныхПродажах, Строка.ОтчетОРозничныхПродажах.СуммаОплатыНаличных);
				
				НовСтрока	=	Объект.ВыемкаДенежныхСредств.Добавить();
				НовСтрока.Выемка	=	НовДок.Ссылка;
				НовСтрока.Сумма		=	Строка.ОтчетОРозничныхПродажах.СуммаОплатыНаличных;
				
				НовСтрока.ПКО = СоздатьПКО_ВыручкаНаличка(
					Объект.Дата,
					Объект.КассаККМ, 
					НовСтрока.Выемка,
					НовСтрока.Сумма, 
...
Функция ЗаполнитьОтчетОПродажах()
	...
	Если отчетыПоПродажам.Количество() = 1 Тогда
		...
		Объект.ВыручкаЗаСменуПоКассе = 
			Объект.ОплатаНаличными 
		   +Объект.ОплатаПоБанковскимКартам - Объект.ВозвратОтПокупателяБезНал
		; 
		Объект.ОплатаПоБанковскимКартам		= Объект.ОплатаПоБанковскимКартам - Объект.ВозвратОтПокупателяБезНал;
		
		Объект.ВозвратОтПокупателяНал 		= возвратОтПокупателяНАЛ;
		Объект.ВозвратОтПокупателяБезНал 	= возвратОтПокупателяКарта;
		//КонецВик
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Продавцы.Количество() = 0 тогда
		...
		Отказ = Истина;
	Иначе
		ПоискДублейСтрокИПустыхЗначенийВТЧПродавцы(Объект.Продавцы,Отказ);	
	КонецЕсли; 
	
	Если Объект.ОтчетыОРозничныхПродажах.Количество() = 0 тогда
		Сообщить("Не заполнены Отчеты О Розничных Продажах!");		
		Отказ = Истина;
	КонецЕсли; 
...
&НаКлиенте
 Процедура ПоискДублейСтрокИПустыхЗначенийВТЧПродавцы(ТЧ, Отказ) 
	...
	Для  каждого СтрокаТЧ  Из ТЧ Цикл
		Если не ЗначениеЗаполнено(СтрокаТЧ.Продавец) тогда
			...
			Индекс = Объект.Продавцы.Индекс(СтрокаТЧ);
		...
		Отбор.Очистить();
		Отбор.Вставить("Продавец", СтрокаТЧ.Продавец);
		...
		Если Строки.Количество() > 1 Тогда
			Для  каждого НайденныеСтроки  Из Строки Цикл
				...
				Если Строки.Найти(НайденныеСтроки) > 0 Тогда
					СтрокаСообщения = "Продавец в строке № " + Строки[0].НомерСтроки + " совпадает со строкой № " + НайденныеСтроки.НомерСтроки;
		...
		Если флЕстьДубли тогда
			Прервать;
		КонецЕсли;
		
		// у магазинов, где две кассы иногда не заполняется тип продавца
		// Толстоухов 24.03.2026 ++
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ТипПродавца) Тогда
			СтрокаТЧ.ТипПродавца = ОсновнойПродавец();
		КонецЕсли;	
		// Толстоухов 24.03.2026 --	
...
Функция ОбновитьЗаголовок_ПродавцыПодробно()
	Элементы.ПродавцыПодробно.Заголовок = ПолучитьЗаголовокСервер();
КонецФункции

&НаСервере
Функция ПолучитьЗаголовокСервер()
СтрОсновной	=	"";
	СтрВторой	=	"";
	
	Для каждого стр из Объект.Продавцы Цикл
		
		Если стр.ТипПродавца = Перечисления.ТБКТипыПродавцов.ОсновнойПродавец тогда
			СтрОсновной	=	"(Основной) " + стр.Продавец;	
		ИначеЕсли стр.ТипПродавца = Перечисления.ТБКТипыПродавцов.ВторойПродавец тогда
			СтрВторой	=	"(Второй) " + стр.Продавец;
		КонецЕсли;;		 
		
	КонецЦикла;
	
	СтрВозврата = "";
	Если ЗначениеЗаполнено(СтрОсновной) тогда
		 СтрВозврата	=	СтрОсновной;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрВторой) тогда
		 СтрВозврата	=	?(ЗначениеЗаполнено(СтрВозврата), СтрВозврата + Символы.ПС + СтрВторой, СтрВторой);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОсновнойПродавец()
	Возврат Перечисления.ТБКТипыПродавцов.ОсновнойПродавец;
КонецФункции	

&НаКлиенте
Процедура ВнестиДанныеПоВыручке(Команда)	
	//ЗаписатьНаСервере();
	ПараметрыФормы = Новый Структура("ДанныеПоВыручке", Объект.ДанныеПоВыручке);       
	ПараметрыФормы.Вставить("ОплатаНаличными", Объект.ОплатаНаличными);
	ПараметрыФормы.Вставить("ОплатаПоБанковскимКартам", Объект.ОплатаПоБанковскимКартам);
	
	Если Объект.ДанныеПоВыручке.Количество() тогда
		ПараметрыФормы.Вставить("НаличныйZ", Объект.ДанныеПоВыручке[0].ZОтчет);
		ПараметрыФормы.Вставить("БезНаличныйZ", Объект.ДанныеПоВыручке[1].ZОтчет);
		ПараметрыФормы.Вставить("НаличныйФакт", Объект.ДанныеПоВыручке[0].Фактически);
		ПараметрыФормы.Вставить("БезНаличныйФакт", Объект.ДанныеПоВыручке[1].Фактически);
	КонецЕсли;
	ПараметрыФормы.Вставить("ДанныеПоВыручке", Объект.ДанныеПоВыручке);
	ПараметрыФормы.Вставить("ТерминальнаяСверкаНеВышла", Объект.ТерминальнаяСверкаНеВышла);

	ОткрытьФорму("Документ.ТБК_ВедомостьОПродажахЗаДень.Форма.ФормаДанныеПоВыручке",ПараметрыФормы,ЭтаФорма,,,, Новый ОписаниеОповещения("ПослеВнесенияДанныхПоВыручке", ЭтаФорма) );
КонецПроцедуры

Процедура ПослеВнесенияДанныхПоВыручке(Результат, Параметры) экспорт
	Если Результат<> Неопределено тогда
		Объект.ДанныеПоВыручке.Очистить();
		
		НовСтрока = Объект.ДанныеПоВыручке.Добавить();
		НовСтрока.ТипДС			=	"Наличные";  
		НовСтрока.ZОтчет		=	Результат.НаличныйZ;
		НовСтрока.Фактически	=	Результат.НаличныйФакт;
		
		НовСтрока = Объект.ДанныеПоВыручке.Добавить();
		НовСтрока.ТипДС	=	"Безналичные";  
		НовСтрока.ZОтчет		=	Результат.БезНаличныйZ;
		НовСтрока.Фактически	=	Результат.БезНаличныйФакт;
