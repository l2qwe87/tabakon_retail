...
Функция RemovedОffice(вхПараметры) ЭКСПОРТ //Марк
	Попытка
		...
		Если Сред(НомерДокумента,3,1) = "5" тогда
			НомерДокументаС5	=	лев(НомерДокумента,2) + "0" + Прав(НомерДокумента,6);
		КонецЕсли;
		
		ДатаДокумента = ОбщегоНазначения.РазобратьДату(вхПараметры.дата);
		
		//Марк 2025-11-20
		ДокументЗаявка = Документы.ТБК_ЗаявкаОтМагазина.НайтиПоНомеру(НомерДокументаС5,ДатаДокумента);
