...
Функция ActivateNewPrice(вхПараметры) ЭКСПОРТ
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |			ИЛИ ПрименениеЦенНоменклатуры.Комментарий ПОДОБНО &Комментарий1)
	      	 	             |	И НЕ ПрименениеЦенНоменклатуры.Проведен
	      	 	             |	И НЕ ПрименениеЦенНоменклатуры.ПометкаУдаления");
	Запрос.УстановитьПараметр("Комментарий", "Одноразовые устройства.НЕ принимать, будет проведена только после согласования автоматически.");
	Запрос.УстановитьПараметр("Комментарий1", "Жидкости, Картриджи НЕ принимать, будет проведена только после согласования автоматически.");
