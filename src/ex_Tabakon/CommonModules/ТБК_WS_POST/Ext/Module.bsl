		...
		Если вхПараметры.Свойство("ЕстьФотки") Тогда
			...
			если файл_Каталог_photo.Существует() Тогда
				...
				Если НЕ файл_Каталог_chat.Существует() Тогда
					СоздатьКаталог(файл_Каталог_chat);	
				КонецЕсли;
			иначе
...
//Процедура UploadРhotoFTP(вхПараметры) //Марк 2024-02-12
//	ЗаписьЖурналаРегистрации("Информация",,,,"Зашел в обмен FTP");
//	СтукрутраFTP = ОбщегоНазначения.ПолучитьСтруктуруПодключенияFTP();
//	Соединение = Новый FTPСоединение(СтукрутраFTP.ИмяFTPСервера, СтукрутраFTP.Порт, СтукрутраFTP.Пользователь, СтукрутраFTP.Пароль);
//	
//	//Проверка сущестоывние главного каталога
//	ОсновнойКаталог = Соединение.НайтиФайлы(СтукрутраFTP.КаталогНаСервере + "photo/");
//	Если ОсновнойКаталог.Количество() = 0 Тогда Возврат; КонецЕсли;
//	
//	КаталогИб = СтрокаСоединенияИнформационнойБазы();
//	КаталогИб = СтрЗаменить(КаталогИб, "File=""", "");
//	КаталогИб = СтрЗаменить(КаталогИб, """;", "");
//	
//	//Вначале фотки ТБКЗаявок
//	КаталогТБК = Соединение.НайтиФайлы(СтукрутраFTP.КаталогНаСервере + "photo/TBKzayavki/");
//	если КаталогТБК.Количество() >= 1 тогда
//		Файлы = Соединение.НайтиФайлы(СтукрутраFTP.КаталогНаСервере + "photo/TBKzayavki/","*");
//		//Проверка существоание каталога на диске для ТБК
//		ИмяКаталогаФоток = КаталогИб + "\photo\TBKzayavki\";
//		файл = Новый Файл(ИмяКаталогаФоток);
//		если НЕ файл.Существует() Тогда СоздатьКаталог(ИмяКаталогаФоток); КонецЕсли;
//		
//		для каждого Файл Из Файлы Цикл
//			если СтрНайти(Файл.Имя, "_Shipped") тогда Продолжить; КонецЕсли;
//			ПутьКФайлу = ПолучитьИмяВременногоФайла(Файл.Имя);
//			Соединение.Получить(Файл.ПолноеИмя, ПутьКФайлу);
//			ПереместитьФайл(ПутьКФайлу, КаталогИб + "\photo\TBKzayavki\" + Файл.Имя); 
//			Соединение.Переместить(Файл.ПолноеИмя, Файл.Путь + Файл.ИмяБезРасширения + "_Shipped" + Файл.Расширение); 
//		КонецЦикла;
//		
//	КонецЕсли;
//	 	
//	//ФОТО для чата
//	КаталогТБК = Соединение.НайтиФайлы(СтукрутраFTP.КаталогНаСервере + "photo/chat/");
//	если КаталогТБК.Количество() >= 1 тогда //Проверка каталога для чата
//		
//		//Проверка существоание каталога на диске для чата
//		ИмяКаталогаФоток = КаталогИб + "\photo\chat\";
//		файл = Новый Файл(ИмяКаталогаФоток);
//		если НЕ файл.Существует() Тогда СоздатьКаталог(ИмяКаталогаФоток); КонецЕсли;
//		
//		Файлы = Соединение.НайтиФайлы(СтукрутраFTP.КаталогНаСервере + "photo/chat/","*");
//		для каждого Файл Из Файлы Цикл
//			если СтрНайти(Файл.Имя, "_Shipped") тогда Продолжить; КонецЕсли;
//			ПутьКФайлу = ПолучитьИмяВременногоФайла(Файл.Имя);
//			Соединение.Получить(Файл.ПолноеИмя, ПутьКФайлу);
//			ПереместитьФайл(ПутьКФайлу, КаталогИб + "\photo\chat\" + Файл.Имя); 
//			Соединение.Переместить(Файл.ПолноеИмя, Файл.Путь + Файл.ИмяБезРасширения + "_Shipped" + Файл.Расширение); 
//		КонецЦикла;
//	КонецЕсли;
//	ЗаписьЖурналаРегистрации("Информация",,,,"Конец обмен FTP");
//КонецПроцедуры
