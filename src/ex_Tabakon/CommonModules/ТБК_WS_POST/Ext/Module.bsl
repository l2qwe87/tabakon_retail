...
Функция WriteRequest(вхПараметры) ЭКСПОРТ
	Попытка
		...
		//Марк 2024-12-10 повторная отпавка заявки
...
Функция ActivateNewPrice(вхПараметры) ЭКСПОРТ
	Коммент = СокрЛП(вхПараметры.comment);
	...
	Если Рез.Следующий() тогда
		...
		Исключение
			Возврат "Не удалось провести!"
		КонецПопытки;
		
		Возврат "Успех";
	иначе
		Возврат "Нет переоценки!"
	КонецЕсли;   	
КонецФункции
...
Функция SetTaxPatent(вхПараметры) Экспорт
	...
	Версия = мРасширений[0].Версия;
	Если НЕ Найти(Версия, "1.7.4.6") Тогда
		
		эл =  Новый Структура();
		эл.Вставить("ОбщийОтвет", Ложь);
		эл.Вставить("ОписаниеОшибки", "Магазин не обновился. версия " + Версия);
