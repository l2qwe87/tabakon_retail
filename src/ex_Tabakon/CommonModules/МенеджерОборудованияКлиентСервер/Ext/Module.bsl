...
Функция УстановитьКлише_Сервер(НазвениеКонтанетыКлише) экспорт
	Попытка
		Fptr = ПодключитьВнешнююКомпоненту("AddIn.Fptr10");
		Fptr = Новый COMобъект("AddIn.Fptr10");
		Fptr.open();
		...
		Если не Fptr.getParamString(Fptr.LIBFPTR_PARAM_UNIT_VERSION) = "3.0.1245" тогда
			...
			//Марк 2025-04-10 
			СистемаНалогообложения = ОбщегоНазначенияВызовСервера.ПолучитьСистемуНалогообложенияСрезПоследнихПоПервойОгранизации();
			
			Если СистемаНалогообложения <> Неопределено Тогда
			
				Fptr.setParam(Fptr.LIBFPTR_PARAM_SETTING_ID, 188);
				Fptr.setParam(Fptr.LIBFPTR_PARAM_SETTING_VALUE, "Вид налогообложения: " + СистемаНалогообложения);
			
				Fptr.writeDeviceSetting();   	
				Fptr.commitSettings();
				ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая(НазвениеКонтанетыКлише, истина);
			иначе
				ОбщегоНазначенияВызовСервера.Установить_ЗначениеКонстанты_Общая(НазвениеКонтанетыКлише, Ложь);	
			КонецЕсли; 
			//КонецМарк			
		...
		иначе
			...
			Если СистемаНалогообложения <> Неопределено Тогда
				...
				Fptr.setParam(Fptr.LIBFPTR_PARAM_SETTING_ID, 189);
				Fptr.setParam(Fptr.LIBFPTR_PARAM_SETTING_VALUE, "Вид налогообложения: " + СистемаНалогообложения);	
