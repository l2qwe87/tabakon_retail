Функция ПолучитьКоличествоБаллов(ID, Товары) экспорт
	...
	Имя			=	Сред(КолБаллов_Ответ,Разделитель+1);
	...
	Если КолБаллов >0 тогда
		...
		Пока Найти(ГруппыНоменклатуры,";") цикл
			...
			МассивНоменклатуры.Добавить(Справочники.Номенклатура.НайтиПоКоду(КодГруппы));
			
			ГруппыНоменклатуры = Сред(ГруппыНоменклатуры,13);
		КонецЦикла;
		МассивНоменклатуры.Добавить(Справочники.Номенклатура.НайтиПоКоду(ГруппыНоменклатуры));
...
Функция ОбновитьКоличествоБаллов(КолБаллов, Товары) экспорт
	Прокси = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User", "http://mx.tabakon.ru:1777/ut/ws/tbk?wsdl");
	Если Прокси = Неопределено тогда 
		Сообщить("Ошибка поключения к УТ");
		Возврат 0; 
...
Функция СписатьБаллы(КолБаллов, Товары) экспорт
	Прокси = ОбщегоНазначенияВызовСервера.ПолучитьПрокси("123456", "WS_User", "http://mx.tabakon.ru:1777/ut/ws/tbk?wsdl");
	Если Прокси = Неопределено тогда 
		Сообщить("Ошибка поключения к УТ");
		Возврат 0; 
