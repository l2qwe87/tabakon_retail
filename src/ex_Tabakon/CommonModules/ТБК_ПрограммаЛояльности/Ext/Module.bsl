...
Функция СписатьБаллы(КолБаллов, Товары, Адрес_ВебСоединение, ТБК_IDКлиента) экспорт
	...
	Если КолБаллов >0 тогда
		...
		Пока Найти(ГруппыНоменклатурыМожноСписывать,";") цикл
			...
			Ном =  Справочники.Номенклатура.НайтиПоКоду(КодГруппы);
			Если не Ном.Пустая() тогда  
				Если МассивНоменклатурыМожноСписывать.Найти(Ном) = Неопределено тогда
					МассивНоменклатурыМожноСписывать.Добавить(Ном);
				КонецЕсли;
			КонецЕсли;
		...
		Для каждого СТрокаТовар из Товары цикл
			...
			Если НЕ Запрос.Выполнить().Пустой() тогда //можно списать
				...
				ПолныйКод	=	 СТрокаТовар.Номенклатура.ПолныйКод();
				Поз			=	0;
