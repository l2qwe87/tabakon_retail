...
Функция СписатьБаллы(КолБаллов, Товары, Адрес_ВебСоединение, ТБК_IDКлиента) экспорт
	...
	Если КолБаллов >0 тогда
		...
		//2025-05-26 по просьбе Козьминой - максимум 2 списания в день   
		Если ТекущаяДата()>'20250617' тогда
			Попытка
				Ответ_МожноСписатьПоКоличеству = Прокси.get_MozhnoSpisatPoKolichestvy(ТБК_IDКлиента);
				Если НЕ Ответ_МожноСписатьПоКоличеству тогда
					Сообщить("Списание невозможно. Вы уже списывали 2 раза баллы сегодня!");
					Возврат 0;                          	
				КонецЕсли;
			Исключение
				Сообщить("Ошибка подключения к УТ");
		...
		Для каждого Строка из УсловияПрограммы цикл  
			ВозможныйПроцентСписания			=	Число(Строка.ВозможныйПроцентСписания);
	        ГруппыНоменклатурыЗапрет			=	Строка.ГруппыНоменклатуры;
			//Марк 2025-01-21
			ГруппыНоменклатурыМожноСписывать	=	Строка.ГруппыНоменклатурыМожно;
			//КонецМарк
		КонецЦикла;
		...
		МассивНоменклатурыНельзяСписывать = Новый Массив;
		Пока Найти(ГруппыНоменклатурыЗапрет,";") цикл
			КодГруппы	=	Сред(ГруппыНоменклатурыЗапрет,1,11);
			
			//Ном =  Справочники.Номенклатура.НайтиПоКоду(КодГруппы);
			//Если не Ном.Пустая() тогда  
				Если МассивНоменклатурыНельзяСписывать.Найти(КодГруппы) = Неопределено тогда
					МассивНоменклатурыНельзяСписывать.Добавить(КодГруппы);
				КонецЕсли;
			//КонецЕсли;
			
			ГруппыНоменклатурыЗапрет = Сред(ГруппыНоменклатурыЗапрет,13);
		КонецЦикла;

	
		МассивНоменклатурыМожноСписывать = Новый Массив;
		Пока Найти(ГруппыНоменклатурыМожноСписывать,";") цикл
			КодГруппы	=	Сред(ГруппыНоменклатурыМожноСписывать,1,11);
			
			//Ном =  Справочники.Номенклатура.НайтиПоКоду(КодГруппы);
			//Если не Ном.Пустая() тогда  
				Если МассивНоменклатурыМожноСписывать.Найти(КодГруппы) = Неопределено тогда
					МассивНоменклатурыМожноСписывать.Добавить(КодГруппы);
				КонецЕсли;
			//КонецЕсли;
			
			ГруппыНоменклатурыМожноСписывать = Сред(ГруппыНоменклатурыМожноСписывать,13);
		КонецЦикла;   
		...
		Для каждого СТрокаТовар из Товары цикл
			...
			СТрокаТоварПолныйКод = СТрокаТовар.Номенклатура.ПолныйКод();
			для каждого Элемент из МассивНоменклатурыМожноСписывать Цикл  
				
				Если СтрНайти(СТрокаТоварПолныйКод, Элемент) ТОгда 
					МожноСписать = Истина;
					Прервать;
				КонецЕсли;
			...
			для каждого Элемент из МассивНоменклатурыНельзяСписывать Цикл  
			...
			//Проверяем в группе доступа к списанию товар или нет
			//Запрос	=	Новый Запрос("ВЫБРАТЬ
			//      	 	             |	Номенклатура.Ссылка КАК Ссылка
			//      	 	             |ИЗ
			//      	 	             |	Справочник.Номенклатура КАК Номенклатура
			...
			//Марк 2025-01-21
			//Запрос.УстановитьПараметр("Мас",МассивНоменклатуры);
			//Запрос.УстановитьПараметр("Мас",МассивНоменклатурыМожноСписывать);
			//КонецМарк
			//Запрос.УстановитьПараметр("Ссылка",СТрокаТовар.Номенклатура);	

			//Если НЕ Запрос.Выполнить().Пустой() тогда //можно списать
			Если МожноСписать Тогда 		
				
				//Новое 2024-05-08 Отдельный возможный процент списания в разрезе групп. Какая группа ниже в иерархии - главная.
				НужныйВозможныйПроцентСписания	=	ВозможныйПроцентСписания;
			
			
				ПолныйКод	=	 СТрокаТоварПолныйКод;
			//КонецМарк
				Поз			=	0;

				Для каждого Строка из УсловияПрограммы[0].ГруппыСписания цикл
					
					ГруппаСписания	=	Справочники.Номенклатура.НайтиПоКоду(Строка.НоменклатураКод);
					Если не ГруппаСписания.Пустая() тогда						
							
						Поз_Усл	= Найти(ПолныйКод,Строка.НоменклатураКод);	
						Если Поз_Усл >Поз тогда 
							НужныйВозможныйПроцентСписания	=	Число(Строка.ВозможныйПроцентСписанияГруппы);
							Поз = Поз_Усл;						
						КонецЕсли;
				...
				СТрокаТовар.СуммаРучнойСкидки		=	Цел(Мин((СТрокаТовар.сумма/100)	 * НужныйВозможныйПроцентСписания, КолБаллов));
