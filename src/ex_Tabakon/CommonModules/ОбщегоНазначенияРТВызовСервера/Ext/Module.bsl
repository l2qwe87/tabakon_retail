...
Процедура УдалитьНеПробитыеВозвраты() Экспорт
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ЧекККМ.Ссылка КАК ЧекККМВозврат,
	                      |	ЧекККМ.ЧекККМПродажа КАК ЧекККМПродажа
	                      |ПОМЕСТИТЬ Вт_ЧекиВозврат
	                      |ИЗ
	                      |	Документ.ЧекККМ КАК ЧекККМ
	                      |ГДЕ
	                      ...
	                      |	ВозвратТоваровОтПокупателяТовары.ЧекККМ КАК ЧекККМ,
	                      |	Вт_ЧекиВозврат.ЧекККМВозврат КАК ЧекККМВозврат
	                      |ИЗ
	                      |	Документ.ВозвратТоваровОтПокупателя.Товары КАК ВозвратТоваровОтПокупателяТовары
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ Вт_ЧекиВозврат КАК Вт_ЧекиВозврат
	                      |		ПО ВозвратТоваровОтПокупателяТовары.ЧекККМ = Вт_ЧекиВозврат.ЧекККМПродажа
	                      |ГДЕ
	                      |	ВозвратТоваровОтПокупателяТовары.Ссылка.Проведен
	                      |	И ВозвратТоваровОтПокупателяТовары.Ссылка.Дата >= &Дата
	                      |	И (Вт_ЧекиВозврат.ЧекККМВозврат = ЗНАЧЕНИЕ(Документ.ЧекККМ.ПустаяСсылка)
	                      |			ИЛИ Вт_ЧекиВозврат.ЧекККМВозврат ЕСТЬ NULL
	                      |			ИЛИ НЕ Вт_ЧекиВозврат.ЧекККМВозврат.Проведен)
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	ВозвратТоваровОтПокупателяТовары.Ссылка,
	                      |	ВозвратТоваровОтПокупателяТовары.ЧекККМ,
	                      |	Вт_ЧекиВозврат.ЧекККМВозврат");
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбъектВозвратТоваров = Выборка.ВозвратТоваров.ПолучитьОбъект();	
		ОбъектВозвратТоваров.УстановитьПометкуУдаления(Истина);	
