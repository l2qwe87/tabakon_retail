...
Процедура ТБКОткрытьФормуПодбораНоменклатурыПоШтрихкодам(НеизвестныеШтрихкоды, ФормаВладелец, ОповещениеОЗакрытии)
	...
	Если ЕстьПравоСопоставленияНоменклатуры Тогда
		...
		Если НеизвестныеШтрихкоды.Количество() Тогда
			Для каждого НеНайденныйШтрихкод Из НеизвестныеШтрихкоды Цикл
	...
	Иначе
		...
		Для Каждого Штрихкод Из НеизвестныеШтрихкоды Цикл
			...
			Если ТипЗнч(Штрихкод) = Тип("Структура") Тогда
				...
				ОбщегоНазначенияВызовСервера.ДобавитьДанныеШтрихкодаК_ТБК_ЗапросДанныхУ_УТ(Штрихкод.Штрихкод);
			...
			//КонецМарк
				...
				ОбщегоНазначенияВызовСервера.ДобавитьДанныеШтрихкодаК_ТБК_ЗапросДанныхУ_УТ(Штрихкод);
