...
Функция ТБКДанныеПоШтрихкодамEAN(ШтрихкодыEAN, ПараметрыСканирования = Неопределено, GTINПотребительскихУпаковок = Неопределено, СоздаватьНоменклатуру = Ложь, МРЦ = Неопределено) экспорт
	...
	ШтрихкодированиеИСПереопределяемый.ТБКПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыСканирования, МРЦ);
	...
	Для Каждого ЭлементДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		Если Не ДопустимаПродукцияИСМП
			И ШтрихкодированиеОбщегоНазначенияЕГАИСКлиентСервер.ЭтоВидПродукцииИСМПЕГАИС(ЭлементДанных.ВидПродукции) Тогда
...
Функция ТБКОбработатьВводШтрихкода(Форма, ВходящиеДанные, КэшированныеЗначения, ПараметрыСканирования = Неопределено) Экспорт
	...
	////Марк 2025-02-19 статус Продан пробивать по бк (подставляется бк 'будто отсканировали шк')
