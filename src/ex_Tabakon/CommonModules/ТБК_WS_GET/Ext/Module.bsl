...
Функция GetAnnulledCancelledReceipts(вхПараметры) Экспорт
	...
	Попытка
		...
		Пока рез.Следующий() Цикл
			...
			Продавец = ?(ЗначениеЗаполнено(рез.ПродавецСтроки), рез.ПродавецСтроки, рез.ПродавецШапки);
			...
			эл.Вставить(КодСтатусаЧека(рез.СтатусЧекаККМ),   СокрЛП(рез.СтатусЧекаККМ));
			эл.Вставить(КодВидаОперации(рез.ВидОперации),    СокрЛП(рез.ВидОперации));
...
Функция КодСтатусаЧека(Знач Статус)
	
	Если Статус = Перечисления.СтатусыЧековККМ.Аннулированный Тогда
		Возврат "Annulled";
	ИначеЕсли Статус = Перечисления.СтатусыЧековККМ.Отложенный Тогда
		Возврат "Postponed";
	ИначеЕсли Статус = Перечисления.СтатусыЧековККМ.Пробитый Тогда
		Возврат "Punched";
	ИначеЕсли Статус = Перечисления.СтатусыЧековККМ.Архивный Тогда
		Возврат "Archived";
	КонецЕсли;
	
	Возврат "";   // статус не заполнен - чек в работе
	
КонецФункции

Функция КодВидаОперации(Знач ВидОперации)
