...
Функция GetAnnulledCancelledReceipts(вхПараметры) Экспорт
	...
	Попытка
		...
		Если ДатаКон <= ДатаНач Тогда
			// Интервал схлопнулся: курсор уже на актуальной границе.
			// Отдаём пустой ответ и НЕ двигаем границу вперёд.
		...
		Если КоличествоЧеков >= МаксЧеков Тогда
			// Лимит достигнут - возможно, отдали не всё. Границу ставим по
			// последнему включённому чеку, иначе УТ сдвинет курсор за непрочитанные
			// данные и потеряет их навсегда.
...
Функция GetMarksForWriteOff(вхПараметры) Экспорт
	...
	Попытка
		...
		// Режем по маркам, а не по чекам целиком: граница интервала включительная,
		// поэтому чек, попавший под обрез, приедет в следующей пачке заново и целиком.
		// Дублей это не создаёт - в УТ ключ регистра сама марка.
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 5000
		|	ЧекККМ.Дата КАК ДатаЧека,
		...
		Если Результ.Строк >= МаксСтрок Тогда
			// Лимит достигнут - возможно, отдали не всё. Границу ставим по последней
			// включённой марке, иначе УТ сдвинет курсор за непрочитанные данные
			// и потеряет их навсегда.
			Результ.Обрезано      = Истина;
			Результ.ГраницаОтвета = ДатаВСтроку(ПоследняяДатаЧека);
		Иначе
			Результ.ГраницаОтвета = ДатаВСтроку(ДатаКон);
