...
Функция GetEffekt(вхПараметры) ЭКСПОРТ 
	Попытка
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	СУММА(ЕСТЬNULL(ПродажиПоДнямОбщие.СуммаЧеки * ДниСКоэффициентом.Коэф, 0)) КАК СуммаПродажПродавца,
		               |	СУММА(ЕСТЬNULL(ПродажиПоДням.СуммаЧеки * ДниСКоэффициентом.Коэф, 0)) КАК СуммаПродажПродавцаВМТ,
