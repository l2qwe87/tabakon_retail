...
Функция GetAnnulledReceipts(вхПараметры) Экспорт
	...
	Попытка
		...
		|	ЧекККМТовары.Продавец КАК ПродавецСтроки,
		...
		Пока рез.Следующий() Цикл
			...
			эл.Вставить("SalerRef",      Строка(Продавец.УникальныйИдентификатор()));
			эл.Вставить("SalerName",     Строка(Продавец));
