	...
	ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(ПараметрыОткрытия.ВидПродукции, Истина)
		...
		Если НашиВидыПродкукцииСЗаменойНаБК.Найти(Строка(ПараметрыОткрытия.ВидПродукции)) <> Неопределено 
			...
			Попытка
				...
				ОбщегоНазначенияВызовСервера._ЗаписьЖурналаРегистрации(ТекстКомментария, УровеньЖурналаРегистрации.Примечание);
...
Процедура ТБКУточнитьДанныеУПользователя(ФормаВладелец, ПараметрыОткрытияФормы, ОповещениеПоЗавершениюУточненияДанных) Экспорт
	...
	ИначеЕсли ПараметрыОткрытияФормы.Операция = "ЗапускПроверкиНаТСПИоТ" Тогда
		...
		//|}.";
		//
		//Если (ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ДанныеШтрихкода.РазрешительныйРежимКодОтвета = "200" 
		//	и (ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ДанныеШтрихкода.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиИСМП.Продан")
		//	или ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ДанныеШтрихкода.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиИСМП.Списан")))
		//	
		//	или (ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ЕстьОшибкиВДеревеУпаковок
		//	Или ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ЕстьОшибки
		//	Или ЗначениеЗаполнено(ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ТекстОшибки)
		//	И ОповещениеПоЗавершениюУточненияДанных.ДополнительныеПараметры.РезультатОбработкиШтрихкода.ТекстОшибки = ТекстОшибки)
		//	Тогда
		//	
		//	Сообщить(ТекстОшибки);
		//	ПараметрыОткрытияФормы.КодМаркировкиДляУточнения.РезультатОбработкиШтрихкода.ОткрытьФормуВводаКодаМаркировки = Истина;
		//	ТБКУточнитьДанныеУПользователя(ФормаВладелец, ПараметрыОткрытияФормы, ОповещениеПоЗавершениюУточненияДанных);
		//	
		//КонецЕсли;
		//-------------------------------------------------------------------
