...
Процедура ТБКСформироватьXMLПакетДляФискализацияЧека(ОбщиеПараметры, ПараметрыФискализации, ВключатьПерсональныеДанные = Ложь, 
	...
	Если ПараметрыФискализации.РевизияИнтерфейса >= 3000 Тогда  // Версия стандарта 3.0 (ОФД 1.1)
		...
		Если ПараметрыФискализации.РевизияИнтерфейса >= 4006 Тогда
			...
			//Марк 2026-01-13 НДС22 
			Если ОбщиеПараметры.Свойство("Интернет") Тогда
			//КонецМарк	
		...
		Если ОбщиеПараметры.Свойство("ДанныеКоррекции") Тогда
			ТБКЗаписатьДанныеКоррекции(ЗаписьXML, ОбщиеПараметры.ДанныеКоррекции, ПараметрыФискализации.РевизияИнтерфейса);
		КонецЕсли;
		...
		Если ОбщиеПараметры.ПозицииЧека <> Неопределено Тогда
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда // Фискальная строка
					...
					ЗначениеСтавкаНДС = ТБКПредставлениеСтавкиНДС(ТекущаяПозиция.СтавкаНДС, ПараметрыФискализации.РевизияИнтерфейса);     
...
&Вместо("ЗаписатьДанныеКоррекции")
Процедура ТБКЗаписатьДанныеКоррекции(ЗаписьXML, ДанныеКоррекции, РевизияИнтерфейса = Неопределено)
...
&Вместо("ПредставлениеСтавкиНДС")
Функция ТБКПредставлениеСтавкиНДС(СтавкаНДС, РевизияИнтерфейса)
