	...
	Для каждого Строка из СписокУстройств цикл
		Если Строка.ТипОборудования = ТипККТ тогда
			Попытка	
				...
				//Марк 2026-06-04
				Если ПараметрыККТ.Свойство("P_DisconnectIfEOT") Тогда
					ПараметрыККТ.P_DisconnectIfEOT = Признак;
				Иначе
					ПараметрыККТ.Вставить("P_DisconnectIfEOT", Признак)	
				КонецЕсли;
				//КонецМарк
...
Функция УстановитьКлише(НазвениеКонтанетыКлише) экспорт
	Ответ = МенеджерОборудованияКлиентСервер.УстановитьКлише_Сервер(НазвениеКонтанетыКлише);
