...
Процедура ПоменятьПараметры(Признак = Истина)
	
	СписокУстройств = МенеджерОборудованияВызовСервера.ОборудованиеПоПараметрам();
	
	ТипККТ =  ПредопределенноеЗначение("Перечисления.ТипыПодключаемогоОборудования.ККТ");
...
Функция ПолучитьСуммуККТ() экспорт
	СуммаККТ = МенеджерОборудованияКлиентСервер.ПолучиьтСуммуККТ_Сервер();
