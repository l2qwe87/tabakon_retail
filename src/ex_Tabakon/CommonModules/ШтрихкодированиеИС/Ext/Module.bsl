...
Процедура ТБКЗаполнитьСведенияПоШтриходамДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
			...
			Если ПропуститьОбработкуСтроки Тогда
				Продолжить;
			КонецЕсли;
	...
	ДанныеПоШтрихкодамEANРезультат = ШтрихкодированиеОбщегоНазначенияИС.ТБКДанныеПоШтрихкодамEAN(
	ШтрихкодыEAN, ПараметрыСканирования, GTINПотребительскихУпаковок, Истина, МРЦ);
	...
	Если ИнформацияПоШтрихкодамEAN.Количество() Тогда
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			КонецЕсли;			
			//Марк 2026-02-16 https://jira.tabkn.ru/browse/KRO-104
			//Костыль мрц в qr указан 100 а ценник на пачке 200
			Если Строка(СтрокаДанных.номенклатура) = "Сигареты Беломор" и Строка(СтрокаДанных.Характеристика) = "100" Тогда
				СтрокаДанных.Характеристика = Справочники.ХарактеристикиНоменклатуры.НайтиПоНаименованию("200",Истина, ,СтрокаДанных.номенклатура);		
			КонецЕсли;
			//КонецМарк
