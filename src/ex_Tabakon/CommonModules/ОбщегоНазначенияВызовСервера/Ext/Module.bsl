...
Функция ЭтоОбезличеннаяМарка(Данные) Экспорт
	...
	Если Найти(Данные,"4660313541520") или Найти(Данные,"4627175411217") или Найти(Данные,"4627175411200") или Найти(Данные,"4627172718210") 
		...
		или Найти(Данные,"2000100914656") или Найти(Данные,"2000100914786")
		или Найти(Данные,"6973761110032") или Найти(Данные,"2001199790374") или Найти(Данные,"04627192382095") или Найти(Данные,"2001199790367")
...
// а также в регистре "ТБКУниверсальныеМарки". В случае неудачи поиска, функция добавляет данные штрихкода и возвращает Неопределено.
//
// Параметры:
...
Функция ПолучитьНоменклатуруИзДанныхСканирования(ДанныеСканирования, АвтоматическиПодгружатьНоменклатуру = Ложь) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |
	                      |ВЫБРАТЬ
	                      |	ТБКУниверсальныеМарки.Номенклатура
	                      |ИЗ
	                      |	РегистрСведений.ТБКУниверсальныеМарки КАК ТБКУниверсальныеМарки
	                      |ГДЕ
	                      |	ТБКУниверсальныеМарки.Марка В (&УниверсальнаяМарки, &УниверсальнаяМаркиБезКрипто)");
	...
	Запрос.УстановитьПараметр("УниверсальнаяМарки", ДанныеСканирования);
	Запрос.УстановитьПараметр("УниверсальнаяМаркиБезКрипто", Сред(ДанныеСканирования,1,СтрДлина(ДанныеСканирования)-4)); //удаление крипто хвоста
