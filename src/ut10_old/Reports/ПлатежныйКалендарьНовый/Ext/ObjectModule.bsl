...
Перем СписокВидДенежныхСредств;

Перем ВидДенежныхСредствИспользовать;
Перем ОрганизацияИспользовать ;
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	КонецЕсли;
	 ...
	 ВыводитьНеоплаченныеДокументы = Ложь;
	 ВыводитьИтоги = Истина;
	 ВыводитьНачальныеДанные = Ложь;
	 
	 СписокВидДенежныхСредств = Новый Структура;
	 СписокВидДенежныхСредств.Вставить("Безналичные" , Перечисления.ВидыДенежныхСредств.Безналичные);
	 СписокВидДенежныхСредств.Вставить("Наличные" , Перечисления.ВидыДенежныхСредств.Наличные);
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь, РапортРуководителю=Ложь) Экспорт
	...
	СтрФильтры = "";
	СтрОшибки="";
	ВидДенежныхСредствИспользовать = ЭтотОбъект.ОсновныеГруппировки[1].Использование ;
	ОрганизацияИспользовать  = ЭтотОбъект.ОсновныеГруппировки[0].Использование;
	
	Если ЭтотОбъект.ВыводитьНачальныеДанные = Ложь Тогда
		ЭтотОбъект.ВыводитьНеоплаченныеДокументы = Ложь;
	КонецЕсли;
	...
	/////  Данные по безналу выводим на конец дня,  а данные по наличным нужны на начало дня ///////////////	
	//ПостроительОстатки.Параметры.Вставить("ДатаНач",НачалоДня(ДатаНач));
	
	ГраницаПериода = Новый Граница(КонецДня(ДатаНач)+1, ВидГраницы.Исключая);
	ПостроительОстатки.Параметры.Вставить("ДатаНач",ГраницаПериода);
	...
	/////  Выводит суммы только по основным кассам
	ЗаполнитьВТаблицеСуммыПоОсновнымКассам( ТабНачальныхОстатков, НачалоДня(ДатаНач));
	///////////////////////////////////////////////////////
	...
	СтруктураПараметров.Вставить("ЗначениеГруппировки10",   Макет.ПолучитьОбласть("Значение|Строка10"));
	СтруктураПараметров.Вставить("ЗначенияПоказателя10",   Макет.ПолучитьОбласть("План|Строка10"));
	СтруктураПараметров.Вставить("ЗначениеГруппировки9",   Макет.ПолучитьОбласть("Значение|Строка9"));
	...
	// Вывод шапки На начало периода
	
	Если ЭтотОбъект.ВыводитьНачальныеДанные Тогда	
	...
	Если ЭтотОбъект.ВыводитьИтоги Тогда 
	...
	/////////// Добавляется таблица, на основании которой выводится отчет
	
	ТаблицаВывода = Новый ТаблицаЗначений ;
	ТаблицаВывода.Колонки.Добавить("Группировка");
	ТаблицаВывода.Колонки.Добавить("Начало");
	
	Для  Индекс = 1 По  мГруппировокПоГоризонтали Цикл
		НазваниеКолонки = "Колонка" + Индекс;
		ТаблицаВывода.Колонки.Добавить(НазваниеКолонки);
	КонецЦикла;
	
	ТаблицаВывода.Колонки.Добавить("Итого");
	ТаблицаВывода.Колонки.Добавить("Уровень");
	ТаблицаВывода.Колонки.Добавить("Расшифровка");
	ТаблицаВывода.Колонки.Добавить("Организация");
	ТаблицаВывода.Колонки.Добавить("ВидДенежныхСредств");
	ТаблицаВывода.Колонки.Добавить("Примечание");
	ТаблицаВывода.Колонки.Добавить("ОсновнаяГруппировка");
	///////////////////////////////////////////////////////////////////////////////////////
	...
	//////////////Обработка ТаблицыВывода и вывод отчета	
	СформироватьТаблицуВывода(ТаблицаВывода, ТаблицаОрганизаций);
	
	Если ВыводитьТаблицуНачОстатков Тогда
		ВставитьТаблицуНачальныхОстатков (ТаблицаВывода, ТабНачальныхОстатков);
	КонецЕсли;
	
	Если   Не ЭтотОбъект.ВыводитьНачальныеДанные Тогда
		УдалитьНачальныеДанные( ТаблицаВывода);	
	КонецЕсли;
	
	ДокументРезультат.НачатьАвтогруппировкуСтрок();
	
	ВывестиТаблицу(ТаблицаВывода,СтруктураПараметров );
...
Процедура РазвернутьСтроку (Выборка, СтруктураПараметров,ТабДок,Номер,ОсновнаяГруппировка,ТекущийОстаток, УровеньЗаписи, СтрокаВывода)
	
	ОбластьЗначенияПоказателя    = ?(УровеньЗаписи = 0,СтруктураПараметров.ЗначенияПоказателя10,СтруктураПараметров.ЗначенияПоказателя);
		...
		Для Каждого Имя Из СтруктураПараметров.МассивПоказатели Цикл
			...
			Если Выборка.НайтиСледующий(ЗначимаяГруппировка.Значение,ГруппировкаГоризонтальная) Тогда
				...
				Если ОсновнаяГруппировка Тогда
					...
					ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя=ТекущийОстаток;
					
					Если ЭтотОбъект.ВыводитьНеоплаченныеДокументы  Тогда
					...
					Иначе
						...
						ТекущийОстаток=ТекущийОстаток+ПланируемыеПоступления	-Заявки;
				...
				ИначеЕсли НЕ Выборка.ВидДвиженияПриходРасход=Null Тогда
					...
					Иначе
						Если Выборка.ВидДвиженияПриходРасход="1" Тогда
							ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя=ПланируемыеПоступления;
						ИначеЕсли Выборка.ВидДвиженияПриходРасход="2" Тогда
							ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя=Заявки;
						КонецЕсли;
			...
			СтрокаВывода[ИндексКолонки] =   ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя ;
...
Процедура ВывестиСтроку(Выборка, СтруктураПараметров, Номер, МассивРасшифровки, ТипЗначения)
	...
	Если  Не ЭтотОбъект.ВыводитьНеоплаченныеДокументы И (Выборка.ВидДвиженияДокПлан = "3" ИЛИ  Выборка.ВидДвиженияДокПлан = "5" ) Тогда
		Возврат;
	КонецЕсли;
	...
	ОбластьОбщийОтступ = СтруктураПараметров.ОбщийОтступ;
	ОбластьЗначениеГруппировки   = ?(УровеньЗаписи = 0,СтруктураПараметров.ЗначениеГруппировки10,СтруктураПараметров.ЗначениеГруппировки);
	ОбластьЗначенияПоказателя    = ?(УровеньЗаписи = 0,СтруктураПараметров.ЗначенияПоказателя10,СтруктураПараметров.ЗначенияПоказателя);
	...
	СтрокаВывода.Уровень = УровеньЗаписи;
	
	Если ОрганизацияИспользовать Тогда	
		СтрокаВывода.Организация = Выборка.Организация;
	КонецЕсли;		 
	
	Если ВидДенежныхСредствИспользовать	Тогда	
		СтрокаВывода.ВидДенежныхСредств = Выборка.ВидДенежныхСредств;
	КонецЕсли;
	
	Если ОсновнаяГруппировка Тогда
		
		ОбластьЗначениеГруппировки.Параметры.Расшифровка = ЗначениеРасшифровкиСтрока;
		СтрокаВывода.ОсновнаяГруппировка = Истина;
		
	Иначе
		
		ОбластьЗначениеГруппировки.Параметры.Расшифровка=ЗначениеРасшифровки;
		СтрокаВывода.ОсновнаяГруппировка = Выборка.ВидДвиженияДокПлан;
	...
	СтрокаВывода.Группировка = ОбластьЗначениеГруппировки.Параметры.ЗначениеГруппировки;
	СтрокаВывода.Расшифровка = ОбластьЗначениеГруппировки.Параметры.Расшифровка;
	...
	Для Каждого Имя Из СтруктураПараметров.МассивПоказатели Цикл
		...
		Если ОсновнаяГруппировка Тогда
			...
			ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя=НачальныйОстаток;
			...
			Если ЭтотОбъект.ВыводитьНеоплаченныеДокументы Тогда
				...
				ТекущийОстатокИтоги =  НачальныйОстаток+ПоВходящимДокументам+ПланируемыеПоступления
				-ПоИсходящимДокументам
				-Заявки;
			...
			Иначе
				Если  ЭтотОбъект.ВыводитьНачальныеДанные    Тогда
					...
					ТекущийОстаток=НачальныйОстаток+ПланируемыеПоступления -Заявки;
					ТекущийОстатокИтоги =  НачальныйОстаток+ПланируемыеПоступления -Заявки;
					
				Иначе 
					ТекущийОстаток=НачальныйОстаток;
					ТекущийОстатокИтоги =  НачальныйОстаток;
				КонецЕсли;
		...
		СтрокаВывода.Начало = ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя;
		...
		Если    УровеньЗаписи = 0 Тогда
			СтрокаВывода.Примечание = "жирный";
		КонецЕсли;	
		
		
		Если НЕ ПустаяСтрока(СтруктураПараметров.ГруппировкаГоризонтальнаяНачало) Тогда
			
			РазвернутьСтроку(Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам,СтруктураПараметров.ГруппировкаГоризонтальнаяНачало, "Все"),СтруктураПараметров,ТабДок,0,ОсновнаяГруппировка,ТекущийОстаток, УровеньЗаписи, СтрокаВывода);
		...
		Если ОсновнаяГруппировка Тогда
			...
			ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя=ТекущийОстатокИтоги;
		...
		СтрокаВывода.Итого = ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя;
		НомерКолонки=НомерКолонки+1;
...
/// Процедуры и функции для формирования и обработкт ТаблицыВывода ////
Процедура ЗаполнитьВТаблицеСуммыПоОсновнымКассам(Таблица, ДатаНач )
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ИЗ
	               |	РегистрНакопления.ДенежныеСредства.Остатки(&ДатаНач, ВидДенежныхСредств = &ВидДенежныхСредств) КАК ДенежныеСредстваОстатки
	               |ИТОГИ
	               |	СУММА(СуммаОстаток)
	               |ПО
	               |	Организация,
	...
	ТаблицаКасс =Запрос.Выполнить().Выгрузить();
	ПараметрыОтбора = Новый Структура;
	
	СписокКасс =  Новый Структура;
	СписокКасс.Вставить("ПинкасовичДВ", Справочники.Кассы.НайтиПоКоду( "000000245"));
	СписокКасс.Вставить("ПинкасовичМО", Справочники.Кассы.НайтиПоКоду( "000000247"));
	СписокКасс.Вставить("Сироткин", Справочники.Кассы.НайтиПоКоду( "000000249"));
	СписокКасс.Вставить("Трубач", Справочники.Кассы.НайтиПоКоду( "000000246"));
	СписокКасс.Вставить("Табакон", Справочники.Кассы.НайтиПоКоду( "000000068"));	
	
	Если   ВидДенежныхСредствИспользовать И ОрганизацияИспользовать Тогда
		
		Для Каждого Строка Из Таблица Цикл
			
			Если НЕ Строка.ВидДенежныхСредств = Перечисления.ВидыДенежныхСредств.Наличные Тогда 
				Продолжить;
			КонецЕсли;
			
			ПараметрыОтбора.Очистить();
			ПараметрыОтбора.Вставить("Организация", Строка.Организация);
			ПараметрыОтбора.Вставить("ВидДенежныхСредств", Перечисления.ВидыДенежныхСредств.Наличные);
			
			Если Строка.Организация = Справочники.Организации.НайтиПоНаименованию("ИП Пинкасович Д.В.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000245");
			ИначеЕсли
				Строка.Организация = Справочники.Организации.НайтиПоНаименованию("ИП Пинкасович М.О.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000247");
			ИначеЕсли
				Строка.Организация = Справочники.Организации.НайтиПоНаименованию("ИП Сироткин М.А.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000249");	
			ИначеЕсли
				Строка.Организация = Справочники.Организации.НайтиПоНаименованию("ИП Трубач А.В.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000246");	
			ИначеЕсли
				Строка.Организация = Справочники.Организации.НайтиПоНаименованию("Табакон") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000068");	
			КонецЕсли;	
			
			ПараметрыОтбора.Вставить("БанковскийСчетКасса", Касса);
			
			СтрокаКасс = ТаблицаКасс.НайтиСтроки(ПараметрыОтбора); 
			Если   СтрокаКасс.Количество() > 0 Тогда
				Строка.СуммаОстаток =  СтрокаКасс[0].СуммаОстаток;
			Иначе
				Если Строка.ВидДенежныхСредств  = Перечисления.ВидыДенежныхСредств.Наличные Тогда
					Строка.СуммаОстаток = 0;
				КонецЕсли;
			КонецЕсли;
			
			ТаблицаОрганизаций = Таблица.Скопировать(,"Организация");
			ТаблицаОрганизаций.Свернуть("Организация");
			
			Для Каждого Строка Из  ТаблицаОрганизаций Цикл
				ПараметрыОтбора.Очистить();
				ПараметрыОтбора.Вставить("Организация", Строка.Организация);
				НужныеСтроки = Таблица.НайтиСтроки(ПараметрыОтбора);
				
				Если  НужныеСтроки.Количество() = 5 Тогда 
					НужныеСтроки[0].СуммаОстаток = НужныеСтроки[1].СуммаОстаток + НужныеСтроки[3].СуммаОстаток;
				ИначеЕсли   НужныеСтроки.Количество() = 3 Тогда
					НужныеСтроки[0].СуммаОстаток = НужныеСтроки[1].СуммаОстаток;
				Иначе 
					Продолжить;
				КонецЕсли;	
			КонецЦикла;
	...
	ИначеЕсли     ОрганизацияИспользовать Тогда
		
		ТаблицаОрганизаций = ТаблицаКасс.Скопировать(,"Организация");
		ТаблицаОрганизаций.Свернуть("Организация");
		
		Для Каждого Строка Из  ТаблицаОрганизаций Цикл
			ПараметрыОтбора.Очистить();
			ПараметрыОтбора.Вставить("Организация", Строка.Организация);
			ОтборОргБезСумм = Таблица.НайтиСтроки(ПараметрыОтбора);
			
			
			
			ПараметрыОтбора.Очистить();
			ПараметрыОтбора.Вставить("Организация", ОтборОргБезСумм[0].Организация);
			
			
			ОтборОрг = Таблица.НайтиСтроки(ПараметрыОтбора);
			СуммаОрганизация = ?(ОтборОрг.Количество() > 0, ОтборОрг[0].СуммаОстаток, 0);
			
			ПараметрыОтбора.Вставить("БанковскийСчетКасса", NULL);
			
			
			Отбор = ТаблицаКасс.НайтиСтроки(ПараметрыОтбора);
			СуммаНалПоОрганизации = ?(Отбор.Количество() > 0, Отбор[0].СуммаОстаток, 0);
			
			Если ОтборОрг[0].Организация = Справочники.Организации.НайтиПоНаименованию("ИП Пинкасович Д.В.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000245");
			ИначеЕсли
				ОтборОрг[0].Организация = Справочники.Организации.НайтиПоНаименованию("ИП Пинкасович М.О.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000247");
			ИначеЕсли
				ОтборОрг[0].Организация = Справочники.Организации.НайтиПоНаименованию("ИП Сироткин М.А.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000249");	
			ИначеЕсли
				ОтборОрг[0].Организация = Справочники.Организации.НайтиПоНаименованию("ИП Трубач А.В.") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000246");	
			ИначеЕсли
				ОтборОрг[0].Организация = Справочники.Организации.НайтиПоНаименованию("Табакон") Тогда
				Касса = Справочники.Кассы.НайтиПоКоду( "000000068");	
			КонецЕсли;	
			
			
			ПараметрыОтбора.БанковскийСчетКасса =  Касса;
			Отбор = ТаблицаКасс.НайтиСтроки(ПараметрыОтбора);
			СуммаПоКассе = ?(Отбор.Количество() > 0, Отбор[0].СуммаОстаток, 0);
			
			
			СуммаОрганизация= СуммаОрганизация - СуммаНалПоОрганизации+ СуммаПоКассе;
			
			ОтборОрг[0].СуммаОстаток = СуммаОрганизация ;
			ОтборОрг[1].СуммаОстаток = СуммаОрганизация ;
			
		КонецЦикла
		
	ИначеЕсли     ВидДенежныхСредствИспользовать Тогда
		
		ПараметрыОтбора.Очистить();
		ПараметрыОтбора.Вставить("ВидДенежныхСредств", Перечисления.ВидыДенежныхСредств.Наличные);
		ОтборВид = Таблица.НайтиСтроки(ПараметрыОтбора);
		...
		Для Каждого Элемент ИЗ СписокКасс     Цикл
			
			
			ПараметрыОтбора.БанковскийСчетКасса =  Элемент.Значение;
			Отбор = ТаблицаКасс.НайтиСтроки(ПараметрыОтбора);
			СуммаПоКассе = ?(Отбор.Количество() > 0, Отбор[0].СуммаОстаток, 0);
			
			
			СуммаНалВсего= СуммаНалВсего + СуммаПоКассе;
			
		КонецЦикла;
		ОтборВид[0].СуммаОстаток= СуммаНалВсего  ;
		ОтборВид[1].СуммаОстаток= СуммаНалВсего ;
...
Функция СложитьСтроки (Массив1, Массив2)
	
	Результат = Новый Массив(Массив1.Количество());
	
	Для n=0 По (Массив1.Количество()-1) Цикл 
	
		Результат[n] =  Массив1[n] + Массив2[n] ;	
...
Функция СложитьСтрокиИтого (Массив1, Массив2, Массив3)
	
	Результат = Новый Массив(Массив1.Количество());
	
	Для n=0 По (Массив1.Количество()-1) Цикл 
		
		Результат[n] =  Массив1[n] + Массив2[n] -  Массив3[n] ;	
...
Функция ПолучитьДанныеИзСтроки (Строка)
	...
	Массив.Добавить(Строка.Начало);
	Для n=2 По 	  (мГруппировокПоГоризонтали+1) Цикл
		
		
		Массив.Добавить(Строка[n]);
	КонецЦикла;
	
	Массив.Добавить(Строка.Итого);
	
	Возврат Массив
	
КонецФункции

Процедура ПередатьДанныеВСтроку (Строка, Массив)
	
	Строка.Начало = Массив[0];
	...
	Строка.Итого = Массив[Массив.Количество()-1] ;
		
КонецПроцедуры

Процедура ВставитьТаблицуНачальныхОстатков (ТаблицаВывода, ТаблицаНачальныхОстатков)
	
	ПараметрыОтбора = Новый Структура;
	Индекс = 0;
	
	Таблица  =  ТаблицаНачальныхОстатков.Скопировать();
	
	
	Если ВидДенежныхСредствИспользовать И ОрганизацияИспользовать Тогда	
		Таблица.Свернуть("Организация, ВидДенежныхСредств, СуммаОстаток");
		
	ИначеЕсли ОрганизацияИспользовать Тогда	
		Таблица.Свернуть("Организация, СуммаОстаток");
		
	ИначеЕсли ВидДенежныхСредствИспользовать Тогда	
		Таблица.Свернуть("ВидДенежныхСредств, СуммаОстаток");
	КонецЕсли;
	...
	Для Каждого Строка Из Таблица  Цикл
		...
		КонецЕсли;
		Если ВидДенежныхСредствИспользовать Тогда
			ПараметрыОтбора.Вставить("ВидДенежныхСредств", Строка.ВидДенежныхСредств);
		КонецЕсли;
		
		Отбор = ТаблицаВывода.НайтиСтроки(ПараметрыОтбора);
		
		
		Если Отбор.Количество() = 0 Тогда
			
			НоваяСтрока = ТаблицаВывода.Вставить(Индекс);
			НоваяСтрока.Группировка = Строка.Организация.Наименование;
			Если  ВидДенежныхСредствИспользовать Тогда
				Если Строка.ВидДенежныхСредств = Null Тогда
					НоваяСтрока.Группировка = Строка.Организация.Наименование;
					НоваяСтрока.ВидДенежныхСредств = Строка.ВидДенежныхСредств;
					
					НоваяСтрока.Уровень = 0;
					НоваяСтрока.Примечание = "жирный";
				Иначе
					НоваяСтрока.Группировка = Строка(Строка.ВидДенежныхСредств);
					НоваяСтрока.ВидДенежныхСредств = Строка.ВидДенежныхСредств;
			...
			ЗаполнитьДанные( НоваяСтрока,Строка.СуммаОстаток );
			
			Индекс = Индекс +1;
			
		Иначе 
			Индекс = ТаблицаВывода.Индекс(Отбор[Отбор.Количество()-1])+1;
		КонецЕсли;
...
Процедура ЗаполнитьДанные(Строка, Сумма )
	
	Для n=1 По  (мГруппировокПоГоризонтали+2)	Цикл
		Строка[n] = Сумма;
	КонецЦикла;
	
КонецПроцедуры

Процедура СформироватьТаблицуВывода(ТаблицаВывода, ТаблицаОрганизаций)
	
	ПараметрыОтбора = Новый Структура;
	Если ОрганизацияИспользовать Тогда	
		Для Каждого Строка Из  ТаблицаОрганизаций Цикл
			Если ВидДенежныхСредствИспользовать Тогда
				Для Каждого Элемент Из СписокВидДенежныхСредств Цикл
					ПараметрыОтбора.Очистить();
					ПараметрыОтбора.Вставить("Организация", Строка.Организация);
					ПараметрыОтбора.Вставить("ВидДенежныхСредств", Элемент.Значение);
					
					ДобавлениеИтоговыхСтрокВТаблицуВывода (ТаблицаВывода, ПараметрыОтбора);
...
Процедура ДобавлениеИтоговыхСтрокВТаблицуВывода (ТаблицаВывода, ПараметрыОтбора)
	
	Отбор = ТаблицаВывода.НайтиСтроки( ПараметрыОтбора);
	
	Приход       = Новый Массив (мГруппировокПоГоризонтали +2);
	НеоплПриход  = Новый Массив (мГруппировокПоГоризонтали +2);
	Расход     = Новый Массив (мГруппировокПоГоризонтали +2);
	НеоплРасход  = Новый Массив (мГруппировокПоГоризонтали +2);
	
	Если Отбор.Количество() > 0 Тогда
		
		ОбнулитьМассив (НеоплПриход);
		ОбнулитьМассив (Приход);
		ОбнулитьМассив (Расход);
		ОбнулитьМассив (НеоплРасход);
		
		
		Для Каждого Элемент из Отбор Цикл 
			
			
			Если Элемент.Группировка = "Планируемые расходы" Тогда
				
				Индекс = ТаблицаВывода.Индекс(Элемент);
				НоваяСтрока = ТаблицаВывода.Вставить(Индекс);
				НоваяСтрока.Группировка = "Итого сумма к использованию";
				Если  ОрганизацияИспользовать Тогда
					НоваяСтрока.Организация = ПараметрыОтбора.Организация;
				КонецЕсли;
				Если  ВидДенежныхСредствИспользовать Тогда
					
					НоваяСтрока.ВидДенежныхСредств = ПараметрыОтбора.ВидДенежныхСредств;
				КонецЕсли;
				НоваяСтрока.Уровень = 1;
				НоваяСтрока.Примечание = "жирный";
				НоваяСтрока.ОсновнаяГруппировка = "7";
				Расход = ПолучитьДанныеИзСтроки(Элемент);
							
			КонецЕсли;
				
			
			Если Элемент.Группировка = "Неоплаченные входящие документы" Тогда
				
				НеоплПриход = ПолучитьДанныеИзСтроки(Элемент);	 
				
			КонецЕсли;
			
			
			Если Элемент.Группировка = "Планируемые поступления" Тогда
				
				Приход = ПолучитьДанныеИзСтроки(Элемент);	 
				
			КонецЕсли;
			
			Если Элемент.Группировка = "Неоплаченные исходящие документы" Тогда
				
				
				НеоплРасход = ПолучитьДанныеИзСтроки(Элемент);	
		...
		Если Индекс <> Неопределено  Тогда
			...
			Индекс2 = ТаблицаВывода.Индекс(Отбор[Отбор.Количество()-1]);
			НоваяСтрока = ТаблицаВывода.Вставить(Индекс2+1);
			НоваяСтрока.Группировка = "Итого планируемый остаток";
			Если  ОрганизацияИспользовать Тогда
				НоваяСтрока.Организация = ПараметрыОтбора.Организация;
			КонецЕсли;
			Если  ВидДенежныхСредствИспользовать Тогда
			...
			НоваяСтрока.Уровень = 1;
			НоваяСтрока.Примечание = "жирный"; 
			НоваяСтрока.ОсновнаяГруппировка = "8";
			
			НачальнаяСумма = ПолучитьДанныеИзСтроки(Отбор[0]);	
			
			Сумма =	СложитьСтроки(НачальнаяСумма, Приход );
			СуммаИтого =  СложитьСтрокиИтого(НачальнаяСумма, Приход,Расход);
			
			ПередатьДанныеВСтроку(ТаблицаВывода[Индекс],Сумма);
			ПередатьДанныеВСтроку(ТаблицаВывода[Индекс2+1], СуммаИтого);
		КонецЕсли;
...
Процедура ВывестиТаблицу(ТаблицаВывода,СтруктураПараметров )
	...
	ЦветУровень0 = Новый Цвет(205,197,191); // Новый Цвет(169,169,169);               
	ЦветУровень1 = Новый Цвет(238,229,222); //Новый Цвет(192,192,192);               
	ЦветУровень2 = Новый Цвет(238,233,233);
	
	Для Каждого Элемент Из ТаблицаВывода Цикл
		
		УровеньЗаписи = Элемент.Уровень;	
		ТабДок.Вывести(ОбластьОбщийОтступ, УровеньЗаписи);
		ОбластьЗначениеГруппировки   = ?(УровеньЗаписи = 0,СтруктураПараметров.ЗначениеГруппировки10,СтруктураПараметров.ЗначениеГруппировки);
		ОбластьЗначенияПоказателя    = ?(УровеньЗаписи = 0,СтруктураПараметров.ЗначенияПоказателя10,СтруктураПараметров.ЗначенияПоказателя);
		
		Если Элемент.Группировка = "Наличные" ИЛИ Элемент.Группировка = "Безналичные" Тогда
			ОбластьЗначениеГруппировки   = СтруктураПараметров.ЗначениеГруппировки9;	
		КонецЕсли;
		
		Если    УровеньЗаписи = 0 Тогда
			
			ОбластьЗначениеГруппировки.ТекущаяОбласть.ЦветФона =  ЦветУровень0 ; 	
			ОбластьЗначенияПоказателя.ТекущаяОбласть.ЦветФона =  ЦветУровень0 ;
			
		ИначеЕсли  УровеньЗаписи = 1 Тогда
			ОбластьЗначениеГруппировки.ТекущаяОбласть.ЦветФона =  ЦветУровень1 ;
			ОбластьЗначенияПоказателя.ТекущаяОбласть.ЦветФона =  ЦветУровень1 ;	
		ИначеЕсли  УровеньЗаписи = 2 Тогда
			ОбластьЗначенияПоказателя.ТекущаяОбласть.ЦветФона =  ЦветУровень2 ;
			ОбластьЗначениеГруппировки.ТекущаяОбласть.ЦветФона =  ЦветУровень2 ;
		Иначе
			
			ОбластьЗначениеГруппировки.ТекущаяОбласть.ЦветФона =  WebЦвета.Белый;
			ОбластьЗначенияПоказателя.ТекущаяОбласть.ЦветФона =  WebЦвета.Белый;
		КонецЕсли;	
		
		Если Элемент.Примечание =  "жирный"  Тогда
			ОбластьЗначениеГруппировки.ТекущаяОбласть.Шрифт=Новый Шрифт(ОбластьЗначениеГруппировки.ТекущаяОбласть.Шрифт,,,Истина);	
			ОбластьЗначенияПоказателя.ТекущаяОбласть.Шрифт=Новый Шрифт(ОбластьЗначенияПоказателя.ТекущаяОбласть.Шрифт,,,Истина);
		Иначе
			ОбластьЗначениеГруппировки.ТекущаяОбласть.Шрифт=Новый Шрифт(ОбластьЗначениеГруппировки.ТекущаяОбласть.Шрифт,,,Ложь);	
			ОбластьЗначенияПоказателя.ТекущаяОбласть.Шрифт=Новый Шрифт(ОбластьЗначенияПоказателя.ТекущаяОбласть.Шрифт,,,Ложь);
			
		КонецЕсли;
		
		ОбластьЗначениеГруппировки.Параметры.ЗначениеГруппировки= Элемент.Группировка;
		ОбластьЗначениеГруппировки.Параметры.Расшифровка= Элемент.Расшифровка;
		ОбластьЗначениеГруппировки.Область().Отступ = УровеньЗаписи;
		
		ТабДок.Присоединить(ОбластьЗначениеГруппировки);
		
		Если  ЭтотОбъект.ВыводитьНачальныеДанные Тогда
			ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя =  Элемент.Начало;
			ТабДок.Присоединить(ОбластьЗначенияПоказателя);
		КонецЕсли;
		
		Для n=2 По  (мГруппировокПоГоризонтали+1) Цикл
			
			ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя =  Элемент[n];	
		...
		Если ЭтотОбъект.ВыводитьИтоги Тогда 
			ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя =  Элемент.Итого;
			ТабДок.Присоединить(ОбластьЗначенияПоказателя);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбнулитьМассив (Массив)
	
	Для  n = 0  По (Массив.Количество() -1) Цикл
...
Процедура УдалитьНачальныеДанные(ТаблицаВывода)	
	
	ТаблицаОрг = ТаблицаВывода.Скопировать(,"Организация,ВидДенежныхСредств");
	ТаблицаОрг.Свернуть("Организация, ВидДенежныхСредств ");
	
	Для Каждого Строка Из  ТаблицаОрг Цикл
		Если Не Строка.ВидДенежныхСредств = NULL Тогда	
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Очистить();
			ПараметрыОтбора.Вставить("Организация", Строка.Организация);
			ПараметрыОтбора.Вставить("ВидДенежныхСредств", Строка.ВидДенежныхСредств);
			...
			Если Отбор.Количество() > 0 Тогда
				...
				ИтоговаяСумма = Отбор[0].Начало;
			КонецЕсли;
			
			ПроверитьСтроки(Строка,  "4", "Планируемые поступления", "Документы планирования поступления" );
			
			ПроверитьСтроки(Строка,  "6", "Планируемые расходы", "Заявки на расходование средств" );
	...
	ПараметрыОтбора.Очистить();
	ПараметрыОтбора.Вставить("Группировка", "Планируемые поступления");
	УбратьНачальныеСтроки  (ПараметрыОтбора);
	
	ПараметрыОтбора.Группировка = "Планируемые расходы";
	УбратьНачальныеСтроки  (ПараметрыОтбора);
	...
	Для Каждого Строка Из  ТаблицаОрг Цикл
		Если Не Строка.ВидДенежныхСредств = NULL Тогда
			...
			УбратьИтоговыеСтроки (ПараметрыОтбора);
...
Процедура ПроверитьСтроки(СтрокаТаблицы, ОсновнаяГруппировка, План, Док )
	ПараметрыОтбора = Новый Структура;
	
	ПараметрыОтбора.Очистить();
	ПараметрыОтбора.Вставить("Организация", СтрокаТаблицы.Организация);
	ПараметрыОтбора.Вставить("ВидДенежныхСредств", СтрокаТаблицы.ВидДенежныхСредств);
	
	ПараметрыОтбора.Вставить("ОсновнаяГруппировка", ОсновнаяГруппировка);
	Отбор = ТаблицаВывода.НайтиСтроки(ПараметрыОтбора);
	Если Отбор.Количество() > 1 Тогда
	
		Для n = 0 По (Отбор.Количество() -1) Цикл
			
			Если НЕ Отбор[n].Начало = 0 Тогда
				
				Если  Отбор[n].Итого = 0 Тогда
					
					Индекс = ТаблицаВывода.Индекс( Отбор[n]);
					ТаблицаВывода.Удалить(Индекс);
...
Процедура УбратьНачальныеСтроки (ПараметрыОтбора)
	...
	Если Отбор.Количество() > 0 Тогда
		...
		Для Каждого Элемент из Отбор Цикл
			...
			Для n =2 По (мГруппировокПоГоризонтали+2)  Цикл 
							
				Если НЕ Элемент[n] = 0 Тогда
					НадоУдалить = Ложь;
					Прервать;
				КонецЕсли;	
			КонецЦикла;
...
Процедура УбратьИтоговыеСтроки (ПараметрыОтбора)
	...
	Если Отбор.Количество() > 0 Тогда
		...
		Для Каждого Элемент из Отбор Цикл
			...
			Если  Элемент[мГруппировокПоГоризонтали+3] > 1 Тогда
		...
		Если НадоУдалить Тогда
			...
			ПараметрыОтбора.Вставить("Группировка", "Итого сумма к использованию");
			
			Отбор = ТаблицаВывода.НайтиСтроки(ПараметрыОтбора);
			Если Отбор.Количество() > 0 Тогда
				Индекс = ТаблицаВывода.Индекс( Отбор[0]);
				ТаблицаВывода.Удалить(Индекс+1);
...
Процедура ОбработкаРасшифровкиСводныхЗначений(Расшифровка) Экспорт;
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Иначе
			...
			// Настройка отчета
			Отчет = Отчеты.ПлатежныйКалендарь28.Создать();
