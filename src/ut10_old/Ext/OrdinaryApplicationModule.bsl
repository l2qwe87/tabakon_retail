...
Перем НеПерепроводитьУстановкуЦен Экспорт;
...
Процедура ПриНачалеРаботыСистемы()
	...
	ИнтеграцияЕГАИСКлиентПереопределяемый.УстановитьВидимостьИнтерфейсаЕГАИС();
...
Процедура СформироватьОтчеты()
	...
	//Пользователи.РолиДоступны("ОтчетПоОстаткам", Пользователи.ТекущийПользователь());

	Если РольДоступна("ОтчетПоОстаткам") И Час(ТекущаяДата()) < 12 Тогда
		Попытка
			Отч = Отчеты.МинусыПоОстаткам.Создать();
			Ф =	Отч.ПолучитьФорму();	
			Отч.СкомпоноватьРезультат(Ф.ЭлементыФормы.Результат, ИСТИНА);
			Ф.Открыть(); 
		Исключение
			Сообщить("Отчет по остаткам не может быть сформирован.");
		КонецПопытки;	
	КонецЕсли;
	// КЧАН
...
Процедура ПолныйДействие() Экспорт
	  ...
	  Об = Обработки.ЛК_ОбменДаннымиИнвентаризации.Создать();
	  Об.ПолучитьФорму("Форма").Открыть();
