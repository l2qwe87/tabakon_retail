...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	//Вик 2021_03_25
	Если Найти(ОбщегоНазначения.УдалениеНезначимыхСимволов(ВРег(НазначениеПлатежа)),"НДС(20%)") тогда
		Если РасшифровкаПлатежа.Количество() >0 тогда
			Если РасшифровкаПлатежа[0].СуммаНДС = 0 тогда
				РасшифровкаПлатежа[0].СуммаНДС  = (СуммаДокумента /120)*20;
				РасшифровкаПлатежа[0].СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//КонецВик
