...
Перем мТЗДокументов, мУжеВСписке;//Вик 20201102
...
//  Структура
//
Функция ПолучитьДанныеДляПечатиПеремещенияТоваров(ДопРекЧАН = Неопределено) Экспорт
...
// НЧАН	
	...
	Док = ?(ДопРекЧАН = Неопределено ИЛИ ДопРекЧАН = "ПодборЯчеекХранения", ЭтотОбъект.Ссылка, ДопРекЧАН);
...
// КЧАН
	...
	//|	ПеремещениеТоваров.Товары.(            // НЧАН
	//|		Номенклатура,
...
//	ВыборкаСтрокТовары = Шапка.Товары.Выбрать();
	...
	ЗапросН.Текст = 
		...
		|	МИНИМУМ(ЛК_СвязиЯчеекНоменклатуры.Ячейка) КАК Ячейка
		|ПОМЕСТИТЬ ВТ
		|ИЗ
		...
		|		ПО ПеремещениеТоваровТовары.Номенклатура = ВТ.Номенклатура
		|ГДЕ
	...
	ЗапросН.УстановитьПараметр("Ссылка", Док);
	Если ДопРекЧАН = "ПодборЯчеекХранения" Тогда
		ЗапросН.Текст = СтрЗаменить(ЗапросН.Текст, "ПеремещениеТоваровТовары.НомерСтроки", "Ячейка,ПеремещениеТоваровТовары.Номенклатура.Наименование");
	КонецЕсли;	
	ВыборкаСтрокТовары = ЗапросН.Выполнить().Выбрать();
...
// НЧАН
	ПЩК = ЛК_РаботаСДокументами.ОпределитьЦепочкуПоставщиков(Док);
	ПараметрыПечати.Вставить("ПредставлениеПоставщика", ПЩК);
...
// КЧАН	
	...
	// НЧАН
	ПараметрыПечати.Вставить("ДляКого", Шапка.ДляКого);
	// КЧАН
	...
	ПараметрыПечати.Вставить("ВыводитьЦены", СкладОтправитель.ВидСклада = Перечисления.ВидыСкладов.НТТ ИЛИ СкладПолучатель.ВидСклада = Перечисления.ВидыСкладов.Розничный);   // НЧАН
...
// НЧАН	
	//НоменклатураСписок = Шапка.Товары.Выгрузить().ВыгрузитьКолонку("Номенклатура");	
	...
	//	|			&МоментВремени,
	//	|			Номенклатура.Ссылка В (&НоменклатураСписок)
	//	|				И Склад.Ссылка = &Склад) КАК ЦеныАТТСрезПоследних";
	//
	//Запрос1.УстановитьПараметр("МоментВремени", МоментВремени());
	//Запрос1.УстановитьПараметр("НоменклатураСписок", НоменклатураСписок);
	//Запрос1.УстановитьПараметр("Склад", ЭтотОбъект.СкладПолучатель);
	//
	//ЦеныМагазина = Запрос1.Выполнить().Выгрузить();
...
// КЧАН
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		Если ПараметрыПечати.ВыводитьКоды Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Сокращение=СокрП(ВыборкаСтрокТовары.Товар);

		ПараметрыПозиции.Вставить("Товар", Сокращение + ФормированиеПечатныхФорм.ПредставлениеСерий(ВыборкаСтрокТовары));
        ПараметрыПозиции.Вставить("Штрихкод", ВыборкаСтрокТовары.Штрихкод);
		...
		//ОтборЦены.Вставить("Номенклатура", ВыборкаСтрокТовары.Номенклатура);  
		//ЦенаМагазинаПоТовару = ЦеныМагазина.НайтиСтроки(ОтборЦены);
		//ПараметрыПозиции.Вставить("Цена", ЦенаМагазинаПоТовару[0].Цена);
		//Цена = НаценкаНаТовар (ВыборкаСтрокТовары.Номенклатура, ЭтотОбъект.СкладПолучатель.ТипЦенРозничнойТорговли);
		//ПараметрыПозиции.Вставить("Цена", Цена);
		

		Если ДопРекЧАН = "ПодборЯчеекХранения" Тогда	
			ЯчейкаТ = ЛК_РаботаСДокументами.ПолучитьАдресаХраненияНоменклатуры(ПараметрыПозиции.Номенклатура);
			ПараметрыПозиции.Вставить("Ячейка", ЯчейкаТ);
		КонецЕсли;
		
		// КЧАН	
		//Изменил Вик 2020_10_29
		//Сумма=ВыборкаСтрокТовары.Цена*ВыборкаСтрокТовары.Количество;
		//ПараметрыПозиции.Вставить("Сумма", Сумма);
		СуммаОб	=	ВыборкаСтрокТовары.Цена*ВыборкаСтрокТовары.Количество;
		ПараметрыПозиции.Вставить("Сумма", СуммаОб);
		СуммаВсего=СуммаВсего+СуммаОб;
		...
		#КонецЕсли
		ПараметрыПозиции.Вставить("СуммаВсего", СуммаВсего);
		Позиции.Добавить(ПараметрыПозиции);
		
		СуммаКол = СуммаКол + ВыборкаСтрокТовары.Количество; // НЧАН
		
	КонецЦикла;
	
	// НЧАН
	ПараметрыПечати.Вставить("СуммаКол", СуммаКол);	
	// КЧАН
...
// НЧАН
	"	Док.Штрихкод КАК ШК,
	|	Док.ЕдиницаИзмерения КАК ЕдИзмерен," +
// КЧАН
	"	0 КАК Цена,
	|	1 КАК Количество,
	|	Док.НомерСтроки КАК НомерСтроки //Старцева
	|ИЗ
	|	Документ.ПеремещениеТоваров.Товары КАК Док
	|ГДЕ
	|	Док.Ссылка = &Док
	|	УПОРЯДОЧИТЬ ПО	//Старцева
	|	НомерСтроки     //Старцева
	|");
...
// НЧАН	
	...
	ОбработкаПечатьЦенников.ИПпоСкладу = ЭтотОбъект.СкладПолучатель.ЮрЛицо;      
...
// НЧАН	
	...
	Если ПараметрЧ = 56 Тогда
		ТД = ОбработкаПечатьЦенников.ПечатьЦенника(0);
		ТД.АвтоМасштаб = Истина;
	ИначеЕсли ПараметрЧ = 33 Тогда
		ТД = ОбработкаПечатьЦенников.ПечатьЦенника(1);
		ТД.АвтоМасштаб = Ложь;
	ИначеЕсли ПараметрЧ = 15 Тогда
		 ТД = ОбработкаПечатьЦенников.ПечатьЦенника(2); //Балашева
		ТД.АвтоМасштаб = Ложь; 
	ИначеЕсли ПараметрЧ = 16 Тогда
		
		ТД = ОбработкаПечатьЦенников.ПечатьЦенника(3);
		ТД.АвтоМасштаб = Ложь; //--	
	КонецЕсли;
...
// НЧАН
Функция ПечатьАннотация()              
	
	ТабДок = ЛК_РаботаСДокументами.ПечатьАннотации(Ссылка);
	Возврат  ТабДок
...
//
Функция ПечатьПеремещениеТоваров(ДопРекЧАН = Неопределено) Экспорт

	// НЧАН	
	ПараметрыПечати = ПолучитьДанныеДляПечатиПеремещенияТоваров(ДопРекЧАН);
	// КЧАН	
	...
	// НЧАН  Подбор ячеек хранения
	Если ДопРекЧАН = "ПодборЯчеекХранения" Тогда
		Макет       = ПолучитьМакет("НакладнаяСПодборомЯчеек");
		//СкладДляПоискаПоЯчейкам = Неопределено;
		//ВвестиЗначение(СкладДляПоискаПоЯчейкам, "Выбор склада с ячейками:" , Тип("СправочникСсылка.Склады"));
	КонецЕсли;	
	// КЧАН
	
	//++	//++штрихкодирование автоматизация склада
	НовыйШК = ЛК_РаботаСДокументами.ВывестиШКСверху(Макет, ТабДокумент, Ссылка);
	//++
	...
	ОбластьМакета.Параметры.Заполнить(ПараметрыПечати);
	//Вик 2020_10_29
	НужноеОснование = ПолучитьНужноеОснование();
	Если ЗначениеЗаполнено(НужноеОснование) тогда
		НужНомер		=	"; №" + НужноеОснование.Номер;
		НужКомментарий	=	СокрЛП(НужноеОснование.Комментарий);
	иначе
		НужНомер		= "";
		НужКомментарий  = "";
	КонецЕсли;
	ОбластьМакета.Параметры.Комментарий = НужКомментарий + НужНомер;
	//КонецВик
	...
	ОбластьШтрихкода = Макет.ПолучитьОбласть(ИмяШапки + "|Штрихкод");
	ОбластьКоличества= Макет.ПолучитьОбласть(ИмяШапки + "|Количество");
	ОбластьДанных    = Макет.ПолучитьОбласть(ИмяШапки + "|Данные");
    ОбластьСуммы     = Макет.ПолучитьОбласть(ИмяШапки + "|Сумма");
	...
	ОбластьШтрихкода = Макет.ПолучитьОбласть(ИмяСтроки + "|Штрихкод");
	ОбластьКоличества= Макет.ПолучитьОбласть(ИмяСтроки + "|Количество");
	ОбластьДанных    = Макет.ПолучитьОбласть(ИмяСтроки + "|Данные");
    ОбластьСуммы     = Макет.ПолучитьОбласть(ИмяСтроки + "|Сумма");
	...
	Для каждого ПараметрыПозиции Из ПараметрыПечати.Позиции Цикл
		...
		ОбластьШтрихкода.Параметры.Заполнить(ПараметрыПозиции);
		ТабДокумент.Присоединить(ОбластьШтрихкода);

		ОбластьКоличества.Параметры.Заполнить(ПараметрыПозиции);
		ТабДокумент.Присоединить(ОбластьКоличества);
	...
	ОбластьШтрихкода = Макет.ПолучитьОбласть("Подвал|Штрихкод");
	ОбластьКоличества= Макет.ПолучитьОбласть("Подвал|Количество");
	ОбластьДанных    = Макет.ПолучитьОбласть("Подвал|Данные");
	ОбластьСуммы     = Макет.ПолучитьОбласть("Подвал|Сумма");
	...
	ТабДокумент.Присоединить(ОбластьШтрихкода);
// НЧАН	
	ОбластьКоличества.Параметры.Заполнить(ПараметрыПечати);  
// КЧАН
	ТабДокумент.Присоединить(ОбластьКоличества);
	ТабДокумент.Присоединить(ОбластьДанных);
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "ЗаписьВRFIDМетки" Тогда
		ЗаписатьВRFIDМетки();
		Возврат;
	ИначеЕсли ИмяМакета = "Аннотация" Тогда
		ТабДокумент = ПечатьАннотация();
	ИначеЕсли ИмяМакета = "ПеремещениеТоваровЯчейки" Тогда
		ТабДокумент = ПечатьПеремещениеТоваров("ПодборЯчеекХранения");
	//Вик 20201029	
	ИначеЕсли ИмяМакета = "ЦенникиВедомость" Тогда
		ТабДокумент1  = ПечатьЦенников(16);
		ТабДокумент = ПечатьПеремещениеТоваров();
	//КонецВик
	КонецЕсли;
	...
	Если ИмяМакета = "ЦенникиВедомость" Тогда
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент1, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, ЭтотОбъект.Метаданные().Представление()));
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов.Вставить("Ценники_1_5",        "Ценники(мал)"); //Балашева  
	СтруктураМакетов.Вставить("Аннотация",          "Аннотация");       // НЧАН
	СтруктураМакетов.Вставить("ПеремещениеТоваровЯчейки",          "Перемещение товаров с поиском ячеек");       // НЧАН
    СтруктураМакетов.Вставить("ЦенникиНоменкл", "ЦенникиНоменклатуры"); //Балашева
	СтруктураМакетов.Вставить("ЦенникиВедомость", "Ценники + Ведомость");//Вик 2020_10_29 
...
Процедура ИнициализироватьНовыйДокумент(ПараметрОбъектКопирования, ПараметрОснование) Экспорт
	...
	Если ЗначениеЗаполнено(СкладПолучатель) тогда
		Если СкладПолучатель.ОрдерныйСклад  тогда
	...
	Если ЗначениеЗаполнено(СкладОтправитель) тогда
		Если СкладОтправитель.ОрдерныйСклад  тогда
			ВидОперации      = Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукцияПоОрдерам;
		КонецЕсли;
	КонецЕсли;
	//КонецВик 
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре,Отказ, Заголовок)
	...
	Если ОтражатьВУправленческомУчете Тогда
		Если ВидОперации = Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукцияПоОрдерам Тогда
			...
			// Проверка остатков при оперативном проведении.  //++-- при любом провелении контроль
			//++Если РежимПроведения = РежимПроведенияДокумента.Оперативный Тогда
				Если СкладОтправитель.Наименование <> "Неучтенка" Тогда 
		...
		Иначе // не по ордерам
			...
			// НЧАН
			Если ЭтотОбъект.Комментарий <> "Фиктивный. Перемещение комиссионных товаров." и СкладОтправитель.Наименование <> "Неучтенка" Тогда
	...
	// НЧАН 	
					...
					ТаблицаПоЦенам = УправлениеРозничнойТорговлей.СформироватьЗапросПоПродажнымЦенам(Дата, СкладПолучатель, ТаблицыДанныхДокумента["ТаблицаПоТоварам"].ВыгрузитьКолонку("Номенклатура"), СС).Выгрузить();
	// КЧАН					
					УправлениеРозничнойТорговлей.ЗаполнитьКолонкуСуммаПродажная(ТаблицыДанныхДокумента["ТаблицаПоТоварам"], ТаблицаПоЦенам);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
		...
		ДляКого            = Основание.Заказчик;
		СкладПолучатель    = Справочники.Склады.НайтиПоНаименованию("Офис");
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Подразделение                = Основание.Подразделение;
		Комментарий                  = Основание.Комментарий;
		СкладПолучатель              = Основание.ДляКого;   //ЛК Андрей
		
		ЭтотОбъект.Основание         =  Основание;          // НЧАН
		
		#Если Клиент Тогда
		Если Строка(СкладПолучатель)="" И НЕ ВвестиЗначение(СкладПолучатель, "Выберите склад получатель", Тип("СправочникСсылка.Склады")) Тогда
			СкладПолучатель = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ОсновнойСклад");
		КонецЕсли;
		#КонецЕсли
		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			...
			Если Строка(ТекСтрокаТовары.ЕдиницаИзмерения)="упак." Тогда
			НоваяСтрока.Количество                 = ТекСтрокаТовары.Количество*10;	
...
ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		Баланс = ДанныеЗаполнения.Баланс;
		ВидОперации = ДанныеЗаполнения.ВидОперации;
		ВнутреннийЗаказ = ДанныеЗаполнения.ВнутреннийЗаказ;
		ИД_77 = ДанныеЗаполнения.ИД_77;
		
		Если не Найти(ДанныеЗаполнения.Комментарий,"Ордер №") тогда //Вик 2021_02_01
			Комментарий = ДанныеЗаполнения.Комментарий;
		КонецЕсли;
		
		НДСвСтоимостиТоваров = ДанныеЗаполнения.НДСвСтоимостиТоваров;
		...
		СкладПолучатель = ДанныеЗаполнения.ДляКого;  
		Сумма = ДанныеЗаполнения.Сумма;
		УчастиевОбъединении = ДанныеЗаполнения.УчастиевОбъединении;
		ЭтотОбъект.Основание = Основание;
		
		Для Каждого ТекСтрокаВозвратнаяТара Из ДанныеЗаполнения.ВозвратнаяТара Цикл
			НоваяСтрока = ВозвратнаяТара.Добавить();
			НоваяСтрока.ВнутреннийЗаказ = ТекСтрокаВозвратнаяТара.ВнутреннийЗаказ;
			НоваяСтрока.ДокументРезерва = ТекСтрокаВозвратнаяТара.ДокументРезерва;
			НоваяСтрока.Количество = ТекСтрокаВозвратнаяТара.Количество;
			НоваяСтрока.Номенклатура = ТекСтрокаВозвратнаяТара.Номенклатура;
			//++Старцева
			//НоваяСтрока.Сосканировано = ТекСтрокаВозвратнаяТара.Сосканировано;
			НоваяСтрока.ДатаСканирования = ТекСтрокаВозвратнаяТара.ДатаСканирования;
			//--Старцева
			НоваяСтрока.УдалитьСпособСписанияОстаткаТоваров = ТекСтрокаВозвратнаяТара.УдалитьСпособСписанияОстаткаТоваров;
			НоваяСтрока.ФактическоеКоличество = ТекСтрокаВозвратнаяТара.ФактическоеКоличество;
			НоваяСтрока.Штрихкод = ТекСтрокаВозвратнаяТара.Штрихкод;
		КонецЦикла;
		Для Каждого ТекСтрокаТовары Из ДанныеЗаполнения.Товары Цикл
			НоваяСтрока = Товары.Добавить();
			НоваяСтрока.ВнутреннийЗаказ = ТекСтрокаТовары.ВнутреннийЗаказ;
			НоваяСтрока.ДокументРезерва = ТекСтрокаТовары.ДокументРезерва;
			НоваяСтрока.ЕдиницаИзмерения = ТекСтрокаТовары.ЕдиницаИзмерения;
			НоваяСтрока.ЕдиницаИзмеренияМест = ТекСтрокаТовары.ЕдиницаИзмеренияМест;
			...
			НоваяСтрока.Коэффициент = ТекСтрокаТовары.Коэффициент;
			НоваяСтрока.Номенклатура = ТекСтрокаТовары.Номенклатура;
			НоваяСтрока.ПорядокВТаблице = ТекСтрокаТовары.ПорядокВТаблице;
			НоваяСтрока.СерияНоменклатуры = ТекСтрокаТовары.СерияНоменклатуры;
			//++Старцева
			//НоваяСтрока.Сосканировано = ТекСтрокаТовары.Сосканировано;
			НоваяСтрока.ДатаСканирования = ТекСтрокаТовары.ДатаСканирования;
			//--Старцева
			НоваяСтрока.Сумма = ТекСтрокаТовары.Сумма;
			НоваяСтрока.УдалитьСпособСписанияОстаткаТоваров = ТекСтрокаТовары.УдалитьСпособСписанияОстаткаТоваров;
			НоваяСтрока.ФактическоеКоличество = ТекСтрокаТовары.ФактическоеКоличество;
			НоваяСтрока.ХарактеристикаНоменклатуры = ТекСтрокаТовары.ХарактеристикаНоменклатуры;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// НЧАН 22.02.2019
	// Отмена записи при получении Дока из ГР по условию.
	...
	Если ПолОбмен И Ссылка <> документы.ПеремещениеТоваров.ПустаяСсылка() И ЛК_РаботаСДокументами.ЗапретаОбменаПоДате(Ссылка.Дата) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;	
	// КЧАН
    ...
    // Запрет изменения Перемещение товаров  с проектом Проверено (продажи). Добавлено 01.11.19 Костин
	
	РазрешеноИзменение = ЛК_РаботаСДокументами.ПроверитьПравоПользователя("Разрешить изменение Перемещение товаров  с проектом Проверено (продажи)");
	
	Если  ( Не РазрешеноИзменение Или Не РольДоступна("ПолныеПрава")) И Проект = Справочники.Проект.НайтиПоНаименованию("Проверено (продажи)") Тогда
	...
	Если  ( Не РазрешеноИзменение Или Не РольДоступна("ПолныеПрава")) И Проект = Справочники.Проект.НайтиПоНаименованию("Проверено (продажи)") И ЭтотОбъект.Модифицированность() = Ложь Тогда
		Сообщить("Проведение запрещено!");
		Отказ = Истина;
		Возврат;
	КонецЕсли; 
	...
	// НЧАН
	//Если Принята Тогда
	//	Проект = Справочники.Проект.НайтиПоКоду("000000034");
	//	ЛК_РаботаСДокументами.УстановитьПринятиеРозницей(Ссылка);
	//КонецЕсли;
	// КЧАН
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++
	если СозданВРознице и не ЭтотОбъект.Принята Тогда
		#если клиент тогда
			предупреждение("документ не сделал движений, т.к. не принят (галочка внизу формы)");	
		#конецЕсли	
		возврат;
	КонецЕсли;			
	
// НЧАН 
	Если ВидОперации <> Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукцияПоОрдерам тогда //Вик 2020_11_26
		ЛК_РаботаСДокументами.СоздатьПереоценкуПоПеремещению(ЭтотОбъект.Ссылка);
	КонецЕсли;
// КЧАН
...
//++ ГК
Процедура СоздатьДокументПереоценки()
	
	Возврат; //++  РАСКОММЕНТИТЬ!
	
	
	//Запрос, вдруг уже есть документ переоценки, связанный с этим перемещением!
	...
	ДокументПереоценки = 0;
	
	Если ТЗ.Количество()= 0 Тогда
		
		ДокументПереоценки = Документы.ПереоценкаТоваровВРознице.СоздатьДокумент();
		//Сообщить("Создан новый документ переоценки");
		ДокументПереоценки.Дата = ТекущаяДата();
		
	Иначе ДП = ТЗ[0].ДокументПереоценки;
		
		ДокументПереоценки = ДП.ПолучитьОбъект();
		//Сообщить("Будет перепроведен документ переоценки: "+ ДокументПереоценки.Ссылка);
		ДокументПереоценки.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		ДокументПереоценки.Товары.Очистить();
		
	КонецЕсли;
	
	ДокументПереоценки.ДокументОснование = ЭтотОбъект.Ссылка;
	ДокументПереоценки.Организация   = Организация;
	ДокументПереоценки.Склад         = СкладПолучатель;
	ДокументПереоценки.Подразделение = Подразделение;
	ДокументПереоценки.Ответственный = Ответственный;
	...
	Для каждого Строчка из Товары Цикл
		...
		//Запрос новой цены (просто срез последних)	
		ЗапросНовойЦены = Новый Запрос;
		ЗапросНовойЦены.Текст = 
		"ВЫБРАТЬ
		|	ЦеныНоменклатурыСрезПоследних.Номенклатура,
		|	ЦеныНоменклатурыСрезПоследних.Цена КАК НоваяЦена,
		|	ЦеныНоменклатурыСрезПоследних.Регистратор.Дата КАК РегДата
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних КАК ЦеныНоменклатурыСрезПоследних
		|ГДЕ
		|	ЦеныНоменклатурыСрезПоследних.Номенклатура = &Номенклатура";
		
		ЗапросНовойЦены.УстановитьПараметр("Номенклатура", Строчка.Номенклатура);
		
		РЗ = ЗапросНовойЦены.Выполнить();
		
		//ТЗ = ЗапросНовойЦены.Выполнить().Выгрузить();
		
		ВыборкаНоваяЦена = РЗ.Выбрать();
		
		Граница = ТекущаяДата(); //++
		
		Если ВыборкаНоваяЦена.Следующий() Тогда
			// Вставить обработку выборки ВыборкаДетальныеЗаписи
			Граница = Новый Граница(НачалоДня(ВыборкаНоваяЦена.РегДата) - 1, ВидГраницы.Исключая);
			Стр = ДокументПереоценки.Товары.Добавить();
		Иначе
			Продолжить;
		КонецЕсли;
		//Запрос новой цены
		
		//Запрос старой цены - срез "предпоследних"
		
		ЗапросСтаройЦены = Новый Запрос;
		ЗапросСтаройЦены.Текст = 
		"ВЫБРАТЬ
		|	ЦеныНоменклатурыСрезПоследних.Номенклатура,
		...
		ЗапросСтаройЦены.УстановитьПараметр("Дата", Граница);
		ЗапросСтаройЦены.УстановитьПараметр("Номенклатура", Строчка.Номенклатура);
		
		
		РезультатЗапроса = ЗапросСтаройЦены.Выполнить();
		//ТЗ = РезультатЗапроса.Выгрузить();
		ВыборкаСтараяЦена = РезультатЗапроса.Выбрать();
		
		Если ВыборкаСтараяЦена.Следующий() Тогда
			Стр.ЦенаВРозницеСтарая = ВыборкаСтараяЦена.СтараяЦена; //++
		КонецЕсли;
		
		//ТЗ = РезультатЗапроса.Выгрузить();
		
		Стр.Номенклатура = Строчка.Номенклатура;
		Стр.ХарактеристикаНоменклатуры = Строчка.ХарактеристикаНоменклатуры;
		Стр.СерияНоменклатуры  = СТрочка.СерияНоменклатуры;
		Стр.Количество         = Строчка.Количество;
		
		//Стр.ЦенаВРознице       = ВыборкаНоваяЦена.НоваяЦена;
		//Стр.ЦенаВРозницеСтарая = ВыборкаСтараяЦена.СтараяЦена;
		
	КонецЦикла;
	
	если ДокументПереоценки.товары.Количество() > 0 Тогда
		Попытка
...
Функция НаценкаНаТовар (НоменклатураСсылка, ТипЦен ) Экспорт
	
Запрос = Новый Запрос;
Запрос.Текст = "ВЫБРАТЬ
               |	НаценкиНаНоменклатурныеГруппы.ЦеноваяКатегория,
               |	МАКСИМУМ(НаценкиНаНоменклатурныеГруппы.ПроцентНаценки) КАК ПроцентНаценки,
               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НаценкиНаНоменклатурныеГруппы.НоменклатурнаяГруппа) КАК НоменклатурнаяГруппа
               |ИЗ
               ...
               |	И НаценкиНаНоменклатурныеГруппы.НоменклатурнаяГруппа = &НоменклатурнаяГруппа
               |
               |СГРУППИРОВАТЬ ПО
               |	НаценкиНаНоменклатурныеГруппы.ЦеноваяКатегория";
			   
			Запрос.УстановитьПараметр("ЦеноваяКатегория", ТипЦен.ЦеноваяКатегория);	
			Запрос.УстановитьПараметр("НоменклатурнаяГруппа", НоменклатураСсылка.НоменклатурнаяГруппа);
		...
		Если Результат.Следующий() тогда
			...
			Цена = Об.РасчетЦеныПоЦеновымГруппам (НоменклатураСсылка, ТипЦен, Результат.ПроцентНаценки);
...
//Вик20201102
Функция ПолучитьНужноеОснование()
	мТЗДокументов = Новый ТаблицаЗначений;
	мТЗДокументов.Колонки.Добавить("Ссылка"); 
	...
	Если ЗначениеЗаполнено(Основание) тогда
		...
		иначе
			...
			Попытка
				ВывестиРодительскиеДокументы(Ссылка); //Взято из формы Структуры подчиненности
				Для каждого Строка ИЗ мТЗДокументов цикл
					Если ТипЗнч(Строка.Ссылка) = ТИП("ДокументСсылка.ПеремещениеТоваров") и Строка.Ссылка <> Ссылка тогда
...
Процедура ВывестиРодительскиеДокументы(ДокументСсылка)
	...
	СтрокаДерева = мТЗДокументов.Добавить();
...
Процедура СоздатьПерезаполнитьРО() экспорт
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	РасходныйОрдерНаТовары.Ссылка
	      	 	             |ИЗ
	      	 	             |	Документ.РасходныйОрдерНаТовары КАК РасходныйОрдерНаТовары
	      	 	             |ГДЕ
	      	 	             |	РасходныйОрдерНаТовары.ДокументПередачи.Ссылка = &Ссылка");
	Запрос.УстановитьПараметр("Ссылка",Ссылка);
	Рез = Запрос.Выполнить().Выгрузить();
	Если Рез.Количество() = 1 тогда
		ДокРО = Рез[0].Ссылка.ПолучитьОбъект();
		Если ДокРО.ПометкаУдаления тогда
			ДокРО.УстановитьПометкуУдаления(Ложь);
		КонецЕсли;
	иначе
		ДокРО = Документы.РасходныйОрдерНаТовары.СоздатьДокумент();
	КонецЕсли;
	
	ДокРО.Дата = Ссылка.Дата + 1;
	
	ДокРО.Заполнить(Ссылка);
	Попытка
		Если ДокРО.Товары.Количество() >0 тогда
			ДокРО.Записать(РежимЗаписиДокумента.Проведение);
			Сообщить("Проведен документ: "+ДокРО.Ссылка);
		КонецЕсли;
	Исключение
...
Процедура СоздатьПерезаполнитьПО(НовыйДок = Неопределено) экспорт
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	...
	Рез = Запрос.Выполнить().Выгрузить();
	Если Рез.Количество() = 1 тогда
		ДокПО = Рез[0].Ссылка.ПолучитьОбъект();
		Если ДокПО.ПометкаУдаления тогда
			ДокПО.УстановитьПометкуУдаления(Ложь);
		КонецЕсли;
	иначе
	...
	иначе
		ДокПО.Дата = НовыйДок.дата + 2;
	КонецЕсли;

	
	ДокПО.Заполнить(Ссылка);
	Попытка
		Если ДокПО.Товары.Количество() >0 тогда
			ДокПО.Записать(РежимЗаписиДокумента.Проведение);
			Сообщить("Проведен документ: "+ДокПО.Ссылка);
		КонецЕсли;
	Исключение
		Сообщить(ОписаниеОшибки());
	КонецПопытки;
