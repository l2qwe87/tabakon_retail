...
Функция ПолучитьПараметрыПечатиСчетаЗаказа(Тип) Экспорт	
	...
	ИначеЕсли ДопКолонка = Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Код Тогда
		...
		Колонка = "Код";
	КонецЕсли;
	ПараметрыПечати.ВыводитьКоды = Ложь; //Вик 2020_11_24 Константу отключать нельзя. артикула требуются для печати чего-то по Мегаполису.
...
//
Функция СоздатьТабличныйДокументПечатиСчетаЗаказа(Тип, ПараметрыПечати,ЭтоОпт = ложь)
	...
	ОбластьМакета.Параметры.Заполнить(ПараметрыПечати);
	Если ЭтоОпт тогда
		ОбластьМакета.Параметры.ПредставлениеПоставщика	=	"Основной ПДВ";
	КонецЕсли;
	ТабДокумент.Вывести(ОбластьМакета);
	...
	//Вик 2020_09_29
	Если ЭтоОпт тогда
		ОбластьМакета = Макет.ПолучитьОбласть("ИмяДоговора");
		ОбластьМакета.Параметры.НаименованиеДоговора	=	ДоговорКонтрагента.Наименование;
		ТабДокумент.Вывести(ОбластьМакета); 
	КонецЕсли;    
	
	Если не ЭтоОпт тогда
		ОбластьМакета = Макет.ПолучитьОбласть("ДопИнфо");
		ОбластьМакета.Параметры.Склад 			= СкладГруппа.Наименование;										
		ОбластьМакета.Параметры.НомерНаСайте 	= СайтНомер;
		ОбластьМакета.Параметры.Адрес 			= СайтАдресДоставки;
		ОбластьМакета.Параметры.ТипОплаты 		= СайтТипОплаты;
		ОбластьМакета.Параметры.СтатусЗаказа 	= СайтСтатусЗаказа;
		ОбластьМакета.Параметры.ТипДоставки 	= СайтТипДоставки;
		ОбластьМакета.Параметры.Информация	 	= СайтДопИнфо;
		Если Оплачен тогда
			ОбластьМакета.Параметры.СтатусОплаты 	= "Оплачен";
		иначе
			ОбластьМакета.Параметры.СтатусОплаты 	= "Не оплачен";
		КонецЕсли;	
		ТабДокумент.Вывести(ОбластьМакета);
	КонецЕсли;
	//КонецВИк  
...
//  Табличный документ - сформированная печатная форма
//
Функция ПечатьСчетаЗаказа(Тип, ЭтоОпт = Ложь)

	Возврат СоздатьТабличныйДокументПечатиСчетаЗаказа(Тип, ПолучитьПараметрыПечатиСчетаЗаказа(Тип),ЭтоОпт);
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "ЗаказОпт" тогда
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураПечатныхФорм = Новый Структура("Заказ,ЗаказКорректировка,Счет,Бланк,ЗаказОпт",
	                                        "Заказ покупателя",
	                                        "Заказ покупателя (с учетом корректировок)",
	                                        "Счет на оплату (с учетом корректировок)",
	                                        "Бланк товарного наполнения",
											"Заказ покупателя (опт)");
...
Функция ЗаполнитьТабличнуюЧастьИзПодбора(ТабличнаяЧасть, ЗначениеВыбора) Экспорт
	...
	Если ТабличнаяЧасть = Товары Тогда
			...
			//ЛК Андрей =================
			Если Строка(ЕдиницаИзмерения) <> "шт." Тогда 
			Зап = Новый Запрос;
			Зап.УстановитьПараметр("Товар", Номенклатура);
			Зап.УстановитьПараметр("ЕдИзмерения", ЕдиницаИзмерения);
			Зап.Текст = "ВЫБРАТЬ
			            |	Штрихкоды.ЕдиницаИзмерения.Коэффициент КАК Коэффициент
			            |ИЗ
			            |	РегистрСведений.Штрихкоды КАК Штрихкоды
			            |ГДЕ
			            |	Штрихкоды.ЕдиницаИзмерения.Ссылка = &ЕдИзмерения
			            |	И Штрихкоды.Владелец.Ссылка = &Товар";
			
			Рез = Зап.Выполнить();
			Выбор = Рез.Выбрать();
			Если Выбор.Следующий() Тогда
				Числ = Выбор.Коэффициент;
			КонецЕсли;
			Зап = Новый Запрос;
			Зап.УстановитьПараметр("Товар", Номенклатура);
			Зап.Текст = "ВЫБРАТЬ
			            |	Штрихкоды.ЕдиницаИзмерения.Ссылка КАК ЕдиницаИзмерения
			            |ИЗ
			            |	РегистрСведений.Штрихкоды КАК Штрихкоды
			            |ГДЕ
			            |	Штрихкоды.ЕдиницаИзмерения.Наименование = ""шт.""
			            |	И Штрихкоды.Владелец.Ссылка = &Товар";
			
			Рез = Зап.Выполнить();
			Выбор = Рез.Выбрать();
			Если Выбор.Следующий() Тогда
				ЕдиницаИзмерения=Выбор.ЕдиницаИзмерения;
			КонецЕсли;
		Иначе
			Числ = 1;
		КонецЕсли;
		
		Если Числ=Неопределено Тогда
			Числ=1;
		КонецЕсли;
		
		//СтруктураОтбора.Вставить("ЕдиницаИзмерения", ЕдиницаИзмерения);  
        СтруктураОтбора.Вставить("Номенклатура", Номенклатура);        //ЛК Андрей. Иначе структура не работает.
		Если Номенклатура.ВестиУчетПоХарактеристикам Тогда
			П = ЛК_РаботаСДокументами.ПолучитьСвеженькуюХарактеристику(Номенклатура);
			Характеристика = ?(П <> Неопределено, П, Характеристика);
	...
	Если СтрокаТабличнойЧасти <> Неопределено Тогда
		...
		Если ТабличнаяЧасть = Товары Тогда
			...
			Если не Сформирован тогда // Вик 2020_12_01
				СуммаСтрокиДоИзменения = СтрокаТабличнойЧасти.Цена * СтрокаТабличнойЧасти.Количество;

				// Нашли, увеличиваем количество в первой найденной строке.
				СтрокаТабличнойЧасти.Количество = СтрокаТабличнойЧасти.Количество + (Количество*Числ);
			...
			//Вик 2020_12_01	
			иначе
				СтрокаТабличнойЧасти.ФактическоеКоличество	 = СтрокаТабличнойЧасти.ФактическоеКоличество + (Количество*Числ);
				СтрокаТабличнойЧасти.ДатаСканирования		 = ТекущаяДата();
			КонецЕсли;
			//КонецВик
		...
		КонецЕсли;
	//Изменил Вик 2020_12_01
	//Иначе
	ИначеЕсли не Сформирован тогда
	//КонецВик
