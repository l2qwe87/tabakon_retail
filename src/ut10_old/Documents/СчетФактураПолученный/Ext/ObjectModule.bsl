...
Функция ПечатьСчетаФактуры1137(ДанныеДляПечати)
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		//Вик 2021_01_18
		Если ЗначениеЗаполнено(ДокументОснование.НомерГТД) тогда
			ОбластьМакета.Параметры.ПредставлениеСтраны   	 = ДокументОснование.Страна;
			ОбластьМакета.Параметры.СтранаПроисхожденияКод   = ДокументОснование.Страна.Код;
			ОбластьМакета.Параметры.ПредставлениеГТД       	 = ДокументОснование.НомерГТД;		
		КонецЕсли;
		//КонецВик
...
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)
	//ОбщегоНазначения.ДобавитьПрефиксУзла(Префикс);//Коммент Вик 2021_01_19 т.к. срабатывает также подписка "ПриУстановкеНовогоНомераДокумента"
КонецПроцедуры
