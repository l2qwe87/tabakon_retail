...
Процедура ПодписатьДокумент(Команда)
	...
	Если ПроверитьЗаполнение() И (ЗначениеЗаполнено(ЭлектронныйДокументСсылка)
		...
		Если ДокументЗаписан() И ПроверитьЗахватНаРедактирование() Тогда
			ПодписатьИОтправитьДокумент(Ложь);
		КонецЕсли;
	КонецЕсли;
...
Процедура Отклонить(Команда)
	...
	Если ДокументЗаписан() И НЕ ЭДОтклонен И ЗначениеЗаполнено(ЭлектронныйДокументСсылка) И МожноОтклонитьДокумент() Тогда
		ПараметрыЭД = Новый Структура("Организация, Отклонить", Объект.Организация, Истина);
...
Функция МожноОтклонитьДокумент()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий
		И НЕ Объект.СтатусДокумента = Перечисления.СтатусыЭД.ПодготовленКОтправке Тогда
		МожноОтклонить = Ложь;
	КонецЕсли;
