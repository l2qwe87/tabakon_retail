...
// Выгружает возврат в транспортный модуль ЕГАИС
//
// Параметры:
//  ФР - Строка                  - идентификатор фискального регистратора
//  ОтображатьСообщения - Булево - признак отображения сообщений об ошибках пользователю
//
// Возвращаемое значение:
//  Структура - результат выгрузки
//
Функция ВыгрузитьДокументВЕГАИС(ФР = Неопределено, ОтображатьСообщения = Истина) Экспорт
	
	ТранспортныйМодуль = Неопределено;
	ПечатьСлипЧека = Истина;
	
	ПараметрыЧека = ПодготовитьПараметрыЧекаДляПередачиВЕГАИС(ТранспортныйМодуль, ФР, ПечатьСлипЧека);
	
	Если ТранспортныйМодуль = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось определить транспортный модуль.'");
	...
	РезультатОперации = ИнтеграцияЕГАИСКлиент.СформироватьПодписьЧекаВЕГАИС(ПараметрыЧека, ТранспортныйМодуль,, ПечатьСлипЧека, ФР, ОтображатьСообщения);
	
	Если РезультатОперации.Результат Тогда
		АдресЧекаЕГАИС = РезультатОперации.Адрес;
		ПодписьЧекаЕГАИС = РезультатОперации.Подпись;
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ФУНКЦИИ ВЗАИМОДЕЙСТВИЯ С СИСТЕМОЙ ЕГАИС

#Если Клиент Тогда
	
// Функция подготовливает параметры возвращаемого чека.
//
// Параметры:
...
//
// Возвращаемое значение:
//  <Массив> - параметры чека
//
Функция ПодготовитьПараметрыЧекаДляПередачиВЕГАИС(ТранспортныйМодуль = Неопределено, ФР = Неопределено, ПечатьСлипЧека = Истина)
	...
	ТранспортныйМодуль = ИнтеграцияЕГАИСПереопределяемый.ИспользуемыйТранспортныйМодуль(Шапка.Склад, Шапка.Организация);
	
	РезультатФР = ВыбратьФискальныйРегистратор(ФР);
...
Функция ВыбратьФискальныйРегистратор(ФР)
	...
	Если ФР = Неопределено Тогда
		...
		Если КоличествоФР = 0 Тогда
			Сообщить(НСтр("ru = 'Чек не пробит, т.к. не подключен фискальный регистратор!'"));
			Возврат Ложь;
