...
Функция ПечатьСчетаФактуры1137(ДанныеДляПечати)
	...
	ОбластьМакета.Параметры.Валюта = "Валюта: наименование, код "
		...
		СокрЛП(ДанныеДляПечати.Валюта.НаименованиеПолное) + ", " + СокрЛП(ДанныеДляПечати.Валюта.Код), 
		"");		
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		//Вик 2021_01_18
		СтруктураГТДСтрана = ПолучитьГТДСтрана(Строчка.Товар);
		
		Если СтруктураГТДСтрана <> Неопределено тогда
			ОбластьМакета.Параметры.СтранаПроисхожденияКод = СтруктураГТДСтрана.Страна.Код;
			ОбластьМакета.Параметры.ПредставлениеСтраны    = СтруктураГТДСтрана.Страна;
			ОбластьМакета.Параметры.ПредставлениеГТД       = СтруктураГТДСтрана.НомерГТД;  
		КонецЕсли;
		//КонецВик
...
//Вик 20210118
Функция ПолучитьГТДСтрана(Номенклатура)
	Запрос	=	Новый запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	 	             |	ПоступлениеТоваровУслугТовары.Ссылка,
	      	 	             |	ПоступлениеТоваровУслугТовары.Ссылка.НомерГТД,
	      	 	             |	ПоступлениеТоваровУслугТовары.Ссылка.Страна
	      	 	             |ИЗ
	      	 	             |	Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
	      	 	             |ГДЕ
	      	 	             |	ПоступлениеТоваровУслугТовары.Ссылка.Проведен
	      	 	             |	И ПоступлениеТоваровУслугТовары.Ссылка.НомерГТД <> &НомерГТД
	...
	Если Рез.Количество() = 1 тогда
		СтруктураВозврата.Вставить("Страна",Рез[0].Страна);
		СтруктураВозврата.Вставить("НомерГТД",Рез[0].НомерГТД);
