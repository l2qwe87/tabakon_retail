...
Функция ПечатьЗаявки() Экспорт
	...
	Макет = ПолучитьМакет("ЗаявкаНаРасходованиеСредств_ЛК"); // НЧАН
	...
	Обл.Параметры.БанковскийСчетКасса = БанковскийСчетКасса;	
	// КЧАН
	...
	Если ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам Тогда
		...
		Обл.Параметры.Сделка = ?(ВидОперации=Перечисления.ВидыОперацийЗаявкиНаРасходование.ОплатаПоставщику, "Статья ДДС", "Сделка"); // НЧАН
		...
		Пока Результат.Следующий() Цикл
			...
			// НЧАН
			Если ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ОплатаПоставщику И РасшифровкаПлатежа.Количество() <> 0 Тогда
				Обл.Параметры.Сделка = Результат.СтатьяДвиженияДенежныхСредств;
			КонецЕсли;
			// КЧАН
	...
	ИначеЕсли ВидОперации=Перечисления.ВидыОперацийЗаявкиНаРасходование.ВыдачаДенежныхСредствПодотчетнику Тогда
		...
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |	ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
		             |	ВЫБОР
		             |		КОГДА НЕ ВзаиморасчетыСПодотчетнымиЛицамиОстатки.СуммаВзаиморасчетовОстаток ЕСТЬ NULL
		             ...
		             |		ИНАЧЕ ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СуммаВзаиморасчетов
		             |	КОНЕЦ КАК ТекущийДолг,
		             |	ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств КАК ЛК_СДДС
		             |ИЗ
		             |	Документ.ЗаявкаНаРасходованиеСредств.РасшифровкаПлатежа КАК ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа
	...
	// НЧАН
	Если ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ПрочийРасходДенежныхСредств И РасшифровкаПлатежа.Количество() <> 0 Тогда
		Обл=Макет.ПолучитьОбласть("ДопСтатьяДДС");
		Обл.Параметры.ЛК_СДДС = РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;
		ТабДокумент.Вывести(Обл);
	КонецЕсли;
	// КЧАН
...
Процедура ПроверитьЗаполнениеТЧ(Отказ, Заголовок)
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		///  Отменена проверка заполнения ячейки Договор по просьбе Емельяновой Ирины 30.11.18	
		  Если Не Платеж.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка()  Тогда
		  ...
		  КонецЕсли;	
			...
			КонецЕсли;
			 ...
			 ///  Отменена проверка заполнения ячейки Организация при незаполненной ячейки Договор по просьбе Емельяновой Ирины 30.11.18
			 Если Не Платеж.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка()  Тогда
...
Процедура ДвиженияПоРегистрам(СтруктураДанныхШапки, Отказ, Заголовок, СтруктураКурсыВалют)
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		//////  Устанавливает данные, чтобы можно было проводить документ без заполнения ДоговораКонтрагента по просьбе Емельяновой Ирины 30.11.18 
		Если Платеж.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка() Тогда
		СтруктураКурсВзаиморасчетов.Курс = 1;	
		СтруктураКурсВзаиморасчетов.Кратность = 1;
	    КонецЕсли;
	    ////////////////////////////////////////////////////////////////////////////////
		
		// НЧАН
		Если ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ВыплатаСотруднику Тогда
			СтруктураКурсВзаиморасчетов.Курс = 0;	
			СтруктураКурсВзаиморасчетов.Кратность = 0;
		КонецЕсли;	
		// КЧАН	
		...
		// НЧАН
		Если ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ВыплатаСотруднику Тогда
			СтрокаДвиженийЗаявки.СуммаУпр = СтрокаДвиженийЗаявки.Сумма;
