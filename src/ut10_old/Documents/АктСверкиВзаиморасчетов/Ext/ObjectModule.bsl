...
Процедура ЗаполнитьПоДаннымУправленческогоУчета(СОтчетами) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаСамогоНачала",   Дата("20100101"));
	...
	|ИЗ 
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания, Регистратор, ," + ТекстФильтра + ") КАК Взаиморасчеты
	|ГДЕ
	|	Взаиморасчеты.СуммаВзаиморасчетовПриход <> Взаиморасчеты.СуммаВзаиморасчетовРасход"
	+ ?(НЕ СОтчетами, "", "") +"";        //ЛК Андрей. Добавлен фильтр по Регистратору, чтобы этот и нижний запрос не выводили разные данные.
	...
	//ЛК Андрей. По словам Татьяны, неверно подбирается начальный остаток и как следствие - остаток на конец. 
	//С эти запросом всё получается верно.
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Взаиморасчеты.Регистратор,
	|	Взаиморасчеты.Период КАК Дата,
	...
	|	КОНЕЦ КАК Кредит
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Обороты(&ДатаСамогоНачала, &ДатаНачала, Регистратор," + ТекстФильтра + ") КАК Взаиморасчеты
	|ГДЕ
	|	Взаиморасчеты.СуммаВзаиморасчетовПриход <> Взаиморасчеты.СуммаВзаиморасчетовРасход"
	+ ?(НЕ СОтчетами, "", "") +
    ...
    ИтогКредит = ТаблицаРезультата.Итог("Кредит");
	ИтогДебет = ТаблицаРезультата.Итог("Дебет");
    ОстатокНаНачало = ИтогДебет-ИтогКредит; 	
