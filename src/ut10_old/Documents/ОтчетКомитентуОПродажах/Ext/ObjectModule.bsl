...
#Если Клиент Тогда
//++ГК	
Функция ПечатьОтчетаЛК(Подробно = Ложь)	
	...
	|	ОтчетКомитентуОПродажах.ДоговорКонтрагента.Наименование КАК НаименованиеДоговора
	|ИЗ
	...
	Макет       = ПолучитьМакет("ОтчетКомитентуОПродажахЛК");
	...
	Запрос.Текст = 
		...
		|	ОтчетКомитентуОПродажахТовары.ЕдиницаИзмерения КАК ЕдиницаХранения,
		|	ОтчетКомитентуОПродажахТовары.ДокументПродажи.Склад КАК Реализатор,
		|	ОтчетКомитентуОПродажахТовары.СуммаПоступления Как СуммаСебестоимость
		|ИЗ
		|	Документ.ОтчетКомитентуОПродажах.Товары КАК ОтчетКомитентуОПродажахТовары
		|ГДЕ
	...
	Иначе
		...
		ОбластьШапкаТаблицы = Макет.ПолучитьОбласть("ШапкаТаблицыКратко");
		ТабДокумент.Вывести(ОбластьШапкаТаблицы);
		
		НомерСтроки = 1;
		ТЗ.Свернуть("Товар, ЕдиницаХранения", "Количество, СуммаСебестоимость");
		
		Для каждого стр из ТЗ цикл
			
			ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаКратко");
			ОбластьСтрока.Параметры.НомерСтроки        = НомерСтроки;
			ОбластьСтрока.Параметры.Заполнить(стр);
			ТабДокумент.Вывести(ОбластьСтрока);
			НомерСтроки = НомерСтроки + 1;
	...
	Итого = ТЗ.Итог("СуммаСебестоимость");
	ОбластьПодвал.Параметры.ВсегоЧислом = Формат(Итого, "ЧДЦ=2");
	ОбластьПодвал.Параметры.ВсегоСтрок = НомерСтроки - 1;
	ОбластьПодвал.Параметры.ВсегоПрописью = ОбщегоНазначения.СформироватьСуммуПрописью(ТЗ.Итог("СуммаСебестоимость"), Ссылка.ВалютаДокумента); 
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	//++ГК	
	ИначеЕсли ИмяМакета = "ОтчетКомитентуОПродажахСРеализатором" Тогда
		ТабДокумент = ПечатьОтчетаЛК(Истина);


	ИначеЕсли ИмяМакета = "ОтчетКомитентуОПродажахКраткий" Тогда
		ТабДокумент = ПечатьОтчетаЛК();
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	//++ГК
	СтруктураМакетов.Вставить("ОтчетКомитентуОПродажахСРеализатором", "Отчет комитенту с Реализатором");
	СтруктураМакетов.Вставить("ОтчетКомитентуОПродажахКраткий", "Отчет комитенту краткий");
	//--ГК
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам,
	...
	Если ОтражатьВУправленческомУчете Тогда
		...
		//Лк Андрей. ВзаиморасчетыСКонтрагентами. 
		//Данное комментирование выполнено из-за необходимости корректно отображать данные. 
		//В случае проблем, добавить ВзаиморасчетыСКонтрагентами в движения и раскомментить отсюда и до //====		
