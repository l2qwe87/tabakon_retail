...
Функция ПечатьПоступлениеТоваров()
	...
	Товар=ВыборкаСтрокТовары.Номенклатура;
//Вставить штрихкод
	Запрос = Новый Запрос;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "Аннотация" Тогда
		ТабДокумент =  ЛК_РаботаСДокументами.ПечатьАннотации(Ссылка);
	КонецЕсли;
	// КЧАН
	...
	Для каждого СтрокаБезЦен ИЗ ТаблицаБезЦен Цикл
		...
		Для Каждого СтрокаЗаказа Из СтрокиПоЗаказу Цикл
			...
			Если СписатьПоЗаказу > 0 Тогда
				...
				// == ЛК == Андрей == Преобразование упаковок в штуки ============
				Если СтрокаЗаказа.Коэффициент=1 Тогда
					СтрокаТабличнойЧасти.ЕдиницаИзмерения           = СтрокаЗаказа.ЕдиницаИзмерения;
					СтрокаТабличнойЧасти.Коэффициент                = СтрокаЗаказа.Коэффициент;
				...
				Иначе 
					...
					Зап = Новый Запрос;
					Зап.УстановитьПараметр("ТекущийТовар", СтрокаЗаказа.Номенклатура);
					Зап.Текст="ВЫБРАТЬ
					|	ЕдиницыИзмерения.Ссылка
					|ИЗ
					|	Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
					|ГДЕ
					|	ЕдиницыИзмерения.Владелец.Ссылка = &ТекущийТовар
					|	И ЕдиницыИзмерения.Коэффициент = 1";
					
					Зап = Зап.Выполнить().Выгрузить();
					Для каждого Строчка Из Зап Цикл
						ЕдИзмер=Строчка.Ссылка;
					КонецЦикла;
				...
				ВременнаяЦена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(
				                            МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаЗаказа.Цена,
				                            Сделка.ВалютаДокумента, ВалютаДокумента,
				...
				СтрокаТабличнойЧасти.Цена=ВременнаяЦена/СтрокаЗаказа.Коэффициент;
...
//Функция устанавливает пустой статус ЕГАИС ТТН указаной в документе и возвращает значение Отказ
//
// Возвращаемое значение:
//  Булево
Функция ПроверитьОчиститьСтатусТТНЕГАИС()
	
	Если ЗначениеЗаполнено(ТоварноТранспортнаяНакладнаяЕГАИС) Тогда
		ТТН = ТоварноТранспортнаяНакладнаяЕГАИС.ПолучитьОбъект();
		ТТН.СтатусОбработкиТТН_ЕГАИС = Перечисления.СтатусыОбработкиТТН_ЕГАИС.ПустаяСсылка();
		Попытка
			ТТН.Записать();
		Исключение
			ОбщегоНазначения.СообщитьОбОшибке("Не удалось очистить статус ТТН ЕГАИС." + Символы.ПС+ОписаниеОшибки());
			Возврат Истина;
		КонецПопытки;
	КонецЕсли;
...
//
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоСтатьямДДС) Экспорт
	...
	СтруктураПолей.Вставить("СтатьяДДС"                            , "СтатьяДДС");
	СтруктураПолей.Вставить("Склад"                                , "Склад");
	СтруктураПолей.Вставить("Цена"                                 , "Цена");
	СтруктураПолей.Вставить("Количество"                           , "Количество");
	СтруктураПолей.Вставить("Сумма"                                , "Сумма");	
	...
	РезультатЗапросаПоСтатьямДДС = ОбщегоНазначения.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "СтатьиДДС", СтруктураПолей,СтруктураСложныхПолей);
	ТаблицаПоСтатьямДДС = ПодготовитьТаблицуСтатей(РезультатЗапросаПоСтатьямДДС, СтруктураШапкиДокумента);
	//--ГК
	...
	//++ГК
	БухгалтерскийУчетРасчетовСКонтрагентами.ПодготовкаТаблицыЗначенийДляЦелейПриобретенияИРеализации(ТаблицаПоСтатьямДДС, СтруктураШапкиДокумента, СтруктураШапкиДокумента.НДСВключенВСтоимость, мВалютаРегламентированногоУчета);
	//--ГК

КонецПроцедуры // ПодготовитьТаблицыДокумента()

//++ГК
Функция ПодготовитьТаблицуСтатей(РезультатЗапросаПоСтатьямДДС, СтруктураШапкиДокумента)
	
	ТаблицаСтатей = РезультатЗапросаПоСтатьямДДС.Выгрузить();
	
	ТаблицаСтатей.Колонки.Добавить("Стоимость", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	ТаблицаСтатей.ЗагрузитьКолонку(ТаблицаСтатей.ВыгрузитьКолонку("Сумма") , "Стоимость");
	ПодготовитьТаблицуСтатейУпр(ТаблицаСтатей, СтруктураШапкиДокумента);
	
	Возврат ТаблицаСтатей; 
	
КонецФункции 

Процедура ПодготовитьТаблицуСтатейУпр(ТаблицаСтатей, СтруктураШапкиДокумента)
	
	//ТаблицаТоваров.Колонки.Добавить("ЦенаВВалютеЗаказа"  , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 2));
	ТаблицаСтатей.Колонки.Добавить("СуммаВзаиморасчетов", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 2));
	ТаблицаСтатей.Колонки.Добавить("СуммаУпр"           , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 2));
	ТаблицаСтатей.Колонки.Добавить("НДСУпр"             , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 2));
	
	Для каждого стр из ТаблицаСтатей Цикл
		стр.СуммаВзаиморасчетов = стр.Сумма;
		стр.СуммаУпр = стр.Сумма;
		стр.НДСУпр = стр.НДС;
	КонецЦикла;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли  ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") тогда 
		...
		Организация 					= Справочники.Организации.НайтиПоКоду("000000103");//Пинкасович
		...
		СкладОрдер 						= Справочники.Склады.НайтиПоКоду("000000218");//Транзит Пинкасович
	...
	Если не ( ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
			или ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг")    //Вик 2020_09_24
			или ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПоставщику"))  тогда 	//Вик 2020_09_24 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЭтоНовый() И ПометкаУдаления Тогда
		...
		Иначе
			Отказ = ПроверитьОчиститьСтатусТТНЕГАИС();
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Организация) Тогда// И СокрЛП(ЭтотОбъект.Организация.Код) = ("000000103") Тогда
		ЭтотОбъект.ОтражатьВБухгалтерскомУчете = ЭтотОбъект.Организация.ОтражатьВРегламентированномУчете;
		ЭтотОбъект.ОтражатьВНалоговомУчете = ЭтотОбъект.Организация.ОтражатьВРегламентированномУчете;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Перем ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоСтатьямДДС;
	...
	//++--  //По просьбе клиента, отключено
	//Если ВидПоступления = Перечисления.ВидыПоступленияТоваров.НаСклад тогда
	//	ТекстОшибки = "";
	//	ЛК_РаботаСДокументами.ПроверкаКоличестваИКоличестваФактического(Товары,Отказ,ТекстОшибки);
	//	Если Отказ тогда
	...
	ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоСтатьямДДС);

	//++ГК Татьяне нужно видеть взаиморасчеты в Актах сверки, с комитентами. Поэтому добавлено
	Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком ИЛИ
	...
	СтруктураПодготовленныхТаблиц = Новый Структура("Товары, Услуги, СтатьиДДС", ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоСтатьямДДС);
...
//	ПроверитьЗаполнениеТабличнойСтатьиДДС(ТаблицаПоСтатьямДДС, СтруктураШапкиДокумента, Отказ, Заголовок);
...
Процедура ЗаписатьГТД(Отказ)
	Если дата >= '20210101' тогда
		Для каждого СТрока из Товары цикл
			СтрокаГТД	=	СокрЛП(строка.НомерГТД);
			
			Если ЗначениеЗаполнено(СтрокаГТД) тогда
				НомерГТД_ссылка = Справочники.НомераГТД.НайтиПоКоду(СтрокаГТД);
				
				Если НомерГТД_ссылка.Пустая() тогда //создаем
					НоваяГТД 					= Справочники.НомераГТД.СоздатьЭлемент();
					НоваяГТД.Код				=	СтрокаГТД;
					НоваяГТД.СтранаГТД			=	Строка.Страна;
					Если ЗначениеЗаполнено(Строка.Марка)  тогда
						НоваяГТД.Комментарий	=	 СокрЛП(Строка.Марка);
					КонецЕсли;
					
					Попытка
						День	=	Число(Сред(СтрокаГТД,10,2));
						Месяц	=	Число(Сред(СтрокаГТД,12,2));
						Год		=	Число("20"+Сред(СтрокаГТД,14,2));
						НоваяГТД.ДатаГТД	=	Дата(Год,Месяц,День);	
					Исключение
						Сообщить("Внимание! Не удалось получить дату гтд, проверьте правильность заполнения! Строка: "+Строка.НомерСтроки,СтатусСообщения.Важное);	
						Отказ = Истина;
					КонецПопытки;
					
					НоваяГТД.Записать(); 					
					
				иначе //дописываем марку если нужно
					Если   ЗначениеЗаполнено(Строка.Марка) тогда
						Если Найти(НомерГТД_ссылка.Комментарий,Строка(Строка.Марка)) = 0 тогда
							Об = НомерГТД_ссылка.ПолучитьОбъект();
							
							Если ПустаяСтрока(Об.Комментарий) тогда
								Об.Комментарий =  СокрЛП(Строка.Марка);	
							иначе
								Об.Комментарий = Об.Комментарий + ", "+ СокрЛП(Строка.Марка);
							КонецЕсли;
							Об.Записать();
						КонецЕсли;
					КонецЕсли;
...
//Вик 2021_02_24
//Скопировано из оприходоваине товаров и переделано на поступление
Процедура ЗаполнитьТоварыПоИнвентаризацииТоваровУпр(ДокументОснование) Экспорт
	...
	// НЧАН
	СкладРозничный = (СкладОрдер.ВидСклада = Перечисления.ВидыСкладов.НТТ) ИЛИ (СкладОрдер.ВидСклада = Перечисления.ВидыСкладов.Розничный) ;	
	// КЧАН
	
	Запрос.УстановитьПараметр("ДокументСсылка",          Ссылка);
	Запрос.УстановитьПараметр("НТТ", 					 СкладРозничный);
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПоступлениеТоваровУслугТовары.Количество ЕСТЬ NULL
	|				ТОГДА 0
	...
	|	МАКСИМУМ(Док.Итого) КАК Итого
	|ИЗ
	|	Документ.ИнвентаризацияТоваровНаСкладе.Товары КАК Док
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
	|		ПО Док.Номенклатура = ПоступлениеТоваровУслугТовары.Номенклатура
	|			И Док.ХарактеристикаНоменклатуры = ПоступлениеТоваровУслугТовары.ХарактеристикаНоменклатуры
	|			И Док.СерияНоменклатуры = ПоступлениеТоваровУслугТовары.СерияНоменклатуры
	|			И Док.Ссылка.Склад = ПоступлениеТоваровУслугТовары.Ссылка.СкладОрдер
	|			И (ПоступлениеТоваровУслугТовары.Ссылка.Проведен)
	|			И (ПоступлениеТоваровУслугТовары.Ссылка <> &ДокументСсылка)
	|			И Док.Ссылка = ПоступлениеТоваровУслугТовары.Ссылка.ДокументОснование
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		КоличествоОприходовать = Выборка.Итого - Выборка.КоличествоОприходованное;
		// КЧАН
		...
		// НЧАН
		Если СкладРозничный И СтрокаТабличнойЧасти.Цена = 0 Тогда
		    СтрокаТабличнойЧасти.Цена = ЛК_РаботаСДокументами.ПолучитьЗакупочную(СтрокаТабличнойЧасти.Номенклатура, 
										СтрокаТабличнойЧасти.ХарактеристикаНоменклатуры, Ссылка.Дата);	
		КонецЕсли;										
		// КЧАН
...
мСтруктураПараметровВзаиморасчетов.Вставить("СтруктураТабличныхЧастей", Новый Структура("Товары, Услуги, СтатьиДДС"));
