...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ТипЗнч(Ссылка.ДокументОснование) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
		Для Каждого Эл Из ТаблицаПлатежейУпр Цикл
			Если НЕ ЗначениеЗаполнено(Эл.ДокументПланированияПлатежа) Тогда
				Эл.ДокументПланированияПлатежа = Ссылка.ДокументОснование;
				Эл.ВключатьВПлатежныйКалендарь = Истина;
			КонецЕсли;	
		КонецЦикла;	
	КонецЕсли;	
	// КЧАН
...
Процедура ПриЗаписи(Отказ)
	Если СтрЧислоВхождений(Номер,Организация.Префикс) > 1 тогда
		Если СтрДлина(Организация.Префикс) = 3 тогда
			НужнаяСтрока = "000"+Прав(Номер,5);
			
		ИначеЕсли СтрДлина(Организация.Префикс) = 2 тогда
			НужнаяСтрока = "00"+Прав(Номер,6);
			
		ИначеЕсли СтрДлина(Организация.Префикс) = 1 тогда
			НужнаяСтрока = "0"+Прав(Номер,7);
		КонецЕсли;  	
		
		Номер = Организация.Префикс + НужнаяСтрока;	
		Записать();
	КонецЕсли;
