...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если СтатьиДДС.Количество() = 0 Тогда
		...
		КонецЕсли;
	Иначе
		СделатьДвиженияПоПодотчетникам();
	КонецЕсли;    
	//--ГК
...
//++ГК
Процедура СделатьДвиженияПоПодотчетникам()
	
	Движ = Движения.ВзаиморасчетыСПодотчетнымиЛицами.Добавить();
	Движ.ВидДвижения = ВидДвиженияНакопления.Расход;
	Движ.Период = Дата;
	Движ.ФизЛицо = ФизЛицо;
	Движ.Валюта = ВалютаДокумента;
	Движ.РасчетныйДокумент = ЭтотОбъект.Ссылка;	
	Движ.Организация = Организация;	
	Движ.СуммаВзаиморасчетов = СуммаДокумента;	
	Движ.СуммаУпр = СуммаДокумента;	
