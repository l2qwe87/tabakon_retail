...
Функция ПечатьРКО()
	...
	Макет         = ПолучитьОбщийМакет("КО2_ЛК");
	...
	//ОбластьМакета.Параметры.ДолжностьРуководителя = РуководительДолжность;  // НЧАН
	...
	СДДСп = ?(РасшифровкаПлатежа.Количество() <> 0, РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств, "");
	ОбластьМакета.Параметры.СтатьяДДС             = СДДСп;
	ОбластьМакета.Параметры.Комменатрий           = Комментарий;
	// КЧАН
...
//<pozdniakov.a
Процедура УстановитьПодразделениеСотрудника() Экспорт
	Подразделение = РегистрыСведений.КадровыйУчетСотрудников.ПолучитьПоследнее(Дата, Новый Структура("Организация,Сотрудник", Организация, Контрагент)).Подразделение;
КонецПроцедуры

Процедура УстановитьСуммуВыплаты() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РасчетыПоЗаработнойПлатеОстатки.Организация,
		|	РасчетыПоЗаработнойПлатеОстатки.Сотрудник,
		|	РасчетыПоЗаработнойПлатеОстатки.СуммаОстаток
		|ИЗ
		|	РегистрНакопления.РасчетыПоЗаработнойПлате.Остатки(
		|			&ДатаДокумента,
		|			Организация = &Организация
		|				И Сотрудник = &Сотрудник) КАК РасчетыПоЗаработнойПлатеОстатки";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СуммаДокумента = ВыборкаДетальныеЗаписи.СуммаОстаток;
	КонецЦикла;
КонецПроцедуры
//pozdniakov.a>
...
//<pozdniakov.a
	//ЛК Андрей =============
	// Движения по регистру расчетов по ЗП сотрудников
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотруднику Тогда
		Для каждого Стр из ВыплатаСотрудникам цикл
			...
			Движение.Период = КонецМесяца(ПериодРасчетовПоЗП);
			Движение.Организация = Организация;
			Движение.Сотрудник = Стр.Сотрудник;
			Движение.ВидРасчета = ВидРасчетов;
			Движение.Сумма = Стр.Сумма;
		КонецЦикла;
		
		// НЧАН
		//Если ТипЗнч(Ссылка.ДокументОснование) = Тип("ДокументСсылка.ЗакрытиеЗаявокНаРасходованиеСредств") Тогда
		Если ТипЗнч(Ссылка.ДокументОснование) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
			ТаблицаДвиженийЗаявки = Движения.ЗаявкиНаРасходованиеСредств;                                          		
			СтрокаДвиженийЗаявки = ТаблицаДвиженийЗаявки.Добавить();
			СтрокаДвиженийЗаявки.ВидДвижения = ВидДвиженияНакопления.Расход;
			СтрокаДвиженийЗаявки.СуммаУпр				= Ссылка.ДокументОснование.СуммаДокумента;
			СтрокаДвиженийЗаявки.Сумма					= Ссылка.ДокументОснование.СуммаДокумента;
			СтрокаДвиженийЗаявки.ЗаявкаНаРасходование	= Ссылка.ДокументОснование;
			СтрокаДвиженийЗаявки.Организация			= Организация;
			СтрокаДвиженийЗаявки.Период 				= Дата;
			СтрокаДвиженийЗаявки.СтатьяДвиженияДенежныхСредств = РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;
		КонецЕсли;	
		// КЧАН
	КонецЕсли;
	//=====================
...
//<pozdniakov.a
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотруднику Тогда
		мСтруктураПараметровДенежныхСредств.Вставить("ФизЛицо",                    Контрагент);
//pozdniakov.a>	
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
		...
		Если Основание.ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ВыплатаСотруднику Тогда
			
			 	СтрокаПлатеж = РасшифровкаПлатежа.Добавить();
	
				ВидОперации     = Перечисления.ВидыОперацийРКО.ВыплатаСотруднику;
				ВидРасчетов     = Справочники.ВидыРасчетовССотрудниками.ВыплатаЗарплаты;
								
				КурсДокумента           = СтруктураКурсаДокумента.Курс;
				КратностьДокумента      = СтруктураКурсаДокумента.Кратность;
				
				КассаПС = Справочники.Кассы.ПустаяСсылка();
				ОснованиеКасса = КассаПС;
				Если Основание.Метаданные().Реквизиты.Найти("Касса") = Неопределено Тогда
					Если Основание.Метаданные().Реквизиты.Найти("БанковскийСчетКасса") <> Неопределено Тогда
						ОснованиеКасса = Основание.БанковскийСчетКасса;
					ИНаче
						ОснованиеКасса = КассаПС;
					КонецЕсли;					
				КонецЕсли;					
				 
				Касса                   = ?( ОснованиеКасса <> Касса, ОснованиеКасса , КассаПС);
				СуммаДокумента = Основание.РасшифровкаПлатежа[0].СуммаПлатежа;
								
				Для каждого Строчка Из Основание.ВыплатаСотрудникам Цикл	
					Выплата   = ВыплатаСотрудникам.Добавить();
					Выплата.Сотрудник = Строчка.Сотрудник;
					Выплата.Сумма = Строчка.Сумма;
				КонецЦикла;	
				
				СтрокаПлатеж.СуммаПлатежа = Основание.СуммаДокумента;
				СтатьяДвидДС = Справочники.СтатьиДвиженияДенежныхСредств.ВыплатаСотруднику;
				Если Основание.РасшифровкаПлатежа.Количество() > 0 Тогда
					СтатьяДвидДС = Основание.РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;
				КонецЕсли;	
				СтрокаПлатеж.СтатьяДвиженияДенежныхСредств = СтатьяДвидДС; 
...
ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.НачислениеЗарплаты") Тогда
	...
	СуммаДокумента=Основание.Всего;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Документ", Основание.Ссылка);
	Запрос.Текст = "ВЫБРАТЬ
	|	НачислениеЗарплатыНачисления.Сотрудник.Ссылка КАК Сотрудник,
	|	НачислениеЗарплатыНачисления.Итог
	|ИЗ
	|	Документ.НачислениеЗарплаты.Начисления КАК НачислениеЗарплатыНачисления
	|ГДЕ
	|	НачислениеЗарплатыНачисления.Ссылка = &Документ";
	...
	Для каждого Строчка Из Запрос Цикл
		...
		Выплата   = ВыплатаСотрудникам.Добавить();
		Выплата.Сотрудник = Строчка.Сотрудник;
		Выплата.Сумма = Строчка.Итог;
	КонецЦикла;	
	
	СтрокаПлатеж.СуммаПлатежа=Основание.Всего;
	СтрокаПлатеж.СтатьяДвиженияДенежныхСредств=Справочники.СтатьиДвиженияДенежныхСредств.ВыплатаСотруднику;
...
КонецЕсли;
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.НачислениеЗарплаты") Тогда
	Ответственный=Пользователи.ТекущийПользователь();
	ОтражатьВУправленческомУчете=Истина;
	ОтражатьВБухгалтерскомУчете=Истина;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ЗначениеЗаполнено(Касса) тогда
		Если НРег(Касса.Наименование) = "долги" тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	//
//<pozdniakov.a
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотруднику Тогда
		Если НЕ ЗначениеЗаполнено(ВидРасчетов) Тогда
			Сообщить("Не заполнен вид расчетов с сотрудником!", СтатусСообщения.ОченьВажное);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если СуммаДокумента <> ВыплатаСотрудникам.Итог("Сумма") Тогда
			Сообщить("Сумма документа и сумма в табл. части не сходится!", СтатусСообщения.ОченьВажное);
			Отказ = Истина;
			Возврат;
		КонецЕсли;	
	КонецЕсли;
...
//<pozdniakov.a
	...
	ТаблицаПлатежейУпр = УправлениеДенежнымиСредствами.ПолучитьТаблицуПлатежейУпр(Дата, ВалютаДокумента, Ссылка, "РасходныйКассовыйОрдер", ВидОперации);
//pozdniakov.a>	
