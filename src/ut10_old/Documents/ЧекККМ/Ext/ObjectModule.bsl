...
Функция ПровестиИРаспечататьЧек(Ответ, Отказ, ФормаДокумента, РучнойРежимПечати = Ложь) Экспорт
	...
	Если УстановленЕНВДОрганизации()
	   И КассаККМ.ФормироватьНефискальныеЧеки Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ЕСТЬNULL(ДокЧек.Товары.Номенклатура.ВидАлкогольнойПродукцииЕГАИС.Маркируемый, ЛОЖЬ) КАК Маркируемая,
		               |		ДокЧек.Товары.Номенклатура.ОбъемДАЛ * 10 КАК Объем,
		               |		Номенклатура.Крепость КАК Крепость,
		               |		ВЫБОР
		               |			КОГДА ДокЧек.Товары.Номенклатура.Производитель <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		               |				ТОГДА ДокЧек.Товары.Номенклатура.Производитель.ИНН
		               |			ИНАЧЕ ДокЧек.Товары.Номенклатура.ПроизводительИмпортерАлкогольнойПродукции.ИНН
		               |		КОНЕЦ КАК ИННПроизводителя,
		               |		ВЫБОР
		               |			КОГДА ДокЧек.Товары.Номенклатура.Производитель <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		               |				ТОГДА ДокЧек.Товары.Номенклатура.Производитель.КПП
		               |			ИНАЧЕ ДокЧек.Товары.Номенклатура.ПроизводительИмпортерАлкогольнойПродукции.КПП
		               |		КОНЕЦ КАК КПППроизводителя,
		               |		ЕСТЬNULL(ДокЧек.Товары.Номенклатура.ВидАлкогольнойПродукцииЕГАИС.КодВидаПродукции, """") КАК КодВидаПродукции
		               |	),
		               |	ДокЧек.Оплата.(
		               |		НомерСтроки,
		               |		ВидОплаты,
		...
		Пока ВыборкаТоваров.Следующий() Цикл
			...
			Если ИспользуетсяРегистрацияВЕГАИС Тогда
				...
				Товар.Наименование = ОбластьТелоЧека.Параметры.НаименованиеТовара;
				Товар.НомерСекции  = 1;
				Товар.Цена         = ВыборкаТоваров.Цена;
				Товар.Количество   = ВыборкаТоваров.Количество;
				Товар.Скидка       = КонечнаяСкидка;
				Товар.СтавкаНДС    = УчетНДС.ПолучитьСтавкуНДС(ВыборкаТоваров.СтавкаНДС);
				
				ШтрихкодМарки = "";
				ВыборкаАкцизныеМарки.Сбросить();
				Если ВыборкаАкцизныеМарки.НайтиСледующий(Новый Структура("КлючСтроки", ВыборкаТоваров.КлючСтроки)) Тогда
					ШтрихкодМарки = ВыборкаАкцизныеМарки.ШтрихкодМарки;
				КонецЕсли;
				
				Штрихкод = ВыборкаТоваров.Штрихкод;
				Если ВыборкаТоваров.АлкогольнаяПродукция И ПустаяСтрока(Штрихкод) Тогда
					Штрихкод = ПолучитьШтрихкодНоменклатуры(ВыборкаТоваров.Номенклатура);
				КонецЕсли;
				
				Товар.ПараметрыАлкогольнойПродукции = Новый Структура;
				Товар.ПараметрыАлкогольнойПродукции.Вставить("АлкогольнаяПродукция", ВыборкаТоваров.АлкогольнаяПродукция);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("Маркируемая"         , ВыборкаТоваров.Маркируемая);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("Штрихкод"            , Штрихкод);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("ШтрихкодМарки"       , ШтрихкодМарки);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("Объем"               , ВыборкаТоваров.Объем);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("Крепость"            , ВыборкаТоваров.Крепость);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("ИННПроизводителя"    , ВыборкаТоваров.ИННПроизводителя);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("КПППроизводителя"    , ВыборкаТоваров.КПППроизводителя);
				Товар.ПараметрыАлкогольнойПродукции.Вставить("КодВидаПродукции"    , ВыборкаТоваров.КодВидаПродукции);
			КонецЕсли;
		...
		Если ИспользуетсяРегистрацияВЕГАИС Тогда
			...
			ПараметрыЧека = ПараметрыЧекаДляПодписиВЕГАИС(ТаблицаТоваров, СуммаОплат - СуммаБезналичныхОплат, СуммаБезналичныхОплат, ТранспортныйМодуль);
			...
			Если НЕ РезультатОперации.Результат Тогда
				Ответ = РезультатОперации.ОписаниеОшибки;
				Отказ = Истина;
				Результат = Ложь;
				
				ОбщегоНазначения.СообщитьОбОшибке("Не удалось сформировать электронную подпись чека в ЕГАИС!" + Символы.ПС + РезультатОперации.ОписаниеОшибки);
		...
		Если ИспользуетсяРегистрацияВЕГАИС Тогда
			Если ЗначениеЗаполнено(ОшибкаТО) Тогда
				ПараметрыЧека[3] = НЕ ПараметрыЧека[3];
				ПараметрыЧекаЕГАИС.Вставить("Возврат", ПараметрыЧека[3]);
				...
				КонецЕсли;
			Иначе
				ПараметрыЧекаЕГАИС.Вставить("НомерЧекаККМ", НомерЧекаККМ);
				ПараметрыЧекаЕГАИС.Вставить("НомерСменыККМ", НомерСменыККМ);
				
				Если НЕ ПустаяСтрока(ПараметрыЧекаЕГАИС.Адрес) И НЕ ПустаяСтрока(ПараметрыЧекаЕГАИС.Подпись) Тогда
					РезультатОперации = ИнтеграцияЕГАИСКлиентПереопределяемый.ПечатьСлипЧека(ФР, ПараметрыЧекаЕГАИС, КассаККМ.ШиринаЛенты);
					Если НЕ РезультатОперации.Результат Тогда
						ОбщегоНазначения.СообщитьОбОшибке(РезультатОперации.ОписаниеОшибки);
					КонецЕсли;
				КонецЕсли;
	...
	Иначе
		...
		Если ИспользуетсяРегистрацияВЕГАИС Тогда
			...
			ПараметрыЧека = ПараметрыЧекаДляПодписиВЕГАИС(ТаблицаТоваров, СуммаНал, СуммаБезНал, ТранспортныйМодуль);
...
// Подготавливает данные для формирования электронной подписи в ЕГАИС
//
...
// Возвращает штрихкод производителя для переданной номенклатуры
//
Функция ПолучитьШтрихкодНоменклатуры(Номенклатура)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Возврат РезультатЗапроса.Выгрузить()[0].Штрихкод;
	КонецЕсли;
