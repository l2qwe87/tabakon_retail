...
Функция ПечатьЦенников()
	...
	ОбработкаПечатьЦенников.ТипЦен = Ссылка.Склад.ТипЦенРозничнойТорговли;    // НЧАН
	ОбработкаПечатьЦенников.ИПпоСкладу = Ссылка.Склад.ЮрЛицо;      // НЧАН
...
Функция ПечатьАннотации()   // НЧАН
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ГДЕ
	               |		ПриходныйОрдерНаТоварыТовары.Ссылка = &Ссылка) КАК ВложенныйЗапрос
	               ...
	               |ВЫБРАТЬ
	               |	ПриходныйОрдерНаТоварыТовары.Номенклатура КАК Номенклатура,
	               |	ПриходныйОрдерНаТоварыТовары.Номенклатура.ДополнительноеОписаниеНоменклатуры КАК ДОН,
	               |	ПриходныйОрдерНаТоварыТовары.Номенклатура.Артикул КАК Артикул,
	               |	ВТ1.Штрихкод
	               |ИЗ
	               |	Документ.ПриходныйОрдерНаТовары.Товары КАК ПриходныйОрдерНаТоварыТовары
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ1 КАК ВТ1
	               |		ПО ПриходныйОрдерНаТоварыТовары.Номенклатура = ВТ1.Номенклатура
	               |ГДЕ
	...
	Макет = Обработки.ЛК_РаботаСПеремещениями.ПолучитьМакет("Аннотация");
	//Макет = ЭтотОбъект.ПолучитьМакет("Аннотация");
	Область = Макет.ПолучитьОбласть("Заголовок");
	Область.Параметры.ПрихОрдер = Строка(ЭтотОбъект.Ссылка);
	...
	Пока Выборка.Следующий() Цикл     
		...
		РисунокШтрихКод.Объект.ТекстКода = Выборка.Штрихкод;
		РисунокШтрихКод.Объект.Сообщение = Выборка.Штрихкод;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "ЗаписьВRFIDМетки" Тогда
		ЗаписатьВRFIDМетки();
		Возврат;
	ИначеЕсли ИмяМакета = "Аннотация" Тогда    // НЧАН
		ТабДокумент = ПечатьАннотации();
	КонецЕсли;
