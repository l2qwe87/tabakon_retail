...
// НЧАН
	...
	ОбластьМакета = Макет.ПолучитьОбласть("Покупатель");
	ОбластьМакета.Параметры.ТекстПокупатель = "Для кого:";
	//ОбластьМакета.Параметры.ПредставлениеПолучателя = Строка(ДляКого) + "           ( " + Строка(ДляКого.Адрес) + " )";
	ОбластьМакета.Параметры.ПредставлениеПолучателя = Строка(ДляКого.Код) + " " + Строка(ДляКого.ЮрЛицо) + "   " + Строка(ДляКого) + "           ( " + Строка(ДляКого.Адрес) + " )";
...
// КЧАН	
	...
	// НЧАН
		...
		//ОбластьСтроки.Параметры.Товар = ВыборкаСтрокТовары.Номенклатура.Наименование;	
	// КЧАН	
		Если ВыводитьКоды Тогда
			Если Тип = "ЗаказПоДаннымПоставщика" Тогда
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			Если не ЗначениеЗаполнено(ТекСтрокаТовары.Размещение) тогда
			НоваяСтрока = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрокаТовары);
			НоваяСтрока.СтавкаНДС  = НоваяСтрока.Номенклатура.СтавкаНДС;
			НоваяСтрока.Заказ      = Основание.Ссылка;
			КонецЕсли;
		КонецЦикла;
