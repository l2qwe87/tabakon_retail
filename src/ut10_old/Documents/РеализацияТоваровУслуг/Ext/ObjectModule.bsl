...
//
Функция ПечатьДокумента(СуммыВРублях = Ложь, этоОптИП = Ложь, ВыводитьЯчейки = Ложь)
	...
	Если этоОптИП тогда
		Макет = ПолучитьМакет("НакладнаяОпт");
	иначе
	...
	//Вик 2020_11_13
	Если этоОптИП тогда
		ОбластьМакета.Параметры.Поставщик 				= "ТУ";
		ОбластьМакета.Параметры.ПредставлениеПоставщика = "ТУ";
	иначе
	//КонецВик 
	...
	Если этоОптИП тогда
		...
		ОбластьМакета.Параметры.Адрес   = ДоговорКонтрагента;//содержит в себе адрес
		ТабДокумент.Вывести(ОбластьМакета);  		
	иначе 
		ОбластьМакета = Макет.ПолучитьОбласть("СотрудникИКомментарий");
		ОбластьМакета.Параметры.Сотрудник   = Шапка.Сотрудник;
	...
	Если ВыводитьЯчейки Тогда
		ОбластьЯчейка  = Макет.ПолучитьОбласть("ШапкаТаблицы|Ячейка");
	КонецЕсли;
	...
	Если ВыводитьЯчейки Тогда
		ТабДокумент.Присоединить(ОбластьЯчейка);
	КонецЕсли;    
	...
	Если ВыводитьЯчейки Тогда
		ОбластьЯчейка  = Макет.ПолучитьОбласть("Строка|Ячейка");
	КонецЕсли;
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		//Вик 20210121
		Если ВыводитьЯчейки тогда
			ОбластьЯчейка.Параметры.Ячейка = ОбщегоНазначения.ПолучитьЯчейку(ВыборкаСтрокТовары.Номенклатура);
			ТабДокумент.Присоединить(ОбластьЯчейка); 			
		КонецЕсли;
		//КонецВик  
	...
	Если ВыводитьЯчейки Тогда
		ОбластьЯчейка = Макет.ПолучитьОбласть("Итого|Ячейка");
	КонецЕсли;
	...
	Если Шапка.УчитыватьНДС Тогда
		...
		Если ВыводитьЯчейки Тогда
			ОбластьЯчейка  = Макет.ПолучитьОбласть("ИтогоНДС|Ячейка");
		КонецЕсли;
...
Функция ПечатьТОРГ12(БезУслуг = Ложь)
		...
		//Убрал Вик 2021_01_25 по просьбе Козьминой Т.
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	//Вик
	ИначеЕсли ИмяМакета = "НакладнаяИП" Тогда
		ТабДокумент = ПечатьДокумента(Истина, Истина);
		
	ИначеЕсли ИмяМакета = "НакладнаяЯч" Тогда
		ТабДокумент = ПечатьДокумента(Истина, , Истина);
	//конецВик		
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Иначе
		...
		СтруктураМакетов.Вставить("НакладнаяИП", "Расходная накладная (опт ИП)");
		СтруктураМакетов.Вставить("НакладнаяЯч", "Расходная накладная (с ячейками)");
...
//++ГК
Процедура ЗаполнитьТоварыПоПеремещениюТоваровУпр(ДокументОснование, ИмяТЧ = "Товары", МассивЗаказов = Неопределено) Экспорт
	...
	Для каждого стр из ДокументОснование.Товары Цикл
		...
		строкаТЧ.ЕдиницаИзмерения           = стр.ЕдиницаИзмерения;
		строкаТЧ.ЕдиницаИзмеренияМест       = стр.ЕдиницаИзмеренияМест;
		строкаТЧ.ХарактеристикаНоменклатуры = Стр.ХарактеристикаНоменклатуры;
		строкаТЧ.СерияНоменклатуры          = Стр.СерияНоменклатуры;
	...
	Если ОтражатьВУправленческомУчете Тогда
		...
		Если ВидПередачи = Перечисления.ВидыПередачиТоваров.СоСклада Тогда
			...
			Если Не РезультатЗапросаКомплектыОпт.Пустой() ИЛИ НЕ РезультатЗапросаТараОпт.Пустой() Тогда
				...
				// Проверка остатков при оперативном проведении.      // ++-- проверка при любом проведении
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) <> Тип("ДокументСсылка.ЗаказПокупателя")
		...
		И ТипЗнч(Основание) <> Тип("ДокументСсылка.ПеремещениеТоваров") //Вик 2020_09_24 
		И ТипЗнч(Основание) <> Тип("ДокументСсылка.ОприходованиеТоваров") Тогда//Вик 2020_09_24 
		возврат;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++--
	Если ЗначениеЗаполнено(Сделка) или ВидПередачи = Перечисления.ВидыПередачиТоваров.ПоОрдеру тогда
		ТекстОшибки = "";
...
Процедура ПроставитьГТД(Основание)
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") тогда
		...
		//Полностью копируем ТЧ Заказы и проставляем ГТД
		Если флЕстьМарки тогда
			Товары.Очистить();
			Товары.Загрузить(Основание.Товары.выгрузить());
			Для каждого СТрока из Товары цикл
				Если ЗначениеЗаполнено(Строка.Марка) и не ЗначениеЗаполнено(Строка.НомерГТД) тогда
					
					Запрос = новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
					                      |	ПоступлениеТоваровУслугТовары.Марка,
					                      |	ПоступлениеТоваровУслугТовары.Ссылка,
					                      |	ПоступлениеТоваровУслугТовары.НомерГТД,
					                      |	ПоступлениеТоваровУслугТовары.Страна
					                      |ИЗ
					                      ...
					                      |	И ПоступлениеТоваровУслугТовары.Марка = &Марка
					...
					Запрос.УстановитьПараметр("Марка",Строка.Марка);
					Запрос.УстановитьПараметр("Дата",Дата);
					Запрос.УстановитьПараметр("ДатаНач",'20201201');

					Рез = Запрос.Выполнить().Выгрузить();
					Если Рез.Количество() = 1 тогда
						Строка.НомерГТД	=	Рез[0].НомерГТД;
						Строка.Страна	=	Рез[0].Страна;	
					КонецЕсли;					
...
//Вик 2021_02_24
//Скопировано из списания товаров и переделано на РТиУ
...
//pozdniakov.a>	
	...
	|       ДокСписание.Цена
	|	 ИЗ
	|       Документ.РеализацияТоваровУслуг.Товары КАК ДокСписание
	|    ГДЕ
	|       ДокСписание.Ссылка <> &ДокументСсылка
	|       И ДокСписание.Ссылка.Проведен
	|       И ДокСписание.Ссылка.ДокументОснование = &ДокументОснованиеСсылка
	|
	|    ОБЪЕДИНИТЬ ВСЕ
	|
	|	 ВЫБРАТЬ 
...
//pozdniakov.a>		
	...
	Если Товары.Найти(0, "Цена") = Неопределено Тогда Возврат; КонецЕсли;
	...
	Для Каждого СтрокаТЧ ИЗ Товары Цикл
		Если СтрокаТЧ.Цена = 0 Тогда
			...
			Если ВыборкаДетальныеЗаписи.НайтиСледующий(СтруктураПоиска) Тогда
				СтрокаТЧ.Сумма = ВыборкаДетальныеЗаписи.Цена * СтрокаТЧ.Количество;
