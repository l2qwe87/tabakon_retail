...
Процедура ЗаполнитьТоварыПоИнвентаризацииТоваровУпр(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	//Изменил Вик 2020_08_07
	//|	сумма(Док.Итого) КАК Итого,
	|	Док.Итого КАК Итого,
	//Вик
...
//pozdniakov.a>	
	...
	|   И  (Док.Итого < 0 ИЛИ Док.ЭтоНедостача = Истина)
	|
	|СГРУППИРОВАТЬ ПО
	|	Док.Ссылка.Склад,
	|	Док.Номенклатура,
...
//<pozdniakov.a
	// Переменная, по значению которой при заполнении ТЧ "Товары" будут идентифицироваться 
	// записи о недостачах товаров
	ФлагСуммыНедостачи = 0.01;
	
	ИтогСуммаНедостачи = 0;
//pozdniakov.a>		
	...
	Пока Выборка.Следующий() Цикл
		...
		КоличествоСписать = (-Выборка.Итого) - Выборка.КоличествоСписанное;
		Если Выборка.ЭтоНедостача И Выборка.Итого > 0 Тогда
			ВычетПоНедостачи = ВычетПоНедостачи + (Выборка.Итого * Выборка.Цена);
		КонецЕсли;	
		// КЧАН
		...
		СтрокаТабличнойЧасти.СерияНоменклатуры          = Выборка.СерияНоменклатуры;
 		СтрокаТабличнойЧасти.ЭтоНедостача               = Выборка.ЭтоНедостача;
...
//<pozdniakov.a
		СтрокаТабличнойЧасти.СуммаНедостачи = Макс(Выборка.Цена*КоличествоСписать, ФлагСуммыНедостачи);
		
		Если СтрокаТабличнойЧасти.СуммаНедостачи <> ФлагСуммыНедостачи Тогда 
			ИтогСуммаНедостачи = ИтогСуммаНедостачи + СтрокаТабличнойЧасти.СуммаНедостачи;
		КонецЕсли;
//pozdniakov.a>
	КонецЦикла;
...
//<pozdniakov.a
	...
	//ОтборСтр.Вставить("ЭтоНедостача", Истина); 	
	//СтрокиНедостачи = Товары.НайтиСтроки(ОтборСтр);
	//СуммаСтрокНед = 0;
	//Для Каждого СтрНед Из СтрокиНедостачи Цикл
	//	СуммаСтрокНед = СуммаСтрокНед + СтрНед.СуммаНедостачи;
	//КонецЦикла;
	//
	//ВсегоНедостача = ВычетПоНедостачи + СуммаСтрокНед;
	
	//Если ВычетПоНедостачи <> 0 Тогда // Пропорциональное деление  // Отменено
	//    Для Каждого СтрНед Из СтрокиНедостачи Цикл
	//		Д = СтрНед.СуммаНедостачи / СуммаСтрокНед;
	//		СтрНед.СуммаНедостачи = СтрНед.СуммаНедостачи - (ВычетПоНедостачи * Д);
	//	КонецЦикла;
	//КонецЕсли;
	
	// КЧАН
	
	// Проверяем наличие недозаполненных сумм недостачи и если таковые имеются - пробуем отыскать розничные цены соответствующих
	// товаров в регистре цен номенклатуры.
	Если Товары.Найти(ФлагСуммыНедостачи, "СуммаНедостачи") = Неопределено Тогда Возврат; КонецЕсли;
		
	СписокСообщений = Новый СписокЗначений;
	стрСообщение = "Не удалось определить сумму недостачи в розничных ценах для номенклатуры: ";
	
	МассивТоваров = Товары.ВыгрузитьКолонку("Номенклатура");	
//ЛК Андрей Отредактирован запрос=============	
Запрос.Текст =
"ВЫБРАТЬ
|	Склады.ТипЦенРозничнойТорговли.Ссылка КАК Ссылка
|ИЗ
...
КонецЕсли;
	...
	Запрос.Текст = 
		...
		|				И ТипЦен.Ссылка = &ТипЦен) КАК ЦеныНоменклатурыСрезПоследних";
	...
	Для Каждого СтрокаТЧ ИЗ Товары Цикл
		Если СтрокаТЧ.СуммаНедостачи = ФлагСуммыНедостачи Тогда
			...
			Если ВыборкаДетальныеЗаписи.НайтиСледующий(СтруктураПоиска) Тогда
				СтрокаТЧ.СуммаНедостачи = ВыборкаДетальныеЗаписи.Цена*СтрокаТЧ.Количество;
			Иначе
				СтрокаТЧ.СуммаНедостачи = 0;
				СписокСообщений.Добавить(стрСообщение+Строка(СтрокаТЧ.Номенклатура)+" (строка " + Строка(СтрокаТЧ.НомерСтроки) + ")");				
			КонецЕсли;
		КонецЕсли;
		
		ИтогСуммаНедостачи = ИтогСуммаНедостачи + СтрокаТЧ.СуммаНедостачи;
	КонецЦикла;
    //ЛК Андрей Убрана автоматическая генерация недостачи===================	
	//Если ИтогСуммаНедостачи > 0 Тогда
	//	РаспределениеНедостачи.Добавить().СуммаНедостачи = ИтогСуммаНедостачи;
	//КонецЕсли;
	
	Если СписокСообщений.Количество() > 0 Тогда
		стрСообщение = "";
		
		Для Каждого Элем ИЗ СписокСообщений Цикл
			стрСообщение = стрСообщение + Элем.Значение + Символы.ПС;
		КонецЦикла;
		
		#Если Клиент Тогда							  
			Сообщить(стрСообщение);
		#Иначе 
			ВызватьИсключение стрСообщение;
		#КонецЕсли
	КонецЕсли;
...
//<pozdniakov.a
// Проверяет наличие записей в ТЧ "РаспределениеНедостачи". Если при этом есть суммы недостач в ТЧ "Товары" - отмена проведения.
Процедура ПроверитьЗаполнениеТабличнойЧастиРаспределениеНедостачи(ЭтотОбъект, ИмяТабличнойЧасти, Отказ, Заголовок)
	...
	Если ТабличнаяЧасть.Количество() = 0 Тогда
		Если Товары.Итог("СуммаНедостачи") <> 0 Тогда
			СтрокаСообщения = "Не заполнен список """ + СокрЛП(ПредставлениеТабличнойЧасти) + """!";
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

// Проверяет правильность заполнения реквизитов ТЧ "Товары", связанных с данными о недостачах.
// Поля данных о недостаче должны быть либо полностью заполнены, либо не заполняться вовсе.
//Процедура ПроверитьЗаполнениеТабличнойЧастиТоварыДанныеНедостачи(ЭтотОбъект, ИмяТабличнойЧасти, Отказ, Заголовок)

	//ПредставлениеТабличнойЧасти = ЭтотОбъект.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Представление();
	//ТабличнаяЧасть = ЭтотОбъект[ИмяТабличнойЧасти];
	//МетаданныеРеквизиты = ЭтотОбъект.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Реквизиты;

	//СтруктураОбязательныхПолей = Новый Структура("Сотрудник, СуммаНедостачи");	
	//
	...
	//	
	//	// Определяем заполненность всех полей, связанных с регистром недостачи
	//	Для каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
	//		ТекЗначение = ЗначениеЗаполнено(СтрокаТаблицы[КлючЗначение.Ключ]);
	//		СтруктураОбязательныхПолей.Вставить(КлючЗначение.Ключ, ТекЗначение);
	//		КонтрольнаяСумма = КонтрольнаяСумма + Число(ТекЗначение);
	//	КонецЦикла;

	//	// Если КонтрольнаяСумма = 0 - записи о недостачах не формируются в регистре недостачи;
	//	// Если КонтрольнаяСумма = 2 - все заполнено, записи в регистре недостачи формируются;
	//	// в остальных случаях - вывод сообщений о незаполненных полях и отмена проведения;
	//	Если КонтрольнаяСумма <> 0 И КонтрольнаяСумма <> 2 Тогда
...
//<pozdniakov.a
	// Проверка корректности заполнения реквизитов ТЧ, связанных с данными о недостачах
	//ПроверитьЗаполнениеТабличнойЧастиТоварыДанныеНедостачи(ЭтотОбъект, ИмяТабличнойЧасти, Отказ, Заголовок);
//pozdniakov.a>	
...
//<pozdniakov.a
	ДвиженияПоНедостачам(СтруктураШапкиДокумента, Отказ);
//pozdniakov.a>
...
//<pozdniakov.a
// Движения по регистру недостач.
Процедура ДвиженияПоНедостачам(СтруктураШапкиДокумента, Отказ)
	Если Отказ Тогда Возврат; КонецЕсли;
	
	Движения.Недостачи.Очистить();
	Движения.Недостачи.Записывать = Истина;
	                                    
	Для Каждого СтрокаТЧ ИЗ РаспределениеНедостачи Цикл
		Движение = Движения.Недостачи.ДобавитьПриход();
		Движение.Период = СтруктураШапкиДокумента.Дата;
		Движение.Сотрудник = СтрокаТЧ.Сотрудник;
		Движение.ДокументНедостачи = СтруктураШапкиДокумента.Ссылка;
		Движение.Сумма = СтрокаТЧ.СуммаНедостачи;                  
	КонецЦикла;
КонецПроцедуры
//pozdniakov.a>	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// НЧАН
	Если НЕ РазрешеноСписание() И НЕ РольДоступна("ПолныеПрава") Тогда
...
//<pozdniakov.a
	...
	//СтруктураПолей.Вставить("СуммаНедостачи"            , "СуммаНедостачи");
//pozdniakov.a>	
...
//<pozdniakov.a
	// Проверить заполнение ТЧ "РаспределениеНедостачи".
	ПроверитьЗаполнениеТабличнойЧастиРаспределениеНедостачи(ЭтотОбъект, "РаспределениеНедостачи", Отказ, Заголовок);
//pozdniakov.a>	
...
// НЧАН
Функция РазрешеноСписание()
	
	РазрешеноСписание = Ложь;
	...
	Запрос.УстановитьПараметр("Настройка", ПланыВидовХарактеристик.НастройкиПользователей.НайтиПоКоду("000000113"));
	...
	Пока В.Следующий() Цикл
		РазрешеноСписание = В.Значение;	
	КонецЦикла;
		
	Возврат РазрешеноСписание;
