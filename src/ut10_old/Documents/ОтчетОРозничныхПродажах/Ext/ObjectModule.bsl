...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	// Укажем, что надо проверить.
	//++было СтруктураОбязательныхПолей = Новый Структура("Номенклатура, Количество, Сумма");
	//++было СтруктураОбязательныхПолей = Новый Структура("Номенклатура, Сумма");
	//Вик 2021_02_15
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		Если СтрокаТаблицы.Услуга <> Истина Тогда // для товаров единица измерения обязательна
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ЕдиницаИзмерения) и СтрокаТаблицы.количество <> 0 Тогда  //++добавил второе условие для отладки
...
//<pozdniakov.a
// Проверяет правильность заполнения реквизитов ТЧ "Продавцы".
Процедура ПроверитьЗаполнениеТабличнойЧастиПродавцы(ЭтотОбъект, ИмяТабличнойЧасти, Отказ, Заголовок)
	...
	СтруктураОбязательныхПолей = Новый Структура("Продавец");	
	
	ЕстьГлавныйПродавец = Ложь;
	...
	Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Если СтрокаТаблицы.ЭтоГлавныйПродавец Тогда ЕстьГлавныйПродавец = Истина; КонецЕсли;
	КонецЦикла;
	
	//Если НЕ ЕстьГлавныйПродавец Тогда
	//	ОбщегоНазначения.СообщитьОбОшибке("В строках списка """ + ПредставлениеТабличнойЧасти + """ не отмечен главный продавец!", Отказ, Заголовок);
	//КонецЕсли;
КонецПроцедуры
//pozdniakov.a>	
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоСкидкам,
	...
	Если ТаблицаВРознице.Количество() > 0 Тогда
		...
		//++в запросе взяли не регистр цен атт, а остатки в рознице
		
		//ИЗМЕНИЛ ВИК 2021_03_19 бывают залипания на выполнении запроса более 5 минут
		
		//Если Дата < Дата(2017, 06, 01) Тогда
		//	ТаблицаПоЦенам = УправлениеРозничнойТорговлей.СформироватьЗапросПоПродажнымЦенам2(Дата, ТаблицаВРознице.ВыгрузитьКолонку("Склад"),
		...
		Для Каждого Строка из ТаблицаВРознице цикл
			Если Прав(Строка.Номенклатура.Наименование,3)=" БК" тогда //на бк достаем цену
				
				ЦенаБК	=	Ценообразование.ПолучитьЦенуАТТ(Склад, Строка.Номенклатура, Строка.ХарактеристикаНоменклатуры, Дата);
				Если ЦенаБК <> Неопределено тогда
					Строка.СуммаПродажная = ЦенаБК * Строка.Количество;
				иначе
					Строка.СуммаПродажная = Строка.Цена * Строка.Количество;
				КонецЕсли;
			иначе
				Строка.СуммаПродажная = Строка.Цена * Строка.Количество;
			КонецЕсли; 			
		...
		Если Не Отказ Тогда
			НаборДвижений.ВыполнитьРасход(ложь);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	// НЧАН 22.02.2019
	// Отмена записи при получении ООРП из ГР по условию.
	// Так как загрузка происходит в режиме ОбменДанными.Загрузка и многие проверки игнорируются через COM обмен,
	// то на момент записи документ записывается как не проведенный , поэтому при установленном 
	// свойстве "Проверено" отменяем запись:
	...
	Если ПолОбмен И Ссылка <> документы.ОтчетОРозничныхПродажах.ПустаяСсылка() И (Ссылка.Проверено ИЛИ ЛК_РаботаСДокументами.ЗапретаОбменаПоДате(Ссылка.Дата)) И НЕ Открытоизформы Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;	
	// КЧАН
	...
	Если не ЭтоНовый() и ЗначениеЗаполнено(Ссылка) и не Открытоизформы тогда
		Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения и ссылка.Проведен и ЛК_РаботаСДокументами.ЕстьДвиженияУООРП(Ссылка) Тогда
			//Если ЭтотОбъект.Проверено ИЛИ ЭтотОбъект.НеЗамещатьПриОбмене  Тогда
				отказ = истина;
				сообщить ("отмену проведения можно сделать только открыв форму документа");
			//КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	//Вик 2021_03_19
	Если Организация.Код = "000000103" тогда //ПДВ
		СуммаВключаетНДС	=	Истина;
		УчитыватьНДС		=	Истина;
		Для каждого СтрокаТовары из Товары цикл
			СтрокаТовары.СтавкаНДС	=	Перечисления.СтавкиНДС.НДС20;
			СтрокаТовары.СуммаНДС	=	(СтрокаТовары.Сумма/120)*20;			
		КонецЦикла;  
	КонецЕсли;
	//КонецВик	
	...
	//++
	Если не ЗначениеЗаполнено(ПоследнийПользователь) Тогда
		ПоследнийПользователь=ПараметрыСеанса.ТекущийПользователь;
	Иначе
		Если ПоследнийПользователь<>ПараметрыСеанса.ТекущийПользователь тогда
			Сообщить("Ответственный не соотвествует текущему пользователю!")
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Ссылка) и дата < дата(2017,06,01) и ложь Тогда
		//++после перепроведения всей базы нашли расхождения по ценам АТТ, а при проведении берется цена не из остатков. а из регистра цен АТТ
		//надо сформировать переоценку
		склады = новый массив;
		склады.Добавить(Склад);
		ТаблицаПоЦенам = УправлениеРозничнойТорговлей.СформироватьЗапросПоПродажнымЦенам(Дата, склады,
		Товары.ВыгрузитьКолонку("Номенклатура")).Выгрузить();
		...
		З.УстановитьПараметр("ДокументОснование", Ссылка);
		...
		ДП.ДокументОснование = Ссылка;
		ДП.Дата = Дата-1;
		ДП.ВидОперации = Перечисления.ВидыОперацийПереоценкаТоваровВРознице.ПереоценкаВРознице;
		ДП.Организация = Организация;
		ДП.Ответственный = Ответственный;
		ДП.Склад = Склад;
		ДП.Комментарий = "Создано по отчету о розн продажах: " + Строка (Ссылка);
		...
		для Каждого стр из Товары цикл
			...
			найдстр = ТаблицаПоЦенам.найтиСтроки(отб);	
			Если найдстр.количество() = 0 Тогда
				
				ДС = ДП.Товары.Добавить();
				ДС.Номенклатура = стр.Номенклатура;
				ДС.ХарактеристикаНоменклатуры = стр.ХарактеристикаНоменклатуры;
				ДС.ЦенаВРознице = стр.Цена;
				ДС.ЦенаВРозницеСтарая = 0;
			ИначеЕсли (найдстр.количество() > 0) и (найдстр[0].Цена <> стр.цена) Тогда
				
				ДС = ДП.Товары.Добавить();
				ДС.Номенклатура = стр.Номенклатура;
				ДС.ХарактеристикаНоменклатуры = стр.ХарактеристикаНоменклатуры;
				ДС.ЦенаВРознице = Стр.Цена;
				ДС.ЦенаВРозницеСтарая = найдстр[0].цена;
...
//	             |	ОтчетОРозничныхПродажахТовары.ГлавныйПродавец
//	             |ИЗ
...
//	
//// НЧАН
//	Если Вопрос("Заполнить ТЧ Продавцы из ТЧ Товары ?", РежимДиалогаВопрос.ДаНет) 
...
//		СтрокаТабличнойЧасти = Продавцы.Добавить();
//		СтрокаТабличнойЧасти.ЭтоГлавныйПродавец=Строка.ГлавныйПродавец;
//		СтрокаТабличнойЧасти.Продавец=Строка.Продавец;
//	КонецЦикла
////++ГРД

	// НЧАН
	Если ЭтотОбъект.ПометкаУдаления И НЕ ЭтотОбъект.Ссылка.ПометкаУдаления Тогда
		ПОМ = ПланыОбмена.ОбменУправлениеТорговлейРозничнаяТорговля.НайтиПоКоду("ГР");
		ПланыОбмена.ЗарегистрироватьИзменения(ПОМ, ЭтотОбъект);
	КонецЕсли;
	// КЧАН

	//Вик 2020_11_27
	Для каждого Строка из Товары цикл
		Если Строка.Количество * Строка.Цена <> Строка.Сумма И Строка.ПроцентСкидкиНаценки = 0 тогда
			РазмерПроцента = (Строка.Количество * Строка.Цена) / 100;
			Строка.ПроцентСкидкиНаценки = ((Строка.Количество * Строка.Цена) -  Строка.Сумма)/РазмерПроцента;
		КонецЕсли;
	КонецЦикла;
	//КонецВик
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам) Экспорт
	...
	СтруктураПолей.Вставить("ДатаПродажи"                 , "ДатаПродажи");  //++
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ОстакиОтрицательные = Ложь;
	//
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	ОтчетОРозничныхПродажахТовары.Номенклатура КАК НоменклатураДок,
	//	|	ОтчетОРозничныхПродажахТовары.Количество КАК КоличествоДок,
	//	|	ОтчетОРозничныхПродажахТовары.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатурыДок,
	//	|	ТоварыВРозницеОстатки.Номенклатура КАК НоменклатураОст,
	//	|	ТоварыВРозницеОстатки.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатурыОст,
	//	|	ТоварыВРозницеОстатки.КоличествоОстаток КАК КоличествоОстатокОст,
	//	|	ОтчетОРозничныхПродажахТовары.Количество - ТоварыВРозницеОстатки.КоличествоОстаток КАК Поле1
	//	|ИЗ
	//	|	Документ.ОтчетОРозничныхПродажах.Товары КАК ОтчетОРозничныхПродажахТовары
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыВРознице.Остатки КАК ТоварыВРозницеОстатки
	//	|		ПО ОтчетОРозничныхПродажахТовары.Номенклатура = ТоварыВРозницеОстатки.Номенклатура
	//	|			И ОтчетОРозничныхПродажахТовары.ХарактеристикаНоменклатуры = ТоварыВРозницеОстатки.ХарактеристикаНоменклатуры
	//	|ГДЕ
	//	|	ОтчетОРозничныхПродажахТовары.Ссылка = &Ссылка";
	//
	//Запрос.УстановитьПараметр("Ссылка", Ссылка);
	...
	//	Если ВыборкаДетальныеЗаписи.Поле1 < 0 Тогда
	//		ОстакиОтрицательные = Истина;
	//		Прервать;
	//	КонецЕсли;
	//КонецЦикла;
	//	
	//Если ОстакиОтрицательные Тогда     //  И НЕ РольДоступна("ПолныеПрава")
	//	Отказ = Истина;
	//	Сообщить("Отрицательные остатки!");
	//КонецЕсли;	
...
//<pozdniakov.a
	ПроверитьЗаполнениеТабличнойЧастиПродавцы(ЭтотОбъект, "Продавцы", Отказ, Заголовок);	
//pozdniakov.a>
