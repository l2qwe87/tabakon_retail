...
Процедура ОтказатьсяОтТТН(Команда)
	...
	Иначе
		...
		Если РезультатЗаписи Тогда
			...
			Элементы.ФормаОтказатьсяОтДанныхВЕГАИС.Доступность = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьКартинкуСтатуса()
	...
	ИначеЕсли Объект.СтатусОбработкиТТН_ЕГАИС = Перечисления.СтатусыОбработкиТТН_ЕГАИС.ПереданОтветВЕГАИС Тогда
		Если ПереданВЕГАИССРасхождениями(Объект.Ссылка) Тогда
...
Функция ПереданВЕГАИССРасхождениями(ТТН)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПоступлениеТоваров.ПереданВЕГАИССРасхождениями
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ПереданВЕГАИССРасхождениями
	Иначе
		Возврат Ложь
	КонецЕсли;
	
	
КонецФункции // ПереданВЕГАИССРасхождениями()
