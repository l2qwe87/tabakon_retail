...
Функция УдалениеДублейЦен()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	УстановкаЦенНоменклатурыТовары.Ссылка.Дата >= &ДатаН
	               |	И УстановкаЦенНоменклатурыТовары.Ссылка.Дата < &ДатаК
	               |	И УстановкаЦенНоменклатурыТовары.ТипЦен = &ТипЦен
	               |	И УстановкаЦенНоменклатурыТовары.Номенклатура = &Номенклатура
	               |	И УстановкаЦенНоменклатурыТовары.ХарактеристикаНоменклатуры = &ХарактеристикаНоменклатуры
	               |	И УстановкаЦенНоменклатурыТовары.Ссылка.ПометкаУдаления = ЛОЖЬ
	...
	Для Каждого стр из ДокОбъект.Товары Цикл
		...
		Пока рез.Следующий() Цикл
			...
			Если фиксдок = неопределено Тогда
				...
				ЕСЛИ ЛОЖЬ ТОГДА фиксдок = Документы.УстановкаЦенНоменклатуры.СоздатьДокумент(); КонецЕСЛИ;
			...
			Для Каждого стрФикс из фиксдок.Товары Цикл
				Если стрФикс.ТипЦен = стр.ТипЦен 
					И стрФикс.Номенклатура = стр.Номенклатура 
