...
Перем ИмяКлючаСтруктуры;// Вик 2020_12_14
...
Процедура ЗаполнитьСписокЗначенийОсновныхРеквизитовТовара(СписокЗначенийCML, НоменклатураСсылка)
	...
	ДобавитьУзелCML(СписокЗначенийCML, "Артикул"	  , НоменклатураСсылка.Код); // НЧАН НоменклатураСсылка.Артикул);
	ДобавитьУзелCML(СписокЗначенийCML, "Код"	  , НоменклатураСсылка.Код);    //++
	ДобавитьУзелCML(СписокЗначенийCML, "Наименование" , ФорматНаименованияДляCML(?(ЗначениеЗаполнено(НоменклатураСсылка.НаименованиеДляСайта), НоменклатураСсылка.НаименованиеДляСайта,НоменклатураСсылка.Наименование ))); //++ было ФорматНаименованияДляCML(НоменклатураСсылка.Наименование));
...
Процедура ЗаполнитьСписокЗначенийРеквизитовТовараДляПакетаПредложений(СписокЗначенийCML, НоменклатураСсылка, ХарактеристикаСсылка)
	...
	ЗаписанЗаголовокВыгрузкиХарактеристик = Ложь; // НЧАН
	...
	// НЧАН
	ДобавитьОстаткиНаСкладахФорматМегагрупп(СписокЗначенийCML,НоменклатураСсылка, ХарактеристикаСсылка, ЗаписанЗаголовокВыгрузкиХарактеристик);
...
// НЧАН
Процедура ДобавитьОстаткиНаСкладахФорматМегагрупп (СписокЗначенийCML,НоменклатураСсылка, ХарактеристикаСсылка, ЗаписанЗаголовокВыгрузкиХарактеристик)
	
	ФорматКоличества = Неопределено;
	
	Выборка = ВыборкаЦенПолучить(НоменклатураСсылка, ХарактеристикаСсылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		ДобавитьНачалоЭлементаCML(СписокЗначенийCML, "ХарактеристикаТовара");						
		ДобавитьУзелCML(СписокЗначенийCML, "Наименование", ФорматНаименованияДляCML("Остаток на складе (" + СокрЛП(Выборка.Склад.Наименование) + ")"));
		ДобавитьУзелCML(СписокЗначенийCML, "Значение", ФорматЗначенияДляCML(Выборка.Остаток, ФорматКоличества));
		ДобавитьКонецЭлементаCML(СписокЗначенийCML);
	КонецЦикла;
...
Процедура ЗаполнитьСписокЗначенийРеквизитовТовара(СтруктураИзменений, СписокЗначенийCML, НоменклатураСсылка, ВыгруженоКартинок, ВыгруженоФайлов)
	...
	Иначе
		...
		ДобавитьУзелCML(СписокЗначенийCML, "Описание", НоменклатураСсылка.ТекстДляСайта); //++ было НоменклатураСсылка.ДополнительноеОписаниеНоменклатуры);
	...
	Если ВыгружатьКартинки Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			// НЧАН
			КодНомен = НоменклатураСсылка.Код;
			ПутьКпапке = РаботаСФайлами.НайтиПапку(КодНомен);
			
			НФП = Новый Массив;
			НФ = НайтиФайлы(ПутьКпапке,"*",Истина);
			Н = 1;
			Для Каждого Стр Из НФ Цикл
				ОписаниеКартинки = Новый Структура;
				ОписаниеКартинки.Вставить("Путь", Стр.ПолноеИмя);
				ОписаниеКартинки.Вставить("Наименование", Стр.ИмяБезРасширения);
				ОписаниеКартинки.Вставить("ПорНомер", Н);
				НФП.Добавить(ОписаниеКартинки);
				Н = Н + 1;
			КонецЦикла;	
			Если НФП.Количество() > 0 Тогда 
				МассивКартинок.Очистить();
				МассивКартинок = НФП;
			КонецЕсли;					
			// КЧАН
...
//		ВыгрузитьОстаткиДляПакета(СписокЗначенийCML, НоменклатураСсылка, ХарактеристикаСсылка);   Отключил 26.12.19
	//КонецЕсли;
	// КЧАН
...
Функция ВыгрузитьКартинку(Номенклатура, ХранилищеСсылка)
	...
	// НЧАН
	ИмяПоХранилищу = "NoStorage";
	Если ТипЗнч(ХранилищеСсылка) <> Тип("СправочникСсылка.ХранилищеДополнительнойИнформации") Тогда	
		Попытка
			Картинка = Новый Картинка(ХранилищеСсылка.Путь);
			ИмяПоХранилищу =  ИмяПоХранилищу + ХранилищеСсылка.ПорНомер;
		Исключение
	...
	//ИмяПоХранилищу    = Строка(ХранилищеСсылка.УникальныйИдентификатор());   // НЧАН
...
Процедура ВыгрузитьДанные() Экспорт
	...
	// НЧАН
	Если НЕ ВыгрузкаCML2 Тогда
		ВыгружатьНаФтп = Истина;
		Если ВыгружатьНаФтп И ЛК_РегламентныеЗадания.ХодОбменаССайтом() = "Сайт не обработал предыдущие файлы" Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;	
	// КЧАН
	...
	// НЧАН
		Если НЕ ВыгрузкаCML2 Тогда
			...
			ВыгрузкаЛК = Истина;
			//ОтправлятьСразу = Истина;
			ВариантЛК = Истина;
			
#Область ДляБитрикса					
		//Если ВыгрузкаЛК  Тогда
		//	
		//	НастройкиП = СохраненныеНастройкиПостроителя.Получить();
		//	СЗБазовый  = НастройкиП.ТаблицаОтбора.Найти("Номенклатура").Значение;
		//	//МассивВыгрженныхОбъектов = Новый Массив;
		//	ИндексФайлаОбменаП = 1;
		//	ИндексФайлаОбменаСтрокойП = "1";
		//	ВыгрузкаЛКСтруктура = Новый Структура;
		//	
		//	ЗапретВыгрузкиНоменклатура = Новый Массив;
		//	Для Каждого Эл Из НастройкаСсылка.ЗапретВыгрузкиНоменклатура.Получить() Цикл
		//		ЗапретВыгрузкиНоменклатура.Добавить(Эл.Значение);					
		//	КонецЦикла;	
		//		
		//	Если ОтправлятьСразу Тогда
		//		ВыгрузкаЛКСтруктура.Вставить("ЗакончитьОбмен", Истина);
		//	Иначе
		//		ВыгрузкаЛКСтруктура.Вставить("ЗакончитьОбмен", Ложь);
		//		РС = РегистрыСведений.ЛК_ОбменССайтом_МВО.СоздатьНаборЗаписей();
		//		РС.Записать();
		//	КонецЕсли;	
		//	
		//	ПрерватьЦикл = Ложь;
		...
		//		Запрос.Текст = 
		//			"ВЫБРАТЬ ПЕРВЫЕ 1000
		//			|	Номенклатура.Ссылка КАК НН,
		//			|	ЛК_ОбменССайтом_МВО.Товар
		//			|ИЗ
		//			|	Справочник.Номенклатура КАК Номенклатура
		//			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЛК_ОбменССайтом_МВО КАК ЛК_ОбменССайтом_МВО
		//			|		ПО Номенклатура.Ссылка = ЛК_ОбменССайтом_МВО.Товар
		//			|ГДЕ
		//			|	ЛК_ОбменССайтом_МВО.Товар ЕСТЬ NULL
		//			|	И НЕ Номенклатура.ПометкаУдаления
		//			|	И НЕ Номенклатура.Ссылка В ИЕРАРХИИ(&ЗапретВыгрузкиНоменклатура)
		//			|	И Номенклатура.Ссылка В ИЕРАРХИИ(&Группы)";
		//		
		//		//Запрос.УстановитьПараметр("ВыгружОбъекты", МассивВыгрженныхОбъектов);
		//		
		//		Если СЗБазовый.Количество() <> 0 Тогда
		//			Запрос.УстановитьПараметр("Группы", СЗБазовый);
		//		Иначе	
		//			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Номенклатура.Ссылка В ИЕРАРХИИ(&Группы)", "");
		//		КонецЕсли;	 
		//		
		//		Если ВыгружатьТолькоИзменения Тогда	
		//			Запрос.Текст = 
		...
		//				|	Справочник.Номенклатура КАК Номенклатура
		//				|ГДЕ
		//				|	Номенклатура.Ссылка В(&ИзмененНоменкл)
		//				|	И НЕ Номенклатура.Ссылка В ИЕРАРХИИ(&ЗапретВыгрузкиНоменклатура)
		//				|;
		//				|
		//				|////////////////////////////////////////////////////////////////////////////////
		...
		//				|	НомСп КАК НомСп
		//				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЛК_ОбменССайтом_МВО КАК ЛК_ОбменССайтом_МВО
		//				|		ПО НомСп.Ссылка = ЛК_ОбменССайтом_МВО.Товар
		//				|ГДЕ
		//				|	ЛК_ОбменССайтом_МВО.Товар ЕСТЬ NULL";

		//			//Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЛК_ОбменССайтом_МВО.Товар ЕСТЬ NULL", "Номенклатура.Ссылка В (&ИзмененНоменкл)");
		//			Запрос.УстановитьПараметр("ИзмененНоменкл", СтруктураИзменений.Товары);	
		//		КонецЕсли;
		//		
		//		Если ЗапретВыгрузкиНоменклатура.Количество() = 0 Тогда
		//			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ Номенклатура.Ссылка В ИЕРАРХИИ(&ЗапретВыгрузкиНоменклатура)", "");	
		//		Иначе
		//			Запрос.УстановитьПараметр("ЗапретВыгрузкиНоменклатура", ЗапретВыгрузкиНоменклатура);
		//		КонецЕсли;	
		//	
		//		Попытка
		//			СВО = Запрос.Выполнить().Выбрать();
		//		Исключение
		//			ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Ошибка,,,"Обмен с сайтом: " + ОписаниеОшибки());
		//			ВызватьИсключение;
		//		КонецПопытки;	
		//			
		//		УстСЗ = Новый СписокЗначений;
		//		
		//		//МассивТекущихОбъектов = Новый Массив;
		//		НЗР = РегистрыСведений.ЛК_ОбменССайтом_МВО.СоздатьНаборЗаписей();
		//		Пока СВО.Следующий() Цикл
		//			УстСЗ.Добавить(СВО.НН);
		//			//МассивВыгрженныхОбъектов.Добавить(СВО.НН);
		//			//МассивТекущихОбъектов.Добавить(СВО.НН);
		//			СтрНЗР = НЗР.Добавить();
		//			СтрНЗР.Товар = СВО.НН;
		//		КонецЦикла;	
		//				
		//		Если УстСЗ.Количество() < 1000 Тогда
		//			//Прервать;
		//			ВыгрузкаЛКСтруктура.Вставить("ЗакончитьОбмен", Истина);
		//			ПрерватьЦикл = Истина;
		//		КонецЕсли;
		//		
		//		
		//		ВыгрузкаЛКСтруктура.Вставить("ИндексФайлаОбмена", ИндексФайлаОбменаП);
		//		ВыгрузкаЛКСтруктура.Вставить("ИндексФайлаОбменаСтрокой",ИндексФайлаОбменаСтрокойП);
		//		ВыгрузкаЛКСтруктура.Вставить("СписокТоваровЛК", УстСЗ);
		//		ИндексФайлаОбменаП = ИндексФайлаОбменаП + 1;
		//		Если ИндексФайлаОбменаП > 0 Тогда
		//			ИндексФайлаОбменаСтрокойП = Формат(ИндексФайлаОбменаП, "ЧГ=");	
		//		КонецЕсли;
		//		
		//		//Если ИндексФайлаОбменаП < 11 Тогда
		//		//	Продолжить;
		//		//КонецЕсли;
		//		
		//		ТоварыУспешноВыгружены = ВыгрузитьТовары(СтруктураИзменений, ВыгрузкаЛКСтруктура);
		//		
		//		Если ТоварыУспешноВыгружены Тогда
		//			НЗР.Записать(Ложь);
		//		КонецЕсли;	
		//		
		//		Если НЕ ТоварыУспешноВыгружены ИЛИ ПрерватьЦикл ИЛИ ВыгружатьТолькоИзменения Тогда
		//			Прервать;
		//		КонецЕсли;
		//		
		//	КонецЦикла;	
		...
		//КонецЕсли;
		//
		//Если ТоварыУспешноВыгружены ИЛИ ВыгружатьТолькоИзменения Тогда
		//	РС = РегистрыСведений.ЛК_ОбменССайтом_МВО.СоздатьНаборЗаписей();
		//	РС.Записать();			
		//КонецЕсли;
...
#КонецОбласти	
			...
			Если ВыгрузкаЛК  Тогда
					...
					ЗапретВыгрузкиНоменклатураМ = Новый Массив;
					Для Каждого Эл Из НастройкаСсылка.ЗапретВыгрузкиНоменклатура.Получить() Цикл
						ЗапретВыгрузкиНоменклатураМ.Добавить(Эл.Значение);					
					КонецЦикла;
					
					Запрос = Новый Запрос;
					Запрос.Текст = 
						"ВЫБРАТЬ
						|	Номенклатура.Ссылка КАК СС
						|ИЗ
						|	Справочник.Номенклатура КАК Номенклатура
						|ГДЕ
						|	Номенклатура.Ссылка В ИЕРАРХИИ(&Ссылка)";
					
					Запрос.УстановитьПараметр("Ссылка", ЗапретВыгрузкиНоменклатураМ);		
					ЗапретВыгрузкиНоменклатура = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СС");

					
					Если ВариантЛК Тогда
						
						ИндексФайлаОбменаП = 0;
						ПрерватьЦикл = Ложь;
						ТоварыИзменения = СтруктураИзменений.Товары;
						
						Пока Истина Цикл
							
							ПакетКоличество = 1000;
							ПакетТоваров = Новый Массив;
								
							Пока ПакетКоличество > 0 Цикл
								Если ТоварыИзменения.Количество() = 0 Тогда
									Прервать;
								КонецЕсли;								
								ТоварВПакет = ТоварыИзменения[0];							
								Если ЗапретВыгрузкиНоменклатура.Найти(ТоварВПакет) <> Неопределено Тогда
									ТоварыИзменения.Удалить(0);
									Продолжить;
								КонецЕсли;	
								ПакетТоваров.Добавить(ТоварВПакет);
								ТоварыИзменения.Удалить(0);
								ПакетКоличество = ПакетКоличество - 1;
								Если ПакетКоличество = 0 Тогда
									Прервать;
								КонецЕсли;
							КонецЦикла;	
									
							Если ПакетТоваров.Количество() < 1000 Тогда
								ПрерватьЦикл = Истина;
							КонецЕсли;
							...
							Попытка
								//ВыгрузкаВариантЛК(ПакетТоваров, ИндексФайлаОбменаСтрокойП);
								ВыгрузкаВФорматеYML(ПакетТоваров, ИндексФайлаОбменаСтрокойП);   // Выгрузка YML
							...
							Если УспешноВыгружено Тогда
								Для каждого ТоварПакета Из ПакетТоваров Цикл
									ПланыОбмена.УдалитьРегистрациюИзменений(УзелОбменаТоварами, ТоварПакета);
								КонецЦикла;
							КонецЕсли;	
							
							Если НЕ УспешноВыгружено ИЛИ ПрерватьЦикл Тогда
								Прервать;
							КонецЕсли;
					...
					Иначе
						...
						НастройкиП = СохраненныеНастройкиПостроителя.Получить();		
						ДопРегаТоваров = НастройкиП.ТаблицаОтбора.Найти("Номенклатура").Значение;
						...
						Пока Истина Цикл
							...
							Для Каждого ТоварВПакет Из СтруктураИзменений.Товары Цикл
								Если ЗапретВыгрузкиНоменклатура.Найти(ТоварВПакет) <> Неопределено Тогда
									Продолжить;
								КонецЕсли;	
								ПакетТоваров.Добавить(ТоварВПакет);
								УстСЗ.Добавить(ТоварВПакет);
								ПакетКоличество = ПакетКоличество - 1;
								Если ПакетКоличество = 0 Тогда
									Прервать;
								КонецЕсли;
							...
							Если УстСЗ.Количество() < 500 Тогда
								ВыгрузкаЛКСтруктура.Вставить("ЗакончитьОбмен", Истина);
								ПрерватьЦикл = Истина;
							КонецЕсли;
							...
							Если НЕ ТоварыУспешноВыгружены ИЛИ ПрерватьЦикл Тогда
								Прервать;
							КонецЕсли;
	...
	// НЧАН
	Если НЕ ВыгрузкаCML2 Тогда
		ЛК_РегламентныеЗадания.ЗагрузкаНаФтпСайта();
	КонецЕсли;	
	// КЧАН
...
Функция ВыгрузитьТовары(СтруктураИзменений, ВыгрузкаЛКСтруктура = Неопределено)
	...
	ВыгрузитьТоварыВКаталогНаДиске(СтруктураИзменений, ВыгрузкаЛКСтруктура);
	...
	// НЧАН
	Если ВыгрузкаЛКСтруктура <> Неопределено Тогда 
		ЗакончитьОбмен = ВыгрузкаЛКСтруктура["ЗакончитьОбмен"];
	Иначе 
		ЗакончитьОбмен = Истина;
	КонецЕсли;	
	// КЧАН
	Если ВыгружатьНаСайт И ЗакончитьОбмен Тогда           //  НЧАн
...
Процедура ВыгрузитьТоварыВКаталогНаДиске(СтруктураИзменений, ВыгрузкаЛКСтруктура = Неопределено) 
	...
	ВыгрузкаЛКСтруктураБЗ = ВыгрузкаЛКСтруктура; // НЧАН
	Для Каждого СтрокаТаблицыКаталогов Из мТаблицаКаталогов Цикл
// НЧАН	
	Если ВыгрузкаЛКСтруктура <> Неопределено Тогда
		УстНом = ПостроительЗапроса.Отбор.Номенклатура;
		УстНом.ВидСравнения = ВидСравнения.ВСписке;
		УстНом.Значение = ВыгрузкаЛКСтруктура["СписокТоваровЛК"];
		УстНом.Использование = Истина;	
	Иначе
		ВыгрузкаЛКСтруктура = Новый Структура;
		ВыгрузкаЛКСтруктура.Вставить("ИндексФайлаОбменаСтрокой", "");
	КонецЕсли;	
// КЧАН		
		...
		ИмяФайлаКаталога = мКаталогНаДиске + "\import" + ИндексФайлаОбменаСтрокой + ВыгрузкаЛКСтруктура["ИндексФайлаОбменаСтрокой"] + ".xml"; // НЧАН
		ИмяФайлаПрайса	 = мКаталогНаДиске + "\offers" + ИндексФайлаОбменаСтрокой + ВыгрузкаЛКСтруктура["ИндексФайлаОбменаСтрокой"] + ".xml"; // НЧАН
		...
		// НЧАН
		//Попытка       // Выгружаем классификатор только в первом файле
		//Если ВыгрузкаЛКСтруктура["ИндексФайлаОбменаСтрокой"] = "1" Тогда
		ВыгрузитьКлассификатор(ОбъектCMLКаталог, СтрокаТаблицыКаталогов);
			//ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Информация,,,"Обмен с сайтом: " + ИмяФайлаКаталога);
		//КонецЕсли;	
		//Исключение
		//	ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Ошибка,,,"Обмен с сайтом: " + ОписаниеОшибки());
		//	ТД = Новый ТекстовыйДокумент;
		//	
		//	Для Каждого Эл Из МассивТекущихОбъектов Цикл
		//		Стр = "" + Эл.УникальныйИдентификатор();
		//		ТД.ДобавитьСтроку(Стр);
		//	КонецЦикла;
		//	ТД.Записать("F:\Легкий Код\Error_Obmen.txt");
		//		
		//	Продолжить;
		//	
		//КонецПопытки;
	...
	ВыгрузкаЛКСтруктура = ВыгрузкаЛКСтруктураБЗ ; // НЧАН
	
КонецПроцедуры

Функция ВыгрузитьКаталогПредложенияНаСайт() Экспорт
...
Функция СформироватьCMLКаталогаПредложений(СтруктураИзменений, ОбъектCMLКаталог, ОбъектCMLПакетПредложений, ПараметрыКаталога)
	...
	// НЧАН Выгрузка остатков по складам
	ВыгрузитьСкладыВПакет(ОбъектCMLПакетПредложений);	
	// КЧАН
...
Функция ОбработатьНачалоЭлемента(ОбъектCML, Знач ИмяЭлемента, ДеревоДокументов);
	...
	Если ИмяЭлемента = "Документ" Тогда
		...
		//Вик 2020_12_14
		НовСтрокаДерева.СтруктураНужныхДанных = Новый Структура;
		//КонцеВик 
...
Функция ОбработатьЗначениеЭлемента(Знач ИмяЭлемента, Знач ЗначениеЭлемента, ДеревоДокументов);
	...
	Если КоличествоДокументов > 0 Тогда
		...
		Если ИмяЭлемента = "Документ.ЗначенияРеквизитов.ЗначениеРеквизита.Наименование" Тогда			
			ИмяКлючаСтруктуры	=	СтрЗаменить(ЗначениеЭлемента," ","");
		КонецЕсли;
		
		Если ИмяЭлемента =  "Документ.Контрагенты.Контрагент.Наименование" тогда
			ТекущаяСтрокаДерева.СтруктураНужныхДанных.Вставить("ФИО",ЗначениеЭлемента);
			
		ИначеЕсли ИмяЭлемента =  "Документ.Контрагенты.Контрагент.Контакты.Контакт.Значение" тогда 
			Если Найти(ЗначениеЭлемента,"@") тогда
				ТекущаяСтрокаДерева.СтруктураНужныхДанных.Вставить("Почта",ЗначениеЭлемента);
			КонецЕсли;
			
			//Если Найти(ЗначениеЭлемента,"+7") тогда
			Если Найти(ЗначениеЭлемента,"79") и не Найти(ЗначениеЭлемента,"@") тогда
				ТекущаяСтрокаДерева.СтруктураНужныхДанных.Вставить("Телефон",ЗначениеЭлемента);
			КонецЕсли; 
			
		ИначеЕсли ИмяЭлемента =  "Документ.Склады.Склад.Ид" тогда 
			ТекущаяСтрокаДерева.СтруктураНужныхДанных.Вставить("СкладГруппа",
													Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ЗначениеЭлемента)));
		
		ИначеЕсли ИмяЭлемента = "Документ.ЗначенияРеквизитов.ЗначениеРеквизита.Значение" Тогда
			ТекущаяСтрокаДерева.СтруктураНужныхДанных.Вставить(ИмяКлючаСтруктуры,ЗначениеЭлемента);
		КонецЕсли;		
		//КонецВик  
...
Функция РазобратьCML(СтрокаCML)
	...
	//Вик 2020_12_14
	//Очень странно сделано, что данные по заказу, например статус, способы доставки и оплаты - находятся в "ЗначениеРеквизитаНаименование" / 
	// "ЗначениеРеквизитаЗначение" и относятся к табличньй части Товары. Я вынес все что необходимо в отдельную структуру
	ДеревоДокументов.Колонки.Добавить("СтруктураНужныхДанных");
	//КонецВик 	
	...
	ИмяКлючаСтруктуры = "";// Вик 2020_12_14
...
Функция ОбработатьВалютуCML(КодВалютыСтрока)
	...
	// НЧАН
	Если КодВалютыСтрока = "RUB" Тогда
		КодВалютыСтрока = "руб";			
	КонецЕсли;	
	// КЧАН
...
Функция СоздатьОбновитьДокументы(ДеревоДокументов, СтруктураСтатистики, МассивДокументовДляПроведения, МассивЗагруженныхДокументов)
	...
	Для Каждого Док Из ДеревоДокументов.Строки Цикл
		...
		Попытка
			...
			// НЧАН
			СтатусЗначениеСайта = "";
			Для Каждого ПолСв Из Док.Строки Цикл
				Если ПолСв.СвойствоНаименование	= "Статус заказа" Тогда
					СтатусЗначениеСайта = ПолСв.СвойствоЗначение; 
					Прервать;
				КонецЕсли;	
			КонецЦикла;
			Если СтатусЗначениеСайта <> "" Тогда
				СтатусЗаказа = Справочники.Сайт_СтатусыЗаказов.НайтиПоНаименованию(СтатусЗначениеСайта);
				Док.ДокументОбъект.СайтСтатусЗаказа = СтатусЗаказа;
			КонецЕсли;	
			// КЧАН
			
			//Вик 2020_12_14
			Попытка
				Если НРег(Док.СтруктураНужныхДанных.Способдоставки) = "самовывоз" тогда
					Док.ДокументОбъект.СайтТипДоставки	= Справочники.Сайт_ТипыДоставки.НайтиПоНаименованию("самовывоз");
				иначе
					Док.ДокументОбъект.СайтТипДоставки	= Справочники.Сайт_ТипыДоставки.НайтиПоНаименованию("доставка");
				КонецЕсли;
				
				Если НРег(Док.СтруктураНужныхДанных.Способоплаты) = "наличные/онлайн при получении" тогда
					Док.ДокументОбъект.СайтТипОплаты	= Справочники.Сайт_ТипыОплаты.НайтиПоНаименованию("Наличные");
				иначе
					Док.ДокументОбъект.СайтТипОплаты	= Справочники.Сайт_ТипыОплаты.НайтиПоНаименованию("Безналичные");
				КонецЕсли;
				
				Склад = Справочники.Склады.ПустаяСсылка();
				Если Док.СтруктураНужныхДанных.Свойство("СкладГруппа",Склад) тогда
					Док.ДокументОбъект.СкладГруппа = Склад;	
				КонецЕсли;

				Если не ЗначениеЗаполнено(Док.ДокументОбъект.СайтСтатусЗаказа) тогда
					Если Док.СтруктураНужныхДанных.Статусзаказа = "[N] Новый" тогда
						Док.ДокументОбъект.СайтСтатусЗаказа	= Справочники.Сайт_СтатусыЗаказов.НайтиПоНаименованию("Новый");
					КонецЕсли;
				КонецЕсли;
				
				Адрес = "";
				Если Док.СтруктураНужныхДанных.Свойство("Адресдоставки",Адрес) тогда
					Док.ДокументОбъект.АдресДоставки = Адрес;	
				КонецЕсли; 
				
				ФИО 	 = "";
				Почта	 = "";
				Телефон	 = "";
				Док.ДокументОбъект.СайтДопИнфо = "";
				Если Док.СтруктураНужныхДанных.Свойство("ФИО",ФИО) тогда
					Док.ДокументОбъект.СайтДопИнфо = ФИО;	
				КонецЕсли; 
				
				Если Док.СтруктураНужныхДанных.Свойство("Почта",Почта) тогда
					Док.ДокументОбъект.СайтДопИнфо = Док.ДокументОбъект.СайтДопИнфо +"; "+ Почта;	
				КонецЕсли; 

				Если Док.СтруктураНужныхДанных.Свойство("Телефон",Телефон) тогда
					Док.ДокументОбъект.СайтДопИнфо = Док.ДокументОбъект.СайтДопИнфо +"; "+ Телефон;	
				КонецЕсли; 				
			...
			Если НовыйДокумент Тогда
				...
				//	
				//КонецЕсли;
				Док.ДокументОбъект.СайтНомер = Док.НомерВходящий; 
				Док.ДокументОбъект.СайтДата = Док.ДокументОбъект.ДатаВходящегоДокументаЭлектронногоОбмена;
				// КЧАН
...
Функция ИдентифицироватьКонтрагентов(ДеревоДокументов)
	...
	Для Каждого СтрокаДД Из ДеревоДокументов.Строки Цикл
		...
		//КонецЕсли;
		КонтрагентСсылка = Справочники.Контрагенты.НайтиПоКоду("АВ0000082");
		// КЧАН
		...
		ДеревоАдресов = СтрокаДД.СтруктураДанныхКонтрагента.ДеревоАдресов;	
		Для Каждого СтрокаДерева Из ДеревоАдресов.Строки Цикл			
			...
			КонецЕсли;
			Если СтрокаДерева.Представление <> "" Тогда
				АдресДоставки = СтрокаДерева.Представление;				
			КонецЕсли;	
		КонецЦикла;
		Док.СайтАдресДоставки = АдресДоставки; 
		// КЧАН
...
// НЧАН
Процедура ЗагрузитьНаСайтИзКаталога(Каталог) Экспорт
	
	мКаталогНаДиске = Каталог;
	ВыгрузитьКаталогПредложенияНаСайт();	
	
КонецПроцедуры	
// КЧАН

// НЧАН
Процедура  ВыгрузитьСкладыВПакет(ОбъектCMLПакетПредложений) Экспорт

	лкТолькоРозничныеСклады = Истина;
	...
	ОбъектCMLПакетПредложений.ЗаписатьНачалоЭлемента("Склады");

	Пока Выборка.Следующий() Цикл

		Если лкТолькоРозничныеСклады И Выборка.Склад.ВидСклада <> Перечисления.ВидыСкладов.Розничный Тогда
			Продолжить;
		КонецЕсли;
		
		ОбъектCMLПакетПредложений.ЗаписатьНачалоЭлемента("Склад");
		
		ЗаписатьТекстовойУзел(ОбъектCMLПакетПредложений, "Ид", СокрЛП(Выборка.Склад.УникальныйИдентификатор()));
		ЗаписатьТекстовойУзел(ОбъектCMLПакетПредложений, "Наименование", ФорматНаименованияДляCML(Выборка.Склад));
...
// НЧАН
Процедура ВыгрузитьОстаткиДляПакета(СписокЗначенийCML, НоменклатураСсылка, ХарактеристикаСсылка)
	...
	ДобавитьНачалоЭлементаCML(СписокЗначенийCML, "КоличествоНаСкладах");
	...
	Пока Выборка.Следующий() Цикл
		...
		ДобавитьНачалоЭлементаCML(СписокЗначенийCML, "КоличествоНаСкладе");
		
		ДобавитьУзелCML(СписокЗначенийCML, "ИдСклада", СокрЛП(Выборка.Склад.УникальныйИдентификатор()));
		ДобавитьУзелCML(СписокЗначенийCML, "Количество", ФорматЗначенияДляCML(Выборка.Остаток, ФорматКоличества));
	...
	Иначе
		...
		Если СписокЗначенийCML.Количество() > 0 Тогда
			
			СписокЗначенийCML.Удалить(СписокЗначенийCML.Количество() - 1);
...
// НЧАН
Функция ВыборкаЦенПолучить(НоменклатураСсылка, ХарактеристикаСсылка) 
	
	//
	Если мМВТ.Таблицы.Найти("ОстаткиСкл") <>  Неопределено Тогда
		ЗапросМВТ = Новый Запрос;
	    ЗапросМВТ.МенеджерВременныхТаблиц = мМВТ; 
	    ЗапросМВТ.Текст = " УНИЧТОЖИТЬ ОстаткиСкл";
	    ЗапросМВТ.Выполнить();    
	КонецЕсли;	
	//
	...
	Запрос.Текст = 	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               	...
	               	|////////////////////////////////////////////////////////////////////////////////
	               	|ВЫБРАТЬ
	               	|	ОстаткиСкл.НоменклатураСсылка,
	               	|	ОстаткиСкл.ХарактеристикаСсылка,
	               	|	ОстаткиСкл.Остаток,
	               	|	ОстаткиСкл.Склад
	               	|ИЗ
	               	|	ОстаткиСкл КАК ОстаткиСкл
	               	|ГДЕ
	               	|	ОстаткиСкл.НоменклатураСсылка = &НоменклатураСсылка
	               	|	И ОстаткиСкл.ХарактеристикаСсылка = &ХарактеристикаСсылка";
...
// НЧАН
Процедура ВыгрузкаВариантЛК(ПакетТоваров, НомерФайла)
	
	Если ПакетТоваров.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;	
			
	//ПутьККаталогуДляЗаписи = "F:\Легкий Код\9_СаняЧ (Данные)\test_Site\"; 
	ПутьККаталогуДляЗаписи  = КаталогВыгрузки + "\1cbitrix\";
	ПутьКФайлуДляЗаписи = ПутьККаталогуДляЗаписи + "Products" + НомерФайла + ".xml";
	ПапкаДляКартинок = ПутьККаталогуДляЗаписи + НомерФайла + "\";
	СоздатьКаталог(ПутьККаталогуДляЗаписи + НомерФайла);
		
	ФайлВыгрузки = Новый ЗаписьXML;
	ФайлВыгрузки.ОткрытьФайл(ПутьКФайлуДляЗаписи);
    ФайлВыгрузки.ЗаписатьОбъявлениеXML();    
	
	ДанныеВыгрузки = СформироватьДанные(ПакетТоваров);
	...
	Пока ДанныеВыгрузки.Следующий() Цикл
		...
		#Если Клиент Тогда
			Если Нсч % 10 = 0 Тогда
				Состояние("" + Нсч + " = " + ДанныеВыгрузки.Ссылка);
			КонецЕсли;
		#КонецЕсли	
		
		Ном = ДанныеВыгрузки.Ссылка;
		НомУИД = Строка(Ном.УникальныйИдентификатор());
		Удаление = Ном.ПометкаУдаления;
				
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("Товар");
	
		Описание = Новый Структура;
		Описание.Вставить("Идентификатор", НомУИД);
		Описание.Вставить("Наименование", Ном.Наименование);
		Если НЕ Удаление Тогда
			Описание.Вставить("НаименованиеДляСайта", Ном.НаименованиеДляСайта);
			Описание.Вставить("ОписаниеДляСайта", Ном.ТекстДляСайта);
		Иначе
			ФайлВыгрузки.ЗаписатьНачалоЭлемента("Удаление");
		КонецЕсли;
		
		ЗаписатьПараметры(ФайлВыгрузки, "Описание", Описание);
		...
		ЗаписатьКартинку(Ном, ПапкаДляКартинок);

		ФайлВыгрузки.ЗаписатьНачалоЭлемента("Структура");
		ЗаписатьСтруктуру(ФайлВыгрузки, Ном);
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
		
		ВыборкаТипДанных = ДанныеВыгрузки.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
		Пока ВыборкаТипДанных.Следующий() Цикл
			// Вставить обработку выборки ВыборкаТипДанных
			
			Т = ВыборкаТипДанных.ТипДанных;
			
			В = ВыборкаТипДанных.Выбрать();
			
			СтрукСвойства = Новый Структура;
			Аттрибуты = Новый Массив;
			АтррибутыСклад = Новый Структура;
			АтррибутыСвойства = Новый Структура;
			
			Пока В.Следующий() Цикл
				ПривПараметр = СтрЗаменить(В.Свойство.Наименование, " ", "");
				ПривПараметр = СтрЗаменить(ПривПараметр, ".", "_");
				ПривПараметр = СтрЗаменить(ПривПараметр, ",", "_");
				ПривПараметр = СтрЗаменить(ПривПараметр, "-", "_");
				ПривПараметр = "_" + ПривПараметр;
				ПривЗнач = В.Значение;
				Если ТипЗнч(ПривЗнач) = Тип("Число") Тогда
					ПривЗнач = Строка(В.Значение);
				ИначеЕсли ТипЗнч(ПривЗнач) <> Тип("Строка") Тогда	
					ПривЗнач = В.Значение.Наименование;
				КонецЕсли; 
				
				СтрукСвойства.Вставить(ПривПараметр, ПривЗнач);
				
				Если Т = "Остатки" Тогда
					АС = Новый Массив;
					АС2 = Новый Массив;
					АС2.Добавить("Название");
					АС2.Добавить(В.Свойство.Наименование);
					АС.Добавить(АС2);
					АС1 = Новый Массив;
					АС1.Добавить("Ид");
					АС1.Добавить(Строка(В.Свойство.УникальныйИдентификатор()));
					АС.Добавить(АС1);
					АтррибутыСклад.Вставить(ПривПараметр, АС);
				КонецЕсли;
				
				Если Т = "Свойства" Тогда
					АС = Новый Массив;
					АС1 = Новый Массив;
					АС1.Добавить("Название");
					АС1.Добавить(В.Свойство.Наименование);
					АС.Добавить(АС1);
					АтррибутыСвойства.Вставить(ПривПараметр, АС);
				КонецЕсли;

			КонецЦикла;	
			
			Аттрибуты.Добавить(АтррибутыСклад);
			Аттрибуты.Добавить(АтррибутыСвойства);

			ЗаписатьПараметры(ФайлВыгрузки, Т , СтрукСвойства, ,Аттрибуты);
...
КонецПроцедуры
// КЧАН
Процедура ВыгрузкаВФорматеYML(ПакетТоваров, НомерФайла)
	
	КаталогНаФТП = "ftp://tabakon.ru/tabakon.ru/public_html/obmen_1C/";
	...
	ФайлВыгрузки.ЗаписатьБезОбработки(символы.ВК);
	ФайлВыгрузки.ЗаписатьБезОбработки(символы.ПС);
	ФайлВыгрузки.ЗаписатьБезОбработки("<!DOCTYPE yml_catalog SYSTEM ""shops.dtd"">");
	
	ДатаЯ = Формат(ТекущаяДата(), "ДФ='yyyy-MM-dd ЧЧ:ММ'" ); // 2019-03-06 13:04
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("yml_catalog");
	ФайлВыгрузки.ЗаписатьАтрибут("date", ДатаЯ);	
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("shop");
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("name");
	ФайлВыгрузки.ЗаписатьТекст("Табакон");	
	ФайлВыгрузки.ЗаписатьКонецЭлемента();
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("company");
	ФайлВыгрузки.ЗаписатьТекст("Табакон");	
	ФайлВыгрузки.ЗаписатьКонецЭлемента();
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("currencies");
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("currency");
			ФайлВыгрузки.ЗаписатьАтрибут("id", "RUB");	
			ФайлВыгрузки.ЗаписатьАтрибут("rate", "1");	
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
	ФайлВыгрузки.ЗаписатьКонецЭлемента();
	...
	Запрос.Текст = 
		...
		|	Номенклатура.Родитель.Код КАК РК
		|ИЗ
		|	Справочник.Номенклатура КАК Номенклатура
		|ГДЕ
		|	Номенклатура.ЭтоГруппа
		|	И Номенклатура.ИспользоватьВИерархииСайта = ИСТИНА";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	В = РезультатЗапроса.Выбрать();
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("categories");
		
	Пока В.Следующий() Цикл
		
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("category");
		ФайлВыгрузки.ЗаписатьАтрибут("id", В.Код);
		Если В.РК <> null Тогда
			ФайлВыгрузки.ЗаписатьАтрибут("parentId", В.РК);
		КонецЕсли;	
		ФайлВыгрузки.ЗаписатьТекст(В.Наименование);		
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
	...
	ФайлВыгрузки.ЗаписатьНачалоЭлемента("offers");
	...
	Пока ДанныеВыгрузки.Следующий() Цикл
		...
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("offer"); // Товар
		ФайлВыгрузки.ЗаписатьАтрибут("id", Ном.Код) ;
		
		Если Удаление Тогда
			ФайлВыгрузки.ЗаписатьНачалоЭлемента("Удаление");
			ФайлВыгрузки.ЗаписатьТекст("ПОМЕТКА УДАЛЕНИЯ");
			ФайлВыгрузки.ЗаписатьКонецЭлемента();
			ФайлВыгрузки.ЗаписатьКонецЭлемента();
			Продолжить;
		КонецЕсли;
		
		//ФайлВыгрузки.ЗаписатьНачалоЭлемента("price");
		//ФайлВыгрузки.ЗаписатьТекст("RUB");
		//ФайлВыгрузки.ЗаписатьКонецЭлемента();

		ФайлВыгрузки.ЗаписатьНачалоЭлемента("currencyId");
		ФайлВыгрузки.ЗаписатьТекст("RUB");
		ФайлВыгрузки.ЗаписатьКонецЭлемента();

		Если Ном.Родитель <> Справочники.Номенклатура.ПустаяСсылка() Тогда
			РодительСтруктура = ПолучитьРодителяСУчетомСтруктуры(Ном.Родитель);
			ФайлВыгрузки.ЗаписатьНачалоЭлемента("categoryId");
			ФайлВыгрузки.ЗаписатьТекст(РодительСтруктура.Код);
			ФайлВыгрузки.ЗаписатьКонецЭлемента();
		КонецЕсли;	
	
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("name");
		ФайлВыгрузки.ЗаписатьТекст(Ном.Наименование);
		ФайлВыгрузки.ЗаписатьКонецЭлемента();

		ФайлВыгрузки.ЗаписатьНачалоЭлемента("article");
		ФайлВыгрузки.ЗаписатьТекст(Ном.Артикул);
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
		
		ФайлВыгрузки.ЗаписатьНачалоЭлемента("description");
		ФайлВыгрузки.ЗаписатьТекст(Ном.ТекстДляСайта);
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
		
		ИмяФ = ЗаписатьКартинку(Ном, ПапкаДляКартинок);
		Если ИмяФ <> "" Тогда
			ФайлВыгрузки.ЗаписатьНачалоЭлемента("picture");
			ФайлВыгрузки.ЗаписатьТекст(КаталогНаФТП + НомерФайла + "/" + ИмяФ);
			ФайлВыгрузки.ЗаписатьКонецЭлемента();
		КонецЕсли;	
		...
		Пока ВыборкаТипДанных.Следующий() Цикл
			...
			Если Т = "Цены" Тогда
				ФайлВыгрузки.ЗаписатьНачалоЭлемента("prices");
	            СтрокаЗ = "price";
			ИначеЕсли Т = "Свойства" Тогда
				ФайлВыгрузки.ЗаписатьНачалоЭлемента("characteristics");
	            СтрокаЗ = "characteristic";
			ИначеЕсли Т = "Остатки" Тогда
				ФайлВыгрузки.ЗаписатьНачалоЭлемента("ostatki");
	            СтрокаЗ = "ostatok";	
			КонецЕсли;
			
			Пока В.Следующий() Цикл
				
				ФайлВыгрузки.ЗаписатьНачалоЭлемента(СтрокаЗ);
				ФайлВыгрузки.ЗаписатьАтрибут("name", Строка(В.Свойство)); 
				Если Т = "Остатки" Тогда
					ФайлВыгрузки.ЗаписатьАтрибут("id", В.Свойство.Код); 
				КонецЕсли;
				Если Т = "Свойства" Тогда
					ФайлВыгрузки.ЗаписатьТекст(В.Значение.Код);
				Иначе
					ФайлВыгрузки.ЗаписатьТекст(Строка(В.Значение));
				КонецЕсли;	
				ФайлВыгрузки.ЗаписатьКонецЭлемента();
	...
	ФайлВыгрузки.ЗаписатьКонецЭлемента(); // offers
	ФайлВыгрузки.ЗаписатьКонецЭлемента(); // shop
	ФайлВыгрузки.ЗаписатьКонецЭлемента(); // yml_catalog
...
// НЧАН
Функция ПолучитьРодителяСУчетомСтруктуры(ТекРодитель)
	
	РодНач = ТекРодитель;
	
	// Вариант 1. Первая папка с использованием на сайте
	//Пока НЕ РодНач.ИспользоватьВИерархииСайта Цикл
	//	Если РодНач.Родитель = Справочники.Номенклатура.ПустаяСсылка() Тогда
	//		Прервать;
	//	КонецЕсли;	
	//	РодНач = РодНач.Родитель;
	//КонецЦикла;	
	
	// Вариант 2. Допустимы только первые 2 группы от корня дерева
	РодПред = Справочники.Номенклатура.ПустаяСсылка();
	Пока РодНач.Родитель <> Справочники.Номенклатура.ПустаяСсылка() Цикл				
		РодПред = РодНач;		
		РодНач = РодНач.Родитель;
	КонецЦикла;
	Если РодПред.ИспользоватьВИерархииСайта Тогда
		РодНач = РодПред;
	КонецЕсли;	
...
// НЧАН
Функция ЗаписатьКартинку(Ном, ПапкаДляКартинок)
	
	НомУИД = Строка(Ном.УникальныйИдентификатор());
	Кар = Ном.ОсновноеИзображение;
	ФорматКартинкиРазрешен = Истина;
	
	Если ВыгружатьКартинки И Кар <> Неопределено И Кар <> Справочники.ХранилищеДополнительнойИнформации.ПустаяСсылка() Тогда
		Картинка = Кар.Хранилище.Получить();
	Иначе 
		Возврат "";
	КонецЕсли;	
	...
	Иначе
		Если НЕ (ФорматКартинкиОбъекта = ФорматКартинки.GIF
			...
			Попытка
				ФорматКартинкиОбъекта = Картинка.Преобразовать(ФорматКартинки.JPEG);
			Исключение
				ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Ошибка,,,"Обмен с сайтом: " + "Не удалось преобразовать формат картинки. Номенклатура: " + Ном + ". Преобразование из " + Строка(ФорматКартинкиОбъекта) + " в JPEG");
				Возврат "";
			КонецПопытки;
		КонецЕсли;
		РасширениеФайлаКартинки = Строка(ФорматКартинкиОбъекта);
	...
	Если НЕ ФорматКартинкиРазрешен Тогда
		ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Ошибка,,,"Обмен с сайтом: " + "Формат основного изображения неизвестен или запрещен. Номенклатура: " + Ном);
		Возврат "";
	КонецЕсли;
	
	Попытка
		Картинка.Записать(ПапкаДляКартинок + НомУИД + "." + РасширениеФайлаКартинки);
	Исключение
		ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Ошибка,,,"Обмен с сайтом: " + "Не удалось записать файл картинки на диск. Номенклатура: " + Ном);
		Возврат "";
	КонецПопытки;	
	
	Возврат НомУИД + "." + РасширениеФайлаКартинки;
...
// НЧАН
Функция СформироватьДанные (ПакетТоваров) 
	
	//СвойстваТовара = Новый Массив;
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("1Товарная группа", Истина));
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Бренд", Истина));
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Вес", Истина));
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Гарантия", Истина));	
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Комплектация", Истина));	
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Материал", Истина));	
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Покрытие", Истина));	
	//СвойстваТовара.Добавить(ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Комплектация", Истина));	
	...
	Запрос.Текст = 
		...
		|	ЗначенияСвойствОбъектов.Свойство,
		|	ЗначенияСвойствОбъектов.Значение,
		|	Номенк.Ссылка КАК Ссылка,
		|	""Свойства"" КАК ТипДанных
		|ИЗ
		|	Номенк КАК Номенк
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
		|		ПО Номенк.Ссылка = ЗначенияСвойствОбъектов.Объект
		|ГДЕ
		...
		|						ИЗ
		|							Номенк КАК Номенк)
		|					И ТипЦен = &ИнтерентЦена) КАК ЦеныНоменклатурыСрезПоследних
		...
		|						Номенк.Ссылка
		|					ИЗ
		|						Номенк КАК Номенк)) КАК ТоварыВРозницеОстатки
		|		ПО Номенк.Ссылка = ТоварыВРозницеОстатки.Номенклатура
		|ГДЕ
	...
	Запрос.УстановитьПараметр("Объекты", ПакетТоваров);
	Запрос.УстановитьПараметр("ИнтерентЦена", Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000120"));
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаСсылка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Возврат ВыборкаСсылка;
	
КонецФункции	
// КЧАН

// НЧАН
Процедура ЗаписатьСтруктуру (ФайлВыгрузки, Ном)
	
	Родитель = Ном.Родитель;
	
	Если Родитель = Неопределено ИЛИ Родитель = Справочники.Номенклатура.ПустаяСсылка() Тогда
		Возврат;
	КонецЕсли;
				
	ПарРодителя = Новый Структура;
	ПарРодителя.Вставить("Наименование", Родитель.Наименование);
	ПарРодителя.Вставить("ИД", Строка(Родитель.УникальныйИдентификатор()));
	ЗаписатьПараметры(ФайлВыгрузки, "Родитель", ПарРодителя, Ложь);
		
	ЗаписатьСтруктуру(ФайлВыгрузки, Родитель);
...
// НЧАН
Процедура ЗаписатьПараметры(ФайлВыгрузки, Название, СтруктураПараметров, Закрывать = Истина, Атрибуты = Неопределено)
	
	ФайлВыгрузки.ЗаписатьНачалоЭлемента(Название);

	Для Каждого Пар Из СтруктураПараметров Цикл
		
		ФайлВыгрузки.ЗаписатьНачалоЭлемента(Пар.Ключ);
		Если Атрибуты <> Неопределено И Атрибуты.Количество() <> 0 Тогда
			Для Каждого Аттр Из Атрибуты Цикл
				Если Аттр.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
				Если НЕ Аттр.Свойство(Пар.Ключ) Тогда
					//Сообщить("Ошибочное свойство аттрибута = " + Пар.Ключ);
					Продолжить;
				КонецЕсли;
				ТекАтрр = Аттр[Пар.Ключ];
				Если ТекАтрр <> Неопределено Тогда
					Для Каждого Эл Из ТекАтрр Цикл
						ФайлВыгрузки.ЗаписатьАтрибут(Эл[0], Эл[1]);
					КонецЦикла;	
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;	
		ФайлВыгрузки.ЗаписатьТекст(Пар.Значение);
		ФайлВыгрузки.ЗаписатьКонецЭлемента();
