...
Процедура СформироватьСтруктуруТаблицыИмпорта(Таб, ТабПоле) Экспорт
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "СуммаКомиссии",          Т_Число15_2,"Сумма комиссии", Видимая); //Вик 2020_09_28
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "Мерчант", Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(12)),"Мерчант", Видимая); //Вик 2020_10_02
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ФактДатаЭквайринга",   Т_Дата,"Факт дата эквайринга", Видимая); //Вик 2020_10_02
	...
	Пока Лев(Врег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			Если Импорт_Загружаемые[Тег] = Истина Тогда
				...
				//Вик 2020_09_28
				Если Тег = "НАЗНАЧЕНИЕПЛАТЕЖА" тогда
					...
					Если СимволыКомиссия > 0 и ДлинаКомиссии > 0 тогда
						Попытка
							...
							СтрокаДокумента.СуммаКомиссии	=	СуммаКомиссии;
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
	...
	ПроверитьПоИННИСчету(ИННДляПоискаДок, СчетДляПоискаДок, ВидОперацииДокумента);
...
//Установка типа сделки документа
	...
	//Вик 2020_09_28
	Попытка
		ОбъектДокумента.СуммаКомиссии		=	ИсходныеДанные.СуммаКомиссии;
		ОбъектДокумента.Мерчант				=	ИсходныеДанные.Мерчант;
		ОбъектДокумента.ФактДатаЭквайринга	=	ИсходныеДанные.ФактДатаЭквайринга;
	Исключение
	КонецПопытки;
	//КонецВик 
КонецПроцедуры
...
Процедура ПроверитьПоИННИСчету(Инн, Счет, ВидОперацииДокумента)
	...
	Запрос.УстановитьПараметр("НомерСчета", Счет);
	Запрос.УстановитьПараметр("ИННКонтрагента", Инн);
	...
	Если РезультатЗапроса.Количество() = 0 Тогда
		...
		|	И БанковскиеСчета.Владелец ССЫЛКА Справочник."+?(ВидОперацииДокумента = Перечисления.ВидыОперацийППИсходящее.ПереводНаДругойСчет 
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПереводНаДругойСчет,"Организации","Контрагенты")+"
		|	"+?(НЕ ПустаяСтрока(ИННКонтрагента)," И БанковскиеСчета.Владелец.ИНН = &ИННКонтрагента","");
		...
		Если РезультатЗапроса.Количество() > 0 Тогда
			...
			Банк = Справочники.Банки.НайтиПоРеквизиту("КоррСчет", "30101810400000000225");
			КонтрагентПоИНН = РезультатЗапроса[0].Владелец;
			//добавим счет в этого контрагента
			НовыйСчет = Справочники.БанковскиеСчета.СоздатьЭлемент();
			НовыйСчет.Владелец = КонтрагентПоИНН;
			НовыйСчет.НомерСчета = Счет;
			НовыйСчет.ВалютаДенежныхСредств = Справочники.Валюты.НайтиПоКоду("643");
			НовыйСчет.Банк = Банк;
			НовыйСчет.Наименование = "Новый счет (Загрузка из Клиента банка)";
			Попытка                  			
				НовыйСчет.Записать();
				Сообщить("Создан новый счет у контрагента " + КонтрагентПоИНН + " . Не забудьте внести необходимые правки!");
			Исключение
				тест = 1;			
			КонецПопытки;
		КонецЕсли;
