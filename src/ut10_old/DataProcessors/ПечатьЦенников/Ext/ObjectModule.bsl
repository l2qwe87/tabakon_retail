...
// Параметр: 0 - печать 5х6, 1 - печать 3х3;
	Если Параметр = 0 или Параметр = 6 Тогда
	 	 КоличествоСтолбцов = 5;
		 КоличествоСтрок = 6;
	ИначеЕсли Параметр = 1 или Параметр = 7 Тогда
	...
	ИначеЕсли Параметр = 6 Тогда                              
		 Макет                      = ПолучитьМакет("Ценник2_Новый"); 
		 
	 ИначеЕсли Параметр = 7 Тогда                              
		 Макет                      = ПолучитьМакет("Ценник3_Новый"); 
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		Если СтрокаТаблицы.Печать Тогда			
			...
			Попытка
				...
				Если  Параметр = 0 или Параметр = 6 Тогда
					Если СтрДлина(цел(СтрокаТаблицы.ЦенаРознич)) > 5 тогда						
						Макет.Область("R6C13").Шрифт = Новый Шрифт(Макет.Область("R6C13").Шрифт, , 16);
						Макет.Область("R6C3").Шрифт = Новый Шрифт(Макет.Область("R6C3").Шрифт, , 16); 
					иначе
						Макет.Область("R6C13").Шрифт = Новый Шрифт(Макет.Область("R6C13").Шрифт, , 18);
						Макет.Область("R6C3").Шрифт = Новый Шрифт(Макет.Область("R6C3").Шрифт, , 18);
					КонецЕсли; 
				КонецЕсли;
				
				Если  Параметр = 1 или Параметр = 7 Тогда
					Если СтрДлина(цел(СтрокаТаблицы.ЦенаРознич)) > 5 тогда						
						Макет.Область("R4C15").Шрифт = Новый Шрифт(Макет.Область("R4C15").Шрифт, , 30);
						Макет.Область("R4C1").Шрифт = Новый Шрифт(Макет.Область("R4C1").Шрифт, , 30);
					иначе
						Макет.Область("R4C15").Шрифт = Новый Шрифт(Макет.Область("R4C15").Шрифт, , 35);
						Макет.Область("R4C1").Шрифт = Новый Шрифт(Макет.Область("R4C1").Шрифт, , 35);
					КонецЕсли; 
				КонецЕсли; 
			...
			Для Тмп = 1 По СтрокаТаблицы.Количество Цикл
				...
				//Вик 2021_01_12
				Если Параметр = 6 или Параметр = 7 тогда
					ОбластьЦенника.Параметры.ПрВо	 = "("+СокрЛП(Прав(СтрокаТаблицы.Номенклатура.НаименованиеПолное,  Индекс1-Индекс2-5))+")";
					ОбластьЦенника.Параметры.СтараяЦ =	Формат(СтрокаТаблицы.СтараяЦена,"ЧДЦ=2; ЧРД=.; ЧРГ=' '; ЧГ=0");
				КонецЕсли;
				//КонецВик
