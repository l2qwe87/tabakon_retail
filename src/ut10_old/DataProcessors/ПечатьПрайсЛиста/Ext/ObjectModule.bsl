...
Процедура ВывестиСтрокуПрайса(Выборка, НомерГруппировки, КолонкиРезультата)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЭтоНижнийУровень Тогда
			...
			//Вик 2021_03_19
			Если ВыводитьПроцентНаценки тогда
				Если ВыборкаТипЦен.Количество() = 2 тогда //ТОЛЬКО длкогда два вида цен
					ВыборкаТипЦен.Сбросить();
					ЦенаЗакупка	=	0;
					ЦенаВторая  =   0;
					Пока ВыборкаТипЦен.Следующий() Цикл
						Если ВыборкаТипЦен.ТипЦен.Код = "000000001" тогда //Закупка
							ЦенаЗакупка		=	 ВыборкаТипЦен.Цена;
						иначе
							ЦенаВторая		=	 ВыборкаТипЦен.Цена;
						КонецЕсли;;
						
					КонецЦикла;
					
					Если ЦенаЗакупка >0 тогда
						ПроцентНаценки	=	Окр((ЦенаВторая/ЦенаЗакупка - 1)*100,2);
					иначе
						ПроцентНаценки  = 0;
					КонецЕсли;
				
					Если ПроцентНаценки >0 тогда
						СекцияДопПоле.Параметры.ЗначениеДопПоля	=	Строка(ПроцентНаценки) + "%";
					иначе
...
Процедура ПечатьПрайсЛиста(ДокументРезультат) Экспорт
	...
	Если ВыводитьПроцентНаценки тогда
		Секция = Макет.ПолучитьОбласть("Шапка|ДопПоле");
		Секция.Параметры.ДопПоле = "Процент наценки";
		мТабДок.Присоединить(Секция);
	КонецЕсли;
	//КонецВик
