...
Процедура СоздатьДокументыНаСервере()
	Для каждого СТрока из Объект.ТоварыИДокументы цикл
		Если не Строка.ЭтоНеучтенка тогда
			Если Строка.Количество <> Строка.КоличествоЗамена тогда
				Сообщить("Различаются количества в строке: "+Строка.НомерСтроки,СтатусСообщения.Важное);
				Возврат;				
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	//4 Создаем Ордера на перемещение
	Если НовыйДок <> Неопределено тогда
		НовыйДок.СоздатьПерезаполнитьРО();
		НовыйДок.СоздатьПерезаполнитьПО(НовыйДок);
	КонецЕсли;
	//4
...
&НаСервереБезКонтекста
Функция ПолучитьТЗОстатковПоГруппе(Номенклатура,Склад,СсылкаООРП,Цена)
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	ТоварыВРозницеОстатки.Номенклатура,
	      	 	             |	ТоварыВРозницеОстатки.ХарактеристикаНоменклатуры КАК Характеристика,
	      	 	             |	ВЫРАЗИТЬ(ТоварыВРозницеОстатки.КоличествоОстаток КАК ЧИСЛО(3, 0)) КАК Остаток,
	      	 	             |	ЦеныАТТСрезПоследних.Цена
	      	 	             |ИЗ
	      	 	             |	РегистрНакопления.ТоварыВРознице.Остатки(&дата, ) КАК ТоварыВРозницеОстатки
	      	 	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныАТТ.СрезПоследних(&Дата, ) КАК ЦеныАТТСрезПоследних
	      	 	             ...
	      	 	             |	И ТоварыВРозницеОстатки.Номенклатура.Родитель = &Родитель
	      	 	             |	И ЦеныАТТСрезПоследних.Цена = &Цена
	      	 	             |
	      	 	             |УПОРЯДОЧИТЬ ПО
	      	 	             |	Остаток УБЫВ");	
...
&НаСервере
Процедура ПодобратьБКНаСервере(СсылкаООРП)
	Для каждого СтрокаТовары из Объект.ТоварыИДокументы цикл
		Если Прав(СтрокаТовары.Номенклатура.Наименование,3)=" БК" тогда			
			НоменклатураПодЗамену = ПолучитьНоменклатуруПодЗамену(СтрокаТовары.Номенклатура);
			
			Если НоменклатураПодЗамену.пустая() тогда
				Сообщить("Не найдена номенклатура под замену для номенклатуры:  "+СтрокаТовары.Номенклатура,СтатусСообщения.Важное);
				Продолжить;
			КонецЕсли;
			
			СтрокаТовары.НоменклатураЗамена	 =  НоменклатураПодЗамену;
			СтрокаТовары.ЕдИзмЗамена		 =	СтрокаТовары.НоменклатураЗамена.ЕдиницаХраненияОстатков;
			СтрокаТовары.КоличествоЗамена	 =	СтрокаТовары.Количество;
			
			СообщениеПроХарактеристику = "";
			Если СтрокаТовары.НоменклатураЗамена.ВестиУчетПоХарактеристикам тогда
				
				ХарактеристикаПодЗамену =  ПолучитьХарактеристику(СтрокаТовары.НоменклатураЗамена,СсылкаООРП,СтрокаТовары.Цена);
				
				Если не ХарактеристикаПодЗамену.Пустая() тогда					
					СтрокаТовары.ХарактеристикаНоменклатурыЗамена	= ХарактеристикаПодЗамену;
				иначе
					Сообщить("НЕ НАЙДЕНА ХАРАКТЕРИСТИКА в строке: "+Строка(СтрокаТовары.НомерСтроки),СтатусСообщения.Важное);
				КонецЕсли;
			КонецЕсли;
...
&НаКлиенте
Процедура ПодобратьБК(Команда)
	ПодобратьБКНаСервере(СсылкаООРП);
КонецПроцедуры

&НаСервереБезКонтекста 
Функция ПолучитьНоменклатуруПодЗамену(Номенклатура)
	НужноеНаименование = Сред(Номенклатура.Наименование,1,СтрДлина(Номенклатура.Наименование) - 3);
	
	НужнаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию(НужноеНаименование);
	Возврат НужнаяНоменклатура;
	
КонецФункции

&НаСервереБезКонтекста 
Функция ПолучитьХарактеристику(Номенклатура,Документ,Цена)
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |ГДЕ
	      	 	             |	ЦеныАТТСрезПоследних.Номенклатура = &Номенклатура
	      	 	             |	И ЦеныАТТСрезПоследних.Цена = &Цена
	      	 	             |	И ЦеныАТТСрезПоследних.Склад.ТипЦенРозничнойТорговли = &ТипЦен");
	Запрос.УстановитьПараметр("Номенклатура",Номенклатура);
	Запрос.УстановитьПараметр("ТипЦен",Документ.Склад.ТипЦенРозничнойТорговли);
	Запрос.УстановитьПараметр("Цена",Цена);
	Запрос.УстановитьПараметр("дата",Документ.Дата);
	
	Рез = Запрос.Выполнить().Выгрузить();
	Если рез.Количество() > 0 тогда
		Возврат рез[0].ХарактеристикаНоменклатуры;
	иначе
...
&НаСервереБезКонтекста
Процедура ПоказатьОстаткиПоГруппеНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьОстаткиПоГруппе(Команда)
	ТекД	=	Элементы.ТоварыИДокументы.ТекущиеДанные;
	Если ТекД <> Неопределено тогда
		Если ЗначениеЗаполнено(ТекД.Номенклатура) тогда
			
				ТЗ = ПолучитьТЗОстатковПоГруппе(ТекД.Номенклатура,ТорговаяТочка,СсылкаООРП,ТекД.Цена);
				ТЗ.Колонки.Остаток.Ширина 		 = 8;
				ТЗ.Колонки.Характеристика.Ширина = 8;
				ТЗ.Колонки.Цена.Ширина 			 = 8;
				ТЗ.Колонки.Номенклатура.Ширина 	 = 50;

				НужнаяСтрока = ТЗ.ВыбратьСтроку("Выберите строку");
				Если НужнаяСтрока <> Неопределено тогда
					ТекД.ХарактеристикаНоменклатурыЗамена	=	НужнаяСтрока.Характеристика;
					ТекД.КоличествоЗамена	=	Мин(НужнаяСтрока.Остаток,ТекД.Количество);
					ТекД.НоменклатураЗамена	=	НужнаяСтрока.Номенклатура;
					ТекД.ЕдИзмЗамена		=	ТекД.НоменклатураЗамена.ЕдиницаХраненияОстатков;
					НужнаяХар				=	НужнаяСтрока.Характеристика;					
				КонецЕсли;    	
	...
	ПоказатьОстаткиПоГруппеНаСервере();
КонецПроцедуры
