...
Процедура ПолучитьТаблицуОбеспечения(ТабДок, ПериодДляРасчетаПотребности, ОриентДатаДоставки, КоличествоДнейДляЗаказа, ФлажокП, УчитыватьПоследнийЗаказ = Ложь, ТаблицаЗаказов = Неопределено,УчитыватьТоварыВПути = Ложь) Экспорт
	...
	//	Для каждого СтрокаСклад  из РезультатСклады цикл;
			...
			//Если КоличествоПродаж = 0 И КоличестоКонечныйОстаток<МинОст Тогда
				...
				//Вик 2020_11_25
				КолВоВПути = 0;
				Если УчитыватьТоварыВПути тогда
					КолВоВПути = ПолучитьКолВоВПути(В.Склад,НоваяСтрока.Номенклатура);
					//НоваяСтрока.Потребность = Макс(0,НоваяСтрока.Потребность - КолВоВПути);
					НоваяСтрока.КолВПути = КолВоВПути;
				КонецЕсли;
				//КонецВик
				...
				если ЭтотОбъект.РасширеннаяВерсия Тогда
					...
					ОбластьМагазинСтрока.Параметры.КолВПути 	   = КолВоВПути;//Вик 2020_11_25
				КонецЕсли;
	...
	//			НоваяСтрока.Потребность  = 	(пКоличествоПродаж / пДоступенДляПродажи) * пКоэффициент;
		...
		// КЧАН
			...
			если ЭтотОбъект.РасширеннаяВерсия Тогда
				...
				ОбластьМагазинСтрока.Параметры.КолЗаказ 	   = КолВЗаказе;//Вик 2020_09_30
				ОбластьМагазинСтрока.Параметры.КолВПути 	   = КолВоВПути;//Вик 2020_09_30
				//ОбластьМагазинСтрока.Параметры.МинОст = ЛК_РаботаСДокументами.минимальныйОстаток(РезультатТовары.Номенклатура, СтрокаСклад.Склад);
			КонецЕсли;	
...
Функция ПолучитьКолВоВПути(Склад,Номенклатура)
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	ТоварыКПолучениюНаСкладыОстатки.Номенклатура,
	      	 	             |	-1 * ТоварыКПолучениюНаСкладыОстатки.КоличествоОстаток КАК Остаток
	      	 	             |ИЗ
	      	 	             |	РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки(
	      	 	             |			,
	      	 	             |			Склад = &Склад
	      	 	             |				И Номенклатура = &Номенклатура) КАК ТоварыКПолучениюНаСкладыОстатки");
	Запрос.УстановитьПараметр("Номенклатура",Номенклатура);
	Запрос.УстановитьПараметр("Склад",Склад);
