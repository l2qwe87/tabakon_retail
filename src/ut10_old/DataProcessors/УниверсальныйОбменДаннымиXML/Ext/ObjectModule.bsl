	...
	Если ИмяТипа = "Справочник" 
		...
		Если ВыборкаДляОчисткиДанных
			...
			Если (ИмяТипа = "Справочник"
				...
				КонецПопытки;
		// НЧАН
		Если УдалениеНЧАНг И ИмяТаблицыДляВыборки = "Номенклатура" Тогда
			НужнаИерархия = Ложь;
		КонецЕсли;	
		// КЧАН		
...
// 
Процедура УдалитьОбъектыТипа(ИмяТипаДляУдаления, УдалениеНЧАН = Неопределено) Экспорт
...
// НЧАН	
	...
	Если УдалениеНЧАН <> Неопределено Тогда
		УдалениеНЧАНг = Истина;
	Иначе
		УдалениеНЧАНг = Ложь;
	КонецЕсли;	
// КЧАН 
