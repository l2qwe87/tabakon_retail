...
Перем ОбъектПодключенияОБменПоТочкам; // НЧАН
////////////////////////////////////////////////////////////////////////////////
...
// НЧАН				
Если ТипЗнч(Ссылка)  <> Тип("СправочникСсылка.Склады") Тогда
// КЧАН	
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если Объект <> Неопределено Тогда
				...
				//	
				//	Если Объект.Проверено ИЛИ Объект.НеЗамещатьПриОбмене  Тогда
				//		
				//		Если ТаблицаВыполненныхОпераций <> Неопределено Тогда
...
Функция ВыполнитьВыгрузку(НужноУдалитьПодключениеКИБПриемникуПослеВыгрузкиДанных = Истина, 
	...
	ПослеОшибкиОтложенногоПроведенияПрекратитьОперации = Ложь,
	КоличествоОшибокОтложенногоПроведенияДляПрекращенияОперации = 0, СтруктураНастроекОбменаДанными = "") Экспорт
	...
	Иначе
		...
		мОбработкаДляЗагрузкиДанных = ВыполнитьПодключениеКИБПриемнику(мОбработкаДляЗагрузкиДанных, СтрокаСообщенияОбОшибке); // НЧАН
	...
	Если мПроверятьЗагруженностьПравилПриВыгрузке
		...
		Если ФлагОшибки Тогда
			...
			Если НужноУдалитьПодключениеКИБПриемникуПослеВыгрузкиДанных Тогда
				...
				ОбъектПодключенияОБменПоТочкам = Неопределено;
			КонецЕсли;
	...
	//	ЗаписьОшибок = "";
	//		
	//	Если НЕ ОбъектПодключенияОБменПоТочкам.ОбменПоТочкамАктивен() Тогда
	//		ОбъектПодключенияОБменПоТочкам.АвтообменсРозничнымиТочками(1, ЗаписьОшибок);
	//		ОбъектПодключенияОБменПоТочкам.СбросОбмена();
	//	КонецЕсли;

	//	Если ЗаписьОшибок <> "" Тогда
	//		ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Информация, , , ЗаписьОшибок, );
	//	КонецЕсли;
	
	ПроходКоличество = СтруктураНастроекОбменаДанными.ПроходКоличество;
	Если ЛК_РегламентныеЗадания.КарасныеСтрокиПеремещенияВГР(ОбъектПодключенияОБменПоТочкам) И ПроходКоличество < 3 Тогда 
		ПроходКоличество = ПроходКоличество + 1;
	Иначе
		ПроходКоличество = 0;
		#Если НЕ Клиент тогда
		ЛК_РегламентныеЗадания.ОбменГРПоТочкам(ОбъектПодключенияОБменПоТочкам);
		#КонецЕсли
	КонецЕсли;	
	СтруктураНастроекОбменаДанными.Вставить("ПроходКоличество", ПроходКоличество);
	
	Исключение
	//	ОбъектПодключенияОБменПоТочкам.СбросОбмена();
	//	ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Информация, , , "Ошибка при обмене: " + ОписаниеОшибки() , );
	КонецПопытки;
	ОбъектПодключенияОБменПоТочкам = Неопределено;
	// КЧАН
...
// НЧАН	
		ОбъектПодключенияОБменПоТочкам = ОбъектПодключения.ЛК_РегламентныеЗадания;
		//Попытка 
		//	АО = ОбъектПодключения.ЛК_РегламентныеЗадания.ОбменПоТочкамАктивен();
		//	Если АО Тогда
		//		ОбъектПодключения = Неопределено;
		//	КонецЕсли;	
		//Исключение
		//	ЗаписьЖурналаРегистрации("(ЛК) Ошибки", УровеньЖурналаРегистрации.Информация, , , "Определение фонового задания, " + ОписаниеОшибки(), );
		//КонецПопытки;
// КЧАН
