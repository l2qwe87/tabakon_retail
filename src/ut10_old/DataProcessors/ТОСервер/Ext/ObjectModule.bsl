...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Данные, СШК, ШКН = Неопределено)
	...
	//Вик 2021_02_19
	флЭтоQR = Ложь;
	Если СтрДлина(ШК) = 29 тогда //это QR
		ИсходныйQR =  ШК;
		МРЦ = ОбщегоНазначения.ПолучитьМРЦ(ШК);
		ШК = СтроковыеФункцииКлиентСервер.УдалитьЛидирующиеНули(лев(ШК,14));
		флЭтоQR = Истина;
	КонецЕсли;
	//КонецВик
...
// КЧАН
	...
	|	РегШК.СерияНоменклатуры КАК СерияНоменклатуры,
	|	РегШК.Качество КАК Качество,
	|	РегШК.Штрихкод КАК ШКН
	|ИЗ
	|	РегистрСведений.Штрихкоды КАК РегШК
	|ГДЕ
	|	РегШК.Штрихкод В(&Штрихкод)");
	Запрос.УстановитьПараметр("Штрихкод", МШК);       // НЧАН
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		РезультатЗапроса = РезультатЗапроса.Выбрать();
		РезультатЗапроса.Следующий();
		...
		Иначе // Если ТипЗнч(РезультатЗапроса.Владелец) = Тип("СправочникСсылка.Номенклатура")
			...
			Исключение
				...
				РезультатЗапроса.ЕдиницаИзмерения,
				1,
				СШК, РезультатЗапроса.ШКН);
				//++Старцева
			Исключение
				//Вик 2021_02_19
				Попытка
					НужныйОбъект  =  Клиент.ДокументОбъект;
				Исключение
					НужныйОбъект = Неопределено;
				КонецПопытки;
				
				Если ТипЗнч(НужныйОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") и флЭтоQR тогда
					НужнаяХар = РезультатЗапроса.ХарактеристикаНоменклатуры;
					Если МРЦ > 0 и не ЗначениеЗаполнено(РезультатЗапроса.ХарактеристикаНоменклатуры) тогда
						НужнаяХар = ОбщегоНазначения.ПолучитьХарактеристикуПоМРЦ(МРЦ,РезультатЗапроса.Владелец);
					КонецЕсли;
	...
	// НЧАН
	Если Не ЗначениеЗаполнено(Результат) И Лев(ШК, 2) = "01" Тогда 
		...
		Запрос.Текст = 
			...
			|	И ПеремещениеТоваров.Проведен = ЛОЖЬ
			|	И ПеремещениеТоваров.ПометкаУдаления = ЛОЖЬ";
		
		Запрос.УстановитьПараметр("Дата", ТекущаяДата() - 84600 * 14);   // 
		Запрос.УстановитьПараметр("Номер", Прав(ШК, 11));
		
		РезультатЗапроса = Запрос.Выполнить();
		
		В = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка");
		
		Если В.Количество() > 0 Тогда
				Ответ = Клиент.СШКДокумент(СШК, ШК, В);
	           	Если Ответ Тогда
					Возврат Результат;
				КонецЕсли;
		КонецЕсли;	
...
//
Функция ПолучитьИмяКомпьютераТО() Экспорт   //++ дописано
