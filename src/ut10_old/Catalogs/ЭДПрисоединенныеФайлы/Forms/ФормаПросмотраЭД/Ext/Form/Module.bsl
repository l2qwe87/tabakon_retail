...
Процедура ИзменитьВидимостьДоступность()
	...
	ЭДТитулПродавца = ЭлектронныеДокументыСлужебный.ЭтоТитулФНС(Объект.ВидЭД);
	...
	Если Объект.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если ЭлектронныеДокументыСлужебный.ЭтоТитулФНС(Объект.ВидЭД) Тогда
...
Процедура ВыполнитьПросмотрЭДИзБДСервер(Отказ = Ложь)
	...
	СтруктураПодписей.Вставить("Подписи", ЭЦП.Выгрузить(, "КомуВыданСертификат, НомерСтроки"));
