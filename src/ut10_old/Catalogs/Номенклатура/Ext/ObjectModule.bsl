...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ОбменДанными.Загрузка тогда
		Если НЕ РазрешенаЗапись() И НЕ РольДоступна("ПолныеПрава") Тогда
	...
	Если Не ОбменДанными.Загрузка И Не ЭтоГруппа Тогда
		...
		Если мЭтоНеНовый Тогда
			...
			//++ дописал И значениеЗаполнено(Ссылка.ЕдиницаХраненияОстатков)
			Если Не Услуга И значениеЗаполнено(Ссылка.ЕдиницаХраненияОстатков) И Ссылка.ЕдиницаХраненияОстатков <> ЕдиницаХраненияОстатков И ПолныеПрава.Номенклатура_СуществуютСсылки(Ссылка, СуществуютСсылки) Тогда
	...
	Если НЕ ЭтоНовый() Тогда
		мСвойстваДляПроверки = новый Массив;
		мСвойстваДляПроверки.Добавить("ТабачнаяПродукция");
		мСвойстваДляПроверки.Добавить("ВестиУчетПоХарактеристикам");
		
		Для Каждого эл из мСвойстваДляПроверки Цикл
			Если ЭтотОбъект[эл] <> ЭтотОбъект.Ссылка[эл] Тогда
				ТБК_Лог.ЗаписатьЛог(
					"Номенклатура.ПередЗаписью"
					,"["+эл+"] : ["+ ЭтотОбъект.Ссылка[эл]+"] -> ["+ЭтотОбъект[эл]+"] ["+ЭтотОбъект.Наименование+"] {"+ЭтотОбъект.Ссылка.УникальныйИдентификатор()+"}"
				);
			КонецЕсли;	
		КонецЦикла;
...
Функция РазрешенаЗапись()
	...
	Запрос.УстановитьПараметр("Настройка", ПланыВидовХарактеристик.НастройкиПользователей.НайтиПоКоду("000000116"));
