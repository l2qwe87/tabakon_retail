...
Процедура ПроверитьЧтоДляРозничныхСкладовЗаполненаСуммаПродажная(ДокументОбъект, ИмяТабличнойЧасти,
	...
	Если Константы.АвтоСозданиеЦенАТТ_тест.Получить() и НайденныеСтроки.количество() > 0 Тогда  //++
		СоздатьПереоценку(НайденныеСтроки, НайденныеСтроки[0].Склад);  //++
	КонецЕсли;                                                             //++
...
Процедура СоздатьПереоценку(НайденныеСтроки, склад) //++
	
	Дата = НачалоГода(Дата(2016,01,01))-1;
	ВидОперации = Перечисления.ВидыОперацийПереоценкаТоваровВРознице.ПереоценкаВРознице;
		
	//Поменял Вик 20200922	
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПереоценкаТоваровВРознице.Дата = &Дата
		|	И ПереоценкаТоваровВРознице.ВидОперации = &ВидОперации
		|	И ПереоценкаТоваровВРознице.Организация = &Организация
		|	И ПереоценкаТоваровВРознице.Склад = &Склад
		|	И ПереоценкаТоваровВРознице.Комментарий ПОДОБНО &Комментарий";
	
	Запрос.УстановитьПараметр("Дата", Дата);
	Запрос.УстановитьПараметр("ВидОперации", ВидОперации);
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Склад", Складзапрос);
	...
	док.Дата = НачалоГода(Дата(2016,01,01))-1;
	док.ВидОперации = Перечисления.ВидыОперацийПереоценкаТоваровВРознице.ПереоценкаВРознице;
	
	//Поменял Вик 20200922	
	//док.Организация = Справочники.Организации.НайтиПоКоду("000000100");
	док.Организация = Склад.ЮрЛицо;
	//КонецВик

	док.Склад = склад;
	док.Комментарий = "тест";
	
	Найдено = новый массив;
	Для Каждого стр из НайденныеСтроки цикл;
	  	//НайденныеСтроки[0].Номенклатура
		если Найдено.Найти(стр.номенклатура)=Неопределено Тогда
			нстр = док.Товары.Добавить();
			СоздатьЕдИзмерения(стр.Номенклатура);
			нстр.Номенклатура = стр.Номенклатура;
			нстр.ЦенаВРознице = 1;
			Найдено.Добавить(стр.Номенклатура);
		КонецЕсли;
	КонецЦикла;	
...
Процедура СоздатьЕдИзмерения(ном)
	
	Объект = ном.ПолучитьОбъект();
	флагМодифицирован = ЛОЖЬ;
	Если не ЗначениеЗаполнено(объект.БазоваяЕдиницаИзмерения) Тогда
		объект.БазоваяЕдиницаИзмерения  = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("1");
		флагМодифицирован=ИСТИНА;
	КонецЕсли;
...
//
Функция СформироватьЗапросПоПродажнымЦенам(ДатаЦен, СписокСкладов, СписокНоменклатуры, СС = Неопределено) Экспорт

// НЧАН 
//Подмена получения цен для записи в регистр "Товары в рознице" для товаров МРЦ. Для документа
// "Перемещение" продажная сумма берется из самаого документа
Если ТипЗнч(СС) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда  
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ГДЕ
	                |	ПеремещениеТоваровТовары.Ссылка = &СС";
...
//  РезультатЗапроса - результат выполненого запроса по продажным ценам.
//
Функция СформироватьЗапросПоПродажнымЦенам2(ДатаЦен, СписокСкладов, СписокНоменклатуры, СС = Неопределено) Экспорт
	...
	|			&Дата,
	|			Склад В (&СписокСкладов)
	|				И Номенклатура В (&СписокНоменклатуры)) КАК ТоварыВРозницеОстатки";
...
Функция ПолучитьШтрихКод(Владелец, ЕдиницаИзмерения = Неопределено, ХарактеристикаНоменклатуры = Неопределено,
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	И РегШтрихкоды.Штрихкод <> &Штрихкод");
