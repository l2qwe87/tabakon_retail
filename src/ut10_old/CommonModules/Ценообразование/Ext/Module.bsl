...
Функция СформироватьЗапросПоЦенам(СтруктураЗначений, СпособЗаполненияЦен, МассивНоменклатуры,
	...
	Если СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры Тогда
		...
		//Вик 2020_11_11 иначе в боевой УТ запрос отрабатывает несколько часов.. Пример - РТиУ 4912 от 10.11.20
		флУпращенныйРежим = истина;
		//если флУпращенныйРежим и не СтруктураЗначений.НовыйТипЦен.Рассчитывается тогда
		если флУпращенныйРежим тогда
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |				И СпрНоменклатура.ХарактеристикаНоменклатуры = ЦеныНоменклатуры.ХарактеристикаНоменклатуры,
			               |		Справочник.ТипыЦенНоменклатуры КАК ТипыЦенНоменклатуры
			               |	ГДЕ
			               |		СпрНоменклатура.Номенклатура В(&МассивНоменклатуры)
			               |		И ТипыЦенНоменклатуры.Ссылка = &ТипЦен) КАК БазовыеЦены
...
//Вик 2021_03_01
Функция ПолучитьЦенуАТТ (Склад,Номенклатура,Характеристика,Дата) экспорт
	Если не ЗначениеЗаполнено(дата) тогда
		дата = ТекущаяДата();
	КонецЕсли;
	...
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             ...
	      	 	             |ГДЕ
	      	 	             |	ЦеныАТТСрезПоследних.Склад = &Склад
	      	 	             |	И ЦеныАТТСрезПоследних.Номенклатура = &Номенклатура
	      	 	             |	И ЦеныАТТСрезПоследних.ХарактеристикаНоменклатуры = &ХарактеристикаНоменклатуры");
	Запрос.УстановитьПараметр("Склад",Склад);
	Запрос.УстановитьПараметр("Номенклатура",Номенклатура);
...
Функция ПолучитьПроцентНаценкиОПТ(Номенклатура,ТипЦен) экспорт
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	ПроцентыНаценкиОПТСрезПоследних.ПроцентНаценки
	      	 	             |ИЗ
	      	 	             |	РегистрСведений.ПроцентыНаценкиОПТ.СрезПоследних КАК ПроцентыНаценкиОПТСрезПоследних
	      	 	             |ГДЕ
	      	 	             |	ПроцентыНаценкиОПТСрезПоследних.Номенклатура = &Номенклатура
	      	 	             |	И ПроцентыНаценкиОПТСрезПоследних.ТипЦен = &ТипЦен");
	Запрос.УстановитьПараметр("Номенклатура",Номенклатура);
	Запрос.УстановитьПараметр("ТипЦен",ТипЦен);
	Рез = Запрос.Выполнить().Выгрузить();
	
	Если Рез.Количество() >0 тогда
		Возврат Рез[0].ПроцентНаценки;
	иначе
		Возврат 0;
	КонецЕсли;	
КонецФункции
