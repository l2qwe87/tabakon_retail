...
Процедура СброситьУстановленныйКодНомерОбъекта(ЭтотОбъект, КодНомер, ПодменюДействия = Неопределено, ПолеВводаНомера = Неопределено) Экспорт
	...
	Иначе		
		...
		Если Источник.Дата >= '20210101' и ( ТипЗнч(Источник) = Тип("ДокументОбъект.РасходныйКассовыйОрдер") или
			...
			Если ЗначениеЗаполнено(Источник.Касса) и ОбщегоНазначения.ЭтоРозничнаяКасса(Источник.Касса) тогда
				Префикс = ОбщегоНазначения.ДобавитьПрефиксКассы(Источник.Касса);
			КонецЕсли;
		...
		иначеЕсли Источник.Дата >= '20210101' и ( ТипЗнч(Источник) = Тип("ДокументОбъект.РеализацияТоваровУслуг") или
			...
			Если ЗначениеЗаполнено(Источник.Дата) тогда
				ДопПрефикс = ОбщегоНазначения.ПолучитьПрефиксПоДате(Источник.Дата);
				Префикс    = ЭтотОбъект.Организация.Префикс + ДопПрефикс;
			КонецЕсли;                                 
