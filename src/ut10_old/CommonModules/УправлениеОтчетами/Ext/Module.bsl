...
Процедура ОтправитьДокументПоЭлектроннойПочте(Документ, ИмяФайлаВложения = "", ОбъектПечати = Неопределено) Экспорт
	...
	//ЛК Андрей Добавление склада в название письма===========
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Документ", ОбъектПечати);
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗаказПоставщику.ДляКого.Ссылка КАК Ссылка
	               |ИЗ
	...
	Если Выборка.Следующий() Тогда
		ДляКого=" для склада "+Строка(Выборка.Ссылка);
		ФормаОтправкиДокумента.ТемаСообщения    = ИмяФайлаВложения + ДляКого;
   КонецЕсли;
	//=======================================================
