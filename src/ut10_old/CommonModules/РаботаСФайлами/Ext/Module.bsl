...
//процедура проверяет, существует ли на сервере папка для хранения картинок указанной номенклатуры
//название искомой папки =коду номенклатуры
//если такой папки нет, создает ее
Функция НайтиПапку(КодНомен)	Экспорт

	ПутьКпапке = Константы.ЛК_ПутьКхранилищуКартинок.Получить() + "\" + КодНомен;
...
//процедура проверяет, сохранена ли текущая "основная картинка элемента номенклатуры" в папке на сервере
Процедура СохранитьКартинку(ПутьКпапке, НаборКартинок) Экспорт
	
	//если переменная "НаборКартинок" содержит массив значений
	Если ТипЗнч(НаборКартинок)=Тип("Массив") Тогда
		Для Каждого Стр из НаборКартинок Цикл 
			ФайлИсходник=Новый Файл(Стр);
			ФайлКопия = Новый Файл(ПутьКпапке+"\"+ФайлИсходник.Имя);
			Если Не ФайлКопия.Существует() Тогда
				Картинка=Новый Картинка(Стр);
				Картинка.Записать(ФайлКопия.ПолноеИмя);
			КонецЕсли;
		КонецЦикла;
	//если переменная "НаборКартинок" содержит ссылку на элемент справочника
	ИначеЕсли ТипЗнч(НаборКартинок)=Тип("СправочникСсылка.ХранилищеДополнительнойИнформации") Тогда
		ФайлКопия = Новый Файл(ПутьКпапке+"\"+ НаборКартинок.ИмяФайла);
		Если Не ФайлКопия.Существует() Тогда
			Хран=НаборКартинок.Хранилище;
			Картинка=Хран.получить();	
	        Картинка.Записать(ФайлКопия.ПолноеИмя);
		КонецЕсли;
	КонецЕсли;
...
//процедура устанавливает новую картинку в качестве "основной картинки элемента номенклатуры"
Процедура СделатьКартинкуОсновной(ОбъектВладелец, ОсновноеИзображение, ФайлПуть,ФайлИмя) Экспорт

	ДанныеКартинки = Новый Картинка(ФайлПуть, Ложь);
	НоваяКартинка= Новый ХранилищеЗначения(ДанныеКартинки);	
	ТекущаяКартинка = ОсновноеИзображение.ПолучитьОбъект();
	ТекущаяКартинка.Хранилище = НоваяКартинка;
	ТекущаяКартинка.ИмяФайла = ФайлИмя;
	ТекущаяКартинка.Записать();	
