...
Процедура СписаниеПартий(СтрокаДокумента, СтруктураПараметров, РегистрУчета)
	...
	Если СтрокаДереваПартий <> Неопределено Тогда
		...
		Для Каждого СтрокаПартииРаспределения ИЗ СтрокаДереваПартий.Строки Цикл
			...
			//Вик 20201109
			Если ТипЗнч(СтруктураПараметров.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
				Если ЗначениеЗаполнено(СтруктураПараметров.Регистратор.Контрагент.Родитель) тогда
					Если СтруктураПараметров.Регистратор.Контрагент.Родитель.Код = "АВ0011802" тогда  //папка наши Юр Лица
						СтрокаПартии.СтатусПартии = Перечисления.СтатусыПартийТоваров.Купленный;	
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			//КонецВик
...
Процедура ВыполнитьДопДвиженияПоРезультатуСписанияУпр(СтруктураПараметров, ТаблицаСписания)
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВУправленческомУчете Тогда
		//Вик 20201109
		флНеНадоПроводитьПоРеализованным = Ложь;
		Если ТипЗнч(СтруктураПараметров.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
			Если ЗначениеЗаполнено(СтруктураПараметров.Регистратор.Контрагент.Родитель) тогда
				Если СтруктураПараметров.Регистратор.Контрагент.Родитель.Код = "АВ0011802" тогда  //папка наши Юр Лица
					флНеНадоПроводитьПоРеализованным = Истина;	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		//КонецВик
		...
		Если  не флНеНадоПроводитьПоРеализованным тогда 
...
Процедура ДвижениеПартийТоваров(ОбрабатываемыйДокумент, ТаблицаСписания = Неопределено,
								...
								СтруктураНаборовЗаписей = Неопределено, ПаузаПриПроведенииДнем =ложь) Экспорт
...
Функция ПолучитьДеревоПартийНаСкладахУпр (МоментКон, СтруктураПараметров)
	...
	///++ вычитал что типа дико ускоряет ниже код
	Запрос.Текст =
	"ВЫБРАТЬ
	|    *
...
Процедура ВыполнитьВосстановлениеНаСервере (ДатаКонцаОбработки,
	...
	ЗаполнениеДокументов.ВыполнитьВосстановление (ДатаКонцаОбработки,
						...
						Останавливаться,
						МаксимальноеКоличествоДокументовВТранзакции,
						МаксимальноеКоличествоСтрокВТранзакции, ПаузаПриПроведенииДнем);
