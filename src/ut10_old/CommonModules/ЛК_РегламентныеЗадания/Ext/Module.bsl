...
Процедура ПроведениеПоПартиямНаСервере() Экспорт
	...
	ДатаКонцаОбработки = НачалоОбработки;
	...
	Попытка
		УправлениеЗапасамиПартионныйУчет.ВыполнитьВосстановлениеНаСервере(ДатаКонцаОбработки, 
			Справочники.Организации.ПустаяСсылка(), ОтражатьВУправленческомУчете, Истина, Истина,
			Останавливаться, 1000, 1000, ПаузаПриПроведенииДнем);
	Исключение
