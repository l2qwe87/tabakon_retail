...
Процедура УстановитьЗаголовокФормыДокумента(СтрокаВидаОперации = "", ДокументОбъект, ФормаДокумента) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") Тогда
		Если ДокументОбъект.АктПереданВЕГАИС Тогда
			Заголовок = Заголовок + ". Акт передан в ЕГАИС";
			Если ДокументОбъект.ПереданВЕГАИССРасхождениями Тогда
				Заголовок = Заголовок + " с расхождениями."
			Иначе
				Заголовок = Заголовок + ".";
			КонецЕсли;
		КонецЕсли;
