		...
		Для Каждого ЭлементСоответствия Из СоответствиеФайлПараметры Цикл
			...
			Для Каждого ПутьКПодписи Из ЭлементСоответствия.Значение.Подписи Цикл
				...
				Для Каждого ФайлПодписи Из МассивФайлов Цикл
					ДанныеПодписи = ЭлектронныеДокументыВнутренний.СтруктураДанныхПодписи(ФайлПодписи, МенеджерКриптографии);
	...
	Попытка
		...
		Если ПолучитьФайлДанныхДляОбработки(СтруктураДанных, ПапкаДляРасшифровки, ЭтоПроизвольныйЭД) Тогда
			...
			Если НЕ ЭтоПроизвольныйЭД Тогда
				...
				ИначеЕсли СтруктураФайлаЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
					...
					ОтменитьТранзакцию();
					ТекстОшибки = НСтр("ru = 'Ошибка распаковки пакета:
						|%1
						|Для полученного ответного титула (%2) не найден исходящий ЭД.
						|Возможные причины:
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ПакетЭД, СтруктураФайлаЭД.ВидЭД);
					ТекстСообщения = СтрЗаменить(НСтр("ru = 'Ошибка распаковки пакета: %1 (подробности см. в Журнале регистрации).'"), "%1", ПакетЭД);
					ОбработатьИсключениеПоЭДНаСервере(НСтр("ru = 'Распаковка пакета.'"), ТекстОшибки, ТекстСообщения);
					ФлагОшибки = Истина;
					Возврат Неопределено;
				КонецЕсли;
			...
			Иначе
				Попытка
					...
					Иначе
						...
						ДокументСообщение.Текст           = СоответствиеФайлПараметры.Получить("Текст");
			...
			Если НЕ ФлагОшибки Тогда
				...
				Если ЗаписатьНовыйЭД Тогда
					...
					Если ЗначениеЗаполнено(ДобавленныйФайл) Тогда
						...
						Если Не ЭлектронныеДокументыСлужебный.ЭтоОтветныйТитул(СтруктураФайлаЭД.ВидЭД) Тогда
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Иначе
			...
			Если Не ЕстьПравоНастройкиЭДО(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ru = '%1 «Недостаточно прав для настройки ЭДО»'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
	...
	Если ЭлектронныеДокументыКлиентСервер.ЕстьДействие(Действия, "Утвердить") Тогда
		...
		Для Каждого ТекСтрока Из ТЗ_ЭД Цикл
			...
			Если ЭлектронныеДокументыСлужебный.ЭтоТитулФНС(ТекСтрока.ВидЭД) Тогда
	...
	Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.АктИсполнитель
		...
		Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
			...
			Если ПространствоИменСхемы = "TORGPR" Тогда // Передача товаров, акт результатов работ (Исполнитель).
				ЛицоГрузПолучил = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Файл.Документ.СодФХЖ3.СвЛицОтпГруз", ПространствоИменСхемы);
				СведенияЛица = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Файл.Документ.СодФХЖ3.СвЛицОтпГруз.РабОргПрод", ПространствоИменСхемы);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведенияЛица, "Должность", ДолжностьПоСертификату, Истина, ТекстОшибки);
				...
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ЛицоГрузПолучил, "РабОргПрод", СведенияЛица, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СодФХЖ3, "СвЛицОтпГруз", ЛицоГрузПолучил, Истина, ТекстОшибки);
		...
		ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
			...
			Если ПространствоИменСхемы = "TORGPOK" Тогда // Передача товаров, акт результатов работ (Исполнитель).
				ЛицоГрузПолучил = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Файл.Документ.СодФХЖ4.ГрузПолучил.СвЛицПолГруз", ПространствоИменСхемы);
				СведенияЛица = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Файл.Документ.СодФХЖ4.ГрузПолучил.СвЛицПолГруз.РабОргПок", ПространствоИменСхемы);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведенияЛица, "Должность", ДолжностьПоСертификату, Истина, ТекстОшибки);
				ФИО = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("ФИОТип", ПространствоИменСхемы);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", ФамилияСертификата, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Имя", ИмяСертификата, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведенияЛица, "ФИО", ФИО, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ЛицоГрузПолучил, "РабОргПок", СведенияЛица, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СодФХЖ4.ГрузПолучил, "СвЛицПолГруз", ЛицоГрузПолучил, Истина, ТекстОшибки);
			Иначе
...
Функция РеквизитыСертификата(СертификатЭП) Экспорт
	...
	ПараметрыСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СертификатЭП,
		...
		|Наименование, ОграничитьДоступКСертификату, Пользователь, ПрограммаБанка, ПометкаУдаления");
