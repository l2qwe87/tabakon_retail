...
Процедура ЗагрузитьВТаблицуЗначений(ТаблицаИсточник, ТаблицаПриемник) Экспорт
	...
	Для каждого СтрокаТаблицыИсточника Из ТаблицаИсточник Цикл
		...
		//++
		если ТаблицаИсточник.колонки.найти ("ДатаПродажи") <> Неопределено Тогда
			Если ЗначениеЗаполнено(СтрокаТаблицыИсточника["ДатаПродажи"]) Тогда
				СтрокаТаблицыПриемника.Период = СтрокаТаблицыИсточника["ДатаПродажи"];
			КонецЕсли;
		КонецЕсли;	
...
Процедура ДобавитьПрефиксОрганизации(ДокументОбъект, Префикс) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Префикс) тогда
			Префикс = ДокументОбъект.Организация.Префикс + Префикс;
		иначе
			Префикс = ДокументОбъект.Организация.Префикс;
		КонецЕсли;		
		//КонецВик 
	...
	//ЛК Андрей Добавление для обмена установкой цен
	Если ТипЗнч(ДокументОбъект)=Тип("ДокументОбъект.ПереоценкаТоваровВРознице") Тогда
			Префикс="УТ";
	КонецЕсли;
...
Процедура ПриУстановкеНовогоНомераДокументаПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс) Экспорт
	//Вик 2021_02_01
	Если Источник.Дата >= '20210101' и ( ТипЗнч(Источник) = Тип("ДокументОбъект.РасходныйКассовыйОрдер") или
		ТипЗнч(Источник) = Тип("ДокументОбъект.ПриходныйКассовыйОрдер") ) тогда
		
		
		Если ЗначениеЗаполнено(Источник.Касса) и ЭтоРозничнаяКасса(Источник.Касса) тогда
			Префикс = ДобавитьПрефиксКассы(Источник.Касса);
	...
	иначеЕсли Источник.Дата >= '20210101' и ( ТипЗнч(Источник) = Тип("ДокументОбъект.РеализацияТоваровУслуг") или
		ТипЗнч(Источник) = Тип("ДокументОбъект.ПеремещениеТоваров") или ТипЗнч(Источник) = Тип("ДокументОбъект.СчетФактураВыданный") ) тогда
		
		Если ЗначениеЗаполнено(Источник.Дата) тогда
			ОбщегоНазначения.ДобавитьПрефиксОрганизации(Источник, Префикс);
			ДопПрефикс = ПолучитьПрефиксПоДате(Источник.Дата);
			Префикс    = Префикс + ДопПрефикс;
		КонецЕсли;                                 
...
Функция СформироватьЗапросПоТабличнойЧасти(ДокументОбъект, ИмяТабличнойЧасти, СтруктураПолей,
	...
	// НЧАН
		Если  ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОприходованиеТоваров") Тогда
			СтруктураПолей.Вставить("Количество", "Количество * Док.Коэффициент / ВЫБОР КОГДА Док.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент Есть Null ТОГДА Док.Коэффициент ИНАЧЕ Док.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент КОНЕЦ");			
		КонецЕсли;		
	// КЧАН
    ...
    //++
	Если СтруктураПолей.свойство("Количество") Тогда
		Запрос.Текст = Запрос.Текст + " и Док.Количество <> 0 ";
	КонецЕсли;	
...
КонецФункции
//Вик
Процедура ПроставитьОрганизациюПриИзмененииСклада(Склад,Организация) экспорт
	Организация = Склад.ЮрЛицо;
КонецПроцедуры
//КонецВик

//Вик 2020_12_08
Процедура УстановитьФильтрПоСкладу(ИмяРеквизита, Список, ФормаСписок, Склады, _ВидСравнения = Неопределено, Доступность = Неопределено, флНужноДобавитьАНеОчистить = Ложь) Экспорт
	Перем Отбор;
	Перем Стр;
	
	Если _ВидСравнения = Неопределено Тогда
		_ВидСравнения = ВидСравнения.ВСписке;
	КонецЕсли;
	Отбор = Список.Отбор;
	Найдено = Ложь;
	Для Каждого Стр Из Отбор Цикл
		Если Стр.Имя = ИмяРеквизита Тогда
			...
			Стр.ВидСравнения = _ВидСравнения;
			//Вик 20170712
			Если флНужноДобавитьАНеОчистить Тогда
				Если не ТипЗнч(Склады) = Тип("Массив") и не ТипЗнч(Склады) = Тип("ТаблицаЗначений") тогда
					Стр.Значение = Склады;
				иначе
					Для каждого Строка Из Склады цикл
						Стр.Значение.Добавить(Строка.Ссылка);	
					КонецЦикла;
				КонецЕсли;
			иначе
				Стр.Значение.ЗагрузитьЗначения(Склады);
			КонецЕсли; 
			//КонецВик
			Прервать;
		КонецЕсли;
	...
	Если Не Найдено Тогда
		Стр = Отбор.Добавить(ИмяРеквизита);
		Стр.Имя = ИмяРеквизита;
		Стр.Использование = Истина;
		Стр.ВидСравнения = _ВидСравнения;
		...
		Если флНужноДобавитьАНеОчистить Тогда
			...
			КонецЦикла;
		иначе
			Стр.Значение.Загрузить(Склады);
		КонецЕсли; 
		
	КонецЕсли;
	НастройкаОтбора = ФормаСписок.НастройкаОтбора;
	Стр = НастройкаОтбора.Найти(ИмяРеквизита);
	Если Стр <> Неопределено Тогда
		Если Доступность = Неопределено Тогда
			Стр.Доступность = Ложь;
		Иначе
			Стр.Доступность = Доступность;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
//КонецВик
...
//Вик 2021_01_11
Функция ПолучитьСкладыПоТипу(ВидСклада)экспорт
...
Функция ПолучитьЯчейку(Номенклатура) экспорт
	Запрос	=	Новый запрос("ВЫБРАТЬ
	...
	Если Рез.Количество() >0 тогда
		Возврат Рез[0].Ячейка;
	иначе
		Возврат "";
	КонецЕсли; 	
КонецФункции

Функция СкопироватьФайл_ОбщегоНазначения(ИмяФайлаИсточник, ИмяФайлаНазначение, ВыдаватьСообщения = Ложь, флУдалять = Ложь) Экспорт
	Попытка
...
Функция ЭтоРозничнаяКасса(Касса) экспорт
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	      	 	             |	Склады.Ссылка
	      	 	             |ИЗ
	      	 	             |	Справочник.Склады КАК Склады
	      	 	             |ГДЕ
	      	 	             |	Склады.КассаСклада = &КассаСклада
	      	 	             |	И Склады.ВидСклада = &ВидСклада");
	Запрос.УстановитьПараметр("КассаСклада",Касса);
	Запрос.УстановитьПараметр("ВидСклада",Перечисления.ВидыСкладов.Розничный);
	
	Возврат не Запрос.Выполнить().Пустой();	
КонецФункции

Функция ДобавитьПрефиксКассы (Касса) экспорт
	...
	НужныйПрефикс = "";
	Если Рез.Количество() >0 тогда
		НужныйПрефикс = Лев(Рез[0].Номер,5);	
	КонецЕсли;    
	
	Возврат НужныйПрефикс;	
КонецФункции

Функция ПолучитьПрефиксПоДате(Дата) Экспорт
...
Функция ПолучитьХарактеристикуПоМРЦ(МРЦ,Владелец) экспорт 
	Запрос	=	Новый Запрос("ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Наименование",Строка(МРЦ));
	рез = Запрос.Выполнить().Выгрузить();
	Если Рез.Количество() >0 тогда
