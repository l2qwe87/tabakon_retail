...
Процедура ИзменитьЦеныВалюту(ДокументОбъект, СпособЗаполненияЦен, СтруктураРеквизитовДокумента, 
	...
	//Если СтруктураЗначений.ПерезаполнитьЦеныПоТипу или СтруктураЗначений.ПересчитатьЦеныПоВалюте тогда //Вик 20200923
...
Функция НапечататьПриходРасходСРозничныхСкладовВЦенахАТТ(Док, ВалютаРегламентированногоУчета, ЭтоПриход) Экспорт
	...
	Иначе
		...
		Пока ВыборкаСклады.Следующий() Цикл
			...
			ОбластьСклад.Параметры.Комментарий = Док.Комментарий;
...
Процедура КорректировкаРегистрацииВПоследовательности()
	...
	Для Каждого ТекПоследовательность Из Метаданные.Последовательности Цикл
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если выборка.проведен = NULL Тогда   //++
				Продолжить;
			КонецЕсли;	                         //++
...
Процедура ВыполнитьВосстановление (ДатаКонцаОбработки,
								  ...
								  МаксимальноеКоличествоСтрокВТранзакции, ПаузаПриПроведенииДнем = ложь) Экспорт
	...
	//++граница бывает скидывается. Проверим. Если скинулась - принудительно установим на значение из константы
	Попытка
		Если началодня(Последовательности.ПартионныйУчет.ПолучитьГраницу().дата) = дата (2016,01,01) Тогда
			док = Константы.ТекГраница.Получить();
			Если ЗначениеЗаполнено(док) тогда
				ДокументМоментВремени = Новый МоментВремени(док.Дата, док);
		...
		Иначе
			...
			Если (Бух ИЛИ Нал ИЛИ Упр)
				...
				УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров (ДокументСсылка,
									  ...
									  Истина,
									  СтруктураНаборовЗаписей, ПаузаПриПроведенииДнем);
	...
	// НЧАН   Тест
	//Если Формат(Год(ДокументСсылка.Дата),"ЧГ=0") <> "2018" Тогда
	//	ЗаписьЖурналаРегистрации("(ЛК) Тест", УровеньЖурналаРегистрации.Информация, , , ДокументСсылка , ); 
	//КонецЕсли;
	// КЧАН
		...
		Если ОбработаноСтрок >= МаксимальноеКоличествоСтрокВТранзакции Тогда
			...
			Константы.ТекГраница.Установить(ДокументСсылка); //++
			...
			ПаузаПриПроведенииДнем = ИСТИНА ; // НЧАН
			//++
			Если час(ТекущаяДата())> 9 и час(ТекущаяДата())< 20 и ПаузаПриПроведенииДнем Тогда
				ПаузаКонст = Константы.ПаузаДляПартий.Получить();
				ЛК_РаботаСДокументами.Пауза(ПаузаКонст);
			Иначе
				ПаузаНочь = Константы.ПаузаДляПартийНочь.Получить();
				ЛК_РаботаСДокументами.Пауза(ПаузаНочь);
			КонецЕсли;
