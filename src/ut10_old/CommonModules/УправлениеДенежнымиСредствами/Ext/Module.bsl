...
Функция ПолучитьТаблицуПлатежейУпр(ДатаДокумента,ВалютаДокумента,Документ, ВидПлатежногоДокумента, ВидОперацииДокумента = Неопределено) Экспорт
//pozdniakov.a>	
...
//<pozdniakov.a
	...
	Если ВидОперацииДокумента = Перечисления.ВидыОперацийРКО.ВыплатаСотруднику Тогда
		Запрос.УстановитьПараметр("ПустыеСделки",Неопределено);
		Запрос.УстановитьПараметр("ПустыеРасчетныеДокументы",Неопределено);
	Иначе
		Запрос.УстановитьПараметр("ПустыеСделки",МассивПустыхСделок);
		Запрос.УстановитьПараметр("ПустыеРасчетныеДокументы",МассивПустыхРасчетныхДокументов);
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаявкеУпр(ДокументОбъект,ДокументОснование,мТекущийПользователь) Экспорт
	...
	|	ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СуммаНДС
	|ИЗ
	...
	|		ПО ЗаявкиНаРасходованиеСредствОстатки.ДоговорКонтрагента = ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.ДоговорКонтрагента
	|			И ЗаявкиНаРасходованиеСредствОстатки.СтатьяДвиженияДенежныхСредств = ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств
	|			И ЗаявкиНаРасходованиеСредствОстатки.СуммаОстаток = ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СуммаПлатежа
	|ГДЕ
	|	ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.Ссылка = &ДокументЗаявка";
	
	Запрос.УстановитьПараметр("ДокументЗаявка",ДокументОснование);  // НЧАН ред запрос
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаПлатеж.СтавкаНДС = Выборка.СтавкаНДС;   // НЧАН
			СтрокаПлатеж.СуммаНДС = Выборка.СуммаНДС;     // НЧАН
...
//<pozdniakov.a
	ИначеЕсли ВидОперацииДокумент=Перечисления.ВидыОперацийРКО.ВыплатаСотруднику Тогда
...
Процедура ПровестиСписаниеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	Если ОтраженоВОперУчете И НЕ ПоРасчетномуДокументу И НЕ ПоОбъявлениюНаВзносНаличными Тогда
		...
		Для Каждого СтрокаПлатеж ИЗ ДвиженияПоЗаявкам Цикл
			...
			// НЧАН
			// По просьбе Ирины для отчета "Расчеты с контрагентами".
			//ИспользоватьСтандартныйМеханизм = СтруктураШапкиДокумента.ВидДокумента <> "ПлатежноеПоручениеИсходящее";
			ИспользоватьСтандартныйМеханизм = Истина;
			// КЧАН
		
			Если НЕ СтрокаПлатеж.ДокументПланированияПлатежа.Пустая() И ИспользоватьСтандартныйМеханизм Тогда
