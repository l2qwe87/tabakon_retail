...
Процедура КонтрольСвободныхОстатков_Перемещение()
	...
	Запрос.УстановитьПараметр("Дата",		 мСтруктураШапкиДокумента.Дата); //Вик 2020_12_20	
	...
	|		) КАК ТЧ
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&Дата, Склад = &СкладОтправитель И
	|			Номенклатура В (%ВыборкаПоНоменклатуре%)
	|			) КАК ВремРезервы
	|	ПО ТЧ.Номенклатура = ВремРезервы.Номенклатура 
	|		// СОЕДИНЕНИЕ_Характеристика_ВремРезервы
	...
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки(&Дата,
	|			Склад = &СкладОтправитель
