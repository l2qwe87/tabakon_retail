...
//  Нет.
//
Процедура ВыполнитьРасход(ЗаполнитьПериод = истина) Экспорт

	ОбщегоНазначения.ВыполнитьДвижениеПоРегистру(ЭтотОбъект, ВидДвиженияНакопления.Расход,,ЗаполнитьПериод);
...
Процедура КонтрольОстатков_СкладВШапке_ЗаказВШапке()
	...
	//Вик 2021_03_12 включаем контроль остатков при неоперативном проведении для комплектаций
	Если ТипЗнч(СтруктураШапкиДокумента.Ссылка) = Тип("ДокументСсылка.КомплектацияНоменклатуры") тогда
		Запрос.УстановитьПараметр("Дата", новый МоментВремени(СтруктураШапкиДокумента.Ссылка.Дата,СтруктураШапкиДокумента.Ссылка));
	иначе
	...
	//таблица остатков товаров с учетом серий
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРознице.Остатки(&Дата, Склад = &Склад И
	...
	Иначе  //Если ЗначениеНеЗаполнено(Заказ) Тогда
		Если ЕстьСпособСписания Тогда
			...
			Если ЕстьСерия и ИспользоватьУказаниеСерийНоменклатурыПриРезервировании Тогда
				...
				|		) КАК ТЧ
				|	ЛЕВОЕ СОЕДИНЕНИЕ 
				|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&Дата, Склад = &Склад И ДокументРезерва = &Заказ  И
				|			%ВыборкаПоНоменклатуре%
				|			) КАК ВремРезервы
				|	ПО ТЧ.Номенклатура = ВремРезервы.Номенклатура 
				|		//СОЕДИНЕНИЕ_Характеристика_ВремРезервы
		...
		Иначе //Если ЕстьСпособСписания Тогда
			...
			ТекстЗапросаРезервыПоДокументу = "
            |	ЛЕВОЕ СОЕДИНЕНИЕ 
			|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&Дата, Склад = &Склад И ДокументРезерва = &Заказ
