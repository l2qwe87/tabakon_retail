...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабЧасти, СтруктураШапки, ОтказПроведения, ЗаголовокСообщения) Экспорт
	...
	// НЧАН
	ТПОбмен = СокрЛП(Пользователи.ТекущийПользователь().Код) = "obmen";    // Закомментил так ка не дали добро на выполнение
	ПравоПревышОст = УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровНаСкладе();
	ОтветНЕБулево = ТипЗнч(ПравоПревышОст) <> Тип("Булево");
	Если ОтветНЕБулево И ТПОбмен Тогда
		ПравоПревышОст = Истина;
	КонецЕсли;	
	// КЧАН
	
	Если ПравоПревышОст Тогда
		Возврат;
	КонецЕсли;
...
Процедура КонтрольОстатков_ПеремещениеТоваров()
	...
	//таблица остатков на складах с учетом серий номенклатуры   //++добавлена дата
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки(&Дата, Склад = &Склад И
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&Дата, Склад = &Склад И
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки(&Дата, Склад = &Склад И
	|		%ВыборкаПоНоменклатуре%
	|	//УСЛОВИЕ_Качество
	|	) КАК ТоварыКПередаче
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки(&Дата, Склад = &Склад И
	|		%ВыборкаПоНоменклатуре% 
...
Процедура КонтрольОстатков_Реализация_ОтчетОРознПродажах_ЧекККМ(ДокументОбъект)
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки(&Дата, Склад в (&СписокСкладов) И
	|		%ВыборкаПоНоменклатуре%  
	|		//УСЛОВИЕ_Качество
	|	) КАК Остатки
	|ПО 
	...
	//++
	//Мом = новый МоментВремени(ДокументОбъект.МоментВремени()
	//Граница = новый Граница(ДокументОбъект, ВидГраницы.Исключая);
	Запрос.УстановитьПараметр("Дата", новый граница(ДокументОбъект.МоментВремени(), ВидГраницы.Исключая)); 
...
Процедура КонтрольОстатков_ЗаказВТабЧасти()
	...
	Запрос.УстановитьПараметр("Дата", 					СтруктураШапкиДокумента.Дата);//Вик 2020_12_22
...
Процедура ОбработатьРезультатЗапроса(Выборка,ЕстьСерия,ЕстьРезервыПоДокументуБезСерии)
	Пока Выборка.Следующий() Цикл
		...
		//++
		Если Выборка.ДокументКоличество = null  Тогда
			Продолжить;
		Конецесли;	
