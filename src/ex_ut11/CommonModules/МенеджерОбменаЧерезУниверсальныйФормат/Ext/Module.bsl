...
Функция ДополнитьВыгрузку_Себестоимость(ДанныеИБ, ДанныеXDTO)
	...
	Попытка
		чек = ДанныеXDTO[имяТЧ];
	Исключение
		Возврат ложь; // нет ТоварыПродажа  или  Товары
	КонецПопытки;
...
Процедура TBK_ПослеМержевания_ПКО(ПолученныеДанные, ДанныеИБ, КомпонентыОбмена)
	...
	//Вик 2021_04_01
	Попытка
		ДанныеИБ.Касса = ПолученныеДанные.Касса;
	Исключение
	КонецПопытки;
	//КонецВик
...
//Вик 2021_04_01
&После("Мержевание_РКО")
Процедура TBK_Мержевание_РКО(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	Попытка
		ДанныеИБ.Касса = ПолученныеДанные.Касса;
	Исключение
	КонецПопытки;
...
&Вместо("ЗаполнитьКассуПоВалютеИОрганизации")
Процедура TBK_ЗаполнитьКассуПоВалютеИОрганизации(ПолученныеДанные, КомпонентыОбмена, НаименованиеПодразделения)
	//Касса "ДОЛГИ" принадлежит организации Табакон. поэтому не будем ее сбрасывать на Основную кассу организации при различии во владельцах.
	флЗаходим = истина;
	Попытка
		Если Найти(НРег(ПолученныеДанные.Касса.Наименование),"долги") тогда
			флЗаходим = Ложь;
		КонецЕсли;
	Исключение
	КонецПопытки;
	
	Если флЗаходим тогда
		ПродолжитьВызов(ПолученныеДанные, КомпонентыОбмена, НаименованиеПодразделения);
	КонецЕсли;
КонецПроцедуры
//КонецВик

&Вместо("ПКО_Документ_РеализацияТоваровУслуг_Получение_ПередЗаписьюПолученныхДанных")
Процедура TBK_ПКО_Документ_РеализацияТоваровУслуг_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	// Вставить содержимое метода.
	...
	Если ТипЗнч(ДанныеДляЗаписиВИБ) = Тип("Структура") Тогда
		Если ДанныеДляЗаписиВИБ.Свойство("СкидкиРассчитаны") Тогда
			ДанныеДляЗаписиВИБ.Вставить("СкидкиРассчитаны");
		КонецЕсли;
	КонецЕсли;
	ДанныеДляЗаписиВИБ.СкидкиРассчитаны = истина;
КонецПроцедуры

&Вместо("ПКО_Справочник_БанковскиеСчетаКонтрагентов_Получение_ПередЗаписьюПолученныхДанных")
Процедура TBK_ПКО_Справочник_БанковскиеСчетаКонтрагентов_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	// Вставить содержимое метода.
	ПродолжитьВызов(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена);
	...
	Если ПроверитьНаличиеСвойства(ДанныеДляЗаписиВИБ, "ВалютаДенежныхСредств") Тогда
		Если НЕ ЗначениеЗаполнено(ДанныеДляЗаписиВИБ.ВалютаДенежныхСредств) Тогда
			ДанныеДляЗаписиВИБ.ВалютаДенежныхСредств = ПолучитьВалютуРубли();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьНаличиеСвойства(ДанныеДляЗаписиВИБ, имяСвойства)
	
	Если ТипЗнч(ДанныеДляЗаписиВИБ) = Тип("Структура") Тогда
		Если ДанныеДляЗаписиВИБ.Свойство(имяСвойства) Тогда
			ДанныеДляЗаписиВИБ.Вставить(имяСвойства);
		КонецЕсли;
	КонецЕсли;
	
	Попытка
		чек = ДанныеДляЗаписиВИБ[имяСвойства];
	Исключение
		Возврат ложь;
	КонецПопытки;
	возврат истина;
...
&Вместо("ПКО_Справочник_ХарактеристикиНоменклатуры_ПриОтправкеДанных")
Процедура TBK_ПКО_Справочник_ХарактеристикиНоменклатуры_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	// Вставить содержимое метода.
	...
	//Если НЕ ДанныеXDTO.Свойство("Наименование") ИЛИ НЕ ДанныеXDTO.Свойство("НаименованиеПолное") Тогда
	//	ДанныеXDTO = неопределено;
	//	Возврат;
	//КонецЕсли;
	//
	//Если НЕ ЗначениеЗаполнено(ДанныеXDTO.Наименование) или НЕ ЗначениеЗаполнено(ДанныеXDTO.НаименованиеПолное) Тогда
	//	ДанныеXDTO = неопределено;
	//	Возврат;
	//КонецЕсли;
	
	//
	//Если НЕ ДанныеXDTO.Свойство("НаименованиеПолное") Тогда
	//	ДанныеXDTO = неопределено;
	//	Возврат;
	//КонецЕсли;
	//
	//Если НЕ ЗначениеЗаполнено(ДанныеXDTO.НаименованиеПолное) Тогда
	//	ДанныеXDTO = неопределено;
	//	Возврат;
	//КонецЕсли;
КонецПроцедуры

&Вместо("ПКО_Справочник_ХарактеристикиНоменклатуры_ПриКонвертацииДанныхXDTO")
Процедура TBK_ПКО_Справочник_ХарактеристикиНоменклатуры_ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена)
	// Вставить содержимое метода.
