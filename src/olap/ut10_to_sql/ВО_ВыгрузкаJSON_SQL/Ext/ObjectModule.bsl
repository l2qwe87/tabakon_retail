...
Функция ВыгрузитьПродажи_(датаН, датаК,склад)
	...
	ДанныеВСиквел(массивДанных,"ОтчетОРозничныхПродажах", " [Магазин] ='"+складУИ+"' AND [Дата] >= '"+ДатуВСтрокуСКЛ(НачалоДня(датаН))+"' AND [Дата] <= '"+ДатуВСтрокуСКЛ(КонецДня(датаК))+"' ");
КонецФункции
...
Функция ВыгрузитьВозвраты_(датаН, датаК,склад)
	...
	ДанныеВСиквел(массивДанных,"ВозвратТоваровОтПокупателя", " [Магазин] ='"+складУИ+"' AND [Дата] >= '"+ДатуВСтрокуСКЛ(НачалоДня(датаН))+"' AND [Дата] <= '"+ДатуВСтрокуСКЛ(КонецДня(датаК))+"' ");
КонецФункции
...
Функция ВыгиузкаЗакупку_(датаН, датаК, склад)
	...
	ДанныеВСиквел(массивДанных,"ПоступлениеТоваровУслуг", " [Склад] ='"+складУИ+"' AND [Дата] >= '"+ДатуВСтрокуСКЛ(НачалоДня(датаН))+"' AND [Дата] <= '"+ДатуВСтрокуСКЛ(КонецДня(датаК))+"' ");
КонецФункции
...
Функция ВыгрузитьТоварыОбороты_(датаН, датаК, склад)
	...
	ДанныеВСиквел(массивДанных,"ТоварыОбороты", " [Склад] ='"+складУИ+"' AND [Период] >= '"+ДатуВСтрокуСКЛ(НачалоДня(датаН))+"' AND [Период] <= '"+ДатуВСтрокуСКЛ(КонецДня(датаК))+"' ");
КонецФункции
...
Функция ВыгрузитьТоварыОстатки_(датаН, датаК, склад)
	...
	ДанныеВСиквел(массивДанных,"ТоварыОстатки", " [Склад] ='"+складУИ+"' AND [Период] >= '"+ДатуВСтрокуСКЛ(НачалоДня(датаН))+"' AND [Период] <= '"+ДатуВСтрокуСКЛ(КонецДня(датаК))+"' ");
КонецФункции
...
Функция СправочникНоменклатуры(датаН, датаК) ЭКСПОРТ	
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ПРЕДСТАВЛЕНИЕ(Номен.ОсновнойПоставщик) ОсновнойПоставщик 
	               |ИЗ
	               |	Справочник.Номенклатура КАК Номен";
...
Функция ДатуВСтрокуСКЛ(дата)
	Возврат ФОРМАТ(НачалоДня(дата),"ДФ=""ггггММдд ЧЧ:мм:сс""");	
КонецФункции
...
Процедура ДанныеВСиквел(массивДанных, имяТаблицы, where = "", replace = false)
	...
	балк = ГенерацияSQL_BULK(массивДанных, полныйПуть, имяТаблицы);
	Если балк <> неопределено Тогда
		ВыполниттьSQL(балк, ИСТИНА);	
		Попытка
			ВыполниттьSQL(ГенерацияSQL_INSERT_TRY(имяТаблицы), ЛОЖЬ);	
		Исключение	
			ВыполниттьSQL(ГенерацияSQL_MERGE(имяТаблицы, where, replace), ИСТИНА);
		Конецпопытки;
		
		ВыполниттьSQL(ГенерацияSQL_DROP_Артефактов(имяТаблицы), ИСТИНА);
	КонецЕсли;
...
Функция ГенерацияSQL_INSERT_TRY(имяТаблицы)
	Возврат "SELECT * INTO ["+имяТаблицы+"] FROM ["+имяТаблицы+"_tmp]";
КонецФункции

Функция ГенерацияSQL_MERGE(имяТаблицы, where = неопределено,replace=false)
	
	Если replace Тогда
		Возврат "
		|BEGIN TRAN
		|DELETE FROM ["+имяТаблицы+"]  
		|INSERT INTO ["+имяТаблицы+"] SELECT * FROM ["+имяТаблицы+"_tmp]
		|COMMIT TRAN";
	КонецЕсли;
...
Функция ГенерацияSQL_DROP_Артефактов(имяТаблицы)
	Возврат "DROP TABLE ["+имяТаблицы+"_tmp]"
КонецФункции

Функция ГенерацияSQL_BULK(массивДанных, имяФайла, имяТаблицы)
