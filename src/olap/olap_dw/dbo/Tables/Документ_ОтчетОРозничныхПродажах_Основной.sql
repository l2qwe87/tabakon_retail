CREATE TABLE [dbo].[Документ_ОтчетОРозничныхПродажах_Основной] (
...
CREATE UNIQUE CLUSTERED INDEX [PK_Main]
    ON [dbo].[Документ_ОтчетОРозничныхПродажах_Основной]([Дата] ASC, [Магазин] ASC, [Ссылка] ASC);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_Ref]
    ON [dbo].[Документ_ОтчетОРозничныхПродажах_Основной]([Ссылка] ASC);
